...
public class FuzzyAgent extends Agent {
	...
	public Action chooseAction() {
		...
		if (deal)
			...
			lValues.put(
					utilitySpace.getDomain().getIssues().get(0).getNumber(),
					lValue);
			try {
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		if (opponentAction instanceof Offer) {
			...
			case BUYER:
				try {
					SellOffer[fRound][0] = ((ValueReal) (lBid
			...
			case SELLER:
				try {
					BuyOffer[fRound][0] = ((ValueReal) (lBid
							.getValue(utilitySpace.getDomain().getIssues()
									.get(0).getNumber()))).getValue();
	...
	public static double CalculateCost(double constant, int NoIteration) {
		double cost;
		cost = (double) (java.lang.Math.exp(NoIteration * constant) - java.lang.Math
				.exp(-NoIteration * constant))
				/ (double) (java.lang.Math.pow(java.lang.Math.E, NoIteration
						* constant) + java.lang.Math.pow(java.lang.Math.E,
	...
	protected static boolean Buyer(int i) {
		...
		case 1:
			...
			// if it has not reached the deadline continue negotiation
				...
				Utl = (double) (MaxB - SellOffer[i - 1][0])
						/ (double) (MaxB - MinB);
				if (i > 0) {
					Utl = (double) (MaxB - SellOffer[i - 1][0])
							/ (double) (MaxB - MinB);
					if (CSell == false) {
						 ...
						 */
						if (((double) SellOffer[i - 1][1] <= (double) BuyOffer[i - 1][1])
								&& (i <= deadlineB)) {
				...
				if (BBuy == 0.01 || BBuy == 0.2) {
					A = java.lang.Math
							.exp(java.lang.Math.pow(
									1 - ((double) java.lang.Math.min(i + 1,
											deadlineB) / (double) deadlineB),
									(BBuy))
									* java.lang.Math.log(K));
				} else {
					A = K
							+ ((1 - K) * (java.lang.Math.pow(
				...
				if ((float) tmpUtl < (float) Utl && (CBuy == false)) {
					...
					if (CBuy == true) {
						...
						if (BBuyStretch == 0.01 || BBuyStretch == 0.2) {
							...
							A = K
									+ ((1 - K) * (java.lang.Math
											.pow(((double) java.lang.Math.min(
													i + 1, deadlineB) / (double) deadlineB),
													(1 / BBuyStretch))));
						}
						double tmpBcons = ((percentageB2 * (Bcons)) + ((percentageB1 - percentageB2)
								* (Bcons) * (1 - A)));
					...
					if (CSell == false) {
						...
						if (((double) SellOffer[i - 1][1] <= (double) BuyOffer[i - 1][1])
							...
							if ((double) BuyOffer[i - 1][0] == (double) BuyOffer[i - 1][1]) {
								...
								double[] yParams = { 0.0, 1.0, 1.0, 0.0 };
								inter = Intersection(xParams, yParams);
								double DifUtlOurs = (double) (MaxB - BuyOffer[i - 1][0])
										/ (double) (MaxB - MinB);
								double MidUtl = (double) (MaxB - inter[0])
										/ (double) (MaxB - MinB);
								DifUtlOurs -= MidUtl;
								double DifUtlOps = (double) (MaxB - SellOffer[i - 1][0])
										/ (double) (MaxB - MinB);
				...
				if (LBuy == 3) {
					...
					if (i > LBuy) {
						...
						double tmp3 = java.lang.Math.min(java.lang.Math.max(
								(BuyOffer[i - 1][0]
										+ (double) SellOffer[i - 1 - LBuy][0]
										- (double) SellOffer[i - 1][0] + 2),
								MinB), MaxB);
						double tmpUtl3 = (double) (MaxB - tmp3)
								/ (double) (MaxB - MinB);
					...
					if (i > LBuy) {
						double tmp2 = java.lang.Math
								.min(java.lang.Math
										.max(((double) SellOffer[i - 1 - LBuy][0] / (double) SellOffer[i - 1][0])
												* BuyOffer[i - 1][0], MinB),
										MaxB);
						// double
						...
						double tmpUtl2 = (double) (MaxB - tmp2)
								/ (double) (MaxB - MinB);
	...
	protected static boolean Seller(int i) {
			...
			// if it has not reached the deadline continue negotiation
				...
				if (i > 0) {
					if (CBuy == false) {
						 ...
						 */
						if (((double) SellOffer[i - 1][1] <= (double) BuyOffer[i][1])
								&& (i <= deadlineS)) {
							double[] xParams = new double[4];
					...
					if (CSell == true) {
						...
						if (BSellStretch == 0.01 || BSellStretch == 0.2) {
							...
							A = K
									+ ((1 - K) * (java.lang.Math
											.pow(((double) java.lang.Math.min(
													i + 1, deadlineS) / (double) deadlineS),
													(1 / BSellStretch))));
						}
						double tmpScons = ((percentageS2 * (Scons)) + ((percentageS1 - percentageS2)
								* (Scons) * (1 - A)));
			...
			if (i > deadlineS) {
					...
					if (CBuy == false) {
						...
						if (((double) SellOffer[i - 1][1] <= (double) BuyOffer[i][1])
							...
							if ((double) SellOffer[i - 1][0] == (double) SellOffer[i - 1][1]) {
								...
								double[] yParams = { 0.0, 1.0, 1.0, 0.0 };
								inter = Intersection(xParams, yParams);
								double DifUtlOurs = (double) (SellOffer[i - 1][0] - MinS)
										/ (double) (MaxS - MinS);
								double MinUtl = (double) (inter[0] - MinS)
										/ (double) (MaxS - MinS);
								DifUtlOurs -= MinUtl;
								double DifUtlOps = (double) (BuyOffer[i - 1][0] - MinS)
										/ (double) (MaxS - MinS);
								DifUtlOps = Utl - DifUtlOps;
								// if ((float)DifUtlOurs<=(float)DifUtlOps){
								MaxS = inter[0];
				...
				if (LSell == 3) {
					...
					if (i >= LSell) {
						double tmp3 = java.lang.Math.min(
								java.lang.Math.max((SellOffer[i - 1][0]
										+ BuyOffer[i - LSell][0]
										- BuyOffer[i][0] - rand), MinS), MaxS);
						double tmpUtl3 = (double) (tmp3 - MinS)
								/ (double) (MaxS - MinS);
					...
					if (i >= LSell) {
						...
						double tmp2 = java.lang.Math
								.min(java.lang.Math.max(((double) BuyOffer[i
										- LSell][0] / (double) BuyOffer[i][0])
										* SellOffer[i - 1][0], MinS), MaxS);
						Utl = (double) (BuyOffer[i][0] - MinS)
								/ (double) (MaxS - MinS);
						double tmpUtl2 = (double) (tmp2 - MinS)
								/ (double) (MaxS - MinS);
		...
		case 3:
			...
			// if it has not reached the deadline continue negotiation
				...
				double resource = new Double(MSell
						* ((double) 1 / (double) (i + 1)));
				A = K + ((1 - K) * (1 / (java.lang.Math.exp(resource))));
				double tmp4 = MinS + ((MaxS - MinS) * (1 - A));
				double tmpUtl4 = (double) (tmp4 - MinS)
						/ (double) (MaxS - MinS);
	...
	public static double[] Intersection(double[] xPars, double[] yPars) {
		...
		double ua = 0;
		ua = (double) (((xPars[3] - xPars[2]) * (yPars[0] - yPars[2])) - ((yPars[3] - yPars[2]) * (xPars[0] - xPars[2])))
				/ (double) (((yPars[3] - yPars[2]) * (xPars[1] - xPars[0])) - ((xPars[3] - xPars[2]) * (yPars[1] - yPars[0])));
	...
	public static void FuzzyDist() {
		...
		if (BuyOffer[iteration - 1][0] != BuyOffer[iteration - 1][1]) {
			midPointXB += ((double) ((3 * (java.lang.Math.pow(
					BuyOffer[iteration - 1][0], 2) * BuyOffer[iteration - 1][1]))
					- java.lang.Math.pow(BuyOffer[iteration - 1][1], 3) - (2 * java.lang.Math
					.pow(BuyOffer[iteration - 1][0], 3))) / (double) (6 * (BuyOffer[iteration - 1][0] - BuyOffer[iteration - 1][1])));
			midPointXB = midPointXB
					/ ((double) ((2 * BuyOffer[iteration - 1][1] * BuyOffer[iteration - 1][0])
							- java.lang.Math.pow(BuyOffer[iteration - 1][1], 2) - java.lang.Math
								.pow(BuyOffer[iteration - 1][0], 2)) / (double) (2 * (BuyOffer[iteration - 1][0] - BuyOffer[iteration - 1][1])));
			midPointYB = 0.5;
		} else {
		...
		if (SellOffer[iteration - 1][0] != SellOffer[iteration - 1][1]) {
			...
			midPointXS += ((double) ((3 * (java.lang.Math.pow(
					SellOffer[iteration - 1][1], 2) * SellOffer[iteration - 1][0]))
					- java.lang.Math.pow(SellOffer[iteration - 1][0], 3) - (2 * java.lang.Math
					.pow(SellOffer[iteration - 1][1], 3))) / (double) (6 * (SellOffer[iteration - 1][0] - SellOffer[iteration - 1][1])));
			midPointXS = midPointXS
					/ ((double) ((2 * SellOffer[iteration - 1][1] * SellOffer[iteration - 1][0])
							- java.lang.Math
									.pow(SellOffer[iteration - 1][1], 2) - java.lang.Math
								.pow(SellOffer[iteration - 1][0], 2)) / (double) (2 * (SellOffer[iteration - 1][0] - SellOffer[iteration - 1][1])));
			midPointYS = 0.5;
		} else {
			midPointXS = SellOffer[iteration - 1][0];
		}
		double last = java.lang.Math.sqrt(java.lang.Math.pow(
				(midPointXS - midPointXB), 2)

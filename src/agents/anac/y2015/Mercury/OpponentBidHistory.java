...
public class OpponentBidHistory {
	...
	public void initiPartyRec(int num, UtilitySpace pUtilitySpace){
		...
		this.partyNum = num;
		this.oppAcce = new ArrayList<Bid>(); // the best acceptance from each party
	...
	public void updateOppRec(Bid oppbid, Object sender){
		...
		if ( partyOrder.size() < partyNum-1){
			...
			try{			
				if (fUtilitySpace.getUtility(oppMax.get(partyOrder.indexOf(sender))) < fUtilitySpace.getUtility(oppbid) )

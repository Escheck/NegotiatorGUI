...
public class BidGenerator {
	...
	public static double[] utilitylist(ArrayList<Bid> bids,  TUDMixedStrategyAgent info){
			...
			for(int j=0;j<before;j++) {
				for(int k=0;k<actual;k++) {
					for(int l=0;l<after;l++) {
						valueMatrix[l+actual*after*j+k*after][i] = listValueList.get(i).get(k);
					}
				}
			}
	...
	/*
	 * Returns ArrayList of Bids within the range given, if there isn't any returns closest Bid with higher utility, 
	 * If still there isn't any will return Bid with closest lower utility. 
	 * If there isn't any will return an empty ArrayList
	 * */
	public static ArrayList<Bid> getBidsInRange(ArrayList<Bid> bids, double low_bound, double up_bound, TUDMixedStrategyAgent info)
	{
		...
		if(bids.isEmpty())
			return bidsInRange;
		for(int i=0;i<bids.size();i++) {
			if(low_bound<info.getUtility(bids.get(i))&&up_bound>info.getUtility(bids.get(i))){
		...
		if(bidsInRange.isEmpty()) {
			// There are none inside or larger than range, search for closest smaller 

...
public class XianFaAgent extends AbstractNegotiationParty {
	...
	private Action chooseBid() {
		...
		try {
			...
			e.printStackTrace();
			act = 1;
			return (new Accept()); // best guess if things go wrong.
		}
		try {
	...
	private void updateSelf() {
		...
		if (!opponentA.equals("") && !opponentB.equals("")) {
			...
			if (rounds % 200 == 0) {
				...
				threshold = threshold
						- ((Math.max(cRateA, cRateB) * 0.00515) * ((1 - t)
								* (1 - t) * (1 / discount)));
				threshold = threshold
						- ((Math.max(1 - sRateA, 1 - sRateB) * 0.0116) * ((1 - t)
								* (1 - t) * (1 / discount)));
			}
			// printConsole();
		}
		...
		if (action instanceof Offer) {
			...
			if (myTurnPrev) {
				...
				try {
					...
					else if (utilitySpace.getUtility(opponentABest) < getUtilitySpace()
							.getUtility(partnerBid)) {
						opponentABest = new Bid(partnerBid);
					}
				} catch (Exception e) {
	...
	public void initTree() throws Exception {
		...
		for (int curDepth = 0; curDepth < maxDepth; curDepth++) { // for this
			...
			} else if (utilitySpace.getDomain().getIssue(curDepth).getType() == ISSUETYPE.INTEGER) {
				ii = (IssueInteger) utilitySpace.getDomain().getIssue(curDepth);
				for (int curMember = 0; curMember < tree
						.getSizeOfLevel(curDepth); curMember++) { // for all
																	// nodes in
																	// this
																	// depth
					Node node = new Node(ii.getNumber(), new ValueInteger(
							ii.getUpperBound()), curDepth, 1);
					tree.getMemberInLevel(curDepth, curMember).add(node);
					tree.addNodeInDepth(node, curDepth + 1);
				}
			}
	...
	public Bid getBid(int position) throws Exception {
		...
		values.put(tree.getMemberInLevel(maxDepth, position).getNumber(), tree
				.getMemberInLevel(maxDepth, position).getValue());
			...
			for (Issue iss : bid.getIssues()) {
				for (int i = 0; i < issuesA.size(); i++) {
					if (issuesA.get(i).getIssNr() == iss.getNumber()) {
						double s = issuesA.get(i).getVal(
								bid.getValue(iss.getNumber()));
						if (s > 1)
							s = 1;
						score = score + s;
						break;
	...
	public void initOpp() {
		for (int i = 0; i < utilitySpace.getDomain().getIssues().size(); i++) {
			AIssue iss = new AIssue(utilitySpace.getDomain().getIssues().get(i)
					.getNumber());
			if (utilitySpace.getDomain().getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
			...
			} else if (utilitySpace.getDomain().getIssues().get(i).getType() == ISSUETYPE.INTEGER) {
			...
			if (act == 0) {
				...
				for (int i = 0; i < bid.getIssues().size(); i++) {
					for (int j = 0; j < issuesA.size(); j++) {
						if (bid.getIssues().get(i).getNumber() == issuesA
								.get(j).getIssNr()) {
							if (bid.getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
								issuesA.get(j).setVal(
										bid.getValue(bid.getIssues().get(i)
												.getNumber()), totalA, 0);
							} else {
								// ignore
							}
			...
			if (act == 0) {
				...
				for (int i = 0; i < bid.getIssues().size(); i++) {
					for (int j = 0; j < issuesB.size(); j++) {
						if (bid.getIssues().get(i).getNumber() == issuesB
								.get(j).getIssNr()) {
							if (bid.getIssues().get(i).getType() == ISSUETYPE.DISCRETE) {
								issuesB.get(j).setVal(
										bid.getValue(bid.getIssues().get(i)
												.getNumber()), totalB, 0);
							} else {
								// ignore

...
{	
	...
	public bidSearch(UtilitySpace utilitySpace, negotiatingInfo negotiatingInfo) throws Exception {
	...
	private void initMaxBid() throws Exception{
		...
		for (int i = 0; i < tryNum; i++) {
			try { 
				do{ SimulatedAnnealingSearch(maxBid, 1.0); } while (utilitySpace.getUtility(maxBid) < utilitySpace.getReservationValue());
				if(utilitySpace.getUtility(maxBid) == 1.0){ break; }
			} catch (Exception e) {
	...
	public Bid getBid(Bid baseBid, double threshold) {
		...
		for(Issue issue:negotiatingInfo.getIssues()) {
			switch(issue.getType()) {
				case REAL: 
					try { return (getRandomBid(threshold)); } 
					catch (Exception e) {
		...
		try {
			...
			if (utilitySpace.getUtility(bid) < threshold){ bid = getBidbyAppropriateSearch(baseBid, threshold); } // 閾値以上の効用値を持つ合意案候補を探索
			if (utilitySpace.getUtility(bid) < threshold) { bid = new Bid(maxBid); } // 探索によって得られたBidがthresholdよりも小さい場合，最大効用値Bidを基準とする
		...
		do {
			for(Issue lIssue:issues) 
			{
				...
				case INTEGER:
					...
					break;
				default: throw new Exception("issue type "+lIssue.getType()+" not supported by Atlas3");
				}
			}
	...
	private Bid getBidbyNeighborhoodSearch(Bid baseBid, double threshold) {
		...
		try { for (int i = 0; i < NEAR_ITERATION; i++) { bid = NeighborhoodSearch(bid, threshold); } } 
		catch (Exception e) {
	...
	private Bid getBidbyAppropriateSearch(Bid baseBid, double threshold) {
		...
		try {
			...
			if(negotiatingInfo.isLinerUtilitySpace()){ 
				...
				if(utilitySpace.getUtility(bid) < threshold){ negotiatingInfo.utilitySpaceTypeisNonLiner(); } // 探索に失敗した場合，非線形効用空間用の探索に切り替える
			} 
	...
	private Bid relativeUtilitySearch(double threshold) throws Exception{
		...
		HashMap<Issue, HashMap<Value, Double>> valueRelativeUtility = negotiatingInfo.getValueRelativeUtility();
		ArrayList<Issue> randomIssues = negotiatingInfo.getIssues();
	...
	{	
		...
		ArrayList<Issue> issues = negotiatingInfo.getIssues();
		...
		while (currentTemperature > END_TEMPERATURE) { // 温度が十分下がるまでループ
			...
			for (int i = 0; i < STEP_NUM; i++) { // 近傍のBidを取得する
				...
				if (maxBid == null || nextBidUtil >= utilitySpace.getUtility(maxBid)) { maxBid = new Bid(nextBid); } // 最大効用値Bidの更新
			}
		...
		if (targetBids.size() == 0) { return new Bid(baseBid); } // 境界値より大きな効用値を持つBidが見つからなかったときは，baseBidを返す
		else { return new Bid(targetBids.get(randomnr.nextInt(targetBids.size()))); } // 効用値が境界値付近となるBidを返す
	}
    
	// 近傍探索
	private Bid NeighborhoodSearch(Bid baseBid, double threshold) throws Exception {
		...
		for (Issue issue:issues) {
			...
			for (Value value:values) {
				currentBid.setValue(issue.getNumber(), value); // 近傍のBidを求める
				currenBidUtil = utilitySpace.getUtility(currentBid);
				if (maxBid == null || currenBidUtil >= utilitySpace.getUtility(maxBid)) { maxBid = new Bid(currentBid); }	// 最大効用値Bidの更新
				// 更新
				if (currenBidUtil >= threshold){
					if(targetBids.size() == 0){ 
						targetBids.add(new Bid(currentBid)); 
	...
	private Bid getConvertBidbyFrequencyList(Bid baseBid) {
		try {
			...
			ArrayList<Object> randomOrderOpponents = negotiatingInfo.getOpponents();
...
//			for(Object sender:randomOrderOpponents){ 
				ArrayList<Issue> randomOrderIssues = utilitySpace.getDomain().getIssues();
				Collections.shuffle(randomOrderIssues); // ランダムに並び替える	
				for (Issue issue:randomOrderIssues) {
					Bid nextBid = new Bid(currentBid);
					nextBid.setValue(issue.getNumber(), negotiatingInfo.getValuebyAllFrequencyList(issue)); // 頻出valueを優先してセットする
//					nextBid.setValue(issue.getNumber(), negotiatingInfo.getValuebyFrequencyList(sender, issue)); // 頻出valueを優先してセットする
					if(utilitySpace.getUtility(nextBid) >= utilitySpace.getUtility(currentBid)){ currentBid = new Bid(nextBid); }
				}
//			}
			return currentBid;
		} catch (Exception e) {
    ...
    public ArrayList<Issue> criticalIssue(Bid baseBid) throws Exception { 
		...
		for (Issue issue:issues) {
			...
			for (Value value:values) {
				currentBid.setValue(issue.getNumber(), value); // 近傍のBidを求める
				if(utilitySpace.getUtility(currentBid) != utilitySpace.getUtility(baseBid)){

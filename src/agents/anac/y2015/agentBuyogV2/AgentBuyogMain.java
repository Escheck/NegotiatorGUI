...
public class AgentBuyogMain extends AbstractNegotiationParty {
	...
	private Double getDistance(BidDetails bid, Double infoAKalai,
		...
		try {
			return Math.sqrt((1 - infoA.getAgentDifficulty())
					* Math.pow(
							infoA.getOpponentUtilitySpace().getUtility(
									bid.getBid())
									- infoAKalai, 2)
					+ (1 - infoB.getAgentDifficulty())
					* Math.pow(
							infoB.getOpponentUtilitySpace().getUtility(
									bid.getBid())
									- infoBKalai, 2));
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action instanceof Offer) {
			...
			try {
				totalHistory.add(new BidDetails(bid, utilitySpace
						.getUtility(bid), timeline.getTime()));
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void updateCommonBids(OpponentInfo otherInfo, Bid bid) {
		...
		if (otherInfo.containsBid(bid)) {
			try {
				this.AandBscommonBids.add(new BidDetails(bid, utilitySpace
						.getUtility(bid)));
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void updateOpponentModel(OpponentInfo senderInfo) {
		...
		if (senderInfo.getAgentBidHistory().size() >= minimumHistorySize) {
			LearningFunction function = new LearningFunction(senderInfo
					.getAgentBidHistory().getHistory().get(0)
					.getMyUndiscountedUtil());
			...
			double slopeFromZeroToOne = Math.atan(slopeStandardScale)
					/ (Math.PI / 2);
			double adjustedLeniency = slopeFromZeroToOne + slopeFromZeroToOne
					/ leniencyAdjuster;
		...
		double goldenValue = issueWeightsConstant
				* (1 - (Math.pow(timeline.getTime(), learningTimeController
						+ utilitySpace.getDiscountFactor()))) / numberOfIssues;
		double totalSum = 1D + goldenValue * numberOfUnchanged;
		double maximumWeight = 1D - ((double) numberOfIssues) * goldenValue
				/ totalSum;

		for (Integer issueNumber : changed.keySet()) {
			if (!changed.get(issueNumber)
					&& opponentUtilitySpace.getWeight(issueNumber) < maximumWeight) {
				opponentUtilitySpace
						.setWeight(
								opponentUtilitySpace.getDomain().getObjective(
										issueNumber),
								(opponentUtilitySpace.getWeight(issueNumber) + goldenValue)
										/ totalSum);
			} else {
				...
				if (issueEvaluatorEntry.getKey() instanceof IssueDiscrete) {
							...
							.setEvaluation(
									...
									(int) (issueValuesConstant
											* (1 - Math.pow(
													...
													learningTimeController
															+ utilitySpace
																	.getDiscountFactor())) + ((EvaluatorDiscrete) issueEvaluatorEntry
											.getValue())
											.getEvaluationNotNormalized((ValueDiscrete) opponentsLatestBid
													.getBid()
													.getValue(
															((IssueDiscrete) issueEvaluatorEntry
																	.getKey())
					...
					if (midPoint > opponentsLatestValueForIssueAsInteger) {
						...
						double distanceFromMidPoint = opponentsLatestValueForIssueAsInteger
								- midPoint + 1; // because midPoint is included
												// and I don't want a 0 value.
						double normalizedDistanceFromMidPoint = distanceFromMidPoint
		...
		try {
			...
			if (senderInfo.getAgentBidHistory().size() >= minimumHistorySize) {
				...
				double agentDifficulty = (1 - domainWeight)
						* senderInfo.getLeniency() + domainWeight
						* senderInfo.getDomainCompetitiveness();
		...
		try {
			for (Issue i : senderInfo.getOpponentUtilitySpace().getDomain()
				...
				if (i instanceof IssueDiscrete) {
					changed.put(i.getNumber(), (((ValueDiscrete) first.getBid()
							.getValue(i.getNumber()))
							.equals((ValueDiscrete) second.getBid().getValue(
									i.getNumber()))) ? false : true);
				} else if (i instanceof IssueInteger) {
					changed.put(i.getNumber(), (((ValueInteger) first.getBid()
							.getValue(i.getNumber()))
							.equals((ValueInteger) second.getBid().getValue(
									i.getNumber()))) ? false : true);
				}
			}
		} catch (Exception ex) {
	...
	private void updateOpponentBidHistory(OpponentInfo opponent, Bid bid) {
		...
		try {
			opponent.getAgentBidHistory().add(
					new BidDetails(bid, utilitySpace.getUtility(bid), timeline
							.getTime()));
			...
			if (opponent.getBestBid() == null
				...
				opponent.getBestBids().add(
						new BidDetails(bid, utilitySpace.getUtility(bid),
								timeline.getTime()));
			} else {
				opponent.getBestBids().add(
						new BidDetails(opponent.getBestBid(), utilitySpace
								.getUtility(opponent.getBestBid()), timeline
								.getTime()));

...
//		 if (prev != null) {
//			 double previousOutcome = (Double) prev; //�@‘O‰ñŠl“¾Œø—p’l
...
//	}
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		if (opponentAction != null){
			...
			currBidHistory.add(new BidDetails(opponentLastBid, getUtility(opponentLastBid), 
									timeline.getTime()));
	...
	private void calculateMinimumBidUtil(double time) {
		double reservationValue = utilitySpace.getReservationValueWithDiscount(time);
	...
	private Action finalPhase(){
		...
		return new Offer(list.get(countFinal).getBid());
	}
	...
	private Bid getBidBySA(double target) throws Exception{
		double t = 1000000;
		double cool = 0.999;
		ArrayList<Issue> issues = utilitySpace.getDomain().getRandomBid().getIssues();
		Random rnd = new Random();
		Bid nowBid;
		
		nowBid = utilitySpace.getDomain().getRandomBid();
		Bid nextBid = null;
		...
		while(t > 0.001){
			...
			if (isInt.getUpperBound() == vlInt.getValue()) { // issue‚Ì�Å‘å’l‚Ì‚Æ‚«
				...
				nextBid.setValue(index+1, vlInt);
		...
		while(t > 0.001){
			...
			for (int i=0; i < issues.size(); i++) {
				...
				for (int j=min; j < max; j++) {
					...
					tempBid.setValue(i+1, new ValueInteger(j));

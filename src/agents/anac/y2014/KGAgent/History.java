...
public class History {
	...
	History(ArrayList<Issue> issuelist,KGAgent agent,int ty){
		issues = issuelist;
		this.agent= agent;
		type = ty;
	}
	...
	void AddHistoryNonLin(Bid bid){
		...
		for(int i=0;i<size;i++){
			...
			for(int j=i+1;j<size;j++){
				...
				//System.out.print("add!" + pair.get(0).key +""+pair.get(1).key);
	...
	double SearchMaxPoint(){
		...
		if(list== null){
			gaMain = new GA_Main(new BidGenerationChange(100), new CompMyBidGene(-1));
			...
			gaMain = new GA_Main(in,new BidGenerationChange(100,10), new CompMyBidGene(0));

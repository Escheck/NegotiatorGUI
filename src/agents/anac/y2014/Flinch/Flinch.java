...
public class Flinch extends Agent {
	...
	private double getLambda() {
		...
		return t_lambda_min + (t_lambda_max - t_lambda_min)
				* Math.pow(delta, beta);
	}
	...
	private double getAcceptThreshold(double t) {
		...
		if (Double.isNaN(ret) || Double.isInfinite(ret)) { // @TODO
			System.out
					.println("Errors in calculating acceptThreshold, default to M*p = "
							+ left);
			ret = left;
		}
	...
	private double distance(Bid b1, Bid b2) throws Exception {
		...
		for (int i = 0; i < issues.size(); i++) {
			...
			case DISCRETE: {
				...
				ValueDiscrete v1d = (ValueDiscrete) b1.getValue(issue
						.getNumber());
				ValueDiscrete v2d = (ValueDiscrete) b2.getValue(issue
						.getNumber());
				if (!v1d.equals(v2d)) {
					diff = 1.0;
				}
			...
			case INTEGER: {
				...
				ValueInteger v2i = (ValueInteger) b2
						.getValue(issue.getNumber());
				double abs_diff = (double) Math.abs(v1i.getValue()
						- v2i.getValue());
				diff = abs_diff
						/ (double) (lIssueInteger.getUpperBound() - lIssueInteger
								.getLowerBound());
				break;
			}
	...
	private Bid GA_select(Bid[] population, double[] score, double N)
			throws Exception { // currently using roulette

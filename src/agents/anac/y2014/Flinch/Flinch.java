...
public class Flinch extends Agent
{
    ...
    private double estimatedMaxUtil = 1.0; //estimated best utility for me, default at 1.0
	...
	public Action chooseAction() //main function
	{
		...
		{ 
            ...
            //System.out.println("updated acceptThreshold = " + acceptThreshold);
            //System.out.println("estimatedMaxUtil = " + estimatedMaxUtil);
            //System.out.println("t = " + t + "\tt_lambda = " + t_lambda);
            //if(bestBidFromOpponent != null)
                //System.out.println("U_bbo = " + utilitySpace.getUtility(bestBidFromOpponent));

            round++;

			if(actionOfPartner==null){
                Bid myBid = chooseNextBid();
                myHistory.add(new BidDetails(myBid,utilitySpace.getUtility(myBid)));
                ...
                if(actionOfPartner instanceof Offer) {
                    ...
                    history.add(new BidDetails(partnerBid,offeredUtilFromOpponent,t));
                    if(bestBidFromOpponent == null || offeredUtilFromOpponent > utilitySpace.getUtility(bestBidFromOpponent)){
    ...
    private int estimatedRoundsLeft(){
        ...
        double dur = history.get(n-1).getTime() - history.get(n-1-10).getTime();
    ...
    private double getLambda(){
        ...
        return t_lambda_min + (t_lambda_max - t_lambda_min) * Math.pow(delta,beta);
    }
    ...
    private double getAcceptThreshold(double t){
        ...
        if(t <= t_lambda){
            ...
            try{
                right = Math.min(right, utilitySpace.getUtility(bestBidFromOpponent));
            } catch(Exception e){ }

            ret = middle + (right - middle) * Math.pow((t - t_lambda)/ (1-t_lambda) , 1/beta);
        }

        if(Double.isNaN(ret) || Double.isInfinite(ret)){ //@TODO
            System.out.println("Errors in calculating acceptThreshold, default to M*p = " + left);
            ret = left;
        }
    ...
    private double distance(Bid b1,Bid b2) throws Exception{
        ...
        ArrayList<Issue> issues = b1.getIssues();
                ...
                case DISCRETE:
                    {
                        ...
                        ValueDiscrete v1d = (ValueDiscrete)b1.getValue(issue.getNumber());
                        ValueDiscrete v2d = (ValueDiscrete)b2.getValue(issue.getNumber());
                        if(!v1d.equals(v2d)){
                            diff = 1.0;
                        }
                ...
                case REAL:
                    {
                        ...
                        diff = abs_diff / (lIssueReal.getUpperBound() - lIssueReal.getLowerBound());
                        break;
                    }
                case INTEGER:
                    {
                        IssueInteger lIssueInteger = (IssueInteger)issue;
                        ValueInteger v1i = (ValueInteger)b1.getValue(issue.getNumber());
                        ValueInteger v2i = (ValueInteger)b2.getValue(issue.getNumber());
                        double abs_diff = (double)Math.abs(v1i.getValue() - v2i.getValue());
                        diff = abs_diff / (double)(lIssueInteger.getUpperBound() - lIssueInteger.getLowerBound());
                        break;
                    }

                default: throw new Exception("issue type " + issue.getType()+" not supported by Flinch");
            }
    ...
    private Bid chooseNextBid() throws Exception{
        ...
        for(int i=0;i<candidates.length;i++){
            myUtils[i] = utilitySpace.getUtility(candidates[i]) / estimatedMaxUtil;
...
// ================================
	...
	public Bid getRandomBid() throws Exception
	{
        ...
        for(Issue lIssue:issues) 
        {
            ...
            default: throw new Exception("issue type " + lIssue.getType()+" not supported by Flinch LocalSearch");
            }
        }
...
// =======================
    ...
    private Bid GA_select(Bid[] population,double[] score,double N) throws Exception{ //currently using roulette
    ...
    private Bid GA_crossover(Bid b1,Bid b2) throws Exception{
        ...
        for(int i=k;i<issues.size();i++){
            ...
            ret.setValue(issuenr, b2.getValue(issuenr));
        }
    ...
    private Bid GA_mutate(Bid b) throws Exception{
		ArrayList<Issue> issues=utilitySpace.getDomain().getIssues();
        Random randomnr = new Random();
        int k = (int)randomnr.nextInt(issues.size());
        ...
        switch(issues.get(k).getType()){
            ...
            case REAL:
            {
                ...
                int optionInd = randomnr.nextInt(lIssueReal.getNumberOfDiscretizationSteps()-1);
                v =  new ValueReal(lIssueReal.getLowerBound() + (lIssueReal.getUpperBound()-lIssueReal.getLowerBound())*(double)(optionInd)/(double)(lIssueReal.getNumberOfDiscretizationSteps()));
                break;
            }
            case INTEGER:
            {
            ...
            default: throw new Exception("issue type " + issues.get(k).getType()+" not supported by Flinch LocalSearch");
        }
        assert v != null;

        b.setValue(issuenr,v);
        return b;
    }

    private void updateCandidatesSet(Set<Bid> ret,Bid[] population,double f) throws Exception{
            double t = timeline.getTime();
            for(int i=0;i<POP_SIZE;i++){
    ...
    private Bid[] GA(double f) throws Exception{
        ...
        return (Bid[])ret.toArray(new Bid[0]);
    }

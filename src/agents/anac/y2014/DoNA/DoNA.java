...
public class DoNA extends Agent {
	...
	boolean highReservation;// =  (utilitySpace.getReservationValue() >= 0.8);//(minUtility+0.75*UtilityRange));
	boolean midReservation;//   =  (utilitySpace.getReservationValue() <  0.8 && utilitySpace.getReservationValue() > 0.2);//(minUtility+0.75*UtilityRange) && utilitySpace.getReservationValue() >= (minUtility+0.375*UtilityRange));
	boolean lowReservation;//   =  (utilitySpace.getReservationValue() <= 0.2);//(minUtility+0.375*UtilityRange));
	
	boolean highDiscount;// = (utilitySpace.getDiscountFactor() >= 0.8);
	boolean midDiscount;// = (utilitySpace.getDiscountFactor() < 0.8 && utilitySpace.getDiscountFactor() > 0.2);
	...
	public void init() {
		...
		midReservation   =  (utilitySpace.getReservationValue() <  0.8 && utilitySpace.getReservationValue() > 0.2);//(minUtility+0.75*UtilityRange) && utilitySpace.getReservationValue() >= (minUtility+0.375*UtilityRange));
		...
		midDiscount = (utilitySpace.getDiscountFactor() < 0.8 && utilitySpace.getDiscountFactor() > 0.2);
		...
		try {
			if(utilitySpace.getReservationValue() > 0 && utilitySpace.getReservationValue() < 1 && utilitySpace.getDiscountFactor() < 1)			
				theTimeIAmReadyToInvestInNegotiation=Math.sqrt(
					Math.sqrt(1-utilitySpace.getReservationValue())
					*
		...
		if(isPrinting) System.out.println("m=" + (avg));
		...
		if(isPrinting) System.out.println("S="+stia);
		...
		lastAction = new Offer(maxUtilityBid);
		...
		theMotivationIHaveToInvestInNegotiation = dblMinUtil- utilitySpace.getReservationValue();
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		if (opponentAction instanceof Offer) {
			...
			{
				currentTimeIndex=0;
				averageResponseTime=(timeArr[numOfSamplesForAvgTime-1]-timeArr[0])/(numOfSamplesForAvgTime-1);
				double sumAll = 0;
				for (int i = 1; i < numOfSamplesForAvgTime; i++) {
					sumAll+=Math.pow(timeArr[i]-timeArr[i-1]-averageResponseTime, 2);
				}
				stiaResponseTime = Math.pow(sumAll/(numOfSamplesForAvgTime-1), 0.5);
	...
	public Action chooseAction() {
		...
		if(highReservation || (midReservation && lowDiscount) || theMotivationIHaveToInvestInNegotiation <= 0.1)
		{
			return cleanEndNegotiation();
		}
		...
		try {
			lastBidOfOpponentUtil = lastBidOfOpponent == null? 0: utilitySpace.getUtility(lastBidOfOpponent);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		
		if((lastBidOfOpponentUtil>dblMinUtil || lastBidOfOpponentUtil>myMinUtil) && lastBidOfOpponentUtil > R)
		{
			if(isPrinting) System.out.println("Accept1 Util: " + lastBidOfOpponentUtil);
			if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
				return cleanEndNegotiation();
			return new Accept();
		}
		...
		double currentTime = timeline.getTime();
			...
			{
				Bid Nbid = getBestNigberBid(actionOfPartner.getBidFromAction(actionOfPartner),0);
		...
		if(isPrinting) System.out.println("111111111111111111111");
		if(highDiscount)
		{
			if(NotFirstTime==false)
			{
			...
			{ 
				 ...
				 utilOfBestBidOfOppenent = utilitySpace.getUtility(this.opponentBidHistory.getBestBidInHistory());
			}	catch (Exception e) { 
				System.out.println("Exception in ChooseAction1:"+e.getMessage());
				
				lastAction = cleanOffer(hashBids.get(arrSamples.get(countOffer)),lastBidOfOpponentUtil); // best guess if things go wrong.
				
			}
			
			int FactorOfAverageResponseTime = 1;
			Bid bid;
			if (currentTime >= 1-FactorOfAverageResponseTime*(averageResponseTime+2*stiaResponseTime)) {  // last last moment
				if(utilitySpace.getReservationValue() > utilOfBid)
				{
					if(isPrinting) System.out.println("Enddddddddddddd 1111111111111");
					return cleanEndNegotiation();	
				}
				else
				{
					if(isPrinting) System.out.println("Accept2 Util: " + utilOfBid);
					if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
						return cleanEndNegotiation();
					return new Accept();
				}
			} else if (currentTime >= 1-Math.pow(2,1)*FactorOfAverageResponseTime*averageResponseTime) {  // one before last moment
				if(isPrinting) System.out.println("averageResponseTime: " + averageResponseTime);
				if(isPrinting) System.out.println("Math.pow(2,1): " + Math.pow(2,1));
				if(isPrinting) System.out.println("currentTime: " + currentTime + " / " + (1-Math.pow(2,1)*FactorOfAverageResponseTime*averageResponseTime));
				if(utilitySpace.getReservationValue() > utilOfBestBidOfOppenent && utilitySpace.getReservationValue() > utilOflastBidOfOpponent )
				{
					if(isPrinting) System.out.println("Enddddddddddddd  2222222222222");
					
					return cleanEndNegotiation();
				}
				else if (utilOflastBidOfOpponent >= utilOfBestBidOfOppenent)
				{
					if(isPrinting) System.out.println("Accept3 Util: " + utilOflastBidOfOpponent);
					if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
						return cleanEndNegotiation();
				...
				{
					double dblutility = getUtility(this.opponentBidHistory.getBestBidInHistory());
					if(isPrinting) System.out.println("Offer best history: " + dblutility);
					return new Offer(this.opponentBidHistory.getBestBidInHistory());
				}
			}
			if(isPrinting) System.out.println("222222222222222222222");
				
			return FastLastMomentStrategyWithoutEndNegotiation(dblMinUtil+0.25*stia,dblMinUtil,currentTime);
		}
		if (lowDiscount) 
		{
			return FastStrategy();
		...
		{ 
			...
			lastAction=cleanEndNegotiation(); // best guess if things go wrong. 
		}
		...
		if (currentTime >= 1-FactorOfAverageResponseTime*(averageResponseTime+2*stiaResponseTime)) {  // last last moment
			if(utilitySpace.getReservationValue() > utilOfBid)
			{
				if(isPrinting) System.out.println("Enddddddddddddd  3333333333333");
				
				return cleanEndNegotiation();
			}
			else
			{
				if(isPrinting) System.out.println("Accept4 Util: " + utilOfBid);
				if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
					return cleanEndNegotiation();
			...
			if(utilitySpace.getReservationValue() > utilOfBestBidOfOppenent && utilitySpace.getReservationValue() > utilOflastBidOfOpponent )
			{
				if(isPrinting) System.out.println("Enddddddddddddd  444444444444");
				
				return cleanEndNegotiation();
			}
			else if (utilOflastBidOfOpponent >= utilOfBestBidOfOppenent)
			{
				if(isPrinting) System.out.println("Accept5 Util: " + utilOflastBidOfOpponent);
				if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
					return cleanEndNegotiation();
		...
		return FastLastMomentStrategyWithoutEndNegotiation(dblMinUtil-UtilityFactor+0.25*stia,dblMinUtil-UtilityFactor,currentTime);
	...
	/**
	 * If the utility of the given bid is better than the reservation value - offer it. Otherwise - return EndNegotiation. 
	 * @throws Exception 
	 */
	private Action EndNegotiationOrAcceptOrNewOfferr(Bid myOfferedBid) throws Exception {
		...
		double bestOpponentBidUtility = (bestOpponentBid==null)? utilitySpace.getUtility(this.minUtilityBid) :utilitySpace.getUtility(bestOpponentBid);
		...
		{
			if(isPrinting) System.out.println("Enddddddddddddd  5555555555555555");
		...
		{
			if(isPrinting) System.out.println("Accept6 Util: " + lastBidOfOpponentUtility);
			if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
				return cleanEndNegotiation();
		...
		{
			if(isPrinting) System.out.println("new Offer(this.opponentBidHistory.getBestBidInHistory())");
			return new Offer(this.opponentBidHistory.getBestBidInHistory());
		}
		...
		{
			if(isPrinting) System.out.println("else");
			lastAction = cleanOffer(myOfferedBid,lastBidOfOpponentUtility); 
			MyOfferedUtility = utilitySpace.getUtility(lastAction.getBidFromAction(lastAction));
			if(isPrinting) System.out.println("else MyOfferedUtility 333: " + MyOfferedUtility);
			return lastAction;
		}
	}
	...
	private Bid getRandomBid(double minUtil) throws Exception
	{
		...
		do 
		{
			...
			//System.out.println("currentUtil: " + currentUtil + " / " + tmpMinUtil);
			...
			if(countTimes>=150000)
				flagStopExplor = true;
		} while (currentUtil < tmpMinUtil && otherArrSamples.size() < 500 && (flagStopExplor==false));
	...
	{
		double lastBidOfOpponentUtil = lastBidOfOpponent == null? 0: getUtility(lastBidOfOpponent);
		Bid nextBid=null ;
		try { nextBid = getRandomBid(minUtil); }
		catch (Exception e) { System.out.println("Problem with received bid:"+e.getMessage()+". cancelling bidding"); }
		if (nextBid == null) return cleanOffer(hashBids.get(arrSamples.get(arrSamples.size()-1)),lastBidOfOpponentUtil);
		if(lastBidOfOpponent==null)
			return new Offer(nextBid);
		else
		{
			if(isPrinting) System.out.println("chooseRandomBidAction-lastBidOfOpponentUtil: " + lastBidOfOpponentUtil);
		...
		for(Issue lIssue:issues) 
		{
			...
			case DISCRETE:
				...
				for (int optionIndex = 0; optionIndex < numOfValues; optionIndex++) {
					...
					newBid = currentBid;
					newBid.setValue(lIssue.getNumber(), lIssueDiscrete.getValue(optionIndex));
					if(deep>0)
					{
			...
			case REAL:
				...
				for (int optionInd = 0; optionInd < numOfValues; optionInd++) {
					newBid = currentBid;
					newBid.setValue(lIssue.getNumber(), new ValueReal(lIssueReal.getLowerBound() + (lIssueReal.getUpperBound()-lIssueReal.getLowerBound())*(double)(optionInd)/(double)(lIssueReal.getNumberOfDiscretizationSteps())));
					if(deep>0)
					{
						newBid=getBestNigberBid(newBid,deep-1);
					}
			...
			case INTEGER:
				...
				numOfValues = lIssueInteger.getUpperBound()-lIssueInteger.getLowerBound();
				for (int optionIndex2 = lIssueInteger.getLowerBound(); optionIndex2 < numOfValues; optionIndex2++) {
					newBid = currentBid;
					newBid.setValue(lIssue.getNumber(), new ValueInteger(optionIndex2));
					if(deep>0)
					{
						newBid=getBestNigberBid(newBid,deep-1);
					}
	...
	public Action FastStrategyWithoutEndNegotiation(double VirtulReservation)
	{
		...
		try{
			...
			{
				if(isPrinting) System.out.println("Accept7 Util: " + lastBidOfOpponentUtil);
				if(R>utilitySpace.getUtilityWithDiscount(lastBidOfOpponent, timeline.getTime()))
					return cleanEndNegotiation();
			...
			{
				if(isPrinting) System.out.println("bestOpponentBidUtility: " + bestOpponentBidUtility + " MyOfferedUtility: " + MyOfferedUtility +" VirtulReservation: "+VirtulReservation);
				return new Offer(this.opponentBidHistory.getBestBidInHistory());
			}
			// Reservation is the best choice
			else if(VirtulReservation > MyOfferedUtility)
			{
				if(isPrinting) System.out.println("MyOfferedUtility 111: ");
				double dbltime = timeline.getTime();
				
				if(flagStopExplor==false && dbltime<0.90)
				{
					if(isPrinting) System.out.println("flagStopExplor==false");
				...
				else
				{
					...
					if(otherArrSamples.size()>0)
					{
						if(isPrinting) System.out.println("otherArrSamples.size()>0");
						...
						if(curBid==null)
						{
							if(isPrinting) System.out.println("curBid==null");
							
							int tmpDif = arrSamples.size()-countOffer;
							double randNum = Math.random();
							if(isPrinting) System.out.println("countOffer: " + countOffer + " tmpDif: "+tmpDif+" randNum: " + randNum);
							int randIndex = ((int)(randNum*tmpDif)) + countOffer;
							curBid = hashBids.get(arrSamples.get(randIndex));
							if(isPrinting) System.out.println("randIndex: " + randIndex);
							if(isPrinting) System.out.println("curBid: " + curBid);
						}
					}
					else
					{
						if(isPrinting) System.out.println("otherArrSamples.size()>0 else");
				...
				MyOfferedUtility = utilitySpace.getUtility(lastAction.getBidFromAction(lastAction));
				if(isPrinting) System.out.println("MyOfferedUtility 111: " + MyOfferedUtility);
				
				return lastAction;
			}
			else
			{
				if(isPrinting) System.out.println("MyOfferedUtility 222: ");
				
				countOffer--;
				lastAction = cleanOffer(curBid,lastBidOfOpponentUtil);
				MyOfferedUtility = utilitySpace.getUtility(lastAction.getBidFromAction(lastAction));
				if(isPrinting) System.out.println("MyOfferedUtility 222: " + MyOfferedUtility);
	...
	public Action FastLastMomentStrategyWithoutEndNegotiation(double StartMinUtil, double MinUtil ,double dblMyRoundTime)
	{
		
		int constForNumOfRounds = 100;
		double Delta = (StartMinUtil-MinUtil)/11;
			//double MinUtil = 0.75;
			//double dblMyRoundTime = timeline.getTime()/(1.1*(1-utilitySpace.getReservationValue())); 
			//double dblMyRoundTime = timeline.getTime();
			int FactorOfAverageResponseTime = 1;
			if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {  // Other before-last moments - logarithmic scale
				if(isPrinting) System.out.println("dblMyRoundTime 1111");
				return FastStrategyWithoutEndNegotiation(MinUtil);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 2222");
				return FastStrategyWithoutEndNegotiation(MinUtil+Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 33333");
				return FastStrategyWithoutEndNegotiation(MinUtil+2*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 44444");
				return FastStrategyWithoutEndNegotiation(MinUtil+3*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 55555");
				return FastStrategyWithoutEndNegotiation(MinUtil+4*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 6666");
				return FastStrategyWithoutEndNegotiation(MinUtil+5*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 7777");
				return FastStrategyWithoutEndNegotiation(MinUtil+6*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 88888");
				return FastStrategyWithoutEndNegotiation(MinUtil+7*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 9999");
				return FastStrategyWithoutEndNegotiation(MinUtil+8*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 111 000");
				return FastStrategyWithoutEndNegotiation(MinUtil+9*Delta);
			} else if (dblMyRoundTime >= 1-constForNumOfRounds*FactorOfAverageResponseTime*averageResponseTime) {
				if(isPrinting) System.out.println("dblMyRoundTime 111 111");
				return FastStrategyWithoutEndNegotiation(MinUtil+10*Delta);
			} else {
				if(isPrinting) System.out.println("dblMyRoundTime 111 222");
	...
	public Action cleanOffer(Bid curBid, double lastBidOfOpponentUtil)
	{
		try {
			...
			if(MyOfferedUtility<=lastBidOfOpponentUtil || MyOfferedUtility<R)
			{
				if(isPrinting) System.out.println("Enddddddddddddd  7777777777777777");
	...
	private Action cleanEndNegotiation() {
		...
		if (randIndex==countOffer)
			return new Offer(hashBids.get(arrSamples.get(randIndex)));
		else
	...
	{
		double dblUtilVal = alternativeArrSamples.get(alternativeArrSamples.size()-1);
		...
		if(R>utilitySpace.getUtilityWithDiscount(lastBid, timeline.getTime()))
			return cleanEndNegotiation();
		else

...
public class Group2_OM extends OpponentModel {
	...
	public void init(NegotiationSession negotiationSession,
		...
		try {
			...
			Pair<Integer, Integer> bigyo = issueRanges.get(max_issues
					- min_issues + 1);
			...
			for (Issue issue : domain_issues) {
				...
				histogram.put(an_index, new HashMap<Integer, Integer>(highbound
						- lowbound + 1));
	...
	public double getBidEvaluation(Bid bid) {
		...
		for (Issue issue : domain_issues) {
			...
			int limit = this.negotiationSession.getOpponentBidHistory().size()
					* 1 / (NoOptions + 1);
		...
		double matching_bid_measure = ((double) counter)
				/ ((double) (this.max_issues - this.min_issues + 1));

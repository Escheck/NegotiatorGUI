...
/**
 * This {@code Group2_AS} is an extended AC_next strategy.
 * It will accept any bid that is better than the next that we would propose.
 * It will also accept any bid when the utility is above 0.9. With this we can avoid missing out on a great deal.
 * It will also accept any bid when there is too little time left, as a sort of fail-safe, to avoid 0.0 utility outcomes.
 * 
 * @since 2013-12-13
 * @version 1.0
 */
public class Group2_AS extends AcceptanceStrategy 
{
	 ...
	 * If we set this to, for example 2.0 than we stop when the
	 * opponent's bid is twice as good as the next bid we'll propose
	 * If we set this to 1.0, we accept any bid that is at least as good as ours.
	 */
	private double alpha;
	
	/**
	 * This is the offset factor.
	 * If we set this value > 0 than we accept only bids that are a constant factor
	 * better than the next bid that we would propose.
	 */
	private double beta;
	 ...
	 * @param negotiationSession The environment of the negotiation session. Contains deadline, domain, etc. 
	 * @param offeringStrategy This is the used bidding strategy. 
	 * 			We use this bidding strategy to decide what our next move will be.
	 * @param alpha The scaling factor
	 * @param beta The constant addition factor
	 */
	public Group2_AS(NegotiationSession negotiationSession, OfferingStrategy offeringStrategy, 
	...
	/**
	 * This function gets called by the framework upon initialization.
	 * It offers all commonly used variables (which are also in the constructor).
	 * This function might be redundant. but if left it here in case it actually does something.
	 * @param negotiationSession The environment of the negotiation session. Contains deadline, domain, etc. 
	 * @param offeringStrategy This is the used bidding strategy. We use this bidding strategy to decide what our next 
	 * 			move will be.
	 * @param parameters This is a hashmap that contains string key "a" which represents {@code alpha}, the scaling 
	 * 			factor and string key "b" which represents {@code beta}, the addition factor. 
	 */
	@Override
			...
			HashMap<String, Double> parameters) throws Exception 
	{
		...
		// if alpha or beta are set, use those values, otherwise use there defaults
	...
	{	
		...
		else
		{
			...
			if (AS=="ACGOODENOUGH"){
				// Use AC Good Enough
				// A more genreal AC Next: if the new bid of the opponent is better then any of the ones that we have bid ourselves
				// accept it.
				...
				for (BidDetails bidDetails : history) {
					if (lastOpponentBidUtil>= bidDetails.getMyUndiscountedUtil()) accept=true;
				}
				
 
				
				// Try Ac_next if still false
				if (!accept) accept = alpha * lastOpponentBidUtil + beta >= nextMyBidUtil;
				if (accept) acReason="AC good enough";
...
//			for (int j = 0; j < negotiationSession.getOpponentBidHistory().size(); j++)
...
//		{ 
//			// Accept if T > 1 - "average time of last 10 bids" + "standard deviation of timall bids"
//			boolean ACtime = (negotiationSession.getTimeline().getTotalTime()-negotiationSession.getTimeline().getCurrentTime())<metaStrategy.timing_10avg+metaStrategy.timing_sd*1D;

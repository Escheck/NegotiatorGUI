...
/**
 * This is the actual {@link OfferingStrategy} used.
 * It's called {@code Group2_BS} because in turn it calls different strategies, namely: 
 * {@link OpeningGameStrategy}, {@link MiddleGameStrategy}, {@link EndGameStrategy}. They are as there names suggest
 * called in that particular order. The first switch is made when the {@code foreplayCount} is reached (editable at the
 * top of the {@code Group2_BS.java} source file) and the second switch is made when the Kalai-Somorodisnky
 * point is reached, or more accurately: When the Kalai-Smorodinsky point plus hysteresis and offset are reached.
 * The values for the {@code hysteresis} and {@code offset} can also be found in this {@code Group2_BS.java}
 * source file. Note that these should have the same values as the ones in {@link EndGameStrategy}. 
 */
public class Group2_BS extends OfferingStrategy 
{
	...
	/** Either {@link OpeningGameStrategy}, {@link MiddleGameStrategy} or {@link EndGameStrategy} */
	...
	public Group2_BS(NegotiationSession negotiationSession, OpponentModel opponentModel, 
			OMStrategy opponentModelStrategy, double concessionFactor, double k, double maxTargetUtil, double minTargetUtil)
	{
	...
	/**
	 * I think this gets called on the first run. Just use the next bid as opening bid.
	 */
	@Override
	...
	/**
	 * Instead of directly determining next bid, this method only checks with it's {@code biddingStrategyState} what
	 * bid it should do. This function does determine the actual strategy used (hence the "Meta" in it's classname).
	 */
	@Override
	public BidDetails determineNextBid() 
	{
		try {
			...
			if (thisNumBids!=thisNumBids_now||thatNumBids!=thatNumBids_now)
			{
				...
				//timing_diffTime = (timer.get(thisNumBids_now)-timer.get(thisNumBids));
				timing_mean = SetMath.mean(timer);
				timing_sd = Math.sqrt(SetMath.variance(timer));
				timing_10avg = SetMath.avg(timer, 10);
				//System.out.println(String.format("bd(%4d) t=%.4f td=%.1fms (mean=%.1fms, sd=%.1fms, 10avg=%.1fms)",
			...
			// only do this if not in Displaying raw Exploration
				...
				if ((!switchedToMid) && (negotiationSession.getTime()>  (Math.pow( negotiationSession.getDiscountFactor(), 2 )*0.25)  ) )
				{

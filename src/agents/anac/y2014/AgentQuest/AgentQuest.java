...
public class AgentQuest extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			if (this.partnerAction instanceof Offer) {
				...
				// Maintain best Bid:
				if (this.myUtilitySpace
						.getUtilityWithDiscount(partnerBid, time) > this.myUtilitySpace
	...
	private void addOpponentBidToHistory(Bid partnerBid, double time) {
		if (this.opponentBidHistory.size() < this.MAX_BIDS_TO_STORE) {
			try {
				this.opponentBidHistory.add(new BidDetails(partnerBid,
						this.myUtilitySpace.getUtilityWithDiscount(partnerBid,
								time)));
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private void addBids(double time) {
		for (int i = 0; i < this.MAX_BIDS_TO_SAMPLE; i++) {
			...
			try {
				this.myBidHistory.add(new BidDetails(randomBid,
						this.myUtilitySpace.getUtilityWithDiscount(randomBid,
								time)));
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	private BidDetails getCounterOffer(double time) {
		...
		for (int row = 0; row < 10; row++) {
			...
			for (int col = 0; col < 10; col++) {
				if (col > 0) {
					if ((comparisonMatrix[row][col] < comparisonMatrix[row][col - 1])
	...
	private double computeEuclideanDistancePerBid(
			BidDetails opponentBidDetails, BidDetails myBidDetails, double time)
			throws Exception {
		...
		double myBidUtility;
		opponentBidUtility = this.myUtilitySpace.getUtilityWithDiscount(
				opponentBid, time);
	...
	private double getMyReservationValue(double time) {
		if (this.myUtilitySpace.getReservationValue() != null
				&& this.myUtilitySpace.getReservationValueWithDiscount(time) > 0.0d) {
	...
	private Offer genearteCounterOffer(Offer opponentOffer, double time) {
		...
		if (time > 0.0d) {
			...
			if (this.myUtilitySpace
					.getUtilityWithDiscount(this.myLastBid, time) > this.myUtilitySpace
					.getUtilityWithDiscount(this.myBidHistory
							.getBestBidDetails().getBid(), time)) {
				counterOffer = new Offer(this.myLastBid);
			} else {
				BidHistory tempBidHistory = this.myBidHistory
						.filterBetweenUtility(this.myUtilitySpace
				...
				if (tempBidHistory != null && tempBidHistory.size() > 0) {
					...
					counterOffer = new Offer(this.myLastBid);// retain last
																// offer.
				}
			}
			...
			try {
				...
				if ((this.hardlineCounter < this.alpha)
						&& (concessionProbability.compareTo(new BigDecimal(
								this.beta)) > 0)) {
					...
					if (bidUtility > this.myUtilitySpace
						...
						this.myLastBid = tempBid;
							...
							this.bestBid, time) >= this
							.getMyReservationValue(time)) {
					...
					if (this.counter > 1) // Not the first round.
					{
						...
						counterOffer = new Offer(this.myLastBid);// will execute
																	// just once
																	// i.e., for
																	// round 1.
					}
	...
	private boolean acceptOpponentOffer(Offer opponentOffer, double time) {
		...
		if (opponentOffer != null) {
			try {
				...
				myOfferUtility = this.myUtilitySpace.getUtilityWithDiscount(
						this.myLastBid, time);
				bestBidUtility = this.myUtilitySpace.getUtilityWithDiscount(
						this.bestBid, time);
	...
	public BigDecimal Pconcede(double u, double t1) throws Exception {
		...
		double value = (u - 2. * u * t + 2. * (-1. + t + Math.sqrt(sq(-1. + t)
				+ u * (-1. + 2 * t))))
				/ (-1. + 2 * t);

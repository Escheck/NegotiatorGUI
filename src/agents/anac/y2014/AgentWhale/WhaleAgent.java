...
public class WhaleAgent extends Agent {
	...
	public void endSession(NegotiationResult result) {
		...
		WhaleSessionData sessionData = new WhaleSessionData(
				result.getLastBid(), getBestBidOpponent(),
				result.getMyDiscountedUtility(), result.isAgreement(),
				bidLists, sumUtil, actionType);
	...
	public Action getBestBidByOppo(int type) {
		...
		if (best_util >= utilitySpace.getReservationValueWithDiscount(timeline
				.getTime()) && type == 1) {
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		if (opponentLastBid != null) {
			...
			double offeredUtility = utilitySpace.getUtilityWithDiscount(
					opponentLastBid, time);// discount‚³‚ê‚½‚à‚Ì‚ðŽæ“¾
	...
	public Action actionType1() {
		...
		try {
			...
			if (opponentLastBid != null
					&& utilitySpace.getUtility(opponentLastBid) >= MINIMUM_BID_UTILITY) {
				return new Accept();

			} else if (opponentLastBid != null
					&& utilitySpace.getUtility(opponentLastBid) >= MINIMUM_BID_UTILITY - 0.05) {
				// Œë�·‚ª0.1‚ÌŽž
				if (getRandomTrue(3)) {
					return new Accept();
				}
	...
	public Bid getNeighborBid(Bid bid) {
		try {
			...
			for (int i = 0; i < bid.getIssues().size(); i++) {
				...
				IssueInteger issueInteger = (IssueInteger) bid.getIssues().get(
						change_id);
				...
				Value valueInteger = new ValueInteger(issueValueInt + flag
						* step);// change value

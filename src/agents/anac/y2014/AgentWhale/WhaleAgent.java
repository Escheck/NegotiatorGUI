...
public class WhaleAgent extends Agent {
	...
	public void init() {
		...
		SAmax = utilitySpace.getUtilityWithDiscount(getBidSA(bid, 10000,0.999,0.90,0), 0.0);
		
		//System.out.println("**SAmax "+SAmax);
		setName("WhaleAgent");
		//Serializable prev = this.loadSessionData();// ‘O‰ñ‚ÌŠl“¾Œø—p’l
		//if (prev != null) {
		//		double previousOutcome = (Double) prev;
		//		MINIMUM_BID_UTILITY = Math.max(Math.max(utilitySpace.getReservationValueUndiscounted(),previousOutcome), 0.85);
		//} else {
		//		MINIMUM_BID_UTILITY = Math.max(utilitySpace.getReservationValueUndiscounted(), 0.85);
		//}
		//		
		offered_sum = 0.0;
		offered_count = 0;
	...
	// Žc‚èŽžŠÔ‚ðŒ©‚Ä Offer‚·‚é�@ƒrƒbƒh‚Ì�Å‘å’l‚ð‰º‚°‚é
		...
		if(prev != null){
			...
			prevwData = wData;
				...
				for (int i = 0; i < prevwData.bidLists.size(); i++) {				
					tmpMap.put(prevwData.bidLists.get(i), utilitySpace.getUtility(prevwData.bidLists.get(i)));
				}
				
				//Sort
				TreeMap<Bid, Double> treeMap = new TreeMap<Bid, Double>(new MapComparator(tmpMap));
			...
			if(wData.getUtil >= 0.9 ){
				//‘O‰ñ‚ÌUtility‚ª�‚‚¢Žž => ‚»‚ÌBid‚ð‚·‚®‚ÉOffer‚·‚é‚æ‚¤‚É‚·‚é�iDiscount‚ª‚ ‚é‚Æ‚«�j
				reOfferCount = 1;
				isFirstAction = true;
	...
	public void endSession(NegotiationResult result) {
		...
		WhaleSessionData sessionData = new WhaleSessionData(result.getLastBid(), getBestBidOpponent(),result.getMyDiscountedUtility(),result.isAgreement(), bidLists,sumUtil,actionType);
		...
		//		TreeMap<Bid, Double> treeMap = new TreeMap<Bid, Double>(new IntegerMapComparator(bestMaps));
	...
	public Action preOffer(Bid bid){
		...
		offeredMyBidMap.put(bid.toString(), utilitySpace.getUtilityWithDiscount(bid, timeline.getTime()));
	...
	public Bid getBestSeedBid(){
		...
		Bid bestBid = utilitySpace.getDomain().getRandomBid();
		//‰ß‹Ž‚ÌBid‚æ‚èˆê’è‚Ì’l‚æ‚è‘å‚«‚¢Bid‚ðƒ‰ƒ“ƒ_ƒ€‚ÅSeed‚Æ‚µ‚Ä�Ì—p
		for (Bid bid : offeredBidMap.keySet()) {
			double recent_util = utilitySpace.getUtilityWithDiscount(bid, timeline.getTime());
	...
	public HashMap<Bid, Double> getBestBids(int count){
		...
		//Sort
		TreeMap<Bid, Double> treeMap = new TreeMap<Bid, Double>(new MapComparator(offeredBidMap));
	...
	public Action getBestBidByOppo(int type) {
		...
		double best_util = utilitySpace.getUtilityWithDiscount(bestBid, timeline.getTime());
		...
		if(best_util >= utilitySpace.getReservationValueWithDiscount(timeline.getTime()) && type == 1){
			//‘ŠŽè‚ÌBestBid‚ðOffer‚·‚é
			flag_offer_bestbid_opponent = true;
			
			return preOffer(bestBid);
		}else{
			Bid bid = utilitySpace.getDomain().getRandomBid();// ƒ‰ƒ“ƒ_ƒ€‚ÉBid‚ð�¶�¬
...
//		}
	...
	public Bid getSeedBid() {
		...
		if(getRandomTrue(5)){
			...
			return utilitySpace.getDomain().getRandomBid();// ƒ‰ƒ“ƒ_ƒ€‚ÉBid‚ð�¶�¬
		}else{
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		opponentLastBid = Action.getBidFromAction(opponentAction);// ‘ŠŽè‚ÌlastBid
		
		if(opponentLastBid != null){
			
			//‘ŠŽè‚ÌOffer‚µ‚Ä‚«‚½Bid‚ðŽæ“¾‚µ�A•Û‘¶‚·‚é
			double time = timeline.getTime();//Œo‰ßŽžŠÔ‚ðŽæ“¾
			double offeredUtility = utilitySpace.getUtilityWithDiscount(opponentLastBid,time);//discount‚³‚ê‚½‚à‚Ì‚ðŽæ“¾
		...
		// System.out.println("+ReservationValue : "+utilitySpace.getReservationValue());
//		 System.out.println("++ opponet : "+getUtility(opponentLastBid));
		// TODO lastBid‚ÌŠˆ—p�@‚µ‚«‚¢’l‚ð‰º‚°‚½‚è‚·‚é
...
//			System.out.println(offerBidLists.size());
			//System.out.println("**offeredBidMap.size() = "+offeredBidMap.size());
			//System.out.println("**offered_count = "+offered_count);
			if(offered_count > 10 && offered_count > 2*offeredBidMap.size()){
...
//				System.out.println("**new first offer");
			...
			if (opponentLastBid != null && utilitySpace.getUtility(opponentLastBid) >= MINIMUM_BID_UTILITY) {
				return new Accept();
				
				
			}else if (opponentLastBid != null && utilitySpace.getUtility(opponentLastBid) >= MINIMUM_BID_UTILITY-0.05) {
				//Œë�·‚ª0.1‚ÌŽž
				if(getRandomTrue(3)){
					return new Accept();
				}
		...
		if(timeline.getTime() > 0.995){
			...
			if(util > utilitySpace.getReservationValueWithDiscount(timeline)){ //TODO:Resevation Value‚Æ”ä‚×‚é•K—v‚ ‚è
				return new Accept();
			}
			//Žc‚èŽžŠÔ‚ª�­‚È‚¢Žž
		...
		if(timeline.getTime() > 0.91){
			...
			double util = utilitySpace.getUtilityWithDiscount(getBestBidOpponent(), timeline.getTime());
			if(util < MINIMUM_BID_UTILITY){
				//MINIMUM_BID_UTILITY = util*1.05;//TODO:‚±‚±‚ðƒRƒ�ƒ“ƒgƒAƒEƒg‚·‚é‚Æ“¦‚°‚é“®‚«‚ð‚·‚é‚Ì‚Å‚¢‚¢‚©‚à�H
			}
			//Žc‚èŽžŠÔ‚ª�­‚È‚¢Žž
...
//			Bid bid = getSeedBid();//’T�õ‚ÌSeedBid‚ðŽæ“¾‚·‚é
...
//		System.out.println("hashmap_avg  : "+offered_sum/offeredBidMap.size());
		...
		if(timeline.getTime() > 0.99){
			double util = getUtility(opponentLastBid);
			if(util > utilitySpace.getReservationValue()){ //TODO:Resevation Value‚Æ”ä‚×‚é•K—v‚ ‚è
				return new Accept();
			}
			//Žc‚èŽžŠÔ‚ª�­‚È‚¢Žž
			return getBestBidByOppo(1);//‘Ã‹¦ƒ‚�[ƒh‚ÌŽž�H
...
//		return -4.0/(10+Math.exp(2.0/x))+1;
		...
		//
//		return (-1)/(1+Math.exp(-50*(x-1)))+1;//0.9•t‹ß‚©‚ç‹}�~‰º�@
		return (-1)/(1+Math.exp(-20*(x-1)))+1;//0.9•t‹ß‚©‚ç‹}�~‰º�@
//		return (Math.pow(x, 16)+Math.exp(10*x-15))/(Math.exp(-x)+Math.exp(x))+1;//x=0.8•t‹ß‚©‚ç‰º‚ª‚è�Ay=0.8‚­‚ç‚¢‚Ü‚Å‰º‚ª‚é
//		return Math.sin(x)*Math.cos(x)+Math.exp(-x)-0.05;//�Å�‰ŠÉ‚­�A�ÅŒãŒµ‚µ‚­Œ^
//		double result= Math.sin(x)*Math.cos(x)+Math.exp(-10*x+Math.log(10*x+0.02))+0.5;//‚Q‚Â‚Ìƒs�[ƒN‚ðŽ�‚ÂƒOƒ‰ƒt
...
//			int change_id = i;
			...
			IssueInteger issueInteger = (IssueInteger) bid.getIssues().get(change_id);
			int issueId = issueInteger.getNumber();
			ValueInteger issueValue = (ValueInteger) bid.getValue(issueId);
			int issueValueInt = Integer.valueOf(issueValue.toString()).intValue();
			...
			Value valueInteger = new ValueInteger(issueValueInt+flag*step);// change value
			
			bidChange.setValue(issueId, valueInteger);// change
	 ...
	 * @return
	 */
	public Action getBidLowSA(Bid bid, double T,double cool,double bias,int loop_count) {
...
//				System.out.println(rnd.nextDouble() < p);
				if ((af_cost < bf_cost || rnd.nextDouble() < p ) && af <= MINIMUM_BID_UTILITY_OFFER ) {
					bid = new Bid(bidChange);
					if(af >= bias){// TODO: ‚±‚ê‚Å‚¢‚¢‚Ì‚©�H
						ok_flag = true;
			...
			if(utilitySpace.getUtility(bid) < bias){
				...
				return getBidLowSA(bid, step, cool+t_cool, bias,loop_count+1);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
//			System.out.println("loop_count: "+loop_count);
	...
	public Bid getBidSA(Bid bid, double T,double cool,double bias,int loop_count) {
...
//			System.out.println("loop_count: "+loop_count);
	 ...
	 * @return
	 */
	public Action getBidHighClimb(Bid bid, int loop,double bias,int loop_count) {
		try {
			//T = 10000;
...
class WhaleSessionData implements Serializable{
	...
	Bid offeredBid;
	boolean isAgreement;
	public WhaleSessionData(Bid accesptedBid,Bid offeredBid,double getUtil,boolean isAgreement, ArrayList<Bid>bidLists,double sumUtil,
			int actionType) {

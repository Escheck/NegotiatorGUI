 ...
 public class SortedOutcomeSpace extends OutcomeSpace
 {
   ...
   public List<BidDetails> getBidsinRange(Range r)
   {
     ...
     if ((((BidDetails)this.allBids.get(upperboundIndex)).getMyUndiscountedUtil() <= r.getUpperbound()) && (upperboundIndex > 0)) {
       upperboundIndex--;
     }
 
     if ((((BidDetails)this.allBids.get(lowerboundIndex)).getMyUndiscountedUtil() >= r.getLowerbound()) && (lowerboundIndex < this.allBids.size())) {
       lowerboundIndex++;
     }
     ...
     ArrayList subList = new ArrayList(this.allBids.subList(upperboundIndex, lowerboundIndex));
     return subList;
   }
   ...
   public int getIndexOfBidNearUtility(double utility)
   {
     ...
     int newIndex = -1;
     double closestDistance = Math.abs(((BidDetails)this.allBids.get(index)).getMyUndiscountedUtil() - utility);
 
     if ((index > 0) && (Math.abs(((BidDetails)this.allBids.get(index - 1)).getMyUndiscountedUtil() - utility) < closestDistance)) {
       newIndex = index - 1;
       closestDistance = Math.abs(((BidDetails)this.allBids.get(index - 1)).getMyUndiscountedUtil() - utility);
     }
 
     if ((index + 1 < this.allBids.size()) && (Math.abs(((BidDetails)this.allBids.get(index + 1)).getMyUndiscountedUtil() - utility) < closestDistance)) {
       newIndex = index + 1;
       closestDistance = Math.abs(((BidDetails)this.allBids.get(index + 1)).getMyUndiscountedUtil() - utility);
     }
     else {

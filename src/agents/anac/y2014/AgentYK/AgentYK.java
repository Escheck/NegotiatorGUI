...
public class AgentYK extends Agent {
	...
	public void init()
	{
		...
		for(Issue issue:utilitySpace.getDomain().getIssues()) issueNrs.add(issue.getNumber());

		stopNr = 1 + (int)Math.pow(Math.log(Math.sqrt(totalCombiNum)), 2);
		if(stopNr > 300) stopNr = 300;
		if(stopNr < 100) stopNr = 100;
		seedNr = 1 + (int)logTCN;
		if(seedNr > 30) seedNr = 30;
		if(seedNr < 10) seedNr = 10;
		try {
	...
	public Action chooseAction() {
		...
		try {
			if(actionOfPartner==null) action = new Offer(getAgentID(), myMinBid);
			if(actionOfPartner instanceof Offer) {
				...
				if(isAccept(offeredBid)) {
					...
					if(0 <= leftTime && leftTime < (5+(double)totalTime/30) * FaceOfBuddha && myMinBid != null) {
						FaceOfBuddha--;
						...
						if(FaceOfBuddha == 0) {
							stopNr /= 2;
							seedNr /= 2;
							noValueUtil = (utilitySpace.getUtility(myMinBid) + utilitySpace.getUtility(oppMaxBid)) / 2;
							MINIMUM_BID_UTILITY = utilitySpace.getUtility(oppMaxBid);
						}
					}
					if(0 <= leftTime && leftTime < 2 && myMinBid != null) {
						double diffMyOpp = utilitySpace.getUtility(myMinBid) - utilitySpace.getUtility(oppMaxBid);
						if(diffMyOpp <= 0.25 || utilitySpace.getUtility(myMinBid)/2 <= utilitySpace.getUtility(oppMaxBid)) {
							myBidManager(oppMaxBid);
							if(utilitySpace.getReservationValue() >= utilitySpace.getUtility(oppMaxBid)) return new EndNegotiation(getAgentID());
					...
					if(utilitySpace.getUtility(nextBid) < MINIMUM_BID_UTILITY) nextBid = new Bid(myHistory.getRandom().getBid());
					if(utilitySpace.getUtility(nextBid) < utilitySpace.getUtility(oppMaxBid)) nextBid = new Bid(oppMaxBid);
					if(myBidPool.contains(nextBid.toString())) nextBid = new Bid(myHistory.getRandom().getBid());
					myBidManager(nextBid);
					refreshHistory();
					action = new Offer(getAgentID(), nextBid);
					if(utilitySpace.getReservationValue() >= utilitySpace.getUtility(myMinBid)) action = new EndNegotiation(getAgentID());
				}
			}
		} catch(Exception e) {
			action = new Accept(getAgentID());
	...
	private void refreshHistory() throws Exception {
		if(now > coefRefresh*termRefresh || (1 <= coefRefresh*termRefresh && coefRefresh*termRefresh < 1.2 && now > 0.9) || opponentHistory.size() + preRefresh > 1 + Math.pow(logTCN,  2)) {
			List<BidDetails> oppList = new ArrayList<BidDetails>(opponentHistory.filterBetweenUtility(oppAverage, 1.0).getHistory());
			List<BidDetails> myList = new ArrayList<BidDetails>(myHistory.filterBetweenUtility(0.0, myAverage).getHistory());
			opponentBidElementHistory = new BidElementHistory();
			opponentPairBidElementHistory = new PairBidElementHistory();
			myBidElementHistory = new BidElementHistory();
			myPairBidElementHistory = new PairBidElementHistory();
			opponentHistory = new BidHistory();
			myHistory = new BidHistory();
			HashSet<String> tmpOppBidPool = new HashSet<String>(opponentBidPool);
			...
			for(String str:tmpOppBidPool) {
				if(!opponentBidPool.contains(str)) opponentBidPool.add(str);
			}
			for(String str:tmpMyBidPool) {
				if(!myBidPool.contains(str)) myBidPool.add(str);
			}
			coefRefresh++;
	...
	private void opponentBidManager(Bid oppBid) throws Exception {
		...
		if(!opponentBidPool.contains(oppBid.toString())) {
			...
			if(opponentHistory.isEmpty()) {
				opponentHistory.add(new BidDetails(oppBid, utilitySpace.getUtility(oppBid), now));
				while(bei.hasNext()) opponentBidElementHistory.add(new BidElementDetails(bei.next(), now));
				while(pbei.hasNext()) opponentPairBidElementHistory.add(new PairBidElementDetails(pbei.next(), now));
			} else {
				oppAverage = 0.0;
		...
		if(basedBE > 0 && basedPBE > 0 && oppAverage <= utilitySpace.getUtility(oppBid)) {
			double additional = 0.0;
			bei = new BidElementIterator(oppBid);
			sum = 0;
			while(bei.hasNext()) sum += myBidElementHistory.getWeightedAppearanceCount(bei.next());
			additional += ((double)sum / basedBE);
			pbei = new PairBidElementIterator(oppBid);
			sum = 0;
			while(pbei.hasNext()) sum += myPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
			additional += ((double)sum / basedPBE);
			additional *= (Math.pow((now/df), 2) > 1 ? 1 : Math.pow((now/df), 2));
		...
		} else if(utilitySpace.getUtility(oppMaxBid) < utilitySpace.getUtility(oppBid)) {
			oppMaxBid = new Bid(oppBid);
		}
		bei = new BidElementIterator(oppMaxBid);
		sum = 0;
		while(bei.hasNext()) sum += opponentBidElementHistory.getWeightedAppearanceCount(bei.next());
		baseBE = sum;
		pbei = new PairBidElementIterator(oppMaxBid);
		sum = 0;
		while(pbei.hasNext()) sum += opponentPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
		basePBE = sum;
	}
	...
	private void myBidManager(Bid myBid) throws Exception {
		...
		if(!myBidPool.contains(myBid.toString())) {
			...
			if(myHistory.isEmpty()) {
				myHistory.add(new BidDetails(myBid, utilitySpace.getUtility(myBid), now));
				while(bei.hasNext()) myBidElementHistory.add(new BidElementDetails(bei.next(), now));
				while(pbei.hasNext()) myPairBidElementHistory.add(new PairBidElementDetails(pbei.next(), now));
			} else {
				myAverage = 0.0;
		...
		if(utilitySpace.getUtility(myMinBid) > utilitySpace.getUtility(myBid)) {
		...
		for(BidDetails bd:myHistory.getHistory()) {
			...
			while(bei.hasNext()) basedBE += myBidElementHistory.getWeightedAppearanceCount(bei.next());
			pbei = new PairBidElementIterator(bd.getBid());
			while(pbei.hasNext()) basedPBE += myPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
		}
	...
	private boolean isAccept(Bid offeredBid) throws Exception{
		...
		if(offeredUtil >= utilitySpace.getUtility(myMinBid)*0.999) return true;
	...
	private Bid getHillClimbBid(int stopNr) throws Exception{
		...
		while(c++ < stopNr && timeline.getTime() - now < 0.01 && timeline.getTime() < 1.0) {
	...
	private Bid nextBid(Bid bid, int changeNum) {
		...
		for(Integer issueNumber:changeIssueNrs) {
			...
			switch(domain.getIssue(issueN).getType()) {
			case DISCRETE:
				IssueDiscrete lIssueDiscrete = (IssueDiscrete)domain.getIssue(issueN);
				int optionIndex=randomnr.nextInt(lIssueDiscrete.getNumberOfValues());
				nextBid.setValue(issueNumber, lIssueDiscrete.getValue(optionIndex));
				break;
			case REAL:
				IssueReal lIssueReal = (IssueReal)domain.getIssue(issueN);
				int optionInd = randomnr.nextInt(lIssueReal.getNumberOfDiscretizationSteps()-1);
				nextBid.setValue(issueNumber, new ValueReal(lIssueReal.getLowerBound() + (lIssueReal.getUpperBound()-lIssueReal.getLowerBound())*(double)(optionInd)/(double)(lIssueReal.getNumberOfDiscretizationSteps())));
				break;
			case INTEGER:
				IssueInteger lIssueInteger = (IssueInteger)domain.getIssue(issueN);
				int optionIndex2 = lIssueInteger.getLowerBound() + randomnr.nextInt(lIssueInteger.getUpperBound()-lIssueInteger.getLowerBound());
				nextBid.setValue(issueNumber, new ValueInteger(optionIndex2));
				break;
			default:
			}
		}
	...
	private double evaluateBid(Bid bid) throws Exception {
		...
		if(baseBE > 0 && basePBE > 0) {
			...
			double sum = 0;
			while(bei.hasNext()) sum += opponentBidElementHistory.getWeightedAppearanceCount(bei.next());
			additional += ((double)sum / baseBE);
			PairBidElementIterator pbei = new PairBidElementIterator(bid);
			sum = 0;
			while(pbei.hasNext()) sum += opponentPairBidElementHistory.getWeightedAppearanceCount(pbei.next());
			additional += ((double)sum / basePBE);
			additional *= ((now/df) > 1 ? 1 : (now/df));
			if(additional > (1+(double)(3-FaceOfBuddha)/8)*eval) additional = (1+(double)(3-FaceOfBuddha)/8)*eval;
			if(MINIMUM_BID_UTILITY <= eval && eval < noValueUtil) eval += (allowedAdditional < additional ? allowedAdditional : additional);
		}

		if(timeBonus && !myBidPool.contains(bid.toString()) && myBidPool.size() <= opponentBidPool.size()) eval += now;
	...
	private Bid getRandomBid() throws Exception
	{
		...
		for(Issue lIssue:issues)
		{
			...
			case INTEGER:
				...
				break;
			default: throw new Exception("issue type "+lIssue.getType()+" not supported.");
			}
		}
	...
	private long getTotalCombinationNumber() {
		...
		try {
			...
			for(Issue lIssue:issues) {
				...
				case INTEGER:
					IssueInteger lIssueInteger = (IssueInteger)lIssue;
					c *= (lIssueInteger.getUpperBound() - lIssueInteger.getLowerBound() + 1);
					break;
				default:
					break;
				}

...
public class AgentYK extends Agent {
				...
				if (isAccept(offeredBid)) {
					...
					if (0 <= leftTime
						...
						if (FaceOfBuddha == 0) {
							stopNr /= 2;
							seedNr /= 2;
							noValueUtil = (utilitySpace.getUtility(myMinBid) + utilitySpace
									.getUtility(oppMaxBid)) / 2;
					...
					if (0 <= leftTime && leftTime < 2 && myMinBid != null) {
						...
						if (diffMyOpp <= 0.25
								|| utilitySpace.getUtility(myMinBid) / 2 <= utilitySpace
										.getUtility(oppMaxBid)) {
	...
	private void refreshHistory() throws Exception {
		if (now > coefRefresh * termRefresh
				...
				|| opponentHistory.size() + preRefresh > 1 + Math
						.pow(logTCN, 2)) {
			List<BidDetails> oppList = new ArrayList<BidDetails>(
					opponentHistory.filterBetweenUtility(oppAverage, 1.0)
							.getHistory());
	...
	private void opponentBidManager(Bid oppBid) throws Exception {
		...
		if (!opponentBidPool.contains(oppBid.toString())) {
			...
			if (opponentHistory.isEmpty()) {
				opponentHistory.add(new BidDetails(oppBid, utilitySpace
						.getUtility(oppBid), now));
				while (bei.hasNext())
					opponentBidElementHistory.add(new BidElementDetails(bei
							.next(), now));
				while (pbei.hasNext())
		...
		if (basedBE > 0 && basedPBE > 0
			...
			sum = 0;
			while (bei.hasNext())
				sum += myBidElementHistory.getWeightedAppearanceCount(bei
						.next());
			additional += ((double) sum / basedBE);
			pbei = new PairBidElementIterator(oppBid);
			sum = 0;
			while (pbei.hasNext())
				sum += myPairBidElementHistory.getWeightedAppearanceCount(pbei
						.next());
			additional += ((double) sum / basedPBE);
			additional *= (Math.pow((now / df), 2) > 1 ? 1 : Math.pow(
					(now / df), 2));
		...
		sum = 0;
		while (bei.hasNext())
			sum += opponentBidElementHistory.getWeightedAppearanceCount(bei
					.next());
		baseBE = sum;
	...
	private void myBidManager(Bid myBid) throws Exception {
		...
		if (!myBidPool.contains(myBid.toString())) {
			...
			if (myHistory.isEmpty()) {
				myHistory.add(new BidDetails(myBid, utilitySpace
						.getUtility(myBid), now));
				while (bei.hasNext())
					myBidElementHistory.add(new BidElementDetails(bei.next(),
							now));
				while (pbei.hasNext())
					myPairBidElementHistory.add(new PairBidElementDetails(pbei
							.next(), now));
			} else {
				myAverage = 0.0;
		...
		for (BidDetails bd : myHistory.getHistory()) {
			...
			while (bei.hasNext())
				basedBE += myBidElementHistory.getWeightedAppearanceCount(bei
						.next());
	...
	private Bid nextBid(Bid bid, int changeNum) {
		...
		for (Integer issueNumber : changeIssueNrs) {
			...
			case REAL:
				...
				break;
			case INTEGER:
				IssueInteger lIssueInteger = (IssueInteger) domain
						.getIssue(issueN);

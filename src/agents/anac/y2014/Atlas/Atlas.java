...
//	PrintWriter pw_my_b_c;
	...
	{			
		try {
			...
			if(sessionNr == 0){
				Bid check_b;
				for(int i=0; i < utilitySpace.getDomain().getIssues().size() * 10; i++){
					check_b = getBestBidbySA();
					if(max_bid == null || utilitySpace.getUtility(check_b) >= utilitySpace.getUtility(max_bid)){
	...
	public Action chooseAction()
	{
		...
		try 
		{ 
			if(actionOfPartner==null) action = chooseBidAction();
			if(actionOfPartner instanceof Offer)
			{
				...
				if(partner_bid_count == 0){
					partner_bid_utility_weighted_average = utilitySpace.getUtility(partnerBid);
				}
				else{
					partner_bid_utility_weighted_average = (utilitySpace.getUtility(partnerBid) + partner_bid_utility_weighted_average) / 2;
				}
				// ‘ŠŽè‚ÌBid‚µ‚½Žž��‚ð‹L˜^‚µ‚Ä‚¨‚­
				partner_prev_last_bid_time = partner_last_bid_time;
				partner_last_bid_time = timeline.getTime();
				time_scale = (((partner_last_bid_time - partner_prev_last_bid_time) + time_scale * partner_bid_count) / (partner_bid_count + 1));
				last_time = 1.0 - time_scale * ERROR_TIME_SCALE_LIM;
				partner_bid_count++;
				
				// get current time
				double time = timeline.getTime();
				double offeredUtilFromOpponent = utilitySpace.getUtility(partnerBid);
...
//				pw_pa_u.println();
				...
				else if(utilitySpace.getUtility(partnerBid) > utilitySpace.getUtility(best_partner_bid)){
					best_partner_bid = new Bid(partnerBid);
				}
				...
				if (isAcceptable(partnerBid, offeredUtilFromOpponent, myOfferedUtil, time))
...
//			}
	...
	private boolean isAcceptable(Bid partnerBid, double offeredUtilFromOpponent, double myOfferedUtil, double time) throws Exception
	{		
		// ‘ŠŽè‚ÌŒø—p’l
		...
		if (P >= border_bid_utility && P >= utilitySpace.getUtility(best_partner_bid)){
			return true;
		} 
		
		// ‘¦Œˆ”»’è
		if(!tension && false_best_agreement_bid != null && P >= utilitySpace.getUtility(false_best_agreement_bid)){
			// ‚±‚Ì‚Ü‚ÜŒð�Â‚ð‘±‚¯‚½�ê�‡
			double parameter  = partner_guard_num + partner_attack_num;
			// —˜“¾(‘ŠŽè‚ªŽã‹C‚ÌŽž)
			double get_point =  utilitySpace.getDiscountFactor() - utilitySpace.getUtility(false_best_agreement_bid);
			// ‘¹Ž¸(‘ŠŽè‚ª‹­‹C‚ÌŽž)
			double lost_point = false_agreement_average_utility_with_discount - utilitySpace.getUtility(false_best_agreement_bid);

			// Šú‘Ò’l(‘ŠŽè‚ÌŽp�¨‚Í‹­‹CorŽã‹C‚Å‚»‚ê‚¼‚ê‚ÌŠm—¦‚Í‰ß‹Ž‚ÌŒð�Â‚ðŽQ�Æ‚·‚é)
			double mean_point = get_point * ((partner_guard_num + 0.5)/(parameter + 1)) + lost_point * ((partner_attack_num + 0.5)/(parameter + 1));
...
//				first_offer_decision_flag = true;
		...
		// ”ñ�í—p
		if (!tension && timeline.getTime() > (last_time - time_scale * 0) && P > utilitySpace.getReservationValue()){
			attack_flag = true;
			return true;
		} 
	...
	{		
		...
		if(!tension){
			...
			if(timeline.getTime() > (last_time - time_scale * 2) && best_partner_bid != null && false_best_agreement_bid != null){
				attack_flag = true;
				// �¬‚³‚¢•û
				if(timeline.getTime() > (last_time - time_scale * 1)){
					if(utilitySpace.getUtility(false_best_agreement_bid) < utilitySpace.getUtility(best_partner_bid)){
						if(utilitySpace.getUtility(false_best_agreement_bid) > utilitySpace.getReservationValue()){
							return false_best_agreement_bid;
						}
					}
					else{ 
						if(utilitySpace.getUtility(best_partner_bid) > utilitySpace.getReservationValue()){
			...
			if(timeline.getTime() > (last_time - time_scale * 1) && !tension && false_best_agreement_bid == null && best_partner_bid != null && utilitySpace.getUtility(best_partner_bid) > utilitySpace.getReservationValue()){
				attack_flag = true;
				return best_partner_bid;
			}
			if(timeline.getTime() > (last_time - time_scale * 1) && !tension && best_partner_bid == null && false_best_agreement_bid != null && utilitySpace.getUtility(false_best_agreement_bid) > utilitySpace.getReservationValue() ){
				attack_flag = true;
				return false_best_agreement_bid;
			}
		}
...
//		pw_my_b_c.println();
		...
		if(prev_partner_bid != null || last_bid != null){
			...
			catch (Exception e) { System.out.println("Problem with received bid(Near:last):"+e.getMessage()+". cancelling bidding"); }
		}
		
		// ‹ß–T’T�õ‚ÌŒ‹‰Ê�C–ž‘«‚Ì‚¢‚­Bid‚ð“¾‚ç‚ê‚È‚©‚Á‚½�ê�‡�CSA‚ÅBid‚ð’T�õ
		if(bid == null || utilitySpace.getUtility(bid) < border_bid_utility || (tension && utilitySpace.getUtility(bid) < utilitySpace.getUtility(max_bid))){
			try{
				for(int i=0; i < SA_ITERATION; i++){
					if(tension){
						//‹­‹C
						check_bid = getBestBidbySA();
						if(bid == null || utilitySpace.getUtility(check_bid) > utilitySpace.getUtility(bid)){
						...
						if(bid == null || (utilitySpace.getUtility(bid) < border_bid_utility || (utilitySpace.getUtility(check_bid) < utilitySpace.getUtility(bid)) && utilitySpace.getUtility(check_bid) > border_bid_utility)){
							bid = new Bid(check_bid);
						}
					}
				}
			}
			catch (Exception e) { System.out.println("Problem with received bid(SA):"+e.getMessage()+". cancelling bidding"); }
		}
		
		// �Å‘åBid‚Ì�X�V
		if(bid != null && utilitySpace.getUtility(max_bid) <= utilitySpace.getUtility(bid)){
			max_bid = new Bid(bid);	
		}
		...
		if(last_bid != null){
			// ‹­‹C‚Ì˜A�Ÿ”»’è
			if(true_best_agreement_bid != null && tension && first_offer_flag){
				return true_best_agreement_bid;
			}
			// �ÄŒ»”»’è
			if(repeat_flag && utilitySpace.getDiscountFactor() < 1.0){
				// ‹­‹C‚Ì�ÄŒ»”»’è
				if(tension && true_best_agreement_bid != null && true_repeat_time <= timeline.getTime()){
					repeat_flag = false;
					return true_best_agreement_bid;
				}
				
				// Žã‹C‚Ì�ÄŒ»”»’è
				if(!tension && false_best_agreement_bid != null && false_repeat_time <= timeline.getTime()){
					repeat_flag = false;
					return false_best_agreement_bid;
				}
			}
...
//		pw_my_u.println();
	...
	private void getBorderUtility() throws Exception{
		...
		// ‹«ŠE’l‚Ì�X�V
		border_bid_utility = timeline.getTime() * (1.0 - utilitySpace.getDiscountFactor()) + utilitySpace.getDiscountFactor();	
		
		// Discount‚ª�‚‚¢‚Æ‚«‚É�÷•à‚µ‚·‚¬‚é‚±‚Æ‚ð–h‚®‚½‚ß‚É�CŠeŽp�¨‚Ì1�í–Ú‚ÍŠî–{‹«ŠE’l‚ð‚Æ‚ç‚È‚¢‚æ‚¤‚É‚·‚é
		if((tension && true_failure_num == 0)||(!tension && false_agreement_num == 0)){
...
//		pw_my_b.println();
		...
		if(tension){
			...
			if(true_best_agreement_bid != null && border_bid_utility < utilitySpace.getUtility(true_best_agreement_bid)){
				border_bid_utility = utilitySpace.getUtility(true_best_agreement_bid);
			}
		}
		else{
			// Žã‹C‚Ì�ê�‡
			...
			if(SAMPLE_NUM < partner_bid_count){
				compensate = (prev_offeredUtilFromOpponent - partner_bid_utility_weighted_average) * prev_offeredUtilFromOpponent * (1.0 - utilitySpace.getReservationValue());
				
				// ŽžŠÔŒo‰ß‚Æ‘ŠŽè‚Ì�Å—ÇBid‚É‚æ‚é�÷•à‚ð—˜—p‚µ‚½•â�³‚ð“K—p
				if(best_partner_bid != null){
					time_compensate = timeline.getTime() * (border_bid_utility - utilitySpace.getUtility(best_partner_bid)) * utilitySpace.getUtility(best_partner_bid) * (1.0 - utilitySpace.getReservationValue());
				...
				if(compensate * (1.0 + time_compensate) > time_compensate && false_agreement_num > 0){
					border_bid_utility = border_bid_utility - compensate * (1.0 + time_compensate);
				}
				else{
				...
				if(timeline.getTime() > 0.90){
					border_bid_utility = border_bid_utility - 10 * (timeline.getTime() - 0.90) * (border_bid_utility - utilitySpace.getUtility(best_partner_bid));
				}
			}
		}
			
		// ‚à‚µ�C‹«ŠE�ü‚ª�Å—Ç’ñŽ¦Bid‚ÌŒø—p’l‚æ‚è’á‚¢Žž�C‹«ŠE’l‚ð‚»‚ÌŒø—p’l‚É‚·‚é
		if(false_best_agreement_bid != null && border_bid_utility < utilitySpace.getUtility(false_best_agreement_bid)){
			border_bid_utility = utilitySpace.getUtility(false_best_agreement_bid);
		}
		
		// ‚à‚µ�C‹«ŠE�ü‚ªŒ»�Ý‚ÌŒð�Â‚Ì�Å—Ç’ñŽ¦Bid‚ÌŒø—p’l‚æ‚è’á‚¢Žž�C‹«ŠE’l‚ð‚»‚ÌŒø—p’l‚É‚·‚é
		if(best_partner_bid != null && border_bid_utility < utilitySpace.getUtility(best_partner_bid)){
...
//		pw_my_b_t.println();
	...
	private Bid getBestBidbySA() throws Exception
	{
		...
		// �‰Šú‰ð‚Ì�¶�¬
        if(prev_partner_bid == null){
        	current_b = utilitySpace.getDomain().getRandomBid();
        }else{
		...
		do{
			...
			while(temperature > 0.0001){				
				...
				ArrayList<Issue> issues = utilitySpace.getDomain().getIssues(); // ‘Sissue‚ÌŽæ“¾

				// ‹ß–T‚ÌBid‚ðŽæ“¾‚·‚é
				for (int i = 0; i < step_num; i++) {
					int issueIndex = randomnr.nextInt(issues.size()); // issue‚Ì”ÍˆÍ“à‚Åindex‚ðƒ‰ƒ“ƒ_ƒ€‚ÉŽw’è
					IssueInteger lIssueInteger = (IssueInteger) issues.get(issueIndex); // Žw’è‚µ‚½index‚Ìissue
					...
					ValueInteger issueValue = (ValueInteger) next_b.getValue(issueNumber); // ‘I‚ñ‚¾issue‚É‘Î‰ž‚·‚évalue
					int issueValueInt = Integer.valueOf(issueValue.toString()).intValue(); // ‘I‚ñ‚¾issue‚É‘Î‰ž‚·‚évalue(integer)
					...
					next_b.setValue(issueNumber, new ValueInteger(next_v)); // ‹ß–T‚ÌBid‚ð‹�‚ß‚é
				}
		...
		} while(utilitySpace.getUtility(bid) < utilitySpace.getReservationValue());	
	...
	{	
		...
		for(int i=0;i<lim;i++){
			...
			for(int j=min;j<=max;j++){
				current_b.setValue(issueNumber, new ValueInteger(j)); // ‹ß–T‚ÌBid‚ð‹�‚ß‚é
				current_u = utilitySpace.getUtility(current_b);
...
//		csv�o—Í—p
//		fw = new FileWriter("/Users/Mori/Program/R/anac/Atlas_Session_mu"+ sessionNr + ".csv", false);
//		pw_my_u = new PrintWriter(new BufferedWriter(fw));
//		// ‘ŠŽè‚Ì’ñŽ¦‚µ‚½Bid‚ÌŒø—p’l
//		fw_p = new FileWriter("/Users/Mori/Program/R/anac/Atlas_Session_Data_pu" + sessionNr + ".csv", false);
//		pw_pa_u = new PrintWriter(new BufferedWriter(fw_p));
//		// Ž©•ª‚Ì’ñŽ¦‚µ‚½Bid‚Ì‹«ŠE�ü
//		fw_b = new FileWriter("/Users/Mori/Program/R/anac/Atlas_Session_Data_mb" + sessionNr + ".csv", false);
//		pw_my_b = new PrintWriter(new BufferedWriter(fw_b));
//		// Ž©•ª‚Ì’ñŽ¦‚µ‚½Bid‚Ì�d‚Ý•t‚«•â�³�Ï‚Ý‹«ŠE�ü
//		fw_b_t = new FileWriter("/Users/Mori/Program/R/anac/Atlas_Session_Data_mbt" + sessionNr + ".csv", false);
//		pw_my_b_t = new PrintWriter(new BufferedWriter(fw_b_t));
//		// Ž©•ª‚Ì’ñŽ¦‚µ‚½Bid‚Ì•â�³�Ï‚Ý
//		fw_b_c = new FileWriter("/Users/Mori/Program/R/anac/Atlas_Session_Data_mbc" + sessionNr + ".csv", false);
		...
		if(sessionNr > 0 && prev != null){
			...
			if(is_agreement && last_tension){
				...
				if(false_best_agreement_bid == null){
					...
					double get_utility_true   = (utilitySpace.getDiscountFactor() - false_agreement_average_utility_with_discount) * (sessionsTotal - (sessionNr));
					// ‘¹Ž¸�i‹­‹C‚ÅŽ¸”s‚µ‚½‚Æ‚«‚ÉŽ¸‚¤Œø—p’l�i1‰ñ•ª�j�j
					double lost_utility_true  = utilitySpace.getReservationValueWithDiscount(1.0) - false_agreement_average_utility_with_discount;
					// Šú‘Ò’l(‘ŠŽè‚ÌŽp�¨‚Í‹­‹CorŽã‹C‚Å‚»‚ê‚¼‚ê‚ÌŠm—¦‚Í‰ß‹Ž‚ÌŒð�Â‚ðŽQ�Æ‚·‚é)
					double utility_true = get_utility_true * ((partner_guard_num) / (parameter)) + lost_utility_true * ((partner_attack_num) / (parameter));
					// Šú‘Ò’l‚É‹­‹C‚Å‚ÌŽ¸”s‚ÌŽÀ�Ñ’l‚ð”½‰f
					utility_true += (utilitySpace.getReservationValueWithDiscount(1.0) - false_agreement_average_utility_with_discount) * true_failure_num;
			...
			if(sessionNr > 1 && utilitySpace.getDiscountFactor() < 1.0 && true_failure_num == 0){
				tension = true;
			}
...
//			pw_my_b_c.close();
			...
			if(isAgreement){
				...
				if(tension){
					...
					if(true_best_agreement_bid == null || utilitySpace.getUtility(lastBid) >= utilitySpace.getUtility(true_best_agreement_bid)){
					...
					if(!first_offer_decision_flag){
						...
						if(attack_flag){
							...
							if(false_best_agreement_bid != null){
								...
								if(utilitySpace.getUtility(lastBid) < utilitySpace.getUtility(false_best_agreement_bid) && ((tension && last_tension) || (!tension && !last_tension))){
									partner_attack_num++;
								}
						...
						// �‡ˆÓ‚µ‚½�Å‘åŒø—p’lBid‚ð‹L˜^
						if(false_best_agreement_bid == null || utilitySpace.getUtility(lastBid) >= utilitySpace.getUtility(false_best_agreement_bid)){
						...
						if(utilitySpace.getUtilityWithDiscount(lastBid, timeline.getTime()) > false_agreement_best_utility){
							false_agreement_best_utility = utilitySpace.getUtilityWithDiscount(lastBid, timeline.getTime());
						}
												
						// •½‹Ï’l‚ð‹L˜^
						false_agreement_average_utility_with_discount = ((false_agreement_average_utility_with_discount * false_agreement_num) + utilitySpace.getUtilityWithDiscount(lastBid, timeline.getTime())) / (false_agreement_num + 1);
						false_agreement_num++;
												
						// ‹­‹C••ˆóƒtƒ‰ƒO‚ÌŠm’è
						if(utilitySpace.getDiscountFactor() < utilitySpace.getUtilityWithDiscount(lastBid, timeline.getTime()) && utilitySpace.getDiscountFactor() < 1.0 && utilitySpace.getUtility(false_best_agreement_bid) < 1.0){
			...
			MySessionData mySessionData = new MySessionData(tension, isAgreement, lastBid, max_bid, true_best_agreement_bid, false_best_agreement_bid, 
					true_failure_num, false_agreement_num, false_agreement_average_utility_with_discount, partner_attack_num, partner_guard_num,
					last_time, timeline.getTime(), tension_true_seal_flag, true_repeat_time, false_repeat_time, false_agreement_best_utility);
...
class MySessionData implements Serializable {
	...
	public MySessionData(Boolean tension, Boolean isAgreement, Bid lastBid, Bid max_bid, Bid true_best_agreement_bid, Bid false_best_agreement_bid,
		int true_failure_num, int false_agreement_num, double false_agreement_average_utility_with_discount,  double partner_attack_num, double partner_guard_num,
		double last_time, double last_bid_time, boolean tension_true_seal_flag, double true_repeat_time, double false_repeat_time, double false_agreement_best_utility){

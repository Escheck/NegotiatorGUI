...
public class Simpatico extends Agent {
		...
		try {
			for (int i = 0; i < issuesToChange.length; i++) {
				...
				case DISCRETE:
					...
					break;
				case REAL:
					IssueReal issueReal = (IssueReal) initialBid.bid
							.getIssues().get(issuesToChange[i]);
				...
				case INTEGER:
					...
					break;
				default:
					throw new Exception("issue type not supported by "
							+ getName());
				}
			}
		} catch (Exception e) {
	...
	private BidTuple getDiscreteIssueVariation(BidTuple bid, int issueId,
		...
		IssueDiscrete issueDiscrete = (IssueDiscrete) tmpBid.getIssues().get(
				issueId);
		...
		if (issueDiscrete.getNumberOfValues() >= temIssueIndex + variation
			...
			ValueDiscrete value = issueDiscrete.getValue(temIssueIndex
					+ variation);
	...
	private BidTuple getIntegerIssueVariation(BidTuple bid, int issueId,
		...
		Bid tmpBid = new Bid(bid.bid);
		BidTuple bidTupleTmp = null;
		IssueInteger issueInteger = (IssueInteger) tmpBid.getIssues().get(
				issueId);
		int temIssueValue = ((ValueInteger) tmpBid.getValue(issueId))
				.getValue();

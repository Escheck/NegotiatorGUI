...
public class Simpatico extends Agent
{
	...
	private int bidToSubmitBack = 0; // used in the last moments, we propose back the best of the opponents bids
	...
	{					
		...
		bidToSubmitBack = 0; // used in the last moments, we propose back the best of the opponents bids
	...
	private BidTuple SearchNeighbourhood(BidTuple initialBid, int depth)
	{
		...
		int numberOfIssuesToChange = (int)(nIssues*percentageOfIssuesToChange);
		if(numberOfIssuesToChange > 5) numberOfIssuesToChange = 5;
		...
		try
		{
			for(int i = 0; i < issuesToChange.length; i++)
			{
				...
				switch(initialBid.bid.getIssues().get(issuesToChange[i]).getType()) {
				case DISCRETE:
					bidTupleTmp = getDiscreteIssueVariation(initialBid, issuesToChange[i], 1);
					if(bidTupleTmp!=null)
					{
						if(bidTupleTmp.utility>bestBid.utility) bestBid = bidTupleTmp;
						BidTuple returnedBid = SearchNeighbourhood(bidTupleTmp, depth-1);
						if(returnedBid.utility>bestBid.utility) bestBid = returnedBid;
					}
					...
					break;
				case REAL:
					IssueReal issueReal = (IssueReal)initialBid.bid.getIssues().get(issuesToChange[i]);
					double range = issueReal.getUpperBound()-issueReal.getLowerBound();
					double variation = range * 0.05;
					
					bidTupleTmp = getRealIssueVariation(initialBid, issuesToChange[i], rand.nextFloat()*variation);
					if(bidTupleTmp!=null)
					{
						if(bidTupleTmp.utility>bestBid.utility) bestBid = bidTupleTmp;
						BidTuple returnedBid = SearchNeighbourhood(bidTupleTmp, depth-1);
					...
					break;
				case INTEGER:
					bidTupleTmp = getIntegerIssueVariation(initialBid, issuesToChange[i], 1);
					if(bidTupleTmp!=null)
					{
						if(bidTupleTmp.utility>bestBid.utility) bestBid = bidTupleTmp;
						BidTuple returnedBid = SearchNeighbourhood(bidTupleTmp, depth-1);
					...
					break;
				default: throw new Exception("issue type not supported by " + getName());
				}
			}
		}
		catch (Exception e) 
	...
	private void saveBestOpponentBids(BidTuple bid)
	{
		...
		{
			if(bestOpponentOffers.get(i).utility>bid.utility) insertPosition++;
		}
		bestOpponentOffers.add(insertPosition, bid);
		if(bestOpponentOffers.size()>10) bestOpponentOffers.remove(10);
	}
	
	
	private BidTuple getDiscreteIssueVariation(BidTuple bid, int issueId, int variation) throws Exception
	{
		Bid tmpBid = new Bid(bid.bid);
		BidTuple bidTupleTmp = null;
		IssueDiscrete issueDiscrete = (IssueDiscrete)tmpBid.getIssues().get(issueId);
		int temIssueIndex = issueDiscrete.getIndex(issueDiscrete);
		if(issueDiscrete.getNumberOfValues() >= temIssueIndex+variation && temIssueIndex+variation >= 0)
		{
			ValueDiscrete value = issueDiscrete.getValue(temIssueIndex+variation);
			tmpBid.setValue(issueId, value);
	...
	private BidTuple getRealIssueVariation(BidTuple bid, int issueId, double variation) throws Exception
	{
		Bid tmpBid = new Bid(bid.bid);
		BidTuple bidTupleTmp = null;
		IssueReal issueReal = (IssueReal)tmpBid.getIssues().get(issueId);
		int temIssueValue = ((ValueInteger)tmpBid.getValue(issueId)).getValue();
		if(issueReal.getUpperBound() >= temIssueValue+variation && issueReal.getLowerBound() <= temIssueValue+variation)
		{
	...
	private BidTuple getIntegerIssueVariation(BidTuple bid, int issueId, int variation) throws Exception
	{
		Bid tmpBid = new Bid(bid.bid);
		BidTuple bidTupleTmp = null;
		IssueInteger issueInteger = (IssueInteger)tmpBid.getIssues().get(issueId);
		int temIssueValue = ((ValueInteger)tmpBid.getValue(issueId)).getValue();
		if(issueInteger.getUpperBound() >= temIssueValue+variation && issueInteger.getLowerBound() <= temIssueValue+variation)
		{
	...
	public Action chooseAction()
	{
		...
		{
			if(maximumRoundTime<time-lastRoundTime) maximumRoundTime=time-lastRoundTime;
		...
		{ 		
			...
			if(actionOfPartner instanceof Offer)
			{
				...
				if (time >= 0.99 &&  offeredUtilFromOpponent>= 0.75){
					...
					debug("OFFER: STRAIGHT ACCEPT, TIME IS RUNNING OUT, GOOD OFFER", 3);
					return action = new Accept(getAgentID());
				}
				
				BidTuple opponentBid = new BidTuple(partnerBid, offeredUtilFromOpponent);
				saveBestOpponentBids(opponentBid);
				
				if (bestOpponentOffer==null || bestOpponentOffer.utility < offeredUtilFromOpponent)
				...
				if ((time>=1-(maximumRoundTime*1.09))){
					...
					if(bidToSubmitBack>=bestOpponentOffers.size()) bidToSubmitBack=0;
					debug("OFFER: BEST OF THE OPPONENTS NR " + bidToSubmitBack, 3);
					return action = new Offer(getAgentID(), bestOpponentOffers.get(bidToSubmitBack++).bid);
				}
				...
				BidTuple bestBidInTheVicinity = SearchNeighbourhood(opponentBid);
				debug("bestBidInTheVicinity.utility"+bestBidInTheVicinity.utility);
				if (bestBidInTheVicinity.utility < minimumBidUtility){
					debug("4. bad search, going to pick a random");
					action = chooseRandomBidAction();
					
					// if the random returned a null, which means we couldnt find a bid before time ran out, 
					//  we offer back the best opponent bid
					if(((Offer)action).getBid() == null && bestOpponentOffer.utility >= 0.5)
					{
						debug("action == null && bestOpponentOffer.utility >= 0.5");
						return action = new Offer(getAgentID(), bestOpponentOffer.bid);	
					}
				...
				if(NeverFoundHighUtilityInNeighbourhood)
				{
					if(time > 0.8)
					{
						debug("6. time > 0.8, maximumsearchdepth = " + (int)(initialSearchDepth*1.5));
						maximumSearchDepth = (int)(initialSearchDepth*1.5);
					} 
					else if(time > 0.5)
					{
						debug("7. time > 0.5, maximumsearchdepth = " + (int)(initialSearchDepth*1.2));
			...
			return action = new Offer(getAgentID(), bestOpponentOffer.bid); // best guess if things go wrong. 
		}
	...
	private void updateOpponentProfile(Bid partnerBid, double offeredUtilFromOpponent) 
	{
		countOpponentOffers++;
		...
		if((numberOfCooperativeUtilities/countOpponentOffers) > cooperationthreshold)
		{
			opponentIsCooperative = true;
	...
	private boolean isAcceptable(double offeredUtilFromOpponent, double myOfferedUtil, double time) throws Exception
	{
		...
		acceptanceBidUtility = initialMinimumBidUtility * Math.pow(discount_utility_factor,time);
	...
	private Action chooseRandomBidAction() 
	{
		...
		catch (Exception e) { debug("Problem with received bid:"+e.getMessage()+". cancelling bidding"); }              
	...
	private Bid getRandomBid() throws Exception
	{
		...
		debug("R 2.Generated a random bid. current utility: " + getUtility(bid) + ", tolerance: " + minimumUtilTolerance+ "; tries: " + absoluteTries);
		...
		if(randomnr.nextFloat()<randomSearchRatio)
		{
			...
			bid = SearchNeighbourhood(new BidTuple(bid, getUtility(bid)), maximumSearchDepthForOurBids).bid;
		}
	...
	private void updateMinimumBidUtility(Bid partnerBid, double offeredUtilFromOpponent){
	...
	{
		if(priority >= 4) System.out.println(line);
	}

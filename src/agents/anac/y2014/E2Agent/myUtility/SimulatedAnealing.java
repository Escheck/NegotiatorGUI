...
public class SimulatedAnealing {
     ...
     * @param startBid æŽ¢ç´¢é–‹å§‹Bid
     * @param thresholdUtility åŠ¹ç”¨å€¤é–¾å€¤
     * @param kmax æœ€å¤§ãƒ«ãƒ¼ãƒ—å›žæ•°
     * @return æœ€é�©Bid
     * @throws Exception 
     */
    public BidStorage run(Bid startBid, double targetUtility, int kmax) throws Exception {
        int k = 0;
        ...
        while(k < kmax) {
            ...
            // é�·ç§»ã�™ã‚‹ã�‹ã�©ã�†ã�‹
            boolean trans = methods.calculateWheterToUpdateBid(
                    targetUtility, utility, neighbourUtility, temperature);
            ...
            if(trans) {	
                ...
                // æœ€é�©åŠ¹ç”¨å€¤æ›´æ–°
                if(Math.abs(targetUtility - bestUtility)
    ...
    class RandomSearch implements MethodForSA {
        ...
        public Bid searchNeighbourBid(Bid bid) {
            ...
            IssueInteger lIssueInteger
                = (IssueInteger) issues.get(randomnr.nextInt(issues.size()));
            ...
            neighbourBid.setValue(
                    lIssueInteger.getNumber(), new ValueInteger(optionIndex));
            return neighbourBid;
        }
        ...
        public boolean calculateWheterToUpdateBid(double targetUtility,
            ...
            double diff =
                Math.abs(targetUtility - neighbourUtility) - Math.abs(targetUtility - utility);
            double p = 1.0;
    ...
    class StepSearch implements MethodForSA {
        ...
        public Bid searchNeighbourBid(Bid bid) {
        	...
        	for(int i=0; i<indexes; ++i) {
	            ...
	            IssueInteger lIssueInteger = (IssueInteger) issues.get(targetIssueIndex);
	            int issueIndexMin = lIssueInteger.getLowerBound();
	            int issueIndexMax = lIssueInteger.getUpperBound();
	            int currentIndex;
	            try {
	                currentIndex = ((ValueInteger)bid.getValue(targetIssueIndex+1)).getValue();
	            } catch (Exception e) {
	                e.printStackTrace();
	            ...
	            neighbourBid.setValue(
	                    targetIssueIndex + 1, new ValueInteger(currentIndex));
        	}
            return neighbourBid;
        }
...
interface MethodForSA {
	...
	/**
     * é�·ç§»ç¢ºçŽ‡ã�®ç®—å‡º
     * @param utility åŠ¹ç”¨å€¤
     * @param neighbourUtility è¿‘éš£ã�®åŠ¹ç”¨å€¤
     * @param temperature æ¸©åº¦
     * @return é�·ç§»ç¢ºçŽ‡
     */
	//double calculateTransitionProbability(double targetUtility,
    //        double utility, double neighbourUtility, double temperature);
	boolean calculateWheterToUpdateBid(double targetUtility,
            double utility, double neighbourUtility, double temperature);
}

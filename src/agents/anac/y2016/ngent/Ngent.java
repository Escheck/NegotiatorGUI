...
public class Ngent extends AbstractNegotiationParty {
		...
		private void ChangeIssueScore(Bid bid, String caseName) {
			try {
				...
				if (caseName.equals(string_AddIssueScore)) {
					...
					addedScore = -(1 - Math.exp(-timeline.getTime()
							/ discountingFactor * 50));
				}
				// if(debug)
				// {
				...
				for (Issue lIssue : Issues) {
					...
					case REAL:
						...
						ValueReal valueReal = (ValueReal) bid.getValue(lIssue
								.getNumber());
						int indexReal = (int) ((valueReal.getValue() - lIssueReal
								.getLowerBound())
								/ (lIssueReal.getUpperBound() - lIssueReal
										.getLowerBound()) * lIssueReal
				...
				for (Issue Issue1 : Issues) {
					...
					case DISCRETE:
						...
						if (domainSize > largeDomainSize) {
							score += Math.log(issueScore.get(
									lIssueDiscrete.getNumber() - 1).get(index));
						} else {
							score += issueScore.get(
									lIssueDiscrete.getNumber() - 1).get(index);
						}
						break;
					...
					case REAL:
						...
						if (domainSize > largeDomainSize) {
							score += Math.log(issueScore.get(
									lIssueReal.getNumber() - 1).get(indexReal));
						} else {
	...
	public void init(AbstractUtilitySpace utilitySpace, Deadline deadlines,
		...
		try {
			...
			this.offeredBidsSD = (this.MaximumUtility - this.utilitySpace
					.getUtility(this.bid_minimum_utility)) / 4;
			...
			this.opponentBidHistory1.initializeDataStructures(utilitySpace
					.getDomain());
			this.opponentBidHistory2.initializeDataStructures(utilitySpace
					.getDomain());
			...
			this.bidsUtilityScore = UpdateBidsUtilityScore(this.bidsUtilityScore);
	...
	public AgentID getPartyId() {
		return this.partyId == null ? new AgentID(
				String.format("CUHKAgent2016Test")) : this.partyId;
	}
		...
		try {
			...
			// else if (ActionOfOpponent instanceof Offer)
				...
				if (this.discountingFactor == 1) {
					this.agentData1 = updateConcedeDegree_nonDF(this.agentData1);
					this.agentData2 = updateConcedeDegree_nonDF(this.agentData2);
				} else if (this.discountingFactor <= 0.5) {
					this.agentData1 = updateConcedeDegree_smallDF(this.agentData1);
					this.agentData2 = updateConcedeDegree_smallDF(this.agentData2);
				} else {
				...
				if (estimateRoundLeft(true) > 10) {
					...
					if (IsAccept && !IsTerminate) {
						...
						if (this.utilitySpace.getUtility(oppPreBid) > this.reservationValue) {
							action = new Accept(getPartyId());
							// if(debug)
							// {
							...
							// System.setOut(file);
							// System.out.println("Compare Accept & Terminate, we accept the offer");
							// }
						} else {
							action = new EndNegotiation(getPartyId());
							// if(debug)
							...
							// FileOutputStream(fileName, true));
							// System.setOut(file);
							// System.out.println("Compare Accept & Terminate, we terminate the negotiation");
							// }
						}
					} else {
						...
						if (this.concedeToOpponent == true) {
							...
							Bid bid2 = opponentBidHistory2
									.getBestBidInHistory();
							if (this.utilitySpace.getUtility(bid1) > this.utilitySpace
									.getUtility(bid2)) {
								bid = bid2;
							} else if (this.utilitySpace.getUtility(bid1) == this.utilitySpace
									.getUtility(bid2) && Math.random() < 0.5) {
								bid = bid2;
							} else {
								bid = bid1;
							}
							...
							// FileOutputStream(fileName, true));
							// System.setOut(file);
							// System.out.println("we offer the best bid in the history and the opponent should accept it. Bid:"
					...
					if (timeline.getTime() > 0.99
						...
						if (this.utilitySpace.getUtility(oppPreBid) > this.avgUtilitythreshold) {
							action = new Accept(getPartyId());
							bid = oppPreBid;
						} else {
							bid = BidToOffer_original();
							...
							if (IsAccept && !IsTerminate) {
								...
								if (this.toughAgent == true) {
									...
									action = new Offer(getPartyId(), bid);
									// this.toughAgent = true;
									// System.out.println("this is really the last chance"
	...
	public void receiveMessage(AgentID sender, Action opponentAction) {
		...
		try {
																...
																// w.r.t. us
				{
					...
					if ((this.ActionOfOpponent instanceof Offer)) {
						...
						if (!this.agentData1.GetIsReceivedOppFirstBid()) {
							...
							// FileOutputStream(fileName, true));
							// System.setOut(file);
							// System.out.println("Enter SetFirstBidInform AgentData1! round:"
							// + round + ", CountOpp:" + countOpp +
							// ", EachRoundCount:" + EachRoundCount);
							// }
																		...
																		// us
				{
					...
					if ((this.ActionOfOpponent instanceof Offer)) {
						...
						if (!this.agentData2.GetIsReceivedOppFirstBid()) {
							...
							// FileOutputStream(fileName, true));
							// System.setOut(file);
							// System.out.println("Enter SetFirstBidInform AgentData2! round:"
							// + round + ", CountOpp:" + countOpp +
							// ", EachRoundCount:" + EachRoundCount);
							// }
			...
			System.out.println(e.getMessage()
					+ " exception in method receiveMessage");
		}
	}
	...
	private void UpdateOfferedBidsStat(double bidU) {
		try {
			...
			this.offeredBidsSD = Math
					.sqrt((this.offeredBidsRound * this.offeredBidsSD
							* this.offeredBidsSD + (bidU - this.offeredBidsAvg)
	...
	private boolean AcceptOpponentOffer(Bid opponentBid, Bid ownBid) {
		try {
			...
			if (currentUtility >= this.avgUtilitythreshold
				...
				// file = new PrintStream(new FileOutputStream(fileName, true));
				// System.setOut(file);
				// System.out.println("currentUtility >= this.avgUtilitythreshold || currentUtility >= nextRoundUtility: true, AvgU:"
				...
				if (currentMaximumUtility > predictMaximumUtility
					...
					// true));
					// System.setOut(file);
					// System.out.println("currentMaximumUtility > predictMaximumUtility && timeline.getTime() > this.avgConcedeTime: true, currentMaximumUtility:"
					...
					if (utilitySpace.getUtilityWithDiscount(opponentBid,
						...
						// true));
						// System.setOut(file);
						// System.out.println("utilitySpace.getUtilityWithDiscount(opponentBid, timeline) >= currentMaximumUtility - 0.01: true, DFoppBid:"
				...
				// retrieve the opponent's biding history and utilize it
					...
					// true));
					// System.setOut(file);
					// System.out.println("currentMaximumUtility > this.avgUtilitythreshold * Math.pow(this.discountingFactor, timeline.getTime()): true, currentMaximumUtility:"
	...
	private void calculateBidsBetweenUtility() {
		try {
			...
			this.SpacebetweenDivisions = (this.MaximumUtility - this.MinimumUtility)
			...
			while (myBidIterator.hasNext()) {
				...
				for (int i = 0; i < this.NumberofDivisions; i++) {
					if (this.utilitySpace.getUtility(b) <= (i + 1)
	...
	private Bid regeneratebid(double bidUtil) {
		...
		try {
			...
			// {
			int check = 0;
			for (this.CurrentrangeNumber = this.NumberofDivisions - 1; check == 0; this.CurrentrangeNumber--) {
				if (bidUtil > ((this.CurrentrangeNumber + 1)
						* this.SpacebetweenDivisions + this.MinimumUtility)) {
					break;
				}
				Bids.addAll(this.bidsBetweenUtility
						.get(this.CurrentrangeNumber));
			}
			// }
			// else
			// {
				...
				if (Bids.isEmpty()) {
					...
					for (int i = 0; i < Bids.size(); i++) {
						...
						if (dist < min) {
							...
							// }
								...
								&& this.utilitySpace.getUtility(bid) > this.utilitySpace
										.getUtility(ans)) {
							ans = bid;
						}
					}
					...
					if (Math.random() > 0.95) {
						LinkedList<Bid> sortedBidsList = SortBidsList(Bids,
								distList);
						ans = sortedBidsList
								.get((int) (sortedBidsList.size() / 2 + (int) (Math
										.random() * (Bids.size() - sortedBidsList
										.size() / 2))));
						// if(debug)
						// {
						// file = new PrintStream(new FileOutputStream(fileName,
			...
			System.out.println(e.getMessage()
					+ " Exception in method regeneratebid");
			ans = this.bid_maximum_utility;
		}
	...
	private HashMap<Bid, Double> UpdateBidsUtilityScore(
		...
		try {
			...
			for (; myBidIterator.hasNext();) {
				...
				bidsUtilityScore
						.put(bid,
								(this.utilitySpace.getUtility(bid) / this.MaximumUtility));
			}
		} catch (Exception e) {
			System.out.println("Exception in UpdateBidsUtilityScore: "
					+ e.getMessage());
		}
	...
	private AgentData updateConcedeDegree_smallDF(AgentData agentData) {
		...
		try {
			...
			double temp = this.concedeTime_original + weight
					* (1 - this.concedeTime_original)
					* Math.pow(opponnetToughnessDegree, gama);
			agentData = updateConcedeDegree_smallDF_UpdateConcedeTime(
					agentData, temp);
			// if(debug)
			// {
			// file = new PrintStream(new FileOutputStream(fileName, true));
				...
				if (DFname.equals("Small")) {
					...
					if (this.domainSize > 100) {
						...
						if (relCountUtility > (bidsUtil.size() / 2)) {
							...
							// FileOutputStream(fileName, true));
							// System.setOut(file);
							// System.out.println("updateConcedeDegree_UpdateMinU: numOfBids < 100, above medium:");
							// }

							LinkedList<Bid> sortedBidsList = SortBidsList(Bids);
							for (int i = sortedBidsList.size() / 2; i < sortedBidsList
									.size(); i++) {
								double bidUtil = (Double) this.utilitySpace
							...
							agentData.relStdevUtility = Math
									.sqrt(relSqRootOfAvgUtility
											/ relCountUtility);
		...
		try {
			...
			if (relStdevUtility < 0.1) {
				...
				for (Iterator g = bidsUtil.iterator(); g.hasNext();) {
					...
					if (bidUtil >= agentData.oppFirstBidUtility
							&& (bidUtil < (relAvgUtility - relStdevUtility) || bidUtil > (relAvgUtility + relStdevUtility))) {
						relSqRootOfAvgUtility += (bidUtil - relAvgUtility)
								* (bidUtil - relAvgUtility);
						relCountUtilityInSmallSD++;
					}
				}
				relStdevUtility = Math.sqrt(relSqRootOfAvgUtility
						/ relCountUtilityInSmallSD);
			}
	...
	private void updateConcedeDegree_largeDF(AgentData agentData) {
		try {
			...
			agentData.concedePartOfdiscountingFactor = this.discountingFactor - 1;
			agentData.concedePartOfOppBehaviour = (((((agentData.relCountLowerBoundMid2 / agentData.relCountUpperBoundMid2) * agentData.slopeOfSlopeOfSessionMax2) - ((agentData.relCountLowerBoundMid1 / agentData.relCountUpperBoundMid1) * agentData.slopeOfSlopeOfSessionMax1)) / this.k) / agentData.relStdevUtility)
					- this.N;
	...
	private AgentData MeasureConcedePartOfOppBehaviour(AgentData agentData) {
		try {
			...
			if (agentData.count < this.FirstTimeInterval) {
				if (utilitySpace.getUtility(agentData.oppPreviousBid) > agentData.utility_maximum_from_opponent_Session1) {
				...
				if (utilitySpace.getUtility(agentData.oppPreviousBid) > agentData.utility_maximum_from_opponent_Session2) {
					agentData.utility_maximum_from_opponent_Session2 = utilitySpace
							.getUtility(agentData.oppPreviousBid);
				}
			} else if (agentData.count < this.ThirdTimeInterval) {
				if (utilitySpace.getUtility(agentData.oppPreviousBid) > agentData.utility_maximum_from_opponent_Session3) {
				...
				agentData.midPointOfSlopeSessionMax1 = (agentData.utility_maximum_from_opponent_Session2 + agentData.utility_maximum_from_opponent_Session1) / 2;
				agentData.midPointOfSlopeSessionMax2 = (agentData.utility_maximum_from_opponent_Session3 + agentData.utility_maximum_from_opponent_Session2) / 2;
	...
	private LinkedList<Bid> SortBidsList(LinkedList<Bid> Bids) {
		...
		try {
			...
			Collections.sort(list_Data,
					new Comparator<Map.Entry<Bid, Double>>() {
						public int compare(Map.Entry<Bid, Double> entry1,
							...
							return (entry1.getValue().compareTo(entry2
									.getValue()));
						}
					});
			...
			System.out.println(e.getMessage()
					+ " exception in method SortBidsList");
		}
	...
	private double CalWhetherOppFirstBidGoodEnough(double minThreshold,
		...
		try {
			...
			double relSumUtility = 0;
			double relCountUtility = 0;
			for (int i = sortedBidsList.size() / 2; i < sortedBidsList.size(); i++) {
				relSumUtility += this.utilitySpace.getUtility(sortedBidsList
						.get(i));
				relCountUtility += 1;
				// if(debug)
				// {
	...
	private void printBidAllValues(Bid bid) {
		try {
			...
			System.out.println("Exception in printAllBidValues(): "
					+ e.getMessage());
		}
	}
	...
	private Bid BidToOffer_original() {
		...
		try {
			...
			this.avgConcedeTime = (this.agentData2.concedeTime + this.agentData1.concedeTime) / 2;
			...
			if (currentTIme <= this.avgConcedeTime) {
				if (this.discountingFactor <= 0.5) {
					this.agentData1.minThreshold = (this.MaximumUtility * this.discountingFactor)
							/ Math.pow(this.discountingFactor,
									this.agentData1.concedeTime);
					this.agentData2.minThreshold = (this.MaximumUtility * this.discountingFactor)
							/ Math.pow(this.discountingFactor,
				...
				if (this.discountingFactor <= 0.5) {
					this.agentData1.utilitythreshold = (this.MaximumUtility * this.discountingFactor)
							/ Math.pow(this.discountingFactor, currentTIme);
					this.agentData2.utilitythreshold = (this.MaximumUtility * this.discountingFactor)
							/ Math.pow(this.discountingFactor, currentTIme);
			...
			this.avgUtilitythreshold = (this.agentData1.utilitythreshold + this.agentData2.utilitythreshold) / 2;
	...
	private boolean OtherAcceptCondition(Bid oppPreviousBid, boolean IsAccept) {
		try {
			if (timeline.getTime() >= ((this.agentData1.concedeTime + this.agentData2.concedeTime) / 2)
					&& this.utilitySpace.getUtility(oppPreviousBid) >= this.minUtilityUhreshold) {
				// if(debug)
				// {
				// file = new PrintStream(new FileOutputStream(fileName, true));
				// System.setOut(file);

...
public class Caduceus extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		if (isBestOfferTime()) {
			...
			if (bid != null) {
				...
				System.out.println("Offered bid util:"
						+ this.getUtilityWithDiscount(previousBid));
				System.out
						.println("My good offer for everyone (considering nash product):"
								+ this.getUtility(bid));
				...
				if (this.getUtilityWithDiscount(bid) < selfReservationValue) {
					...
					if (bid == null) {
						System.err
								.println("Failed to generate bid with getRandomBid()");
					}

				}

				if (this.getUtilityWithDiscount(previousBid) > (this
						.getUtilityWithDiscount(bid)) + 0.2) {
				...
				System.err
						.println("Failed to generate bid with getMyBestOfferForEveryone!");
			}

			return new Offer(bid); // conceding to Nash
		}
	...
	private Bid getMyBestOfferForEveryone(double time) {
		...
		for (Map.Entry<AgentID, Opponent> entry : this.opponentProfiles
				.getOpponentProfiles().entrySet()) {
		...
		try {
			...
			if (npc.nashBid == null) {
				Bid bestBid = this.getBestBid();
				CounterOfferGenerator offerGenerator = new CounterOfferGenerator(
						bestBid, this);
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (sender != null) {
			if (action instanceof Offer) {
				...
				SaneBid saneBid = new SaneBid(uglyBid,
				...
				if (this.opponentProfiles.getOpponentProfiles().get(sender) == null)
					...
					opponentProfile = this.opponentProfiles
							.getOpponentProfiles().get(sender);

				previousBid = uglyBid;
				Bid previousBid = null;

				if (!opponentProfile.getBidHistory().isEmpty())
					previousBid = opponentProfile.getBidHistory().get(
							opponentProfile.getBidHistory().size() - 1);

				Iterator<Pair<SaneIssue, SaneValue>> iterator = saneBid
						.getIterator();
				...
				while (iterator.hasNext()) {
					...
					if (previousBid != null) {
						for (Map.Entry<Integer, Value> previousBidEntry : previousBid
								.getValues().entrySet()) {
							Value previousBidValue = previousBidEntry
									.getValue();
							if (previousBidValue.toString().equalsIgnoreCase(
									value.name)) {
				...
				this.opponentProfiles.getOpponentProfiles().put(sender,
						opponentProfile);
			}
		}
	}
	...
	public SaneUtilitySpace getMySaneUtilitySpace() {
		if (mySaneUtilitySpace == null) {
			...
			mySaneUtilitySpace
					.initWithCopy((AdditiveUtilitySpace) this.utilitySpace);
		}
	...
	private double getRoundValue() {
		...
		double roundValue = (2 * Math.pow(this.getTimeLine().getTime(), 2))
				- (101 * this.getTimeLine().getTime()) + 100;

...
public class ParsAgent2 extends AbstractTimeDependentNegotiationParty {
	...
	public Bid bestMutualCenters() {
		...
		if (utilitySpace.getUtility(clusterA.get(centerindex).getCenter()) > utilitySpace
				...
				try {
					...
					if (utilitySpace.getUtility(b) > getMyutility()) {
						...
						for (int k = 0; k < maxBids.size(); ++k) {
							if (utilitySpace.getUtility(maxBids.get(k)) < utilitySpace
									.getUtility(b)) {
								index = k;
								break;
							}
		...
		for (int j = 0; j < dissues.size(); ++j) {
			...
			if (dissues.get(j) instanceof IssueDiscrete) {
				...
				if (!b1.getValues()
						.get(keyVals[j])
						.toString()
						.equals(b2.getValues().get(keyCenterVals[j]).toString())) {
					distance += 1;
				}
			} else {
				float dis = ((Float.parseFloat(b1.getValues().get(keyVals[j])
						.toString())) - (Float.parseFloat(b2.getValues()
						.get(keyCenterVals[j]).toString()))
						/ dissues.get(j).getChildCount());
	...
	public double getMyutility() {
		...
		if (clusterIsRefreshA && clusterA != null) {
			...
			clusterHistoryA.distributionFactor.add(
					clusterHistoryA.distributionFactor.size(), new Float(
							unormalA));
			...
			for (int i = 0; i < clusterHistoryA.distributionFactor.size(); ++i) {
				...
				for (int j = i + 1; j < clusterHistoryA.distributionFactor
					...
					for (int k = 0; k < numberOfclusters; ++k) {
						for (int l = 0; l < numberOfclusters; ++l) {
							temp1 = calDist(clusterHistoryA.prevCenters.get(i)
									.get(k), clusterHistoryA.prevCenters.get(j)
									.get(l));
							if (temp1 > max1)
								max1 = temp1;
						}
			...
			meanDistributionA = (meanDistributionA / clusterHistoryA.distributionFactor
					.size()) / 1.33f;
			meanDistanceA = (meanDistanceA / clusterHistoryA.distributionFactor
					.size()) / issueSize;

			float w1 = 0.4f, w2 = 0.2f, w3 = 0.4f;
			concessionA = (w1 * maxCenterDistanceA + w2 * meanDistanceA - w3
					* meanDistributionA);
			myConcession = concessionA < concessionB ? concessionA
					: concessionB;
			myConcessionRate = myConcession > 0 && myConcession > 0.15d ? myConcession
					: 0.15d;
			...
			clusterHistoryB.distributionFactor.add(
					clusterHistoryB.distributionFactor.size(), new Float(
							unormalB));
			...
			for (int i = 0; i < clusterHistoryB.distributionFactor.size(); ++i) {
				...
				for (int j = i + 1; j < clusterHistoryB.distributionFactor
					...
					for (int k = 0; k < numberOfclusters; ++k) {
						for (int l = 0; l < numberOfclusters; ++l) {
							temp = calDist(clusterHistoryB.prevCenters.get(i)
									.get(k), clusterHistoryB.prevCenters.get(j)
									.get(l));
							if (temp > max)
								max = temp;
			...
			meanDistributionB = (meanDistributionB / clusterHistoryB.distributionFactor
					.size()) / 1.33f;
			meanDistanceB = (meanDistanceB / clusterHistoryB.distributionFactor
					.size()) / issueSize;

			// System.out.println("for Opponent A All ===============================meanDistributionA ==  "
			// +
			// meanDistributionA + " meanDistanceA ==" +
			// meanDistanceA + "  maxCenterDistanceA==" +
			// maxCenterDistanceA);
			// System.out.println("for Opponent B All ===============================meanDistributionB ==  "
			// +
			...
			concessionB = (w1 * maxCenterDistanceB + w2 * meanDistanceB - w3
					* meanDistributionB);
			myConcession = concessionA < concessionB ? concessionA
					: concessionB;
			myConcessionRate = myConcession > 0 && myConcession > 0.15d ? myConcession

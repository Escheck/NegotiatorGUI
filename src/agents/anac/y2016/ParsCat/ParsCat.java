...
public class ParsCat extends AbstractNegotiationParty {
	...
	public void init(AbstractUtilitySpace utilSpace, Deadline dl,
		...
		this.utilSpace = utilSpace;// read utility space
		TimeLineInfo = tl; // read time line info
	...
	private Bid getRandomBid() throws Exception {
		...
		while (counter == 1000) {
			...
			while (((getUtility(bid) < (tresh - xxx)) || (getUtility(bid) > (tresh + xxx)))
					&& (counter < 1000));
			check = check + .01;
		}
		...
		if ((getUtility(bid) < getUtility(OtherAgentsBidHistory
				.getBestBidDetails().getBid())) && getNumberOfParties() == 2)
	...
	public String getDescription() {
		return "example party group ParsCat";
	}

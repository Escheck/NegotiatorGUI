...
public class BRAMAgent2 extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			if (actionOfPartner == null) {
				...
				if (offeredUtility >= threshold)// If the utility of the bid
					...
					if (opponentBidsArray.size() < OPPONENT_ARRAY_SIZE) {// In
						...
						// Add the new bid of the opponent and update the
						// statistics
						opponentBidsArray.add(opponentBid);
						updateStatistics(opponentBid, false);
					...
					((offeredUtility < this.utilitySpace
							...
							.getTime() > 177.0 / 180.0))
							&& (this.utilitySpace
									.getReservationValueWithDiscount(timeline) > this.utilitySpace
									.getUtilityWithDiscount(bidToOffer,
											timeline))) {
			...
			if (actionOfPartner != null)
				System.out
						.println("BRAMAgent accepted the offer beacuse of an exception");
		}
		return action;
	}
	 ...
	 * 
	 * @param bidToUpdate
	 *            - the bid that we want to update its statistics
	 * @param toRemove
	...
	private void updateStatistics(Bid bidToUpdate, boolean toRemove) {
		try {
			...
			System.out.println("BRAM - Exception in updateStatistics: "
					+ toRemove);
		}
	...
	private double getNewThreshold() {
		...
		if (timeline.getTime() < 60.0 / 180.0)
			tresholdBestBidDiscount = maxUtil - (maxUtil - minUtil)
					* THRESHOLD_PERC_FLEXIBILITY_1;
		else if (timeline.getTime() < 150.0 / 180.0)
			tresholdBestBidDiscount = maxUtil - (maxUtil - minUtil)
					* THRESHOLD_PERC_FLEXIBILITY_2;
		else if (timeline.getTime() < 175.0 / 180.0)
			tresholdBestBidDiscount = maxUtil - (maxUtil - minUtil)
					* THRESHOLD_PERC_FLEXIBILITY_3;
		else
			tresholdBestBidDiscount = maxUtil - (maxUtil - minUtil)
					* THRESHOLD_PERC_FLEXIBILITY_4;
	...
	private Bid getBidToOffer() {
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				...
				if (currBid == null) {
					System.out
							.println(" BRAM - currBid in getBidToOffer is NULL");
				} else {
					// System.out.println(" BRAM - currBid: " +
					// currBid.toString());
					double currUtility = utilitySpace.getUtilityWithDiscount(
							currBid, timeline);
		...
		try {
			...
			for (Issue lIssue : issues) {
				...
				case DISCRETE:
					...
					if (opponentBidsStatisticsDiscrete == null)
						System.out
								.println("BRAM - opponentBidsStatisticsDiscrete IS NULL");
			...
			System.out
					.println("BRAM - Exception in createBidByOpponentModeling function");
		}
		return bid;
	}
	...
	private Bid selectCurrentBidFromOurBidsArray() {
		...
		if (this.utilitySpace.getUtilityWithDiscount(toSend, timeline) < threshold) {
			...
			bidsCountProposalArray[lastPositionInBidArray]++;// update the
																// number of
																// times that
																// this bid was
																// offered
			...
			lastPositionInBidArray = newIndex;// update the last position - this
												// is an indication to the last
												// bid that was offered
			bidsCountProposalArray[newIndex]++;// update the number of times

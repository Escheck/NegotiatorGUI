...
public class OpponentModel {
	...
	/**
	 * HashMap that keeps track for each issue the number of times it has been changed, with the issue as its key
	 */
	private HashMap<Issue, Integer> issueChanged;

	/**
	 * Keeps a HashMap 'matrix' containing references for each issue to their set of values, which refer to the
	 * number of times this value has been offered
	 */
	...
	/**
	 * Updates the model by analyzing the latest bid our opponent has made, given as a parameter
	 * 
	 * @param nextBid Bid
	 * @throws Exception
	 */
	private void updateModel(Bid nextBid) throws Exception{
		...
		for(Issue issue : issueList){
			...
			int c = hm.get(((ValueDiscrete)nextBid.getValue(issue.getNumber())).getValue()) + 1;
			hm.put( ((ValueDiscrete)nextBid.getValue(issue.getNumber())).getValue(), c);
			valuesOffered.put(issue, hm);

			//calculate and add the relative frequency for the offered value for this issue
			relativeFrequency.get(issue).add(c/((double)bidCounter));
		}
		
		//some complicated stuff going on here, check report for explanation (chapter on KDE)
		...
		for(Issue i : issueList){
			...
			for(double d : frequencyComponents){
				for(int index=0; index<100; index++){					
					probabilityDensity[index]+=(Math.exp(-Math.pow((kernel[index]-d),2)/2)/(index+1));					
				}
			}
...
//		System.out.println("\n\nISSUE WEIGHTS: "+issueWeights.toString()+"\n\n");
	...
	public String getPreferredValueForIssue(Issue i){
		int max = 0;
		String current = "";
		for(java.util.Map.Entry<String, Integer> e : valuesOffered.get(i).entrySet()){
			if(e.getValue() >= max){
				current = e.getKey();
				max = e.getValue();
			}			
	...
	public double getEstimatedUtility(Bid bid){		
		...
		double totalUtil = 0;
		for(Issue i : issueList){
			//we know it must be discrete because we only have discrete issues..
			IssueDiscrete id = (IssueDiscrete)i;
			valueCounts = valuesOffered.get(i);
			int maxcount = 0;
			for(ValueDiscrete vald : id.getValues()){
				maxcount = (valueCounts.get(vald.getValue()) > maxcount) ? valueCounts.get(vald.getValue()) : maxcount;
			}
			
			try {
				// normalize the frequency-counts by the highest count (which will be utility 1) 
				totalUtil += (valueCounts.get(((ValueDiscrete)bid.getValue(i.getNumber())).getValue())/(double)maxcount)*issueWeights.get(i);
			} catch (Exception e) {
				return -1; //something went wrong - no bids yet probably, just return -1
			}
		...
		if(discountedDomain){
			...
			totalUtil = totalUtil * Math.pow(discountFactor,timeline.getTime());
		}
	...
	/**
	 * Returns true iff this opponentmodel was properly initialized (ie. no REAL or INTEGER issues..)
	 * 
	 * @return boolean
	 */
	...
	private void addValuesOffered(Issue i){
		switch(i.getType()) {
			case DISCRETE:
				...
				break;
			case REAL:
				//opponentmodel does not yet know how to handle REAL or INTEGER values;
	 ...
	 * 
	 * @param consecutiveDiff number of consecutive unique bids our opponent has made
	 * @return boolean
	 */
	public boolean isStalling(int consecutiveDiff){
		//if there were any non-unique bids among the last 3 offered bids, our opponent
		//has stalled

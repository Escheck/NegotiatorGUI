		...
		try {
			...
			if (actionOfPartner == null) {
				...
				if (actionOfPartner instanceof Offer) {
					...
					double offeredUtility = this.utilitySpace
							.getUtility(((Offer) actionOfPartner).getBid());
					if (MaxUtility == MinUtility) {
						MinUtility = offeredUtility;
						action = new Offer(this.getAgentID(),
								this.utilitySpace.getMaxUtilityBid());
					} else {
						if (offeredUtility >= (MinUtility + (MaxUtility - MinUtility)
								* AcceptanceLimit)) {
							action = new Accept(this.getAgentID());
						} else {
							Bid newOffer = this.utilitySpace.getDomain()
									.getRandomBid();
							while (this.utilitySpace.getUtility(newOffer) <= (MinUtility + (MaxUtility - MinUtility)
									* offerLimit)) {
								newOffer = this.utilitySpace.getDomain()
										.getRandomBid();
							}

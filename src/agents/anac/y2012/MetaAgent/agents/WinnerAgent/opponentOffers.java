...
public class opponentOffers {
	...
	private Hashtable<Issue, Hashtable<Key, Integer>> _offers; //a data structure that indexes opponent offers by issues and counts the number of times each value was proposed
	private ArrayList<Issue> _issues; //the issues in this domain
	private Hashtable<Issue, List<Key>> _sortedValuesKeys; //for each issue, holds the list of possible values by decreasing number of times proposed
	private int _totalNumOfOffers; //counter for the number of offers received from the opponent
	private Hashtable<Issue,Double> _issueWeights; // holds the approximate learned weights of the issues
	private Hashtable<Issue, Hashtable<Key, Double>> _valuesUtil; //holds the approximate learned utility for each value
			...
			for(Issue lIssue : _issues) 
			{
				...
				switch(lIssue.getType()) 
				{
					...
					case INTEGER:
						...
						break;
					default: throw new Exception("issue type "+lIssue.getType()+" not supported by this agent");
				}
	...
	/**
	 * sorts the list of keys for values for the given issue according to the value counts
	 * invoked each time a new bid arrives and updates the location in the list of the proposed value
	 * if needed
	 */
	...
	public boolean updateBid(Bid b) throws Exception
	{
		...
		_ourAvgUtilFromOppOffers = (((_ourAvgUtilFromOppOffers)*(_totalNumOfOffers-1))+utilForUsFromBid)/_totalNumOfOffers;
		_oppConcessionRate=Math.max(1,_ourAvgUtilFromOppOffers/prevAvgUtil);//assuming no opponent will harden it's position in the long term, we allow min concession of 1
			...
			switch(v.getType()) 
			{
				...
				case REAL:
					...
					while(realKeys.hasMoreElements())
					{
						...
						{
							currentCount = (Integer)_offers.get(lIssue).get(currKey);
			...
			for (Issue currIssue : _issues)
			{
				...
				switch(currVal.getType()) 
				{
					case DISCRETE:
						...
						oppUtil=oppUtil+_issueWeights.get(currIssue)*_valuesUtil.get(currIssue).get(k);
					...
					case REAL:
						...
						while(realKeys.hasMoreElements())
						{
							...
							if(currKey.isInRange(realVal))
							{
								oppUtil = oppUtil+_issueWeights.get(currIssue)*_valuesUtil.get(currIssue).get(currKey);
								break;
							}					
						}
						break;
					case INTEGER:
						...
						break;
					default: throw new Exception("issue type "+currIssue.getType()+" not supported by this agent");
				}
			}// for
			return oppUtil;
		}
	...
	/**
	 * calculates the normalized weights of the issues according to their variance
	 * issue with the lowest variance has the highest weight (assuming that this is the reason
	 * the other agent won't compromise a lot about this issue). we compare the maximal counts
	 * of values offered by the opponent as an approximation to the variance (when the maximal count
	 * is higher the variance is lower).
	 * we define stub(i) for an issue as count(mostFrequentValueOffered)/sum for all issues(mostFrequentValueOffered)
	 * this models the opponent's stubbornness for each issue.
	 * the weight is calculated by stub(i,j)/sum(stub(i))
	 */
	private Hashtable<Issue, Double> getIssueWeights()
	{
		...
		for (Issue currIssue : _issues)
		{
			currNormalaizedCount = (_offers.get(currIssue).get(_sortedValuesKeys.get(currIssue).get(0)));
	...
	private double totalNormalizedCounts()
	{
		...
		for (Issue currIssue : _issues)
		{
			totalNormalizedCount = totalNormalizedCount+_offers.get(currIssue).get(_sortedValuesKeys.get(currIssue).get(0));
		}
		return totalNormalizedCount;
	}
	
	/**
	 * calculates the utility for the given value
	 * we approximate the utility by dividing the number of offers made with this value 
	 * by the total number of offers made. a high ratio indicates a high utility from this value
	 */
	 ...
	 * higher importance for that issue. we approximate the variance by the count of the most frequent
	 * proposed value)
	 */
	public List<Issue> getIssuesByCounts()
	{
		...
		try
		{
			...
			for (Issue newIssue : _issues)
			{
				...
				int index = 0;
				newIssueMaxCount = _offers.get(newIssue).get(newIssuekeys.get(0));
				List<Key> currIssuekeys;
				while (newIssueMaxCount < currIssueMaxCount && index<sortedIssues.size())
				{
					currIssue = sortedIssues.get(index);
					currIssuekeys = _sortedValuesKeys.get(currIssue);
					currIssueMaxCount = _offers.get(currIssue).get(currIssuekeys.get(0));
					index++;
				}
				// found place in list
	...
	public void printUtilityFunction()
	{
		...
		while(it.hasNext())
		{
			...
			Iterator<Entry<Key, Double>> valueIt = _valuesUtil.get(currIssue).entrySet().iterator();
			while(valueIt.hasNext())
			{
				currValueEntry = valueIt.next();
				System.out.println(" value "+currValueEntry.getKey()+ "\t\tutility: "+currValueEntry.getValue()+"\t\tcount "+_offers.get(currIssue).get(currValueEntry.getKey()));
			}
		}
	}
	...
	/*
	 * returns a vector with all bids proposed by opponent over the given threshold
	 */
	public Vector<Bid> getOpponentBidsAboveThreshold(double threshold)
	{
		Vector<Bid> bids=new Vector<Bid>();
		int stopIndex=(int)Math.floor((threshold-0.5)*20); //index of the lowest bin we should return offers from
		try
		{
			for (int i=10;i>=stopIndex;i--)
				{
					...
					for (Bid b:currSet)
					{
						if ((i!=stopIndex) || (_utilitySpace.getUtility(b)>threshold))
						{
							bids.add(b);
						}
	...
	public String toString()
	{
		...
		Iterator<Entry<Issue,Hashtable<Key,Integer>>> issueIterator= _offers.entrySet().iterator();
		Issue currIssue;
		...
		while (issueIterator.hasNext())
		{
			...
			Iterator<Entry<Key,Integer>> valueIterator=currValues.entrySet().iterator();
			Key currKey;

...
public class TMFAgent extends Agent
{
	...
	public void init()
	{
		...
		for(Issue issue : issues)
		{
			...
			if (issue.getType() == ISSUETYPE.DISCRETE){
				...
				opponentUtilityEstimator.add(new HashMap<Value, Double>());
				EvaluatorInteger ei = new EvaluatorInteger();

				//putting the opponent best offer
				if (ei.getEvaluation(((IssueInteger)issue).getLowerBound()) < ei.getEvaluation(((IssueInteger)issue).getUpperBound()))
					opponentUtilityEstimator.get(max_i).put(new ValueInteger(((IssueInteger)issue).getLowerBound()), 0.0); 
				else
					opponentUtilityEstimator.get(max_i).put(new ValueInteger(((IssueInteger)issue).getUpperBound()), 0.0);
					*/
			}
				...
				/*opponentUtilityEstimator.add(new HashMap<Value, Double>());
				opponentUtilityEstimator.get(max_i).put(new ValueReal(((IssueReal)issue).getLowerBound()), 0.0);
				opponentUtilityEstimator.get(max_i).put(new ValueReal(((IssueReal)issue).getUpperBound()), 0.0);*/
			}
			maxValue.add(0.0);
		}
		}
		...
		for (Bid b : allBids){
			try {
				relevantBids.add(new ComparableBid(b,utilitySpace.getUtility(b)));
	...
	@Override
	public String getName() {return "TMF-Agent";}

	public void ReceiveMessage(Action opponentAction) 
	{
		//estimating time of one bid-response time (like Round Trip Time in communication)
		...
		if (lastBid == null){ 
			//remove all unrelevance bids = all bids below the first opponent bid or all bids below the reservation value.
			try {
				double minimumRelevant = Math.max(utilitySpace.getUtility(b), utilitySpace.getReservationValueUndiscounted());
				while (relevantBids.size() > 0 && (utilitySpace.getUtility(relevantBids.get(relevantBids.size()-1).bid) < minimumRelevant))
		...
		lastBid = b;
		try {
			if (utilitySpace.getUtility(BestOpponentBid) < utilitySpace.getUtility(b))
				BestOpponentBid = b;
		} catch (Exception e) {}
	}
	...
	private void AddOpponentBidToModel (Bid b, boolean isAgreed){
		...
		for (int i = 0; i < utilitySpace.getDomain().getIssues().size(); i++) {
			...
			if (issue.getType() == ISSUETYPE.DISCRETE){
				...
				int k = Math.min (10, ((IssueInteger)issue).getUpperBound() - ((IssueInteger)issue).getLowerBound());
				int bucket = (int) Math.round((double)(currValue - ((IssueInteger)issue).getLowerBound()) / (((IssueInteger)issue).getUpperBound() - ((IssueInteger)issue).getLowerBound()) * k); 
				...
				int k = 10;
				int bucket = (int) Math.round((currValue - ((IssueInteger)issue).getLowerBound()) / (((IssueInteger)issue).getUpperBound() - ((IssueInteger)issue).getLowerBound()) * k); 
				v = bucket + 0.0; //not a real value - it just the "bucket name"
			}
			else return;
	...
	public Action chooseAction()
	{
		...
		if (!HaveMoreTimeToBid(true)){ //if have time for one last bid, offer the opponent Best possible bid for us (or accept it, if the opponent just offer it)
			try {
				if (utilitySpace.getUtility(BestOpponentBid) <= utilitySpace.getUtility(lastBid)){
					if (utilitySpace.getUtility(lastBid) > utilitySpace.getReservationValueUndiscounted())
						return new Accept();
				}
				else if (utilitySpace.getUtility(BestOpponentBid) > utilitySpace.getReservationValueUndiscounted())
					return new Offer(BestOpponentBid);
			} catch (Exception e) {e.printStackTrace();} 
			return new EndNegotiation(); //if the opponent's best offer is worse than the reservation value, end the negotiation.
		}
		if (!HaveMoreTimeToBid(false)){ //if have no more time for bidding (it about to have time-out), accept the opponent's offer, if it's high than the reservation value 
			try {
				if (utilitySpace.getUtility(lastBid) > utilitySpace.getReservationValueUndiscounted())
					return new Accept();
			} catch (Exception e) {e.printStackTrace();}
			return new EndNegotiation(); //else, take the reservation value.
		...
		ComparableBid cb = relevantBids.get(0); // get <best bid , it's utility>;
		double best; //best bid's utility
		try {
			best = alpha * cb.utility + (1-alpha) * GetEstimatedOpponentUtility(cb.bid);
		} catch (Exception e1) {
			e1.printStackTrace();
			best = 0.0;
		}
		int j = Math.max(1, lastPropose - 3000); //for efficient reasons, don't go over all possible bids - only on 10,000 bids at most (around our last proposal)
		for (int i = j; i < relevantBids.size() && i<lastPropose + 7000; i++) {
			double curr;
			ComparableBid currBid = relevantBids.get(i);
			
			try{	
				curr = alpha * currBid.utility + (1-alpha) * GetEstimatedOpponentUtility(currBid.bid);
			} catch (Exception e) {
				e.printStackTrace();
				curr = 0.0;
			}
		...
		try {
			if (lastBid != null && utilitySpace.getUtility(lastBid) > cb.utility ) //if the opponent offered better offer (to us) than what we decided to offer, we accept this offer.  
				a = new Accept(); 
			else if (cb.utility < utilitySpace.getReservationValueUndiscounted()) //if our offer is worse than the reservation value we can achieve, we'll end the negotiation. 
	...
	private ArrayList<Bid> GetDiscreteBids (){
		...
		try {bids.add(new Bid(utilitySpace.getDomain(), issusesFirstValue));}
       	catch (Exception e) {return null;}     	
		
		for (Issue issue:utilitySpace.getDomain().getIssues()){ //for every issue
       		ArrayList<Bid> tempBids = new ArrayList<Bid>(); //create a list of bids
       		ArrayList<Value> issueValues = new ArrayList<Value>();
       		if (issue.getType() == ISSUETYPE.DISCRETE){
       			ArrayList<ValueDiscrete> valuesD = (ArrayList<ValueDiscrete>) ((IssueDiscrete)issue).getValues(); //get list of options/values for this issue
       			for (Value v : valuesD){
       				issueValues.add(v);
       			}
       		}
       		...
       		for(Bid bid:bids){ //for each bid seen so far (init bids list)
   				for (Value value : issueValues) { //for every value
   					HashMap<Integer, Value> bidValues = new HashMap<Integer, Value>(); //make new ("empty") bid - only values.
   					for (Issue issue1: utilitySpace.getDomain().getIssues()) //go over all issues
   						try{
   							bidValues.put(issue1.getNumber(),bid.getValue(issue1.getNumber())); //each issue is entered
   						} catch(Exception e) {e.printStackTrace();}
   					bidValues.put(issue.getNumber(), value);
   					try { 
   						Bid newBid = new Bid(utilitySpace.getDomain(), bidValues);
   						tempBids.add(newBid); 
   					} catch(Exception e) {e.printStackTrace();}
   				}
   			}
	...
	private double GetEstimatedOpponentUtility (Bid b){
		...
		for (HashMap<Object, Double> h : opponentUtilityEstimator){
			try {
				...
				if (issue.getType() == ISSUETYPE.DISCRETE)
					d += h.get((ValueDiscrete)b.getValue(i))/maxValue.get(count);
				else if (issue.getType() == ISSUETYPE.INTEGER){
	...
	private double getHardness (){
		double alpha = 0,
				x = timeline.getTime(),
				y = utilitySpace.getDiscountFactor() <= 0.0 ? 0.0 : 1-utilitySpace.getDiscountFactor();
	...
	private boolean HaveMoreTimeToBid (boolean wantToMakeTwoProposals){
		if (wantToMakeTwoProposals && 1 - timeline.getTime() > 2 * EstimatedRTT + devRTT){
			return true;
		}
	...
	private Value GetRepresentorOfBucket(int bucket, Issue issue, int k, boolean isInteger){
		...
		if (isInteger){
			EvaluatorInteger ei = new EvaluatorInteger();
			boolean upperIsTheBest = ei.getEvaluation(((IssueInteger)issue).getUpperBound()) > ei.getEvaluation(((IssueInteger)issue).getLowerBound());
			if (upperIsTheBest){
				if (bucket < k){
					ans = ((double)(bucket+1))/k;
					ans = ans * (((IssueInteger)issue).getUpperBound() - ((IssueInteger)issue).getLowerBound()) +  ((IssueInteger)issue).getLowerBound() - 1;
				}
				else
		...
		boolean upperIsTheBest = ei.getEvaluation(((IssueReal)issue).getUpperBound()) > ei.getEvaluation(((IssueReal)issue).getLowerBound());
		if (upperIsTheBest){
			if (bucket < k){
				ans = ((double)(bucket+1))/k;

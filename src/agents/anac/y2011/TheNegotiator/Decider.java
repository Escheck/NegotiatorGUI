...
public class Decider {
	...
	public void setPartnerMove(Action action) {
		if(action instanceof Offer) {
			...
			try {
				bidsCollection.addPartnerBid(bid, agent.utilitySpace.getUtility(bid), timeManager.getTime());
			} catch (Exception e) {
	...
	public Action makeDecision() {
		...
		Action action = acceptor.determineAccept(phase, threshold, agent.timeline.getTime(), movesLeft);

...
public class OpponnentModel {
									...
									// Screen-Size/Brand)
	{
		...
		class NumericValues implements GahbonValueType {
			...
			public void UpdateImportance(Value OpponentBid /*
				...
				double AverageFrequencyIndex = BidFrequencyIndexSum
						/ OpponentBidCountToConsider;

				NormalizedVariance = 0;
				for (int i = 0; i < DiscretizationResolution; ++i) {
					double Distance = (AverageFrequencyIndex - i)
							/ (DiscretizationResolution - 1);
					NormalizedVariance += (double) (this.ValueFrequencies[i])
							* Distance * Distance;
				}
			...
			public double GetExpectedUtilityByValue(Value V) {
				return Math.min(
						1,
						Math.max(0,
								1 - Math.abs(TranslateValue(V) - FirstValue)));
			}
		...
		class DiscreteValues implements GahbonValueType {
			...
			private double GetOptionImportance(int OptionIndex) {
				int FirstOptionBonus = UpdatesCount
						/ (OptionOccurrencesCountByIndex.length); // the first
																	// choice
																	// always
																	// gets a
																	// large
			...
			public void UpdateImportance(Value OpponentBid /*
				...
				Integer incommingOptionIndex = OptionIndexByValue
						.get(OpponentBid);
				...
				for (int OptionIndex = 0; OptionIndex < OptionOccurrencesCountByIndex.length; ++OptionIndex) {
					...
					MostImportantValueOccurrencesAndBonuses = Math.max(
							MostImportantValueOccurrencesAndBonuses,
							OptionImportance);
				...
				for (Integer currentOptionIndex : OptionIndexByImportance
						.values()) {
					AverageImportanceRank += currentOptionRank
							* GetOptionImportance(currentOptionIndex);
					++currentOptionRank;
				}
				...
				for (Integer currentOptionIndex : OptionIndexByImportance
					...
					IssueImportanceRankVariance += OptionOccurrencesCountByIndex[currentOptionIndex]
							* // Occurrence count of current option
			...
			public double GetExpectedUtilityByValue(Value V) {
				...
				return GetOptionImportance(ValueIndex)
						/ MostImportantValueOccurrencesAndBonuses;
			}
			...
			public void INIT(negotiator.issue.Issue I) {
				...
				OptionIndexByValue = new TreeMap<ValueDiscrete, Integer>(
						DIComparer);
		...
		public void INIT(Issue I) {
			...
			if (I instanceof IssueDiscrete) {
				...
				IssueBase = new IssueDiscrete(DI.getName(), DI.getNumber(),
						values);
				Issue = new DiscreteValues();
				Issue.INIT(I);
			} else if (I instanceof IssueReal) {
				IssueReal RI = (IssueReal) I;
				IssueBase = new IssueReal(RI.getName(), RI.getNumber(),
						RI.getLowerBound(), RI.getUpperBound());
				Issue = new NumericValues();
				Issue.INIT(I);
			} else if (I instanceof IssueInteger) {
				IssueInteger II = (IssueInteger) I;
				IssueBase = new IssueReal(II.getName(), II.getNumber(),
						II.getLowerBound(), II.getUpperBound());
				Issue = new NumericValues();
				Issue.INIT(I);
			}
	...
	public OpponnentModel(UtilitySpace utilitySpace, Timeline timeline) {
		...
		for (int IssueIndex = 0; IssueIndex < IA.size(); ++IssueIndex) {
			...
			IPIndexByIssueNumber
					.put(IA.get(IssueIndex).getNumber(), IssueIndex);
		}
	}
	...
	public void UpdateImportance(Bid OpponentBid /*
		...
		for (int IssueIndex = 0; IssueIndex < IssuesByIndex.length; ++IssueIndex) {
			...
			IssuesByIndex[IssueIndex].Issue.UpdateImportance(OpponentBid
					.getValue(IssueID));

			double IssueVariance = IssuesByIndex[IssueIndex].Issue
			...
			if (0 == IssueVariance)
				...
				MinimalNonZeroVariance = Math.min(IssueVariance,
		...
		TotalIssueOptionsVariance /= MinimalNonZeroVariance
				* (1.0 / ZeroVarianceToMinimalVarianceWeight); // we now count
																// importance of
																// issue with
																// units
			...
			for (int IssueIndex = IssuesByIndex.length - 1; IssueIndex >= 0; --IssueIndex) {
				if (0 == IssuesByIndex[IssueIndex].Issue
					...
					IssuesByIndex[IssueIndex].ExpectedWeight = VarianceUnit
							/ IssuesByIndex[IssueIndex].Issue
									.GetNormalizedVariance();
		...
		try {
			for (int IssueIndex = 0; IssueIndex < IssuesByIndex.length; ++IssueIndex) {
				...
				UtilitySum += IssuesByIndex[IssueIndex].ExpectedWeight
						* IssuesByIndex[IssueIndex].Issue
								.GetExpectedUtilityByValue(B.getValue(IssueID));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Value ImproveValue(int IssueNumber, ValueDiscrete ValToImprove)
			throws Exception {
		TreeMap<String, ValueDiscrete> ValueTranslator = ValueTranslation
				.get(IssueNumber);
		...
		IssuePrediction IS = IssuesByIndex[IPIndexByIssueNumber
				.get(IssueNumber)];
	...
	public TreeMap<Double, Bid> FilterBids(TreeMap<Double, Bid> Bids,
			int DesiredResultEntries) throws Exception {
		...
		// int EntryContraction = Bids.size() /
		// Math.max(1,DesiredResultEntries);
		...
		if (resultBids.size() < DesiredResultEntries / 10
				|| resultBids.size() < 20)
			return Bids;
		return resultBids;
	}

...
public class IssueManager {
	...
	double Noise = 0.4; // 0 to 1 - Tells us how varied the opponent's bids are,
						// lately
	// My State:
	...
	public Bid GetMaxBidWithNoCost() throws Exception {
		...
		for (Issue issue : this.US.getDomain().getIssues()) {
			...
			case INTEGER:
				...
				justBidding = justBidding.putValue(issue.getNumber(),
						new ValueInteger(integerIssue.getUpperBound()));
				maxUtil = US.getUtility(justBidding);

				justBidding = justBidding.putValue(issue.getNumber(),
						new ValueInteger(integerIssue.getLowerBound()));
				tmpUtil = US.getUtility(justBidding);

				if (maxUtil > tmpUtil)
					maxBid = maxBid.putValue(issue.getNumber(),
							new ValueInteger(integerIssue.getUpperBound()));
				else
					maxBid = maxBid.putValue(issue.getNumber(),
							new ValueInteger(integerIssue.getLowerBound()));
			...
			case REAL:
				...
				justBidding = justBidding.putValue(issue.getNumber(),
						new ValueReal(realIssue.getUpperBound()));
				maxUtil = US.getUtility(justBidding);

				justBidding = justBidding.putValue(issue.getNumber(),
						new ValueReal(realIssue.getLowerBound()));
				tmpUtil = US.getUtility(justBidding);

				if (maxUtil > tmpUtil)
					maxBid = maxBid.putValue(issue.getNumber(), new ValueReal(
							realIssue.getUpperBound()));
				else
					maxBid = maxBid.putValue(issue.getNumber(), new ValueReal(
							realIssue.getLowerBound()));
			...
			case DISCRETE:
				...
				maxBid = maxBid.putValue(issue.getNumber(),
						discreteIssue.getValue(maxUtilValIndex));
				break;
			}
		}
	...
	public IssueManager(AdditiveUtilitySpace US, Timeline T, OpponnentModel om) {
		...
		for (int i = 0; i < US.getDomain().getIssues().size(); ++i) {
			...
			if (I.getType() == ISSUETYPE.DISCRETE) {
				...
				DifferentValuesCountPerIssueNum.put(ID.getNumber(),
						ID.getNumberOfValues());
				OutgoingValueAppeareancesByIssueNum.put(ID.getNumber(),
						new TreeMap<String, Integer>());
			} else if (I.getType() == ISSUETYPE.REAL) {
				DifferentValuesCountPerIssueNum.put(I.getNumber(),
						(int) DiscretisationSteps);
				OutgoingValueAppeareancesByIssueNum.put(I.getNumber(),
						new TreeMap<String, Integer>());
			} else if (I.getType() == ISSUETYPE.INTEGER) {
				IssueInteger II = (IssueInteger) I;
				DifferentValuesCountPerIssueNum.put(
						I.getNumber(),
						Math.min((int) DiscretisationSteps, II.getUpperBound()
								- II.getLowerBound() + 1));
	...
	void ClearIncommingStatistics() {
		for (Issue I : US.getDomain().getIssues())
			IncomingValueAppeareancesByIssueNum.put(I.getNumber(),
					new TreeMap<String, Integer>());
	...
	private void addPossibleValue(Issue issue, Value val) {
		...
		if (this.relevantValuesPerIssue.get(issue).size() > 0)
			randIndex = Math.abs(random200.nextInt())
					% this.relevantValuesPerIssue.get(issue).size();
	...
	private void buildIssueValues(Bid firstOppBid) throws Exception {
		...
		for (Issue issue : this.US.getDomain().getIssues()) {
			...
			justBidding = justBidding.putValue(issue.getNumber(),
					firstOppBid.getValue(issue.getNumber()));
			double utilityWithOpp = this.US.getEvaluation(issue.getNumber(),
					justBidding);
			...
			case INTEGER:
				...
				iStep = Math.max(1, (int) ((intIssue.getUpperBound() - intIssue
						.getLowerBound()) / totalSteps));

				for (int i = intIssue.getLowerBound(); i <= intIssue
						.getUpperBound(); i += iStep) {
					justBidding = justBidding.putValue(issue.getNumber(),
							new ValueInteger(i));
					double utilityWithCurrent = this.US.getEvaluation(
							issue.getNumber(), justBidding);
				...
				AddedValues += Math.abs(intIssue.getUpperBound()
						- intIssue.getLowerBound());
				break;
			case REAL:

				IssueReal realIssue = (IssueReal) issue;
				double oneStep = (realIssue.getUpperBound() - realIssue
						.getLowerBound()) / (DiscretisationSteps - 1);
				for (double curr = realIssue.getLowerBound(); curr <= realIssue
						.getUpperBound(); curr += oneStep) {
					justBidding = justBidding.putValue(issue.getNumber(),
							new ValueReal(curr));
					double utilityWithCurrent = this.US.getEvaluation(
							issue.getNumber(), justBidding);
	...
	public void learnBids(Bid firstOppBid) throws Exception {
		...
		Iterator<Entry<Issue, ArrayList<Value>>> MyIssueIterator = relevantValuesPerIssue
				.entrySet().iterator();
		...
		// bids
		BuildBid(new HashMap<Integer, Value>(), 0,
				0.05 * Math.pow(GetDiscountFactor(), 0.6) + startTime);
	...
	private void BuildBid(HashMap<Integer, Value> B, int EntrySetIndex,
			double EndTime) {
		...
		if (T.getTime() < EndTime) {
			if (EntrySetIndex < IssueEntries.size()) {
				Entry<Issue, ArrayList<Value>> currentEntry = IssueEntries
						.get(EntrySetIndex);
				...
				try {
					...
					Bid newBid = new Bid(US.getDomain(),
							new HashMap<Integer, Value>(B));

					double BidUtil = US.getUtility(newBid);

					while (UtilityCollisionsLeft > 0
							&& Bids.containsKey(BidUtil)) {
	...
	public double GetNextRecommendedOfferUtility() {
		...
		if (Time < 0.85 * GetDiscountFactor())
			...
			Max /= Min;
				...
				&& (BestEverOpponentBidUtil <= 2 * (1.0 / PlayerCount) || // never
																			// accept
																			// an
																			// offer
																			// with
																			...
																			// than
																			// that
				Time <= (1 - 3 * (TimeDiffEnd - TimeDiffStart)
			...
			TimeDiffEnd = TimeDiffStart = Time;
				...
				&& (BestEverOpponentBidUtil <= 2 * (1.0 / PlayerCount) || // never
																			...
																			// that
				Time <= (1 - 3 * (TimeDiffEnd - TimeDiffStart)
						/ RoundCountBeforePanic))) // until last few rounds
		{
			TimeDiffEnd = Time;
			++RoundCountBeforePanic;
	...
	String EstimateValue(Issue I, Value v) throws Exception {
		...
		case INTEGER:
			...
			if (Step != 0) {
				int totalSteps = (int) Math.min(DiscretisationSteps,
	...
	TreeMap<Integer, TreeMap<String/* value.ToString() */, Integer>> IncomingValueAppeareancesByIssueNum = new TreeMap<Integer, TreeMap<String, Integer>>(); // tells
																																								// how
																																								// many
																																								// times
																																								// each
																																								...
																																								// incoming
																																								// bids
	TreeMap<Integer, TreeMap<String/* value.ToString() */, Integer>> OutgoingValueAppeareancesByIssueNum = new TreeMap<Integer, TreeMap<String, Integer>>(); // tells
																																								// how
																																								// many
																																								// times
																																								// each
																																								...
																																								// incoming
																																								// bids
	TreeMap<Integer, Integer> DifferentValuesCountPerIssueNum = new TreeMap<Integer, Integer>(); // tells
																									// how
																									// many
																									// different
																									// values
																									...
																									// per
																									// Issue
	int CountdownToStatisticsRefreshing = 20; // when hits zero, recalculate
												// AverageDistance and reset
												// Noise
	...
	void AddMyBidToStatistics(Bid OutgoingBid) throws Exception {
		++OutgoingBidsCount;
		for (Issue I : US.getDomain().getIssues()) {
			String bidValueEstimation = EstimateValue(I,
					OutgoingBid.getValue(I.getNumber()));
			if (OutgoingValueAppeareancesByIssueNum.get(I.getNumber())
					.containsKey(bidValueEstimation)) {
				OutgoingValueAppeareancesByIssueNum.get(I.getNumber()).put(
						bidValueEstimation,
						OutgoingValueAppeareancesByIssueNum.get(I.getNumber())
								.get(bidValueEstimation) + 1);
			} else {
				OutgoingValueAppeareancesByIssueNum.get(I.getNumber()).put(
						bidValueEstimation, 1);
			}
		}
	}
	...
	public void ProcessOpponentBid(Bid IncomingBid) throws Exception {
		if (CountdownToStatisticsRefreshing > 0) {
			...
			for (Issue I : US.getDomain().getIssues()) {
				String bidValueEstimation = EstimateValue(I,
						IncomingBid.getValue(I.getNumber()));
				if (IncomingValueAppeareancesByIssueNum.get(I.getNumber())
						.containsKey(bidValueEstimation)) {
					IncomingValueAppeareancesByIssueNum.get(I.getNumber()).put(
							bidValueEstimation,
							IncomingValueAppeareancesByIssueNum.get(
									I.getNumber()).get(bidValueEstimation) + 1);
				} else {
					IncomingValueAppeareancesByIssueNum.get(I.getNumber()).put(
							bidValueEstimation, 1);
				}
			...
			for (Issue I : US.getDomain().getIssues()) {
				for (String val : OutgoingValueAppeareancesByIssueNum.get(
						I.getNumber()).keySet()) {
					if (IncomingValueAppeareancesByIssueNum.get(I.getNumber())
							.containsKey(val)) {
						float outgoingVal = ((float) (OutgoingValueAppeareancesByIssueNum
								.get(I.getNumber()).get(val)))
								/ OutgoingBidsCount;
						float incomingVal = (((float) (IncomingValueAppeareancesByIssueNum
								.get(I.getNumber()).get(val))) / CountdownLength);
						float diff = outgoingVal - incomingVal;
						float diffSqr = diff * diff; // 0 to 1

						CurrentSimilarity += (1.0 / US.getDomain().getIssues()
								.size())
								* (1.0 / DifferentValuesCountPerIssueNum.get(I
										.getNumber())) * (1 - diffSqr);
					}
				}
			}
			...
			if (CurrentSimilarity > PreviousAverageDistance) {
				...
				Noise += NoiseDecreaseRate; // Apparently, the opponent just
											// gave up some of his util
			} else {

...
public class Gahboninho extends Agent {
	...
	public void init() {
		...
		IM = new IssueManager((AdditiveUtilitySpace) utilitySpace, timeline, OM);
	...
	public Action chooseAction() {
		try {
			...
			if (WereBidsFiltered == false
					&& (timeline.getTime() > IM.GetDiscountFactor() * 0.9 || timeline
							.getTime() + 3 * IM.BidsCreationTime > 1)) /*
																		 * we
																		 * must
																		 * filter
																		 * to
																		 ...
																		 */
			{
				...
				int DesiredBidcount = (int) (RoundCount * (1 - timeline
						.getTime()));
		...
		if (FirstActions >= 0 && timeline.getTime() < 0.15) {
			...
			myBid = IM.GenerateBidWithAtleastUtilityOf(0.925 + utilDecrease
					* FirstActions);
			--FirstActions;
		} else {

...
public class AgentUtils {
	...
	public double getAgentUtil(Bid bid) {
		...
		for(int i=0;i<nIssues;i++) {
			...
			ValueDiscrete VD = (ValueDiscrete) bidValuesHashMap.get(issueIds[i]);
			if(issueList.get(i).getValues().contains(VD))				
				utility=utility+issueList.get(i).getDoubleValue(VD)*weight;
		}
		return utility;
	}
	
	// based on the most recent bids received recalculates the issue and value weights
	public void recalculateUtilFunction() {
		ArrayList<Issue> bidIssueList = bidHistory.getHistory().get(0).getBid().getIssues();
		...
		for(int i=0; i<nIssues;i++) {
			...
			double[] valueEvaluation = new double[100]; // random large number, just so we don't have to use a List
			for(int j=0; j<bidHistory.getHistory().size();j++) {
				ValueDiscrete currentValue =  (ValueDiscrete) bidHistory.getHistory().get(j).getBid().getValues().get(bidIssueList.get(i).getNumber());
				if(!valueList.contains(currentValue)) {
					valueList.add(currentValue);
				}
				valueEvaluation[(valueList.indexOf(currentValue))]=valueEvaluation[valueList.indexOf(currentValue)] + bidWeights[j];
			}
			int k=0;
			do { 
				try {
					issueList.get(i).setEvaluationDouble(valueList.get(k), valueEvaluation[k]);
				} catch (Exception e) {
					e.printStackTrace();
				}
				k++;
	...
	// Returns the number of changes in values for each issue through the entire bid history
	private int[] calculateNChanges() {
		...
		for(int i=0; i<nIssues;i++) {
			for(int j=0; j<bidHistory.getHistory().size();j++) {				
				try {					
					if( oldValue!=null && !oldValue.equals(bidHistory.getHistory().get(j).getBid().getValue(issueIds[i]))) {
						nChanges[i]++;
					}			
					oldValue=bidHistory.getHistory().get(j).getBid().getValue(issueIds[i]);
				} catch (Exception e) {
					e.printStackTrace();
				}		
	...
	// Gives a weight to each bid we received, the first bids are worth much more than the last ones

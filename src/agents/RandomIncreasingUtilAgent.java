 ...
 * We only consider random bids with utility higher than the maximum
 * and we search at most MAX_SEARCH times in the random space for a better bid.
 * of the opponent utilities we saw so far.
 */
public class RandomIncreasingUtilAgent extends Agent
{
    private Action actionOfPartner=null;
    private static final double MINIMUM_BID_UTILITY = 0.5; // obviously we will fail if there are no bids with this utility!
	...
	private Bid getRandomBid() throws Exception
    {
    	...
    	// create a random bid with utility>MINIMUM_BID_UTIL AND utility>max_opponent_util
       ...
       int nr_tries=0;
    	do {
	        ...
	        nr_tries++;
    	} while ((bidutility<MINIMUM_BID_UTILITY || bidutility<max_opponent_utility) && nr_tries<MAX_SEARCH);
    	
    	if (bidutility<MINIMUM_BID_UTILITY || bidutility<max_opponent_utility) // reached search limit? 
    	{
    		// could not find a better bid than we received, let's offer that best bid so far...

...
public class Group3 extends AbstractNegotiationParty {
	...
	private List<BidHistory> bidhistorylist; // List of the lists of Bids received, one list for every agent
	private List<AgentID> partylist;	// List of Agents	
	private ArrayList<Bid> possibleBids;	// List of all possible Bids in this utility Space
	private ArrayList<Bid> alreadyProposed;	// List of all Bids, we received and then reproposed
	 ...
	 */
	public Group3(UtilitySpace utilitySpace,
	...
	//	System.out.println("My Next Bid Utility"+ Strategy.nextBidUtility(this));
		
		// Check if we should offer an offer we received already, we will only resend an offer one time
	...
	//		System.out.println("Offering previous bid, of Utility "+ getUtility(toOffer));
			return new Offer(toOffer);
		}
		
		//Generate new offer with that desired utility
		Bid toOffer=Strategy.calculateMyBid(this);
	//	System.out.println("Generating a new Bid, of Utility "+ getUtility(toOffer));	
		return new Offer(toOffer);
	}
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if(action instanceof Offer){		
			if(!partylist.contains(/*action.getAgent()*/ ((AbstractNegotiationParty) sender).getPartyId())) { 
				// We have never seen this agent
				//System.out.println("New Agent: " + action.getAgent().toString());
				partylist.add(((AbstractNegotiationParty) sender).getPartyId()); // add it to our list
				BidHistory newAgentBidHistory = new BidHistory(); // create a new agent list and add it.
				bidhistorylist.add(newAgentBidHistory);
				// create a new agentUtils and add it
				AgentUtils newAgentUtils = new AgentUtils(((AbstractNegotiationParty) sender).getPartyId(), newAgentBidHistory, utilitySpace.getNrOfEvaluators());
				agentUtilsList.add(newAgentUtils);
			}
			lastbid = Action.getBidFromAction(action);
			//add the bid to the bidhistory
			bidhistorylist.get(partylist.indexOf(((AbstractNegotiationParty) sender).getPartyId())).add(new BidDetails(lastbid, getUtility(lastbid)));
			//Loop through our agent List to find the one that sent the message and update his AgentUtils
			for(int i=0;i<agentUtilsList.size();i++) {
				if(agentUtilsList.get(i).agent==((AbstractNegotiationParty) sender).getPartyId()) {
					agentUtilsList.get(i).recalculateUtilFunction();
					break;
				}
			}
 			//System.out.println(this.getPartyId().toString() + " Received bid of utility:" + getUtility(lastbid));
		}
	...
	public int roundDeadline() {
		return  (int) deadlines.get(DeadlineType.ROUND);
	}

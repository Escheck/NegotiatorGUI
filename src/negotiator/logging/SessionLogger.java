...
public class SessionLogger
{
    ...
    // 'owner' of this logger: creating class. Used for reference in logging messages
    private Object owner;

    /**
     * Command line interface
     *
     * @param owner The object creating this class. Used for reference in log messages
     */
     ...
     *
     * @param listener The class that should receive messages from this logger
     */
     ...
     *
     * @param listener The listener that should stop receiving messages from this logger
     */
     ...
     * @param params format arguments in case message is a format string.
     *
     */
    public void logMessage(String message, Object... params)
    {
        LogMessageEvent event = new LogMessageEvent(owner, "Nego", String.format(message, params));
        for (MultipartyNegotiationEventListener listener : listeners)
            listener.handleLogMessageEvent(event);
    }

    /**
     * Log the sessions most recent action as an offer. Called after every offer.
     *
     * @param session The session to get the most recent action out of
     * @param parties The parties involved in the offer
     * @param agree flag indicating whether the offer is an agreement or not
     */
    public void logBid(Session session, List<NegotiationParty> parties, boolean agree)
    {
        ...
        if (action instanceof Offer)
        {
            ...
            for (NegotiationParty party : MediatorProtocol.getNonMediators(parties)) utils.add(party.getUtility(bid));
            MultipartyNegotiationOfferEvent event
                    = new MultipartyNegotiationOfferEvent(owner, round, turn, time, action, utils, agree);
     ...
     * @param agreement The agreement if any (NULL otherwise)
     */
    public void logSession(Session session, Bid agreement)
    {
        for (MultipartyNegotiationEventListener listener : listeners)
            listener.handleMultipartyNegotiationEvent(new MultipartyNegotiationSessionEvent(owner, session, agreement));
    }

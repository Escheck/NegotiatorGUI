...
public class OfferBestN extends OMStrategy {
	 ...
	 * @param model
	 *            opponent model strategy used by this opponent model strategy.
	 * @param n
	 *            amount of best bids from which a random bid is selected.
	 */
	public OfferBestN(NegotiationSession negotiationSession,
			OpponentModel model, int n) {
	...
	public void init(NegotiationSession negotiationSession,
		...
		if (parameters.get("n") != null) {
			...
			throw new Exception(
					"Constant \"n\" for amount of best bids was not set.");
		}
	...
	public BidDetails getBid(List<BidDetails> allBids) {
		...
		ArrayList<BidDetails> oppBids = new ArrayList<BidDetails>(
				allBids.size());
		for (BidDetails bidDetail : allBids) {
			Bid bid = bidDetail.getBid();
			BidDetails newBid = new BidDetails(bid,
					model.getBidEvaluation(bid), negotiationSession.getTime());
			oppBids.add(newBid);
		}
		...
		try {
			nextBid = new BidDetails(opponentBestBid, negotiationSession
	...
	public Set<BOAparameter> getParameters() {
		...
		set.add(new BOAparameter("n", new BigDecimal(3.0),
				"A random bid is selected from the best n bids"));

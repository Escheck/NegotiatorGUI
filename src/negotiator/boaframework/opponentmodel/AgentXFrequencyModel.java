...
public class AgentXFrequencyModel extends OpponentModel {
	 ...
	 * @param i The issue for which a ValueProcessor is needed
	...
	public DiscreteValueProcessor getValueProcessor(IssueDiscrete i){
		...
		if (index != -1) return valueProcessList.get(index);
	 ...
	 * @param b The bid done by the opponent
	 ...
	 * @param issueIndex The index of the issue. Same index as in the ArrayList provided by 
	 * utilitySpace.getDomain().getIssues()
	 * @param valueOfIssue The value from that issue for which the normalized rank is required
	...
	public double getBidEvaluation(Bid bid) {
		...
		// Taking into account that Opponent Issue list is in the same our of the domain
		int nrIssues = negotiationSession.getUtilitySpace().getDomain().getIssues().size();
		
		for (int i = 0; i < nrIssues; i++) {
			try {
				//It was needed to use an auxiliar variable startingBidIssue, because we found out it varies from domain to domain
				ValueDiscrete valueOfIssue = (ValueDiscrete) bid.getValue(i+startingBidIssue);
				double w = getIssueProcessor().getWeightByIssue( (IssueDiscrete) negotiationSession.getUtilitySpace().getDomain().getIssue(i));

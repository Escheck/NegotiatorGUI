 ...
 * 
 * This model increases its weights only when the opponent does not
 * constanstly offer the same bid.
 * The values of this model are scaled before they are used to
...
public class InoxAgent_OM extends OpponentModel {
	...
	/** Smaller version of the opponent's bidding history, containing only unique bids */
	...
	public void updateModel(Bid opponentBid, double time) {
		// Do nothing with the model if the opponent's bidding history is too small for analysis
	...
	public double getBidEvaluation(Bid bid) {
		double result = 0;
		try {
			// If opponent is friendly, conceding opponent we can use the normal frequency
	...
	/**
	 * Method used to estimate the total number of rounds by evaluating
	 * the iteration time in the last 10 rounds.
	 */

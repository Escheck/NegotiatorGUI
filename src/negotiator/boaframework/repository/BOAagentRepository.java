...
public class BOAagentRepository {
	...
	public ArrayList<String> getOfferingStrategies() {
		return new ArrayList<String>(repositoryParser.getBiddingStrategies()
				.keySet());
	}
	...
	public ArrayList<String> getAcceptanceStrategies() {
		return new ArrayList<String>(repositoryParser.getAcceptanceConditions()
				.keySet());
	}
	...
	public ArrayList<String> getOpponentModels() {
		return new ArrayList<String>(repositoryParser.getOpponentModels()
				.keySet());
	}
	...
	public ArrayList<String> getOMStrategies() {
		return new ArrayList<String>(repositoryParser.getOMStrategies()
				.keySet());
	}
	...
	public String toXML() {
		...
		for (Entry<String, BOArepItem> entry : repositoryParser
				.getBiddingStrategies().entrySet()) {
			buffer.append(entry.getValue().toXML());
		}
		buffer.append("\t</biddingstrategies>\n");
		buffer.append("\t<acceptanceconditions>\n");
		for (Entry<String, BOArepItem> entry : repositoryParser
				.getAcceptanceConditions().entrySet()) {
			buffer.append(entry.getValue().toXML());
		}
		buffer.append("\t</acceptanceconditions>\n");
		buffer.append("\t<opponentmodels>\n");
		for (Entry<String, BOArepItem> entry : repositoryParser
				.getOpponentModels().entrySet()) {
			buffer.append(entry.getValue().toXML());
		}
		buffer.append("\t</opponentmodels>\n");
		buffer.append("\t<omstrategies>\n");
		for (Entry<String, BOArepItem> entry : repositoryParser
				.getOMStrategies().entrySet()) {
			buffer.append(entry.getValue().toXML());
		}
		buffer.append("\t</omstrategies>\n");
	...
	public int getItemsCount() {
		return repositoryParser.getBiddingStrategies().size()
				+ repositoryParser.getAcceptanceConditions().size()
				+ repositoryParser.getOMStrategies().size()
				+ repositoryParser.getOpponentModels().size();
	}
	...
	public void removeComponent(BOArepItem removed) {
		...
		case ACCEPTANCESTRATEGY:
			repositoryParser.getAcceptanceConditions()
	...
	public void addComponent(BOArepItem newComponent) {
		...
		case BIDDINGSTRATEGY:
			repositoryParser.getBiddingStrategies().put(newComponent.getName(),
					newComponent);
			break;
		case OPPONENTMODEL:
			repositoryParser.getOpponentModels().put(newComponent.getName(),
					newComponent);
			break;
		case ACCEPTANCESTRATEGY:
			repositoryParser.getAcceptanceConditions().put(
					newComponent.getName(), newComponent);
			break;
		case OMSTRATEGY:
			repositoryParser.getOMStrategies().put(newComponent.getName(),
					newComponent);
			break;
		default:
			break;

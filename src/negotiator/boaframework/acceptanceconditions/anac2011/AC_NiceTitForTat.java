...
public class AC_NiceTitForTat extends AcceptanceStrategy {
	...
	public Actions determineAcceptability() {
		...
		// NOTE: I deleted the code which would give the best bid instead of false!
	}
	...
	public boolean isAcceptable() {
		...
		// AC_NEXT
		try {
			if (negotiationSession.getUtilitySpace().getUtility(opponentBid.getBid()) >= myNextBid.getMyUndiscountedUtil()) {
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		ArrayList<BidDetails> recentBetterBids = ((NiceTitForTatSAS) helper).filterBetween(offeredUndiscountedUtility, 1, now - window, now);

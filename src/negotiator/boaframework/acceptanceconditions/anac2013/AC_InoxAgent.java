...
public class AC_InoxAgent extends AcceptanceStrategy {
	 ...
	 */
	public void init(NegotiationSession negoSession, OfferingStrategy strat, OpponentModel oppModel, HashMap<String, Double> parameters) throws Exception {
		this.negotiationSession = negoSession;
		this.offeringStrategy = strat;
		this.opponentModel = oppModel;
		discountFactor = negotiationSession.getDiscountFactor();
	...
	public Actions determineAcceptability() {
		...
		if (negotiationSession.getOwnBidHistory().getWorstBidDetails() != null) {
			nextMyBidUtil = negotiationSession.getDiscountedUtility(negotiationSession.getOwnBidHistory().getWorstBidDetails().getBid(),time);
		}
		
		// Determine utility of the opponent bid to be evaluated
		double lastOpponentBidUtil = negotiationSession.getDiscountedUtility(negotiationSession.getOpponentBidHistory().getLastBid(),time);
		
		// Break if we are making an offer that is worse than the reservation value
		if (nextMyBidUtil < negotiationSession.getUtilitySpace().getReservationValueWithDiscount(time)) {
			return Actions.Break;
		// Accept if the opponents offer is better than the one we are about to make (or very close to it in discounted domains)
		} else if (nextMyBidUtil <= lastOpponentBidUtil+0.05*(1-discountFactor)) {
			return Actions.Accept;
		// Accept if the opponents offer is better than the utility from acceptUtil()
		} else if (lastOpponentBidUtil >= acceptUtil(time)) {
			return Actions.Accept;
		// Break if the reservation value appears to be more attractive than continuing negotiation
		} else if (!realmedian && medianutil*Math.pow(discountFactor, time) < negotiationSession.getUtilitySpace().getReservationValueUndiscounted()) {
			return Actions.Break;
		}
		// Reject and make our own offer if none of the above conditions were fulfilled
		return Actions.Reject;
	}
	
	/**
	 * Method that returns the minimal utility at which we accept an
	 * opponents offer. The function used depends on time and the
	 * opponents best discounted offer.
	 * @param t Normalized current time in the negotiation
	 */
	public double acceptUtil(double t) {
		double finalVal = negotiationSession.getDiscountedUtility(negotiationSession.getOpponentBidHistory().getBestDiscountedBidDetails(negotiationSession.getUtilitySpace()).getBid(), t);
		finalVal = Math.max(Math.max(finalVal,negotiationSession.getUtilitySpace().getReservationValueWithDiscount(t)),medianutil*Math.pow(discountFactor, t));

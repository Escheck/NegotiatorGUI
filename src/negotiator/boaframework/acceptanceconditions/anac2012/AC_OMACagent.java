...
public class AC_OMACagent extends AcceptanceStrategy {
	...
	public Actions determineAcceptability() {
		...
		double myOfferedUtil = negotiationSession.getDiscountedUtility(
				offeringStrategy.getNextBid().getBid(), time);
		double offeredUtilFromOpponent = negotiationSession
				.getDiscountedUtility(negotiationSession
						.getOpponentBidHistory().getLastBid(), time);
	...
	public boolean bidAlreadyMade(Bid a) {
		...
		for (int i = 0; i < negotiationSession.getOwnBidHistory().size(); i++) {
			if (a.equals(negotiationSession.getOwnBidHistory().getHistory()
					.get(i).getBid())) {
				result = true;
			}
		}

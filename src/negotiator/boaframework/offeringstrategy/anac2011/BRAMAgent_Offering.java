...
public class BRAMAgent_Offering extends OfferingStrategy {
	...
	@Override
	public void init(NegotiationSession domainKnow, OpponentModel om,
	...
	public void initializeAgent(NegotiationSession negoSession,
			OpponentModel om, OMStrategy oms) {
		...
		try {
			...
			maxUtility = negotiationSession.getUtilitySpace()
					.getUtilityWithDiscount(bestBid,
	...
	public BidDetails determineNextBid() {
		round++;
		threshold = ((BRAMAgentSAS) helper).getNewThreshold(
				ourBidsArray.get(ourBidsArray.size() - 1), bestBid);// Update
																	// the
																	// threshold
																	// according
																	// to the
			...
			try {
				...
				if (opponentBidsArray.size() < OPPONENT_ARRAY_SIZE) {// In this
					...
					if (opponentModel instanceof NoModel) {
						...
						threshold = ((BRAMAgentSAS) helper).getNewThreshold(
								ourBidsArray.get(ourBidsArray.size() - 1),
								getBidToOffer());// Update the threshold
													// according to the discount
													// factor
						bidToOffer = omStrategy.getBid(outcomespace, threshold)
								.getBid();
					}
				}

				nextBid = new BidDetails(bidToOffer, negotiationSession
						.getUtilitySpace().getUtility(bidToOffer),
			...
			for (Issue lIssue : issues) {
				...
				case DISCRETE:
					if (opponentBidsStatisticsDiscrete != null
							&& opponentBidsStatisticsDiscrete
	...
	private Bid getBidToOffer() {
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				...
				if (currBid != null) {
					double currUtility = negotiationSession.getUtilitySpace()
							.getUtilityWithDiscount(currBid,
									negotiationSession.getTime());
		...
		if (ourBidsArray.size() == 1) {
			...
			Collections.sort(ourBidsArray, new Comparator<Bid>() {
				...
				public int compare(Bid bid1, Bid bid2) {
					...
					double utility2 = 0.0;
					try {
						utility1 = negotiationSession.getUtilitySpace()
								.getUtility(bid1);
						utility2 = negotiationSession.getUtilitySpace()
								.getUtility(bid2);
	...
	private Bid createBidByOpponentModeling() {
		...
		try {
			...
			ArrayList<Issue> issues = negotiationSession.getIssues();
			...
			for (Issue lIssue : issues) {
					...
					for (int i = 0; i < valueList.size(); i++) {
						...
						if (indx >= first && indx <= last) {
							int lNrOfOptions = lIssueReal
									.getNumberOfDiscretizationSteps();
							double lOneStep = (lIssueReal.getUpperBound() - lIssueReal
									.getLowerBound()) / lNrOfOptions;
							double lowerBound = lIssueReal.getLowerBound();
							double realValueForBid = lowerBound + lOneStep
									* indx + random100.nextDouble() * lOneStep;
				...
				bid = new Bid(negotiationSession.getUtilitySpace().getDomain(),
						valuesToOfferPerIssue);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void fillBidsArray(double startTime) {
		...
		if (TEST_EQUIVALENCE) {
			BidIterator iterator = new BidIterator(negotiationSession
		...
		try {
			...
			for (Issue lIssue : issues) {
				...
				case DISCRETE:
					...
					IssueDiscrete lIssueDiscrete = (IssueDiscrete) lIssue;
					int optionIndex = random200.nextInt(lIssueDiscrete
							.getNumberOfValues());
				...
				case REAL:
					...
					ValueReal value = new ValueReal(lIssueReal.getLowerBound()
							+ lOneStep * lOptionIndex + random200.nextDouble()
							* lOneStep);
	...
	private Bid selectCurrentBidFromOurBidsArray() {
		...
		// ADDED *********************************//
		if (this.negotiationSession.getUtilitySpace().getUtilityWithDiscount(
				toSend, negotiationSession.getTimeline()) < threshold) {

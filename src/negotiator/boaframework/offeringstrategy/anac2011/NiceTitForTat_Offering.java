...
public class NiceTitForTat_Offering extends OfferingStrategy {
	...
	public BidDetails determineNextBid() {
		...
		double maximumOfferedUtilityByOpponent = negotiationSession.getOpponentBidHistory().getBestBidDetails().getMyUndiscountedUtil();
	...
	{
		ArrayList<BidDetails> firstBidList = ((NiceTitForTatSAS) helper).filterBetweenTime(0, TIME_USED_TO_DETERMINE_OPPONENT_STARTING_POINT);
		BidHistory firstBids = new BidHistory(firstBidList);
		double firstBidsMinUtility;
		if (firstBids.size() == 0)
			firstBidsMinUtility = negotiationSession.getOpponentBidHistory().getFirstBidDetails().getMyUndiscountedUtil();
		else
	...
	private Bid getBestBidForSelf(ArrayList<BidDetails> bids) {
		...
		for (int i = 0; i < bids.size(); i++) {
			try {
				if (negotiationSession.getUtilitySpace().getUtility(bids.get(i).getBid()) > 
				negotiationSession.getUtilitySpace().getUtility(maxBid)) {
	...
	public double getUtility(Bid bid) {
    	return negotiationSession.getUtilitySpace().getUtilityWithDiscount(bid, negotiationSession.getTimeline().getTime());
    }
}

 ...
 * for the opponent.
 * 
 * However, if the opponent shows non-conceding behaviour, this strategy will reset to the
 * best possible bid.
 * 
 * Over time the window that is used to pick bids from is increased, this will happen more
 * quickly on a discounted domain.
 * 
...
public class InoxAgent_Offering extends OfferingStrategy {
	 ...
	 * 
	 * This bidding strategy attempts to concede to the opponent by selecting their
	 * best bids in a growing window, starting at it's own best bids.
	 * In the case that the opponent shows non-conceding behaviour, this agent "resets"
	 * it's offer.
	 */
	@Override
	public BidDetails determineNextBid() {
		...
		if (!medianDecided || (roundsLeft <= 4 && !realmedian)) {
			int opplocation = outcomespace.getIndexOfBidNearUtility(negotiationSession.getOpponentBidHistory().getFirstBidDetails().getMyUndiscountedUtil());
			List<BidDetails> alloutcomes = outcomespace.getAllOutcomes();
		...
		BidDetails bestOppBidDetails = negotiationSession.getOpponentBidHistory().getBestDiscountedBidDetails(negotiationSession.getUtilitySpace());
		
		// If some bid of the opponent was already good for us, make that offer again
		if (bestOppBidDetails.getMyUndiscountedUtil() > negotiationSession.getOwnBidHistory().getWorstBidDetails().getMyUndiscountedUtil()) {
		...
		if (time < 0.99) {
			...
			// early on in the negotiation, concede according to our utility
				...
				if (outcomespace.getAllOutcomes().get(lastBidIndex+1).getMyUndiscountedUtil() > Math.max(Math.max(medianutil,negotiationSession.getOpponentBidHistory().getBestBidDetails().getMyUndiscountedUtil()),negotiationSession.getUtilitySpace().getReservationValueUndiscounted())) {
					lastBidIndex += 1;
		...
		// If the opponent has at some point made a better offer than we are about to make, make that offer instead
	 ...
	 * @param oHS The size of the opponents bid history
	 */
	private boolean oppNotConceding(int oHS) {
		double minLastOppUtils = 1.0;
		Iterator<BidDetails> bidIter = negotiationSession.getOpponentBidHistory().getHistory().subList(Math.max(oHS - 4,0), oHS).iterator();
		...
		return (minLastOppUtils >= negotiationSession.getOpponentBidHistory().getLastBidDetails().getMyUndiscountedUtil());
	}
	
	/**
	 * Calculate the size of the window that bids are evaluated in.
	 * The window size increases with time using a 20th order function
	 * in an undiscounted domain, the order decreases when discounts
	 * have more influence.
	 */
	...
	/**
	 * Determines most fair bid from a list of bids,
	 * using the bid that is closest to a rotating line
	 * which converges to the Kalai-Smorodinsky line.
	 * A requirement that the offer needs to be at least 0.12
	 * better for us is imposed as a safety measure.
	 * 
	 * Also updates the lastBidIndex corresponding to the 
	 * index of this bid in the outcome space.
	 */
	private BidDetails kalaiSmor(List<BidDetails> bList, int startIndex, double t) {
		...
		while (it.hasNext()) {
			...
			currIndex++;
			double itKalai = Math.min(itBid.getMyUndiscountedUtil(),a*opponentModel.getBidEvaluation(itBid.getBid()));
			double optKalai = Math.min(optBid.getMyUndiscountedUtil(),a*opponentModel.getBidEvaluation(optBid.getBid()));
			if ( itKalai > optKalai && itBid.getMyUndiscountedUtil() >= (opponentModel.getBidEvaluation(itBid.getBid())+0.12)) {
				optBid = itBid;
	...
	/**
	 * Method used to estimate the number of rounds that are left
	 * by using the time between iteration for the last 10 rounds.
	 */

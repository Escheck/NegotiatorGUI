...
public class IAMHaggler_Test_Offering extends OfferingStrategy {
	...
	public void init(NegotiationSession negotiationSession,
		...
		if (parameters.containsKey("r")) {
			...
			System.out.println("Using default " + amountOfRegressions
					+ " for amount of regressions.");
		}
		if (parameters.containsKey("s")) {
			double value = parameters.get("s");
			amountOfSamples = (int) value;
		} else {
			amountOfSamples = TournamentConfiguration.getIntegerOption(
					"deadline", 10) / 2;
			System.out.println("Using default " + amountOfSamples
	...
	public BidDetails determineOpeningBid() {
		if (!negotiationSession.getOpponentBidHistory().isEmpty()) {
			double myUndiscountedUtil = negotiationSession
					.getOpponentBidHistory().getLastBidDetails()
					.getMyUndiscountedUtil();
			double time = negotiationSession.getTime();
			concessionModel.updateModel(myUndiscountedUtil, time);
			System.out
					.println("IAMHagglerOpponentConcessionModel initialized with u = "
							+ myUndiscountedUtil + ", t = " + time);
	...
	public BidDetails determineNextBid() {
		...
		double myUndiscountedUtil = negotiationSession.getOpponentBidHistory()
				.getLastBidDetails().getMyUndiscountedUtil();
		double time = negotiationSession.getTime();
		int round = ((DiscreteTimeline) negotiationSession.getTimeline())
				.getRound();
		concessionModel.updateModel(myUndiscountedUtil, time);
		...
		if (negotiationSession.getTime() > 0.5) {
			...
			System.out.println("variances: "
					+ variancesWriter.getBuffer().toString());
	...
	public Set<BOAparameter> getParameters() {
		...
		set.add(new BOAparameter("r", new BigDecimal(36.0),
				"Amount of regressions"));
		set.add(new BOAparameter("s", new BigDecimal(100.0),
				"Amount of time samples"));

...
public class OpponentBidHistory {
    ...
    public OpponentBidHistory(OpponentModel model, OMStrategy oms) {
    	this.model = model;
    	this.oms = oms;
    ...
    public Bid ChooseBid(List<Bid> candidateBids, Domain domain, UtilitySpace space) {
    	...
    	if (!(model instanceof NoModel)) {
    		...
    		for (Bid bid : candidateBids) {
    			try {
    				list.add(new BidDetails(bid, space.getUtility(bid)));
    			} catch (Exception e) {
    				e.printStackTrace();
    			}
    		}
    		return oms.getBid(list).getBid();
    	} else {
	        ...
	        try {
	            if (candidateBids.size() < upperSearchLimit) {
	                ...
	                for (int i = 0; i < upperSearchLimit; i++) {
	                    ...
	                    realIndex = discreteIndex = integerIndex = 0;
	                    for (int j = 0; j < issues.size(); j++) {
	                        Value v = candidateBids.get(issueIndex).getValue(issues.get(j).getNumber());
	                        switch (issues.get(j).getType()) {
	                            case DISCRETE:
	                                if (opponentBidsStatisticsDiscrete == null) {
	                                    System.out.println("opponentBidsStatisticsDiscrete is NULL");
    ...
    public double concedeDegree(UtilitySpace utilitySpace) {
        ...
        //System.out.println("the opponent's toughness degree is " + bidCounter.size() + " divided by " + utilitySpace.getDomain().getNumberOfPossibleBids());
   ...
   //Another way to predict the opponent's concession degree
    public double getConcessionDegree() {
        ...
        //System.out.println("the history length is" + bidHistory.size() + "concessiondegree is " + concessionDegree);

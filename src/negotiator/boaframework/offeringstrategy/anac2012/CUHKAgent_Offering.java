...
public class CUHKAgent_Offering extends OfferingStrategy {
	...
	public CUHKAgent_Offering(NegotiationSession negoSession,
	...
	public void init(NegotiationSession negoSession, OpponentModel model,
		...
		maximumOfBid = negoSession.getUtilitySpace().getDomain()
		...
		this.bid_maximum_utility = negoSession.getUtilitySpace()
				.getMaxUtilityBid();
		this.utilitythreshold = negoSession.getUtilitySpace().getUtility(
				bid_maximum_utility); // initial utility threshold
		...
		if (negoSession.getUtilitySpace().getDiscountFactor() <= 1D
			...
			this.discountingFactor = negoSession.getUtilitySpace()
					.getDiscountFactor();
		}
		this.chooseUtilityThreshold();
		this.calculateBidsBetweenUtility();
		this.chooseConcedeToDiscountingDegree();
		this.opponentBidHistory.initializeDataStructures(negoSession
		...
		if (negoSession.getUtilitySpace().getReservationValue() > 0) {
			this.reservationValue = negoSession.getUtilitySpace()
					.getReservationValue();
		}
	}
	...
	public BidDetails determineNextBid() {
		...
		try {
			...
			this.timeLeftBefore = negotiationSession.getTimeline()
					.getCurrentTime();
			BidDetails bid = null;
			...
			if (negotiationSession.getOpponentBidHistory().isEmpty()) {
				...
				this.opponentBidHistory.updateOpponentModel(negotiationSession
				...
				if (negotiationSession.getOwnBidHistory().size() == 0) {
					...
					if (estimateRoundLeft(true) > 10) {// still have some rounds
						...
						if (negotiationSession.getTimeline().getTime() > 0.9985
							...
							bid = new BidDetails(bid1, negotiationSession
									.getUtilitySpace().getUtility(bid1));
							...
							if (negotiationSession.getUtilitySpace()
								...
								if (this.estimateRoundLeft(true) < 2) {
									Bid bid2 = opponentBidHistory
											.getBestBidInHistory();
									bid = new BidDetails(bid2,
											negotiationSession
													.getUtilitySpace()
													.getUtility(bid2));
								} else {
									Bid possibleBid = opponentBidHistory
											.ChooseBid(candidateBids,
													negotiationSession
															.getUtilitySpace()
															.getDomain());
			...
			this.ownBidHistory
					.addBid(bid.getBid(),
							(AdditiveUtilitySpace) negotiationSession
									.getUtilitySpace());
			this.timeLeftAfter = negotiationSession.getTimeline()
					.getCurrentTime();
	...
	private BidDetails BidToOffer() {
		...
		try {
			 ...
			 * minimumOfBid = this.MaximumUtility - decreasingAmount_3; } else
			 * if (this.discountingFactor > 1 - decreasingAmount_4 &&
			 * this.maximumOfBid >= 300000) { minimumOfBid = this.MaximumUtility
			 * - decreasingAmount_4; }
			 */else {// the general case
				if (negotiationSession.getTimeline().getTime() <= this.concedeToDiscountingFactor) {
					...
					this.utilitythreshold = maximumOfBid
							...
							* Math.pow(
									(negotiationSession.getTimeline().getTime() / this.concedeToDiscountingFactor),
									alpha1);
				} else {
			...
			Bid bestBidOfferedByOpponent = opponentBidHistory
					.getBestBidInHistory();
			if (negotiationSession.getUtilitySpace().getUtility(
			...
			if (bidReturned == null) {
				...
				bidReturned = this.negotiationSession.getUtilitySpace()
						.getMaxUtilityBid();
			}
		} catch (Exception e) {
		...
		try {
			return new BidDetails(bidReturned, negotiationSession
					.getUtilitySpace().getUtility(bidReturned));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	 ...
	 * System.out.println(e.getMessage() +
	 * "Exception in method AcceptOpponentOffer part 1"); } try {
	 * nextRoundUtility =
	 ...
	 * " we predict we can get at most " + predictMaximumUtility +
	 * "we concede now to avoid lower payoff due to conflict"); return true; }
	 * else { this.concedeToOpponent = true; return false; } } catch (Exception
	 * e) { System.out.println("exception in Method AcceptOpponentOffer");
	 * return true; } //retrieve the opponent's biding history and utilize it }
	 ...
	 * this.reservationValue+" after discounting is "+currentMaximumUtility); if
	...
	private int estimateRoundLeft(boolean opponent) {
		...
		round = (this.totalTime - negotiationSession.getTimeline()
				.getCurrentTime())
	...
	private void calculateBidsBetweenUtility() {
		BidIterator myBidIterator = new BidIterator(this.negotiationSession
				.getUtilitySpace().getDomain());
		...
		try {
			...
			if (this.maximumOfBid < 20000) {
				while (myBidIterator.hasNext()) {
					...
					for (int i = 0; i < maximumRounds; i++) {
						if (negotiationSession.getUtilitySpace().getUtility(b) <= (i + 1)
								* 0.01 + minUtility
								&& negotiationSession.getUtilitySpace()
										.getUtility(b) >= i * 0.01 + minUtility) {
	...
	private Bid RandomSearchBid() throws Exception {
		...
		ArrayList<Issue> issues = negotiationSession.getUtilitySpace()
				.getDomain().getIssues();
		Bid bid = null;

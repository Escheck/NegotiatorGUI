...
public class AgentLGSAS extends SharedAgentState {
	...
	public AgentLGSAS(NegotiationSession negoSession,
			OpponentBids opponentBids, OpponentModel opponentModel,
			OMStrategy oms) {
		NAME = "AgentLR";
		this.oms = oms;
		this.utilitySpace = (AdditiveUtilitySpace) negoSession
				.getUtilitySpace();
	...
	private void initBids() {
		...
		// System.out.println("before size " + allBids.size() +" time " +
	...
	public BidDetails getNextBid(double time) {
		...
		try {
			...
			currentAction = new BidDetails(newBid,
					...
					if (time - lastTimeLeft > 0.008) {
						...
						if (index >= allBids.size())
							...
							for (int i = numPossibleBids; i <= index; i++) {
								...
								double utiliy = getUtilityOfOpponentsBid(
		...
		if (!(opponentModel instanceof NoModel)) {
			try {
				currentAction = oms.getBid(outcomeSpace,
						utilitySpace.getUtility(currentAction.getBid()));
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	private ArrayList<Bid> filterBids(ArrayList<Bid> bids,
		...
		// System.out.println("need to remove " + bids.size() +" removed" +
		// bidstoRemove.size() + "time " );
	...
	public double getUtilityOfOpponentsBid(Domain domain, Bid bid) {
		double utility;
		if (opponentModel instanceof NoModel) {
			utility = opponentBids.getOpponentBidUtility(
					utilitySpace.getDomain(), bid);
		} else {

...
public class TheNegotiatorSAS extends SharedAgentState {
	...
	public int calculateMovesLeft() {
		...
		if (queue.isEmpty()) {
			...
			movesLeft = (int)Math.floor((1.0 - negotiationSession.getTime()) / (total / (double)queueSize));
		}
	...
	public void calculatePropArray() {
		...
		double min = possibleBids.get(possibleBids.size() - 1).getMyUndiscountedUtil();
		...
		if(propArray[0] == 0) {
			...
			maxThresArray2[1] = possibleBids.get(propArray[0] - 1).getMyUndiscountedUtil(); // -1 to correct for array offset of zero
		}
		...
		if(propArray[0] == 0 && propArray[1] == 0){
			...
			maxThresArray2[2] = possibleBids.get(propArray[0] + propArray[1] - 1).getMyUndiscountedUtil();
		}
	...
	private void createAllBids() {
		...
		for (ArrayList<Pair<Integer, ValueDiscrete>> bidSet : result) {
			...
			try {
				...
				double utility = negotiationSession.getUtilitySpace().getUtility(bid);

...
public class BayesianOpponentModel2 {
...
//	private ArrayList<UtilitySpaceHypothesis> fUSHyps;
	...
	public BayesianOpponentModel2(UtilitySpace pUtilitySpace) {
		//
		...
		for(int i =0; i<fUS.getNrOfEvaluators();i++) {
			...
			case DISCRETE:
				...
				for(int j=0;j<lDiscIssue.getNumberOfValues();j++) 
					lDiscreteEval.addEvaluation(lDiscIssue.getValue(j), (double)j/(lDiscIssue.getNumberOfValues()-1));
				...
				for(int j=0;j<lDiscIssue.getNumberOfValues();j++) 
					lDiscreteEval.addEvaluation(lDiscIssue.getValue(j), 1.0-(double)j/(lDiscIssue.getNumberOfValues()-1));
				lEvaluatorHypothesis = new EvaluatorHypothesis (lDiscreteEval);
				lEvaluatorHypothesis.setProbability((double)1/3);
				lEvalHyps.add(lEvaluatorHypothesis);
				lEvaluatorHypothesis.setDesc("downhill");
				...
				for(int k=1;k<=lTotalTriangularFns;k++) {
					...
					for(int j=0;j<lDiscIssue.getNumberOfValues();j++)
						if(j<k) {
							...
							lEval = 1-(double)(j-k)/(lDiscIssue.getNumberOfValues()-1-k);
							lDiscreteEval.addEvaluation(lDiscIssue.getValue(j), lEval);
						}
					lEvaluatorHypothesis = new EvaluatorHypothesis (lDiscreteEval);
					lEvaluatorHypothesis.setProbability((double)1/3);
	...
	private double getPartialUtility(Bid pBid, int pIssueIndex) {
		double lPartialUtility = 0;		
			...
			for(int k=0;k<fWeightHyps.length;k++) {
				WeightHypothesis lWeightHyp = fWeightHyps[k];
				double p = lWeightHyp.getProbability();
				double u = 0;
				for(int j=0;j<fDomain.getNumberOfIssues();j++) {
					if(pIssueIndex==j) continue;
					u = u + lWeightHyp.getWeight(j)*
							getExpectedEvaluationValue(pBid, j);
				}
				lPartialUtility += u * p;
			}
		...
		if(fBiddingHistory.size()>1) {
			...
			for(int i=0;i<fDomain.getNumberOfIssues();i++) {
				...
				for(int j=0;j<fEvaluatorHyps.get(i).size();j++) {
					...
					lN += lHyp.getProbability()*conditionalDistribution(getPartialUtility(pBid, i)+getExpectedWeight(i)*(Double)(lHyp.getEvaluator().getEvaluation(fUS, pBid, i)), fPreviousBidUtility);					
				}
				//2. update probabilities				
				for(int j=0;j<fEvaluatorHyps.get(i).size();j++) {
					EvaluatorHypothesis lHyp =fEvaluatorHyps.get(i).get(j); 
					lHyp.setProbability(lHyp.getProbability()*conditionalDistribution(getPartialUtility(pBid, i)+getExpectedWeight(i)*(Double)(lHyp.getEvaluator().getEvaluation(fUS, pBid, i)), fPreviousBidUtility)/lN);					
				}
			}			
	...
	public double getExpectedUtility(Bid pBid) {
		...
		for(int i=0;i<fWeightHyps.length;i++) {
			...
			for(int k=0;k<fDomain.getNumberOfIssues();k++) {				
				u = u + lWeightHyp.getWeight(k)*
						getExpectedEvaluationValue(pBid, k);
			}

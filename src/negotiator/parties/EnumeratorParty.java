...
public class EnumeratorParty extends AbstractNegotiationParty {
    ...
    public EnumeratorParty(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed) {
        super(utilitySpace, deadlines, timeline, randomSeed);

        // vars
        Domain dom = getUtilitySpace().getDomain();
        List<Integer> issueSizes = new ArrayList<Integer>();
        for (Issue issue : dom.getIssues()) issueSizes.add(((IssueDiscrete) issue).getNumberOfValues());
    ...
    public Bid generateBid(Integer[] indices) {
        ...
        for (int i = 0; i < indices.length; i++) {
            IssueDiscrete issue = (IssueDiscrete) getUtilitySpace().getDomain().getIssue(i);

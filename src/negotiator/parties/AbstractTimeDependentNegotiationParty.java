 ...
 * Hendrikx to use the SortedOutcomeSpace instead of BidHistory. Adapted by David
 * Festen for multilateral case.
 *
 ...
 *
 * @author David Festen, Tim Baarslag, Mark Hendrikx
 */
public abstract class AbstractTimeDependentNegotiationParty extends AbstractNegotiationParty
{
    ...
    public AbstractTimeDependentNegotiationParty(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed)
    {
     ...
     * if this {@link negotiator.parties.NegotiationParty} is in the
    ...
    {
        Bid nextBid = outcomeSpace.getBidNearUtility(getTargetUtility()).getBid();
        double lastUtil = lastBid != null ? utilitySpace.getUtilityWithDiscount(lastBid, timeline) : 0;
        ...
        if (possibleActions.contains(Accept.class) && nextUtil < lastUtil) return new Accept(getPartyId());

        // Counter offer based actions
        else if(possibleActions.contains(Offer.class)) return new Offer(getPartyId(), nextBid);

        // Voting based actions
        else if(possibleActions.contains(OfferForVoting.class)) return new OfferForVoting(getPartyId(), nextBid);
        else if(possibleActions.contains(Reject.class)) return new Reject(getPartyId()); // Accept is higher up the chain

        // default action
        else return new NoAction(getPartyId());
    }
    ...
    {
        if (arguments instanceof Offer) lastBid = ((Offer) arguments).getBid();
    }
    ...
    public double getTargetUtility()
    {
        ...
        // we have a slight offset because discrete timeline is 1-based, this needs to be addressed
        double offset = timeline instanceof DiscreteTimeline ? 1d / ((DiscreteTimeline) timeline).getTotalRounds() : 0d;
        return 1d - f(timeline.getTime() - offset);
    }

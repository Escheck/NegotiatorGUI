...
public class Group10 extends AbstractNegotiationParty {
	...
	public Group10(UtilitySpace utilitySpace,
	...
	public Action chooseAction(List<Class> validActions) {
		...
		double time_after_walker = ((total_time-1)*(1.0-RANDOM_WALKER_TRESHOLD));
		double temp = (time_left-1.0)/time_after_walker;
		double treshold = CONCEDE_TO + Math.pow(temp,CONCEDE_SPEED)/2;
		double last_utility;
		try {
		...
		if (!validActions.contains(Accept.class) || (last_utility < treshold && (time_left > 1 || first_to_offer == 1))) {
			if ((current_time / total_time) < RANDOM_WALKER_TRESHOLD) {
				final_bid = randomWalker();
			} else {
				if ((int)time_left<3) {
					//Opponent is not conceding. Deadline has been reached.
					//Let them taste a bit of their own medicine. (They have to accept!)
					try{
					...
					ArrayList<Object> senders = opponent_bid_list.getSenders();
					ArrayList<Entry<Pair<Integer,Value>, Integer>> pair_frequency = opponent_bid_list.getMostFrequentIssueValues(senders.get(0));
					ArrayList<Entry<Pair<Integer,Value>, Double>> weighted_list = opponent_bid_list.weightIssueValues(pair_frequency);
					final_bid = concederBid(weighted_list);
				}
			}
			current_time++;
			return new Offer(final_bid);
	...
	private Bid concederBid(ArrayList<Entry<Pair<Integer,Value>, Double>> ordered_pair) {
		int actions_left = (int)(total_time-current_time-1);
		int actions_after_walker = (int)(total_time*(1.0-RANDOM_WALKER_TRESHOLD));
		Bid average_bid = randomWalker();
		//calculate how far in the pairset the best bid occured
		ArrayList<Issue> issues = average_bid.getIssues();
		HashMap<Integer,Value> values = average_bid.getValues();
		ArrayList<Pair<Integer,Value>> issue_pairs = new ArrayList<Pair<Integer,Value>>();
		...
		BigInteger steps_per_action = total_steps.divide(BigInteger.valueOf(actions_after_walker));
		BigInteger previous_step = current_step;
		current_step = steps_per_action.multiply(BigInteger.valueOf(actions_after_walker - actions_left));
		...
		//binary concede bid
		concede_bid = generateConcedeBid(current_step, concede_bid, ordered_pair, max_index);
		for(BigInteger i = previous_step; i.compareTo(current_step) == -1;i=i.add(BigInteger.valueOf(1))) {
			Bid new_bid = new Bid(concede_bid);
			new_bid = generateConcedeBid(i,new_bid, ordered_pair,max_index);
			try{
				if(utilitySpace.getUtility(new_bid)>utilitySpace.getUtility(concede_bid)) {
//					System.out.println("===\n new_bid: "+utilitySpace.getUtility(new_bid)+" concede_bid: "+utilitySpace.getUtility(concede_bid));
					concede_bid = new_bid;
				}
			} catch (Exception e) {}
		}
...
//				System.out.println("Conceded, Utility:"+utilitySpace.getUtility(concede_bid)+" and "+actions_left+" steps to go.");
				return final_bid;
			} else {
//				System.out.println("Average Bid, Utility:"+utilitySpace.getUtility(average_bid)+" and "+actions_left+" steps to go.");
				return average_bid;
			}
		} catch (Exception e) {
			return average_bid;
	...
	private Bid generateConcedeBid(BigInteger step, Bid bid, ArrayList<Entry<Pair<Integer,Value>, Double>> ordered_pair, int issue_count){
		for(int i=0;i<issue_count;i++) {
			//binary split index to steps to include or not (max_index-bits number)
	...
	private Bid randomWalker() {
		ArrayList<Bid> bids = getHighUtilityBids(MINIMAL_WALKER_UTILITY);
...
//		System.out.println("Random bid: "+bids.get(index).toString());
	...
	private ArrayList<Bid> getHighUtilityBids(double minimal_utility) {
		if(high_utility_bids == null) { //singleton
			high_utility_bids = new ArrayList<Bid>();
			SortedOutcomeSpace sorted_outcome = new SortedOutcomeSpace(utilitySpace);
	...
	private double[] getWeights(ArrayList<Bid> high_bids_list) {
		...
		for(int i=0;i<issue_length;i++){
			var[i] = (sum_sq[i] - (sum[i]*sum[i]/datalength))/(datalength - 1);
			sum_var += var[i];
		}
	...
	private double[] getIssueValues(Bid b) {
		...
		for(int j=0;j<size;j++){
			...
			double eval;
			try{
				Evaluator evaluator = utilitySpace.getEvaluator(issue.getNumber());
				eval = evaluator.getWeight()*evaluator.getEvaluation(utilitySpace, b, issue.getNumber());
			} catch (Exception e) {
				eval = 0.0;
			}
			w[j] = eval;
    ...
    protected AgentID partyId = new AgentID("Group 10");

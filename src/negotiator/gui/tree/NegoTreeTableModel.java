...
public class NegoTreeTableModel extends AbstractTreeTableModel implements TreeTableModel {
	...
	//Constructors
	public NegoTreeTableModel(Domain domain) {
		this.domain = domain;
	...
	public NegoTreeTableModel(Domain domain, UtilitySpace utilitySpace) {
		this.domain = domain;
		this.root = domain.getObjectivesRoot();
	...
	/**Calculate the new weights for the tree, and return to caller with the caller's new weight. 
	* This new weight can be different from the requested weight, 
	* for instance if that modification is impossible for some reason.
	* Wouter: newWeight is not used at all. What's the idea?
	*/
	...
	public WeightSlider getWeightSlider(Objective node) {
		...
		if (slider == null) {
			...
			if (utilitySpace != null) {
				...
				// Wouter: if no utilitySpace then we are editing Domain. Why set weight in first place??

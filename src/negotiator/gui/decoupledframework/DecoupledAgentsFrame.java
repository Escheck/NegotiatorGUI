package negotiator.gui.decoupledframework;
...
import negotiator.boaframework.DecoupledComponent;
import negotiator.boaframework.DecoupledParameter;
 ...
 * @version 11-12-11
 */
public class DecoupledAgentsFrame extends JDialog {
    ...
    private DecoupledParameter nullParam;
    private ArrayList<DecoupledAgentInfo> result;
    
    public DecoupledAgentsFrame(Frame frame) {
    	...
    	nullParam = new DecoupledParameter("null", 1, 1, 1);
    }

    public ArrayList<DecoupledAgentInfo> getResult() {
    	initFrameGUI();
	...
	private void initControls() {
    	addOsParam.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				DecoupledParameter param = (DecoupledParameter)new ParameterInput(NegoGUIApp.negoGUIView.getFrame(), "Input parameter").getResult();
				if (param != null) {
    	...
    	saveButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ArrayList<DecoupledAgentInfo> agents = new ArrayList<DecoupledAgentInfo>();
				for (int i = 0; i < agentsModel.getSize(); i++) {
					agents.add((DecoupledAgentInfo) agentsModel.getElementAt(i));
				}
				result = agents;
				dispose();
			}
	...
	private void loadLists() {
		DecoupledAgentRepository dar = DecoupledAgentRepository.getInstance();
	...
	private void generateAgentCombinations() {
		Set<DecoupledComponent> osStrat = generateStrategies(osModel.getElementAt(osList.getSelectedIndex()), getParameters(osParamsModel), "bs");
		Set<DecoupledComponent> asStrat = generateStrategies(asModel.getElementAt(asList.getSelectedIndex()), getParameters(asParamsModel), "as");
		Set<DecoupledComponent> omStrat = generateStrategies(omModel.getElementAt(omList.getSelectedIndex()), getParameters(omParamsModel), "om");
		Set<DecoupledComponent> omsStrat = generateStrategies(omsModel.getElementAt(omsList.getSelectedIndex()), getParameters(omsParamsModel), "oms");
		...
		while (strategyIterator.hasNext()) {
			Set<DecoupledComponent> fullStrat = (Set<DecoupledComponent>) strategyIterator.next();
			Iterator strat = fullStrat.iterator();
			DecoupledComponent os = null, as = null, om = null, oms = null;
			while (strat.hasNext()) {
				DecoupledComponent strategy = (DecoupledComponent) strat.next();
			...
			DecoupledAgentInfo agent = new DecoupledAgentInfo(os, as, om, oms);
			agentsModel.addElement(agent);
		}
	}
	
	private Set<DecoupledComponent> generateStrategies(String classname,
			ArrayList<DecoupledParameter> parameters, String type) {
		...
		Set<DecoupledComponent> strategies = new HashSet<DecoupledComponent>();
		Iterator<Set<Object>> combinationsIterator = result.iterator();

		while (combinationsIterator.hasNext()) {
			// all combinations
			Set<Object> set = combinationsIterator.next();
			DecoupledComponent strat = new DecoupledComponent(classname, type);
	...
	private ArrayList<DecoupledParameter> getParameters(DefaultListModel model) {
		ArrayList<DecoupledParameter> profiles = new ArrayList<DecoupledParameter>();

		if (model != null && model.size() > 0) {
			for (int i = 0; i < model.getSize(); i++) {
				profiles.add((DecoupledParameter) model.getElementAt(i));
			}
		} else {

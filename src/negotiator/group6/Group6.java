...
public class Group6 extends AbstractNegotiationParty {
	 ...
	 */
	public Group6(UtilitySpace utilitySpace,
	...
	public Action chooseAction(List<Class> validActions) {
		try {		
			...
			if(mostRecentBid != null && 
					...
					isAcceptable(mostRecentBid)) {
				return new Accept();
			}
			
			// If there are no previous bids or the agent is not allowed to accept, generate an offer
			mostRecentBid = generateBid();
		} catch(Exception e) {
			e.printStackTrace();
			
			// If something goes wrong, just accept the offer or create one at random
		...
		return new Offer(mostRecentBid);
	}
	
	/**
	 * Checks the acceptability of the Bid.
	 * Only works as planned if at least one bidding round has passed.
	 * 
	 * @param bid to check
	 * @throws Exception 
	 */
	 ...
	 * 
	 * @param time (t)
	 * @param reservationValue (u_r)
	 * @param nrAccepts (n_a)
	 * @param nrOpponents (n)
	 * @return minimal acceptable utility value given it's parameters
	 * @throws Exception outside boundaries
	 */
	...
	private Queue<Issue> getOrderedIssues() {
		PriorityQueue<Issue> issues = new PriorityQueue<Issue>(this.utilitySpace.getDomain().getIssues().size(), new Comparator<Issue>(){
			@Override
			public int compare(Issue i1, Issue i2) {
				return (int) (utilitySpace.getWeight(i1.getNumber()) - utilitySpace.getWeight(i2.getNumber()));
			}
		});
	...
	private Map<Issue, List<Value>> getOpponentMajorityValues() {
		...
		for(Issue issue: this.utilitySpace.getDomain().getIssues()) {
			...
			for(IOpponentModel oppModel : opponentModels.values()) {
				...
				// If there is no entry in the opponent model for this issue, continue
				if(oppModel.getValue(issue.getNumber()) == null) continue;
	...
	private Bid generateBid() throws Exception {
		...
		for(Issue leastImportantIssue : issues) {
			...
			for(Value value : majorityValue) {
				double valueUtility = ((EvaluatorDiscrete) utilitySpace.getEvaluator(leastImportantIssue.getNumber())).getEvaluation((ValueDiscrete) value);
			...
			if(majorityMaxValue == null) break;
			
			// Only concede on issues that we did not concede on in the previous bid
			...
			if(!isAcceptable(resBid)) {
				resBid.setValue(leastImportantIssue.getNumber(), maxBid.getValue(leastImportantIssue.getNumber()));
				break;
			}
		}
	...
	private Bid getMaximumUtilityBid() {
		...
		for(Issue issue : utilitySpace.getDomain().getIssues()) {
			Value value = ((EvaluatorDiscrete) utilitySpace.getEvaluator(issue.getNumber())).getMaxValue();
			if(value == null) break;

 ...
 * @author Reyhan
 */
public class AlternatingOfferMajorityVotingProtocol extends
	...
	/**
	 * Holds the total number of parties in this session (used for checking if
	 * everyone agrees).
	 */
	...
	private Offer acceptedOffer(Round votingRound, Round offerRound) {
		...
		for (int i = 0; i < numOffers * numOffers; i += numOffers) {
			...
			if (votes > mostRecentlyAcceptedOfferVoteCount) {
				mostRecentlyAcceptedOffer = (Offer) voteActions
						.get(offerNumber);
				mostRecentlyAcceptedOfferVoteCount = votes;
				System.out.println("    New most recently accepted bid (votes="
						+ mostRecentlyAcceptedOfferVoteCount + "/"
						+ numberOfAgents + "): " + mostRecentlyAcceptedOffer);
			}
		}

...
/**
 * An adapter for the protocol class. This will implement all the methods int the Protocol interface
 * and return default values for them.
 *
 * @author David Festen
 */
public abstract class ProtocolAdapter implements Protocol {
    ...
    /**
     * This will get called just before the session starts. If some initialization with needs to be
     ...
     * .Round} after the current one. It will however finish all the turns described in the
     ...
     * @return A map where the key is a {@link negotiator.parties.NegotiationParty} that is
     *     responding to a {@link NegotiationParty#chooseAction(java.util.List)} event, and the
     *     value is  a list of {@link NegotiationParty} that are listening to that key party's
     *     response.
     */
    @Override
     ...
     * <p/>
     * Default implementation returns 0 if no agreement or number of parties if agreement exists.
     *
     * @param session the current state of this session
     * @param parties The parties currently participating
    ...
    /**
     * Filters the list by including only the type of negotiation parties. Optionally, this behavior
     * can be reversed (i.e. excluding only the given type of negotiation parties).
     *
     * @param negotiationParties The original list of parties
     * @param negotiationPartyClass The type of parties to include
     *                              (or exclude if inclusionFilter is set to false)
     * @param inclusionFilter If set to true, we include the given type.
     *                        Otherwise, exclude the given type
    ...
    private Collection<NegotiationParty> filter(Collection<NegotiationParty> negotiationParties,
        ...
        Collection<NegotiationParty> filtered =
                new ArrayList<NegotiationParty>(negotiationParties.size());

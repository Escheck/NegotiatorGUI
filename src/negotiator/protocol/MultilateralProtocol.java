 ...
 * {@link MultilateralProtocolAdapter} which implements all the methods in this
...
public interface MultilateralProtocol {
	 ...
	 * a list of all the {@link negotiator.parties.NegotiationPartyInternal} and
	 * the complete {@link Session} which can be used to diversify the round
	 ...
	 */
	Round getRoundStructure(List<NegotiationPartyInternal> parties,
			Session session);

	/**
	 * This will get called just before the session starts. If some
	 * initialization needs to be done by the protocol, it can be done here.
	 *
	 * @param session
	 ...
	 */
	void beforeSession(Session session, List<NegotiationPartyInternal> parties)
	 ...
	 */
	void afterSession(Session session, List<NegotiationPartyInternal> parties);
	 ...
	 */
	boolean isFinished(Session session, List<NegotiationPartyInternal> parties);
	 ...
	 */
	Map<NegotiationPartyInternal, List<NegotiationPartyInternal>> getActionListeners(
	 ...
	 */
	Bid getCurrentAgreement(Session session,
			List<NegotiationPartyInternal> parties);
	 ...
	 */
	int getNumberOfAgreeingParties(Session session,
			List<NegotiationPartyInternal> parties);
	...
	/**
	 * Gets the executor used to box actions that agents can influence. This
	 * counts the action towards agent's time and prevents it from stalling.
	 */
	ExecutorWithTimeout getExecutor();

	/**
	 * Sets the executor used to box actions that agents can influence. This
	 * counts the action towards agent's time and prevents it from stalling.
	 */

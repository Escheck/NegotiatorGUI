...
/**
 * This class will generate the party lists for each negotiation in the tournament. The reason for
 * this generator is that the number of parties can become quite large, which means that at some
 * point, we are unable to enumerate them all. This class will only give the next list parties to
...
public class TournamentGenerator implements Iterable<List<NegotiationParty>>
{
     ...
     *
     * @param config           The configuration used for the tournaments
     * @param indicesGenerator The indices generator used to generate lists of parties
     */
    public TournamentGenerator(Configuration config, TournamentIndicesGenerator indicesGenerator)
    {
        this.config = config;
     ...
     */
    public static NegotiationParty createFrom(PartyRepItem partyRepItem, ProfileRepItem profileRepItem, Session session)
            throws Exception
    {
        ...
        return (NegotiationParty) partyConstructor.newInstance(utilitySpace, session.getDeadlines(), session.getTimeline(), randomSeed);
    }

    /**
     * Create a new instance of the Protocol object from a {@link negotiator.repository.MultiPartyProtocolRepItem}
     *
     ...
     * @throws java.lang.Exception              If {@link negotiator.repository.Repository#copyFrom(negotiator.repository.Repository)} throws an exception.
     */
    public static NegotiationParty createFrom(PartyRepItem partyRepItem, Domain domain, UTILITYSPACETYPE type, Session session) throws Exception
    {
        if (type==null) {
     ...
     *
     * @param partyRepItems          The repository items for parties
     * @param profileRepItems        The repository items for profiles
     * @param mediatorIndex          The index of the mediator in the returned list (usually this is 0)
     * @param mediatorRepItem        The mediator repository item (or null)
     * @param mediatorProfileRepItem The mediator repository profile (or null)
     * @param session              The session used fot this session
    ...
    public static List<NegotiationParty> generateSessionParties(
            ...
            List<ProfileRepItem> profileRepItems,
            List<AgentID> partyIds,
            int mediatorIndex,
            ...
            Session session)
    {
        ...
        return generateSessionParties(
                generateIntegerList(partyRepItems.size()),
                partyRepItems,
                profileRepItems,
                partyIds,
                mediatorIndex,
                mediatorRepItem,
                mediatorProfileRepItem,
                session);
    }
     ...
     * Generate the list of parties in the given session
     *
     * @param partyIndices           The indices used for generating the session
     * @param partyRepItems          The repository items for parties
     * @param profileRepItems        The repository items for profiles
     * @param mediatorIndex          The index of the mediator in the returned list (usually this is 0)
     * @param mediatorRepItem        The mediator repository item (or null)
            ...
            Session session)
    {
        try
        {
            ...
            for (int i = 0; i < profileRepItems.size(); i++)
            {
                ...
                {
                    PartyRepItem partyRepItem = partyRepItems.get(partyIndices.get(i));
                    ProfileRepItem profileRepItem = profileRepItems.get(i);
                    NegotiationParty party = createFrom(partyRepItem, profileRepItem, session);
                    parties.add(party);
                }
            }
            NegotiationParty mediator = generateMediator(
                    mediatorRepItem,
                    mediatorProfileRepItem,
                    profileRepItems.get(0),
                    session);
            if (mediator != null)
     ...
     *
     * @param upToExclusive The number to generate the list from
     * @return a list of integers starting at 0 and going up to, but not including the given number.
     */
     ...
     *
     * @param mediatorRepItem           The mediator repository item
     * @param mediatorProfileRepItem    The mediator repository profile (or null)
     * @param alternativeProfileRepItem If no profile item is given, this is used to get the domain
     * @param session                 The session used fot this session
            ...
            Session session)
    {
        try
        {
            // If mediator set and mediator profile set, add to default mediator spot
            if (mediatorRepItem != null)
            {
                // if mediator profile also set, we'll create it add it at the appropriate index
                if (mediatorProfileRepItem != null)
                {
                    return createFrom(mediatorRepItem, mediatorProfileRepItem, session);
                }
                // if mediator has no profile, we'll base it on one of the agents domains, the
                // assumption here is quite reasonable; all agents operate in the same domain.
                else
                {
                    UTILITYSPACETYPE utilType = extractUtilitySpaceType(alternativeProfileRepItem);
                    Domain someAgentsDomain = createFrom(alternativeProfileRepItem).getDomain();
                    return createFrom(mediatorRepItem, someAgentsDomain, utilType, session);
                }
            }
     ...
     *
     * @param indices The indices of the parties to include
    ...
    List<NegotiationParty> generateSessionParties(List<Integer> indices)
    {
        return generateSessionParties(
                indices,
                config.getPartyItems(),
                config.getPartyProfileItems(),
                null,
                config.getMediatorIndex(),
                config.getMediatorItem(),
                config.getMediatorProfile(),
                config.getSession()
        );
    }
...
//     *
//     * @param partyIndex   The index of the party in the configuration to generate
//     * @param profileIndex The index of the profile in the configuration to use for this party

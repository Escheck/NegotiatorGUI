...
import negotiator.config.ConfigurationException;
...
import negotiator.protocol.AlternatingOfferCounterOfferProtocol;
...
/**
 * Manages the tournament and makes sure that the {@link negotiator.session.SessionManager} are
 * instantiated. It uses the configuration object which is created by the user interface and extracts
 * individual session from configuration object which it wil pass on to the session manager.
 */
public class TournamentManager extends Thread {
    ...
    /**
     * Initializes a new instance of the {@link negotiator.session.TournamentManager} class. The
     * tournament manager uses the provided configuration to find which sessions to run and how
     * many collections of these sessions (tournaments) to run.
     *
     * @param config The configuration to use for this Tournament
     */
    ...
    public void runTournament() throws Exception {
        ...
        for (int tournamentNumber = 0; tournamentNumber < configuration.getNumTournaments(); tournamentNumber++) {
            int sessionNumber = 0;

            // Generator for parties (don't generate them up-front as these can be many)
            Iterable<List<NegotiationParty>> generator = configuration.getPartiesGenerator();
            ...
            for (List<NegotiationParty> partyList : generator) {
                ...
                if (partyList == null) {
                    System.out.println("Error on initializing one or more of the agent");
                    continue;
                }
                ...
                if (!sessionOk) {
                    logger.log("ERROR");
                    for (int i = 0; i < 11; i++) logger.log("");
                    for (NegotiationParty agent : agentList) logger.log(agent.getPartyId().toString());
                    logger.logLine();
                }
     ...
     *
     * @param parties the parties to run the tournament for
     * @throws Exception if some of the required data could not be extracted
     */
    public boolean runSingleSession(List<NegotiationParty> parties) throws Exception {
        try {
            Protocol protocol = configuration.getProtocol();
            Session session = configuration.getSession().copy();

            // TODO: ** hackery ** we should make sure that session gives timeline to agents, not the other way around.
            ...
            try {
                ...
                List<NegotiationParty> agentList = MediatorProtocol.getNonMediators(parties);
                logger.logLine(CsvLogger.logDefaultSession(session, protocol, agentList, runTime));
                return true;
            } catch (Error e) {
                throw new AnalysisException("Unknown error in analyses or logging");
            }
        } catch (InvalidActionError invalidActionError) {
            useConsoleOut(true);
            System.err.println("Agent performed invalid action, skipping current session.");
            return false;
        } catch (NegotiationPartyTimeoutException e) {
            useConsoleOut(true);
            String msg = String.format("Agent %s timed out, skipping current session.", e.getInstigator().getPartyId());
            ...
            useConsoleOut(true);
            System.err.println("Thread or thread execution interrupted, skipping current session.");
            return false;
        } catch (Exception e) {
            useConsoleOut(true);
            System.err.println("Agent caused an unknown exception, skipping current session.");
            e.printStackTrace();
            return false;
        }
    }

 ...
 * A {@link Session} consists of {@link Round} with in turn consists of {@link Turn}.
 * From this session object some information about the current negotiation can be extracted.
 * Important is the {@link Session#startNewRound(Round)} method which is used to add a new round
 * to the session. At this moment (05-08-2014) adding new rounds to the session is the
 * responsibility of the {@link SessionManager}.
 *
 * @author David Festen
 */
public class Session
{
     ...
     */
    private Map<DeadlineType, Object> deadlines;
    ...
    /**
     * Create a new instance of the session object. This should normally only happen once every
     * session. See also the class documentation of {@link Session}.
     *
     * @param deadlines Map of deadline constraints
     */
    public Session(Map<DeadlineType, Object> deadlines)
    {
        this.rounds = new ArrayList<Round>();
        this.deadlines = deadlines;
        if (deadlines.containsKey(ROUND)) {
            this.timeline = new DiscreteTimeline((Integer) deadlines.get(ROUND));
        }
        else if (deadlines.containsKey(TIME)) {
            this.timeline = new ContinuousTimeline((Integer) deadlines.get(TIME));
        }
    }
     ...
     */
    public Map<DeadlineType, Object> getDeadlines()
    {
        return deadlines;
    }
     ...
     *
     * @param deadlines a map of deadline types and values
     */
    ...
    /**
     * Updates the timestamp of this {@link Session}. Use just before starting the negotiation for
     * most accurate timing. Timing is used in for example time deadline constraints. But might
     * also be used in log messages as well as statistics.
     */
    ...
    /**
     * Updates the timestamp of this {@link Session}. Use just after finish the negotiation for
     * most accurate timing. Timing is used in for example time deadline constraints. But might
     * also be used in log messages as well as statistics. If you need to manually set it, consider
     * using the {@link #setRuntimeInNanoSeconds(long)} function.
     */
    ...
    /**
     * Gets the number of rounds currently in this session. When {@link #startNewRound(Round)} is
     * called a new round will be added. Each round already includes all its {@link Turn}s, but some
     * turns might not yet been done.
     *
    ...
    /**
     * Add a round to this session. Make sure it contains all the turns necessary to execute the
     * rounds. Normally the new round will be created by using
     * {@link negotiator.protocol.Protocol#getRoundStructure(java.util.List, Session)}
     *
     *
     * @param round The round to add to this session.
     */
    ...
    public boolean isDeadlineReached()
    {
        ...
        if (deadlines.containsKey(TIME))
        {
            int timeDeadlineInSeconds = (Integer) deadlines.get(TIME);
        ...
        if (deadlines.containsKey(ROUND))
        {
            int roundsDeadline = (Integer) deadlines.get(ROUND);
    ...
    {
        if (timerRunning) return System.nanoTime() - startTime;
        else return stopTime - startTime;
    }

    public double getRuntimeInSeconds()
    {
        return (double)getRuntimeInNanoSeconds() / pow(10, 9); // ns -> s  ( / 10^9 )
    }

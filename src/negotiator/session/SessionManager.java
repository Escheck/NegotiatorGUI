...
/**
 * The {@link SessionManager} is responsible for enforcing the {@link negotiator.protocol.Protocol} during the
 * {@link Session}. This is the entry point for the negotiation algorithm. The protocol and session
...
{
    // maximum utility history to keep for each session (absolute maximum is int.max)
    ...
    // Key represent the even broadcasting parties, value is a list parties that are listening to
    // the events that are send.
    private final Map<NegotiationParty, List<NegotiationParty>> listeners;

    // Command line interface sessionLogger used to log messages to the cli part of the gui in a single
    // negotiation session
    private SessionLogger sessionLogger;

    // holds a history of all the utilities for the agents. Used for logging purposes
    private List<List<Double[]>> agentUtils;

    // utility of most recent agreements. Only holds sane information in case of agreement.
    ...
    /**
     * Initializes a new instance of the {@see SessionManager} object. After initialization this
     * {@see SessionManager} can be {@see #run()}.
     *
     * @param parties  The parties to use in this session (including agents and optionally mediators)
     * @param protocol The protocol to use for this session.
     * @param session  A session object containing preset information (can also be a new instance)
     */
    public SessionManager(List<NegotiationParty> parties, Protocol protocol, Session session)
    {
     ...
     */
    public void run() throws InvalidActionError, InterruptedException, ExecutionException, NegotiationPartyTimeoutException
    {
        ...
        do
        {
            ...
            for (Turn turn : round.getTurns())
            {
                ...
                if (action instanceof Offer)
                {
                    ...
                    Bid currentAgreement = protocol.getCurrentAgreement(session, parties);

                    for (NegotiationParty agent : agents)
                    {
                        int agentId = agents.indexOf(agent);
                        Double[] entry = {session.getRoundNumber() + session.getTurnNumber() / 10d, agent.getUtility(((Offer) action).getBid())};
                        if (agentUtils.get(0).size() < MAX_UTIL_HISTORY)
                            agentUtils.get(agentId).add(entry);

                        if (currentAgreement == null || lastAgreement == null || !currentAgreement.equals(lastAgreement))
                        {
                ...
                // log messages
                sessionLogger.logMessage("  Turn %d: %-13s %s", turnNumber, party, action);
                sessionLogger.logBid(session, parties, protocol.getCurrentAgreement(session, parties) != null);

                // Do not start new turn in current round if protocol is finished at this point
        ...
        if (agreement == null) sessionLogger.logMessage("No agreement found.");
        else
        {
            ...
            for (NegotiationParty agent : agents)
            {
                ...
                Double[] entry = {Double.isNaN(when) ? session.getRoundNumber() : when, agent.getUtility(agreement)};
                agreementUtilities[0][agentId] = entry[0];
                agreementUtilities[1][agentId] = entry[1];
            }
        ...
        sessionLogger.logMessage("Finished negotiation session in %.3fs", runTime);
        try
        {
            sessionLogger.logSession(session, agreement);
            sessionLogger.logMessage(CsvLogger.logSingleSession(session, protocol, agents, runTime));
        }
        catch (Exception e)
        {
    ...
    private boolean checkDeadline() {
        ...
        if (session.isDeadlineReached())
        {
            ...
            sessionLogger.logMessage("Deadline reached: %s", session.getDeadlines());
            return true;
        }
        return false;
    }
    ...
    /**
     * Request an {@link Action} from the {@link negotiator.parties.NegotiationParty} given a list of valid actions and apply
     * it according to {@link negotiator.protocol.Protocol#applyAction(Action, Session)}
     *
     * @param party       The party to request an action of
     * @param validActions the actions the party can choose
     * @return the chosen action-
     */
    private Action requestAction(final NegotiationParty party, final ArrayList<Class> validActions)
            throws InvalidActionError, InterruptedException, ExecutionException, NegotiationPartyTimeoutException {

        // choose action
        Action action = protocol.getTimeOutInSeconds(session) <= 0
                ? party.chooseAction(validActions)
                : chooseActionWithTimeOut(party, validActions);
        ...
        if (!isValid) throw new InvalidActionError(party);
     ...
     *
     * @param actionOwner The Party that initiated the action
     * @param action      The action it did.
     */
    ...
    /**
     * Adds a listener to the logging events. currently used to attach the cli listener
     *
     * @param eventListener The instance listening to logging events
     */
     ...
     * Outer list is a list of agent id's and utilities
     * inner list is a list of double[2] where the first double identifies the round and the second
     * double identifies the util.
     *
     * Example (History of two agents with two rounds):
     *      list[0][0] = {1, 0.90}
     *      list[0][1] = {2, 0.60}
     *      list[1][0] = {1, 0.70}
     *      list[1][1] = {2, 0.80}
     ...
     *
     * result[0][0] = 2.1  <-- round number
     * result[0][1] = 2.1  <-- round number
     * result[1][0] = 0.40 <-- utility
     * result[1][1] = 0.80 <-- utility
     *
    ...
    private static double findLastIndexOfBid(Bid needle, Session session)
    {
        for (int roundIndex = session.getRounds().size() - 1; roundIndex >= 0; roundIndex--)
        {
            ...
            for (int actionIndex = round.getActions().size() - 1; actionIndex >= 0; actionIndex--)
            {
                ...
                if (action instanceof Offer && ((Offer) action).getBid().equals(needle))
    ...
    private Action chooseActionWithTimeOut(final NegotiationParty party, final ArrayList<Class> validActions)
        ...
        try {
            ...
            String msg = String.format("Negotiating party %s timed out. {TIMEOUT=%ds}", party.getPartyId(), timeOut);

...
public class Group9 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class> validActions) {
		...
		// previous bid
		List<BidDetails> myPossibleBids = outcomeSpace
				.getBidsinRange(new Range(myLastUtility, myLastUtility));
		double i = 0.02;
	...
	private void CalculateOpponentUtilityParameters(Object sender) {
		...
		if (bidList.size() == 1) { // the first time
			...
			for (int i = 0; i < issueSize; i++) {
				...
				try {
					...
					if (iss instanceof IssueDiscrete) {
						...
						numValues = ((IssueInteger) iss).getUpperBound()
								- ((IssueInteger) iss).getLowerBound() + 1;
						ValueInteger val = (ValueInteger) first.getValue(isn);
						issueInd = val.getValue()
								- ((IssueInteger) iss).getLowerBound();
					} else if (iss instanceof IssueReal) {
						// put them in 5 bins
						numValues = 5;
						ValueReal val = (ValueReal) first.getValue(isn);
						double normalized = (((IssueReal) iss).getUpperBound() - val
								.getValue())
								/ (((IssueReal) iss).getUpperBound() - ((IssueReal) iss)
										.getLowerBound());
			...
			for (int i = 0; i < issueSize; i++) {
				...
				if (bidList.get(lastBidIndex).getIssues().get(i) != bidList
						.get(lastBidIndex - 1).getIssues().get(i)) {
				...
				try {
					...
					if (iss instanceof IssueDiscrete) {
						issueInd = ((IssueDiscrete) iss)
								.getValueIndex((ValueDiscrete) bidList.get(
										lastBidIndex).getValue(isn));
					} else if (iss instanceof IssueInteger) {
						issueInd = ((ValueInteger) bidList.get(lastBidIndex)
								.getValue(isn)).getValue()
								- ((IssueInteger) iss).getLowerBound();
					} else if (iss instanceof IssueReal) {
						double value = ((ValueReal) bidList.get(lastBidIndex)
								.getValue(isn)).getValue();
	...
	private double approximateUtility(Object party, Bid bid) {
		...
		for (int i = 0; i < bid.getIssues().size(); i++) {
			...
			try {
				...
				if (iss instanceof IssueDiscrete) {
					issueInd = ((IssueDiscrete) iss)
							.getValueIndex((ValueDiscrete) bid.getValue(isn));
				} else if (iss instanceof IssueInteger) {
					issueInd = ((ValueInteger) bid.getValue(isn)).getValue()
							- ((IssueInteger) iss).getLowerBound();
				} else if (iss instanceof IssueReal) {
					double value = ((ValueReal) bid.getValue(isn)).getValue();
					double divider = ((IssueReal) iss).getUpperBound()
							- ((IssueReal) iss).getLowerBound();
					if (divider == 0)
						continue;
					double normalized = (((IssueReal) iss).getUpperBound() - value)
							/ divider;
			...
			for (int j = 0; j < partyIssueCounter.get(party).get(isn).length; j++) {
				maxCount = Math.max(maxCount,
						partyIssueCounter.get(party).get(isn)[j]);
				if (partyIssueCounter.get(party).get(isn)[j] != 0)
					numOptions++;
			}

			// get the weight and the value for this issue
			double value = partyIssueCounter.get(party).get(isn)[issueInd]
					/ (double) maxCount;

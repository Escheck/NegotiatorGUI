...
public class Group9 extends AbstractNegotiationParty
{
	 ...
	 */
	public Group9(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed) {
	...
	public Action chooseAction(List<Class> validActions) {
		// If a negotiating party makes an offer with >0.9 utility, we accept it.
		...
		if(timeline.getTime() > 0.75) {
			...
			concederThreshold = Math.sqrt(Math.sqrt(1.0-progress)); //fourth root
		}
		
		//concede until I think I've conceded more than necessary for an equal gain outcome. 
		...
		// if the bid I made earlier qualifies as an estimated Kalai-Smorodinsky point, I'll offer that bid again.
		if (KS && myLastUtility <= concederThreshold) {
			return new Offer(myLastBid); 
		}
		
		// explore the bids with a personal utility slightly lower than my previous bid
		List<BidDetails> myPossibleBids = outcomeSpace.getBidsinRange(new Range(myLastUtility, myLastUtility));
		double i = 0.02;
		if(myLastUtility <= concederThreshold) { //if the threshold is not yet relevant
		...
		// Bid the selected bid, unless its utility is lower than what we were offered.
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if(!partyBids.containsKey(sender) && !sender.toString().equals("Protocol")) {
	...
	private void CalculateOpponentUtilityParameters(Object sender) {
	    ...
	    if(bidList.size() == 1) { //the first time
	    	...
	    	ArrayList<Issue> issues = first.getIssues();
	    	...
	    	for(int i = 0; i < issueSize; i++) {
	    		...
	    		try {
		    		...
		    		if(iss instanceof IssueDiscrete) {
		    			...
		    			double normalized = (((IssueReal)iss).getUpperBound() - val.getValue())/(((IssueReal)iss).getUpperBound() - ((IssueReal)iss).getLowerBound());
		    			issueInd = (int) Math.floor(normalized*5);
		    			if(issueInd >= 5) issueInd = 4;
		    		}
	    		} catch(Exception e) { }
	    		
	    		partyIssueCounter.get(sender).put(isn, new int[numValues]);
	    		partyIssueCounter.get(sender).get(isn)[issueInd] = 10;
	    		//set counter to 10, we assume they start with their most important
	    	}
	    	...
	    	for(int i = 0; i < issueSize; i++) {
	    		...
	    		try {
		    		...
		    		if(iss instanceof IssueDiscrete) {
		    			issueInd = ((IssueDiscrete)iss).getValueIndex((ValueDiscrete) bidList.get(lastBidIndex).getValue(isn));
		    		} else if(iss instanceof IssueInteger) {
		    			issueInd = ((ValueInteger) bidList.get(lastBidIndex).getValue(isn)).getValue() - ((IssueInteger)iss).getLowerBound();
		    		} else if(iss instanceof IssueReal) {
		    			double value = ((ValueReal) bidList.get(lastBidIndex).getValue(isn)).getValue();
		    			double normalized = (((IssueReal)iss).getUpperBound() - value)/(((IssueReal)iss).getUpperBound() - ((IssueReal)iss).getLowerBound());
		    			issueInd = (int) Math.floor(normalized*5);
		    			if(issueInd >= 5) issueInd = 4;
		    		}
	    		} catch(Exception e) { }
	...
	private double approximateUtility(Object party, Bid bid) {
		...
		for(int i = 0; i < bid.getIssues().size(); i++) {
			...
			if(numOptions == 1 && value == 0.0) //for hardball players who only select 1 option
				value = 0.5; //we dont know how you feel about this subject, so we assume 0.5
			else if(value == 0.0)
				value = 0.1; //you probably dont hate it, even though you didn't choose it

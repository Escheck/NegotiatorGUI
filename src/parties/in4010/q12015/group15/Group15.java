...
public class Group15 extends AbstractNegotiationParty {
	...
	public void init(AbstractUtilitySpace utilSpace, Deadline deadline,
		...
		outcomeSpace.generateAllBids(utilSpace);
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (validActions.contains(Accept.class)
				&& as.determineAcceptability(lastBid, timeline, roundTime)
						.equals(Actions.Accept)) {
			return new Accept();
		}
		...
		if (bestBid == null) {
			bestBid = getRandomBidInInterval(MIN_UTILITY_TO_OFFER,
		...
		try {
			System.out.println("My BID Util: "
					+ utilitySpace.getUtility(bestBid));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void lowerMinUtility() {
		...
		System.out.println("Lowering MIN_UTILITY_TO_OFFER to: "
				+ MIN_UTILITY_TO_OFFER + " in round: " + rounds);
	}
	...
	public List<BidDetails> getPossibleBids() {
		Range range = new Range(MIN_UTILITY_TO_OFFER, MIN_UTILITY_TO_OFFER
				+ OFFER_GROUP_SIZE);
	...
	private Bid getBidWithSmallestHammingDistance(List<BidDetails> bids) {
		...
		for (BidDetails b : bids) {
			...
			for (String opponent : om.getOpponents()) {
				totalDist += getHammingDistance(b.getBid(),
						om.getLastBidFromAgent(opponent));
			}
	...
	private int getHammingDistance(Bid bid1, Bid bid2) {
		...
		for (Issue issue : issues) {
			...
			try {
				if (!bid1.getValue(indexOfIssue).equals(
						bid2.getValue(indexOfIssue))) {
					dist++;
				}
			} catch (Exception e) {
				System.out
						.println("BS Could not calculate Hamming distance between "
								+ bid1 + " and " + bid2);
				e.printStackTrace();
			}
		}
	...
	private Bid getRandomBidInInterval(double min, double max) {
		System.out.println("WARNING: Offering random bid in range: " + min
				+ " - " + max);
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		System.out.println("Receive " + action.getClass() + " at time: "
				+ timeline.getTime() + " from agent " + sender);

...
public class Group7 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (!validActions.contains(Accept.class)) {
		...
		} else if ((myBidHistory.getLastBid() != null && getUtility(this.opBidHistory
				.getCombinedHistory().getLastBid()) > (getUtility(this.myBidHistory
				.getLastBid()) - MAX_DIFF_UTILITY))) {

			logger.logln("");
			logger.logln("-- ACCEPTED");
			logger.logln("My previous bid: " + this.myBidHistory.getLastBid());
			logger.logln("Accepted bid with u = "
					+ getUtility(this.opBidHistory.getCombinedHistory()
							.getLastBid())
					+ ", because little difference with last bid with u = "
					+ getUtility(myBidHistory.getLastBid()));
			logger.logln("");
			return new Accept();

		} else if (this.getTimeLine().getTime() > TIME_ACCEPT_ABOVE_MIN_UTILITY
				&& getUtility(this.opBidHistory.getCombinedHistory()
						.getLastBid()) > this.minUtility()) {
			...
			logger.logln("Accepted bid with u = "
					+ getUtility(this.opBidHistory.getCombinedHistory()
							.getLastBid())
					+ ", because its utility was higher than the minimal required utility u = "
					+ this.minUtility() + ".");
			logger.logln("");
			return new Accept();

		} else if (new Random().nextDouble() < Math.min(1,
				this.probAcceptance())
				&& this.acceptWithProbability()) {

			logger.logln("");
			logger.logln("-- ACCEPTED");
			logger.logln("Accepted because of the acceptance probability of "
					+ this.probAcceptance() + ".");
			logger.logln("");
			return new Accept();
		} else {
	...
	public double probAcceptance() {
		return Math.exp(-(Math.abs(this.maxUtility
				- getUtility(this.opBidHistory.getCombinedHistory()
						.getLastBid())))
	...
	public Bid regularBidMaxOpponentUtilityEstimation() {
		...
		ArrayList<Bid> list = bidSpace.getBids(this.minUtility(),
				this.maxUtility);
		for (Bid myBid : list) {
	...
	public Bid normalBid() {
		Bid myBid = bidSpace.generatePseudoRandomBid(this.minUtility(),
				this.maxUtility);
	...
	public Offer generateOffer(Bid bid) {
		...
		for (Map.Entry<AgentID, IssueEstimator> entry : this.opIssueEstimator
				.getIssueEstimators().entrySet()) {
			logger.logln("Opponent Toughness: "
					+ this.opBidHistory.getHistory(entry.getKey())
							.getToughness(TOUGHNESS_NUM_BIDS));
			logger.logln("Estimated Opponent Utility ("
					+ entry.getKey()
					+ "): "
					+ this.opIssueEstimator.getIssueEstimator(entry.getKey())
							.getUtility(bid));
		}
		logger.logln(" ");
	...
	public double minUtility() {
		return this.maxUtility
				- (this.getTimeLine().getTime() * this.toughness);
	}
	...
	public void adjustToughness() {
		if (this.opBidHistory.getMinToughness(TOUGHNESS_NUM_BIDS) < DEFAULT_TOUGHNESS
			...
			this.toughness = Math.max(MIN_TOUGHNESS,
					this.opBidHistory.getMinToughness(TOUGHNESS_NUM_BIDS));
		} else {
			this.toughness = DEFAULT_TOUGHNESS
					+ this.getAdditionalToughnessNumberOfParties();
		}
	}
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action.getClass() == Offer.class) {
			...
			this.opIssueEstimator.addBid(agentId,
					new BidDetails(new Bid(opponentOffer.getBid()),
							getUtility(opponentOffer.getBid())),

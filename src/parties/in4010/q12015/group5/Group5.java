...
public class Group5 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		try {
			...
			if (acceptBid(bid))
				...
				lastBid = bid;
				return new Offer(new Bid(bid));
			}
		} catch (Exception e) {
			e.printStackTrace();
			return new Accept(); // Accepting is always better than crashing.
		}
	}
	...
	private boolean acceptBid(Bid bid) throws Exception {
		...
		double lastUtility = (lastBid != null) ? utilitySpace
				.getUtility(lastBid) : 0;
		...
		if (time >= 0.95) {
			...
			for (Object opponent : opponentModels.keySet()) {
				opponentBids.addAll(opponentModels.get(opponent).getBidHistory(
						1 - time));
			}
			double maxUtility = 0;

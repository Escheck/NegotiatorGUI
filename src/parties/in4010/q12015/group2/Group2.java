...
public class Group2 extends AbstractNegotiationParty {
	...
	public AgentID getPartyId() {
		return new AgentID(Group2.class.getSimpleName());
	}
	...
	public void receiveMessage(AgentID sender, Action arguments) {
		...
		this.opponentBid = Action.getBidFromAction(arguments);
	...
	public Action chooseAction(List<Class<? extends Action>> actions) {
		...
		return isAcceptable() ? new Accept() : new Offer(this.upcommingBid);
	}
		...
		for (int j = 0; j < 2; j++) {
			for (int i = 0; i < RANDOM_ITERATIONS; i++) {
				...
				if (newUtility > oldUtility
						...
						&& newUtility > bestUtility
						&& isBetterThanOpponents(bid)
						&& (ignoreSuitabilityCheck || isSuitableForOpponents(bid))) {
	...
	private Bid getRandomBid(double target) {
		for (int i = 0; i < RANDOM_ITERATIONS; i++) {
			Bid b = utilitySpace.getDomain().getRandomBid();
	...
	private void modelOpponent(AgentID sender, Action arguments) {
		...
		if ((arguments instanceof Offer)) {
			try {
				((OpponentModel) this.opponentsModels.get(sender.toString()))
						.updateBeliefs(this.opponentBid);
			} catch (Exception e) {
				;
			}
		}
		...
		if ((arguments instanceof Offer)) {
			this.trainingBids.get(sender.toString()).add(
					this.opponentBid.toString().replaceAll("\\s", ""));
		}
	}
	...
	private void buildStatisticalLanguageModel() {
		if (timeline.getTime() > TRAINING_TIME && this.languageModels.isEmpty()) {
			for (Map.Entry<String, List<String>> entry : trainingBids
					.entrySet()) {
				try {
					String opponentId = entry.getKey();
					List<String> opponentBids = entry.getValue();
					PrintWriter writer = new PrintWriter(opponentId + LM_TXT,
							"UTF-8");
					for (int i = 0; i < opponentBids.size(); i++) {
						for (int j = i; j < Math.min(i + LM_ORDER,
								opponentBids.size()); j++) {
					...
					LmReaders.createKneserNeyLmFromTextFiles(files, wi,
							LM_ORDER, new File(opponentId + LM_ARPA),
							new ConfigOptions());
					LmReaders.writeLmBinary(
							LmReaders.readArrayEncodedLmFromArpa(opponentId
									+ LM_ARPA, true), opponentId + LM_BINARY);
			...
			for (Map.Entry<String, List<String>> entry : trainingBids
				...
				try {
					...
					Collection<Entry<String, Double>> dist = getDistributionOverNextWords(
							this.languageModels.get(opponentId), opponentBids)
							.getEntriesSortedByDecreasingCount();
					for (Entry<String, Double> e : dist) {
						if (localResult.size() == TOP_K) {
							break;
						} else if (UNK_SYMBOL.equals(e.getKey())
								|| START_SYMBOL.equals(e.getKey())
								|| END_SYMBOL.equals(e.getKey())
								|| ".".equals(e.getKey())) {
							continue;
						} else {
	...
	private boolean isSuitableForOpponents(Bid bid) {
		...
		return predictedBids.isEmpty()
				|| predictedBids.contains(bid.toString().replaceAll("\\s", ""));
	}
}

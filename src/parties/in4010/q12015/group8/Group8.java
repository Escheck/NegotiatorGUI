...
public class Group8 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> possibleActions) {
		...
		if (getTimeLeft() >= GIVE_IN_THRESHOLD) {
			...
			BID_UTILITY = Math.max(MINIMUM_BID_UTILITY, BID_UTILITY
					- (1.0 - MINIMUM_BID_UTILITY) / (1.0 - GIVE_IN_THRESHOLD)
					* (getTimeLeft() - GIVE_IN_THRESHOLD));
		...
		try {
			...
			if (possibleActions.contains(Accept.class)
					&& canAccept(enemyBids.get(enemyBids.size() - 1))
			...
			return new Accept(); // prevent returning nothing
		}
	}
	...
	private Offer generateBid() throws Exception {
		...
		return new Offer(agentBids.get(nextBid));
	}
	...
	private List<HashMap<Integer, Value>> getAllBidsForIssue(
			ArrayList<Issue> issues, int i) throws Exception {
	...
	private void filterBids() {
		...
		if (history.size() >= FILTER_THRESHOLD) {
			for (Map.Entry<AgentID, OpponentModel> om : opponentModels
					.entrySet()) {

...
public class Group12 extends AbstractNegotiationParty {
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (!otherAgents.containsKey(sender)) {
			OtherAgent newAgent = new OtherAgent(String.valueOf(sender),
					amountIssues);
		...
		if (Action.getBidFromAction(action) != null) {
			BidDetails bidDetail = new BidDetails(
					Action.getBidFromAction(action),
					getUtility(Action.getBidFromAction(action)));
			freqAnalysis(Action.getBidFromAction(action), sender);
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		calculateThreshold();
		if (history.size() != 0
				&& history.getLastBidDetails().getMyUndiscountedUtil() > threshold) {
			return new Accept();
		}

		return new Offer(generateBid(threshold));
	}
	...
	private void freqAnalysis(Bid newBid, Object sender) {
		...
		if (!senderAgent.getHistory().isEmpty()) {
			double[] newWeights = determineWeights(newBid, senderAgent
					.getHistory().getLastBid(), 0.1, senderAgent.getWeights());
	...
	private double[] determineWeights(Bid newBid, Bid prevBid, double n,
			double[] weight) {

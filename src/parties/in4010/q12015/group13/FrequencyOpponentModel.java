...
public class FrequencyOpponentModel {
	...
	public double estimateUtility(Bid b) {
		...
		for (int i = 0; i < issueModels.size(); i++) {
			try {
				ret += weights.get(i)
						* issueModels.get(i).estimateUtility(
								b.getValue(issueNumbers.get(i)));
			} catch (Exception ex) {
				System.err.println("Exception while estimating utility: "
						+ ex.getMessage());
			}
		}
	...
	public void addBid(Bid b) throws Exception {
		...
		if (previousBid != null) {
			for (int i = 0; i < weights.size(); i++) {
				if (b.getValue(issueNumbers.get(i)).equals(
						previousBid.getValue(issueNumbers.get(i)))) {
	...
	public String toString() {
		...
		for (int i = 0; i < issueModels.size(); i++) {
			ret += "Issue " + issueNumbers.get(i) + " (weight = "
					+ weights.get(i) + " )" + ":\n";

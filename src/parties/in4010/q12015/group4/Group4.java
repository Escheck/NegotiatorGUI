...
public class Group4 extends AbstractNegotiationParty {
	...
	public void saveBestBid(Action Action) {
		if (getUtility(Action.getBidFromAction(Action)) < MAXIMUM_BID_UTILITY) {
			if (BEST_BID_OPPONENT_OFFER == null) {
				BEST_BID_OPPONENT_OFFER = Action.getBidFromAction(Action);
				BEST_BID_OPPONENT_OFFER_UTILITY = getUtility(BEST_BID_OPPONENT_OFFER);
			} else {
				if (getUtility(BEST_BID_OPPONENT_OFFER) < getUtility(Action
						.getBidFromAction(Action))) {
					BEST_BID_OPPONENT_OFFER = opponentAction
							.getBidFromAction(Action);
					BEST_BID_OPPONENT_OFFER_UTILITY = getUtility(BEST_BID_OPPONENT_OFFER);
				}
			}

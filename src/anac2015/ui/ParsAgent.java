...
public class ParsAgent extends AbstractTimeDependentNegotiationParty {
     ...
     */
    public ParsAgent(UtilitySpace utilitySpace,
                     Map<DeadlineType, Object> deadlines, Timeline timeline,
                     long randomSeed) {
        // Make sure that this constructor calls it's parent.
        super(utilitySpace, deadlines, timeline, randomSeed);
        Object[] keys = deadlines.keySet().toArray();
        for (int i = 0; i < keys.length; ++i) {
            if (keys[i] instanceof DeadlineType) {
                round = Integer.parseInt(deadlines.get(keys[i]).toString());
            }
        }
                ...
                if (lastAction instanceof Accept) {
                    ...
                    if (utilitySpace.getUtility(lastBid) > getMyutility()) {
                        ...
                        if (utilitySpace.getUtility(b) < getMyutility())
                            return new Offer(getPartyId(),getMybestBid(utilitySpace.getMaxUtilityBid(),
                                                          0));
                        else
                            return new Offer(getPartyId(),b);
...
//            fornullAgent ? "null1" : "null2";
        ...
        if (action != null && action instanceof Offer) {
            ...
            if (withDiscount == null) {
                try {
                    if (utilitySpace.getUtilityWithDiscount(newBid,
                                                            timeline) !=
                        utilitySpace.getUtility(newBid)) {
            ...
            try {
                opBid =
                        new BidUtility(newBid, utilitySpace.getUtility(newBid), System.currentTimeMillis());
                ...
                calculateParamForOpponent((oppAName.equals(agentName) ?
                                           oppAPreferences : oppBPreferences),
                                          newBid);
                lastBid = newBid;
            } catch (Exception e) {
            ...
            try {
                opBid =
                        new BidUtility(lastBid, utilitySpace.getUtility(lastBid),
            ...
            if (bidNN == null ||
                ...
                for (int i = 0; i < isues.size(); ++i) {
                    ...
                    if (keyVal != null &&
                        ...
                        for (int num = 0; num < is.getNumberOfValues();
                             ++num) {
                            if (is.getValue(num).toString().equals(keyVal.get(0).toString())) {
                ...
                try {
                    bid =
new Bid(utilitySpace.getDomain(), (HashMap)map.clone());
                    if (utilitySpace.getUtility(bid) > getMyutility())
                        return bid;
                    else
                        return getMybestBid(utilitySpace.getMaxUtilityBid(),
                                            0);
                } catch (Exception e) {
    ...
    public ArrayList getMutualIssues() {
        ...
        while (twocycle > 0) {
            ...
            for (int i = 0; i < dissues.size(); ++i) {
                if (oppAPreferences.getRepeatedissue().get(dissues.get(i).getName()) !=
                    null) {
                    HashMap vals =
                ...
                if(((HashMap)oppAPreferences.getRepeatedissue().get(dissues.get(i).getName())).size()==0 ||
                   ((HashMap)oppAPreferences.getRepeatedissue().get(dissues.get(i).getName())).size()==0){
                       twocycle--; 
                   }
            }
            if (twocycle != 0) {
    ...
    public void calculateParamForOpponent(OpponentPreferences op, Bid bid) {
        ...
        for (int i = 0; i < dissues.size(); ++i) {
            if (op.getRepeatedissue().get(dissues.get(i).getName()) != null) {
                ...
                HashMap vals =
                    (HashMap)op.getRepeatedissue().get(dissues.get(i).getName());

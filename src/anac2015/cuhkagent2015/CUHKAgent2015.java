package anac2015.cuhkagent2015;
...
//    private Action ActionOfOpponent = null;
    ...
    private Bid bid_minimum_utility;//the bid with the minimum utility over the utility space.
    ...
    public CUHKAgent2015 (UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines, Timeline timeline, long randomSeed)
    {
        // Make sure that this constructor calls it's parent.
        super(utilitySpace, deadlines, timeline, randomSeed);
        try 
        {
            this.NumberOfBid = this.utilitySpace.getDomain().getNumberOfPossibleBids();
            ...
            this.MaximumUtility = this.utilitySpace.getUtility(this.bid_maximum_utility);
            ...
            this.opponentBidHistory1.initializeDataStructures(utilitySpace.getDomain());
            this.opponentBidHistory2.initializeDataStructures(utilitySpace.getDomain());
    ...
    {
        return this.partyId == null ? new AgentID(String.format("CUHKAgent2015")) : this.partyId;
    }
    ...
    {    	
        ...
        try 
        {
            ...
            if ((!possibleActions.contains(Accept.class)) || (this.oppFirstBid_Agent1 == null) || (this.oppFirstBid_Agent2 == null)) 
            {
            ...
            else 
            {
                ...
                this.opponentBidHistory1.updateOpponentModel(this.oppPreviousBid_Agent1, utilitySpace.getDomain(), this.utilitySpace);
                this.opponentBidHistory2.updateOpponentModel(this.oppPreviousBid_Agent2, utilitySpace.getDomain(), this.utilitySpace);
                ...
                {   //other conditions
                    if (estimateRoundLeft(true) > 10)
                    {
                        ...
                        Boolean IsAccept = AcceptOpponentOffer(this.oppPreviousBid_Agent2, bid);
                        ...
                        {
                            if (this.utilitySpace.getUtility(this.oppPreviousBid_Agent2) > this.reservationValue) 
                            {
                                action = new Accept(getPartyId());
                        ...
                        else
                        {
                            ...
                            if (this.concedeToOpponent == true) 
                            {
                                ...
                                Bid bid1 = opponentBidHistory1.getBestBidInHistory();
                                Bid bid2 = opponentBidHistory2.getBestBidInHistory();
                                if(this.utilitySpace.getUtility(bid1)>this.utilitySpace.getUtility(bid2)){
                                    bid = bid2;
                                }else if(this.utilitySpace.getUtility(bid1)==this.utilitySpace.getUtility(bid2) && Math.random()<0.5){
                                    bid = bid2;
                                }else{bid = bid1;}
                                action = new Offer(getPartyId(), bid);
                    ...
                    {//this is the last chance and we concede by providing the opponent the best offer he ever proposed to us
                        ...
                        {//we still have some time to negotiate, 
            ...
            if(debug){
                ...
                System.out.println(Double.toString(this.timeline.getTime())+","+Double.toString(this.AvgUtilitythreshold)+","+Double.toString(this.minUtilityUhreshold));
                System.setOut(SysOut);
            }
        } catch (Exception e) {
            System.out.println("Exception in ChooseAction:" + e.getMessage());
            System.out.println(estimateRoundLeft(false));
            action = new EndNegotiation(getPartyId()); //terminate if anything goes wrong.
        }
        return action;
    }
    ...
    public void receiveMessage(Object sender, Action opponentAction)
    {
        ...
        if(this.countOpp == this.EachRoundCount + 1)//Next Agent w.r.t. us
        {
            if ((this.ActionOfOpponent instanceof Offer))
            {
                ...
                {
                    oppFirstBid_Agent1 = ((Offer)this.ActionOfOpponent).getBid();
                }
                for(int i = 0; i<this.storingcapacity-1; i++){
                    this.OppBids1.set(i,this.OppBids1.get(i+1));
                }
                this.oppPreviousBid_Agent1 = ((Offer)this.ActionOfOpponent).getBid();
                this.OppBids1.set(this.storingcapacity-1,this.oppPreviousBid_Agent1);
            }	
        ...
        else if (this.countOpp == this.EachRoundCount + 2)//Previous Agent w.r.t. us
        {
            if ((this.ActionOfOpponent instanceof Offer))
            {
                if(this.oppFirstBid_Agent2 == null)
                {
                    oppFirstBid_Agent2 = ((Offer)this.ActionOfOpponent).getBid();
                }
                for(int i = 0; i<this.storingcapacity-1; i++){
                    this.OppBids2.set(i,this.OppBids2.get(i+1));
                }
                this.oppPreviousBid_Agent2 = ((Offer)this.ActionOfOpponent).getBid();
                this.OppBids2.set(this.storingcapacity-1,this.oppPreviousBid_Agent2);
            }	
            else if ((this.ActionOfOpponent instanceof Accept))
            {
    ...
    private double BidToOffer() {
        try {
            ...
            AvgConcedeTime = (this.concedeTime_Agent2 + this.concedeTime_Agent1) / 2;

            if (timeline.getTime() <= AvgConcedeTime)
            {
                if (this.discountingFactor <= 0.5)
                {
                    this.minThreshold_Agent1 = (maximumOfBid * this.discountingFactor) / Math.pow(this.discountingFactor, this.concedeTime_Agent1);
                    this.minThreshold_Agent2 = (maximumOfBid * this.discountingFactor) / Math.pow(this.discountingFactor, this.concedeTime_Agent2);
                    this.utilitythreshold_Agent1 = maximumOfBid - (maximumOfBid - this.minThreshold_Agent1) * Math.pow((timeline.getTime() / this.concedeTime_Agent1), alpha1);
                    this.utilitythreshold_Agent2 = maximumOfBid - (maximumOfBid - this.minThreshold_Agent2) * Math.pow((timeline.getTime() / this.concedeTime_Agent2), alpha1);  
                } 
                else 
                {
                    this.utilitythreshold_Agent1 = this.minThreshold_Agent1 + (maximumOfBid - this.minThreshold_Agent1) * (1- Math.sin((Math.PI / 2) * (timeline.getTime() / this.concedeTime_Agent1)));
                    this.utilitythreshold_Agent2 = this.minThreshold_Agent2 + (maximumOfBid - this.minThreshold_Agent2) * (1- Math.sin((Math.PI / 2) * (timeline.getTime() / this.concedeTime_Agent2)));
                }
            } 
            else 
            {
                if (this.discountingFactor <= 0.5)
                {
                    this.utilitythreshold_Agent1 = (maximumOfBid * this.discountingFactor) / Math.pow(this.discountingFactor, timeline.getTime());
                    this.utilitythreshold_Agent2 = (maximumOfBid * this.discountingFactor) / Math.pow(this.discountingFactor, timeline.getTime());
                } 
                else
                {
                    this.utilitythreshold_Agent1 = this.minThreshold_Agent1 + (maximumOfBid - this.minThreshold_Agent1) / (1 - concedeTime_Agent1) * Math.pow((timeline.getTime() - this.concedeTime_Agent1), this.discountingFactor);
                    this.utilitythreshold_Agent2 = this.minThreshold_Agent2 + (maximumOfBid - this.minThreshold_Agent2) / (1 - concedeTime_Agent2) * Math.pow((timeline.getTime() - this.concedeTime_Agent2), this.discountingFactor);
                }
            }
            this.AvgUtilitythreshold = (this.utilitythreshold_Agent2 + this.utilitythreshold_Agent1) / 2;
            ...
            while (!accept){
                ...
                if(count < 2){
                    for(double temputil = this.AvgUtilitythreshold; temputil <= this.MaximumUtility; temputil+=0.01){
                        ...
                        //double temputil = this.utilitySpace.getUtility(tempbid);
                        double pchoose1 = (1 / this.relstdevUtility_Agent1 * (1 / Math.sqrt(2*Math.PI))) * Math.exp(-Math.pow(temputil - this.relavgUtility_Agent1, 2)/(2 * Math.pow(this.relstdevUtility_Agent1, 2)));
                        double pchoose2 = (1 / this.relstdevUtility_Agent2 * (1 / Math.sqrt(2*Math.PI))) * Math.exp(-Math.pow(temputil - this.relavgUtility_Agent2, 2)/(2 * Math.pow(this.relstdevUtility_Agent2, 2)));
                        double pchoose = (pchoose1 + pchoose2) /2;
                        if(pchoose >= Math.random()/* && temputil >= this.utilitythreshold*/){
                            return temputil;
                        }
                    }
                }
    ...
    private boolean AcceptOpponentOffer(Bid opponentBid, Bid ownBid) {
        ...
        if (currentUtility >= this.AvgUtilitythreshold || currentUtility >= nextRoundUtility) {
            return true;
        } else {
            ...
            double currentMaximumUtility1 = this.utilitySpace.getUtilityWithDiscount(opponentBidHistory1.getBestBidInHistory(), timeline);
            double currentMaximumUtility2 = this.utilitySpace.getUtilityWithDiscount(opponentBidHistory2.getBestBidInHistory(), timeline);
            double currentMaximumUtility = Math.min(currentMaximumUtility1,currentMaximumUtility2);
            if (currentMaximumUtility > predictMaximumUtility && timeline.getTime() > this.AvgConcedeTime) {
                try {
                    //if the current offer is approximately as good as the best one in the history, then accept it.
                    if (utilitySpace.getUtilityWithDiscount(opponentBid, timeline) >= currentMaximumUtility - 0.01) {
            ...
            } else if (currentMaximumUtility > this.AvgUtilitythreshold * Math.pow(this.discountingFactor, timeline.getTime())) {
                try {
                    //if the current offer is approximately as good as the best one in the history, then accept it.
                    if (utilitySpace.getUtilityWithDiscount(opponentBid, timeline) >= currentMaximumUtility - 0.01) {
                        return true;
                    } else {
                        System.out.println("test" + utilitySpace.getUtility(opponentBid) + this.AvgUtilitythreshold);
                        this.concedeToOpponent = true;
                        return false;
                    }
                } catch (Exception e) {
...
private void calculateBidsBetweenUtility() {
        ...
        try {
            this.MinimumUtility = Math.min(this.MinimumUtility_Agent1, this.MinimumUtility_Agent2);
            this.SpacebetweenDivisions = (this.MaximumUtility - this.MinimumUtility)/this.NumberofDivisions;
            ...
            this.bidsBetweenUtility.get(this.NumberofDivisions-1).add(counts[this.NumberofDivisions-1],this.bid_maximum_utility);
            ...
            while (myBidIterator.hasNext()) {
                ...
                for (int i = 0; i < this.NumberofDivisions; i++) {
                    if (this.utilitySpace.getUtility(b) <= (i + 1) * this.SpacebetweenDivisions + this.MinimumUtility && this.utilitySpace.getUtility(b) >= i * this.SpacebetweenDivisions + Math.min(this.MinimumUtility_Agent1, this.MinimumUtility_Agent2)) {
    ...
    private void chooseConcedeToDiscountingDegree() {
       ...
       /* if (this.maximumOfBid > 100) {
        ...
        this.AvgConcedeTime = this.minConcedeTime + (1 - this.minConcedeTime) * alpha;
        this.concedeTime_original = this.AvgConcedeTime;
        System.out.println("concedeToDiscountingFactor is " + this.AvgConcedeTime + "current time is " + timeline.getTime());
    }
    /*
    ...
    private Bid regeneratebid(double bidutil){
        ...
        try{
            int check = 0;
            if(this.OppBids1.get(this.storingcapacity-1) == null || this.OppBids2.get(this.storingcapacity-1) == null){
                ans = this.bid_maximum_utility;
            }else{
                for(this.CurrentrangeNumber = 0; check ==0; this.CurrentrangeNumber++){
                    if(bidutil<((this.CurrentrangeNumber+1)*this.SpacebetweenDivisions+this.MinimumUtility)){
                        check = 1;
                        break;
                    }
                }
                ...
                while(Bids.size()<=10){
                    if(this.CurrentrangeNumber <=this.NumberofDivisions-2){
                        for(int i=0;i<this.bidsBetweenUtility.get(this.CurrentrangeNumber+1).size();i++){
                            Bids.add(Bids.size(), this.bidsBetweenUtility.get(this.CurrentrangeNumber+1).get(i));
                        }
                    }else{break;}
                ...
                else
                {
                    ...
                    {
                        for(Bid Bid1: Bids){
                            if(this.utilitySpace.getUtility(Bid1)>this.AvgUtilitythreshold){
                                count = 0;
                                for(int i=0; i<this.storingcapacity; i++){    
                                    if(this.OppBids1.get(i)!=null){
                                        this.averagedistance += calculatedistance(Bid1,this.OppBids1.get(i));
                                        count++;
                                    }
                                    if(this.OppBids2.get(i)!=null){
                                        this.averagedistance += calculatedistance(Bid1,this.OppBids2.get(i));
                                        count++;
                                    }
                                }
                                ...
                                if(this.averagedistance < this.mindistance){
                                    ...
                                    ans = Bid1;
                                } else if(this.averagedistance == this.mindistance && Math.random() <= 0.5){
                                    this.mindistance = this.averagedistance;
                                    ans = Bid1;
                                }
                            }
    ...
    private HashMap<Issue, Value> getBidValues(Bid bid){
        try{
            HashMap<Issue, Value> Values = new HashMap<>();
            ...
            System.out.println("Exception in getBidValues(): " + e.getMessage());
            this.except=18;
        }
        return null;
    }
            ...
            for (Issue Issue1 : this.Issues) {
                switch (Issue1.getType()){
                    ...
                    case REAL:
                        ...
                        break;
                    case INTEGER:
                        ValueInteger valueInteger1 = (ValueInteger) Map1.get(Issue1);
                        ValueInteger valueInteger2 = (ValueInteger) Map2.get(Issue1);
    ...
    private void updateConcedeDegree_smallDiscountingFactor() 
    {
        double gama = 10;
        double weight = 0.1;
        double opponnetToughnessDegree = this.opponentBidHistory2.getConcessionDegree();
        try
        {
            if (IsOppFirstBid_Agent1)
            {
                this.oppFirstBidUtility_Agent1 = utilitySpace.getUtility(this.oppFirstBid_Agent1);
            ...
            {
                this.oppFirstBidUtility_Agent2 = utilitySpace.getUtility(this.oppFirstBid_Agent2);
            ...
            double temp = this.concedeTime_original + weight * (1 - this.concedeTime_original) * Math.pow(opponnetToughnessDegree, gama);
            this.concedeTime_Agent1 = temp;
            this.concedeTime_Agent2 = temp;
            if ((this.concedeTime_Agent1 >= 1) || (this.concedeTime_Agent2 >= 1)) 
            {
   ...
   private void updateConcedeDegree_largeDiscountingFactor_Agent1() {
        try
        {
            ...
            this.concedePartOfdiscountingFactor_Agent1 = this.discountingFactor - 1;       
            this.concedePartOfOppBehaviour_Agent1 = (((((this.relcountLowerBoundMid2_Agent1 /this.relcountUpperBoundMid2_Agent1) * this.slopeOfSlopeOfSessionMax2_Agent1) - ((this.relcountLowerBoundMid1_Agent1 /this.relcountUpperBoundMid1_Agent1) * this.slopeOfSlopeOfSessionMax1_Agent1)) / this.k) / this.relstdevUtility_Agent1) - this.N;
            if(this.startConcede_Agent1 == true)
            {
                i = this.concedePartOfdiscountingFactor_Agent1 + this.concedePartOfOppBehaviour_Agent1;
   ...
   private void updateConcedeDegree_largeDiscountingFactor_Agent2()
   {
        try
        {
            ...
            this.concedePartOfdiscountingFactor_Agent2 = this.discountingFactor - 1;       
            this.concedePartOfOppBehaviour_Agent2 = (((((this.relcountLowerBoundMid2_Agent2 /this.relcountUpperBoundMid2_Agent2) * this.slopeOfSlopeOfSessionMax2_Agent2) - ((this.relcountLowerBoundMid1_Agent2 /this.relcountUpperBoundMid1_Agent2) * this.slopeOfSlopeOfSessionMax1_Agent2)) / this.k) / this.relstdevUtility_Agent2) - this.N;
            if(this.startConcede_Agent2 == true)
            {
                i = this.concedePartOfdiscountingFactor_Agent2 + this.concedePartOfOppBehaviour_Agent2;
   ...
   private void updateConcedeDegree_nonDiscountingFactor_Agent1()
   {
    ...
    private void CalculateMinThreshold_Agent1()
    {
        try 
        {
            ...
            this.relavgUtility_Agent1 = this.relsumUtility_Agent1 / this.relcountUtility_Agent1;
            ...
            for (Iterator f = bidsUtil.iterator(); f.hasNext();)
            {
                ...
                {
                    this.relSqRootOfAvgUtility_Agent1 += (bidUtil - this.relavgUtility_Agent1) * (bidUtil - this.relavgUtility_Agent1);
                }
            }
            this.relstdevUtility_Agent1 = Math.sqrt(this.relSqRootOfAvgUtility_Agent1 / this.relcountUtility_Agent1);
            ...
            if(this.relstdevUtility_Agent1 < 0.1)
            {
                ...
                for (Iterator g = bidsUtil.iterator(); g.hasNext();)
                {
                    ...
                    if (bidUtil >= this.oppFirstBidUtility_Agent1 && (bidUtil < (this.relavgUtility_Agent1 - this.relstdevUtility_Agent1) || bidUtil > (this.relavgUtility_Agent1 + this.relstdevUtility_Agent1)))
                    {
                        this.relSqRootOfAvgUtility_Agent1 += (bidUtil - this.relavgUtility_Agent1) * (bidUtil - this.relavgUtility_Agent1);
                        this.relcountUtilityInSmallSD_Agent1++;
                    }
                }
                this.relstdevUtility_Agent1 = Math.sqrt(this.relSqRootOfAvgUtility_Agent1 / this.relcountUtilityInSmallSD_Agent1);
            }

            this.minThreshold_Agent1 = this.relavgUtility_Agent1 + this.discountingFactor * this.relstdevUtility_Agent1 * this.reservationValue;
        ...
        {
            System.out.println(e.getMessage() + "exception in method CalculateMinThreshold");
        }
    }
    ...
    private void CalculateMinThreshold_Agent2()
    {
        try 
        {
            ...
            this.relavgUtility_Agent2 = this.relsumUtility_Agent2 / this.relcountUtility_Agent2;
            ...
            for (Iterator f = bidsUtil.iterator(); f.hasNext();)
            {
                ...
                {
                    this.relSqRootOfAvgUtility_Agent2 += (bidUtil - this.relavgUtility_Agent2)*(bidUtil - this.relavgUtility_Agent2);
                }
            }
            this.relstdevUtility_Agent2 = Math.sqrt(this.relSqRootOfAvgUtility_Agent2 / this.relcountUtility_Agent2);
            ...
            if(this.relstdevUtility_Agent2 < 0.1)
            {
                ...
                for (Iterator g = bidsUtil.iterator(); g.hasNext();)
                {
                    ...
                    if (bidUtil >= this.oppFirstBidUtility_Agent2 && (bidUtil < (this.relavgUtility_Agent2 - this.relstdevUtility_Agent2) || bidUtil > (this.relavgUtility_Agent2 + this.relstdevUtility_Agent2)))
                    {
                        this.relSqRootOfAvgUtility_Agent2 += (bidUtil - this.relavgUtility_Agent2)*(bidUtil - this.relavgUtility_Agent2);
                        this.relcountUtilityInSmallSD_Agent2++;
                    }
                }
                this.relstdevUtility_Agent2 = Math.sqrt(this.relSqRootOfAvgUtility_Agent2 / this.relcountUtilityInSmallSD_Agent2);
            }

            this.minThreshold_Agent2 = this.relavgUtility_Agent2 + this.discountingFactor * this.relstdevUtility_Agent2 * this.reservationValue;
    ...
    private void CalculateSumAndCount_Agent1(ArrayList<Double> bidsUtil)
    {
        ...
        {
            System.out.println(e.getMessage() + "exception in method CalculateSumAndCount");
        }
    }
    ...
    private void MeasureConcedePartOfOppBehaviour_Agent1()
    {
        try 
        {
            ...
            {
                if (utilitySpace.getUtility(this.oppPreviousBid_Agent1) > this.utility_maximum_from_opponent_Session1_Agent1) 
                {
                        this.utility_maximum_from_opponent_Session1_Agent1 = utilitySpace.getUtility(this.oppPreviousBid_Agent1);
                }
            }
            else if(count_Agent2 < this.SecondTimeInterval)
            {
                if (utilitySpace.getUtility(this.oppPreviousBid_Agent1) > this.utility_maximum_from_opponent_Session2_Agent1) 
                {
                    this.utility_maximum_from_opponent_Session2_Agent1 = utilitySpace.getUtility(this.oppPreviousBid_Agent1);
                }    
            }
            else if(count_Agent2 < this.ThirdTimeInterval)
            {
                if (utilitySpace.getUtility(this.oppPreviousBid_Agent1) > this.utility_maximum_from_opponent_Session3_Agent1) 
                {
                    this.utility_maximum_from_opponent_Session3_Agent1 = utilitySpace.getUtility(this.oppPreviousBid_Agent1);
                }    
            }
            else
            {
                ...
                this.midPointOfSlopeSessionMax1_Agent1 = (this.utility_maximum_from_opponent_Session2_Agent1 + this.utility_maximum_from_opponent_Session1_Agent1) / 2;
                this.midPointOfSlopeSessionMax2_Agent1 = (this.utility_maximum_from_opponent_Session3_Agent1 + this.utility_maximum_from_opponent_Session2_Agent1) / 2;
                this.slopeOfSlopeOfSessionMax1_Agent1 = this.utility_maximum_from_opponent_Session2_Agent1 - this.utility_maximum_from_opponent_Session1_Agent1;
                this.slopeOfSlopeOfSessionMax2_Agent1 = this.utility_maximum_from_opponent_Session3_Agent1 - this.utility_maximum_from_opponent_Session2_Agent1;
        ...
        {
            System.out.println(e.getMessage() + "exception in method CalculateConcedePartOfOppBehaviour");
        }
    }
    ...
    private void MeasureConcedePartOfOppBehaviour_Agent2()
    {
        try 
        {
            ...
            {
                if (utilitySpace.getUtility(this.oppPreviousBid_Agent2) > this.utility_maximum_from_opponent_Session1_Agent2) 
                {
                        this.utility_maximum_from_opponent_Session1_Agent2 = utilitySpace.getUtility(this.oppPreviousBid_Agent2);
                }
            }
            else if(count_Agent2 < this.SecondTimeInterval)
            {
                if (utilitySpace.getUtility(this.oppPreviousBid_Agent2) > this.utility_maximum_from_opponent_Session2_Agent2) 
                {
                    this.utility_maximum_from_opponent_Session2_Agent2 = utilitySpace.getUtility(this.oppPreviousBid_Agent2);
                }    
            }
            else if(count_Agent2 < this.ThirdTimeInterval)
            {
                if (utilitySpace.getUtility(this.oppPreviousBid_Agent2) > this.utility_maximum_from_opponent_Session3_Agent2) 
                {
                    this.utility_maximum_from_opponent_Session3_Agent2 = utilitySpace.getUtility(this.oppPreviousBid_Agent2);
                }    
            }
            else
            {
                ...
                this.midPointOfSlopeSessionMax1_Agent2 = (this.utility_maximum_from_opponent_Session2_Agent2 + this.utility_maximum_from_opponent_Session1_Agent2) / 2;
                this.midPointOfSlopeSessionMax2_Agent2 = (this.utility_maximum_from_opponent_Session3_Agent2 + this.utility_maximum_from_opponent_Session2_Agent2) / 2;
                this.slopeOfSlopeOfSessionMax1_Agent2 = this.utility_maximum_from_opponent_Session2_Agent2 - this.utility_maximum_from_opponent_Session1_Agent2;
                this.slopeOfSlopeOfSessionMax2_Agent2 = this.utility_maximum_from_opponent_Session3_Agent2 - this.utility_maximum_from_opponent_Session2_Agent2;
    ...
    private Bid BidToOffer_original() {
        ...
        try {
            ...
            Bid bestBidOfferedByOpponent1 = opponentBidHistory1.getBestBidInHistory();
            Bid bestBidOfferedByOpponent2 = opponentBidHistory2.getBestBidInHistory();
            Bid bestBidOfferedByOpponent = this.getUtility(bestBidOfferedByOpponent1)>this.getUtility(bestBidOfferedByOpponent2) ? bestBidOfferedByOpponent2 : bestBidOfferedByOpponent1;
            if (utilitySpace.getUtility(bestBidOfferedByOpponent) >= this.AvgUtilitythreshold || utilitySpace.getUtility(bestBidOfferedByOpponent) >= minimumOfBid) {
    ...
    private boolean OtherAcceptCondition(boolean IsAccept) 
    {
        try
        {
            if((timeline.getTime() >= ((this.concedeTime_Agent2 + this.concedeTime_Agent1) / 2)) && (this.utilitySpace.getUtility(((Offer) ActionOfOpponent).getBid()) >= this.minUtilityUhreshold)){
                IsAccept = true;
            }
        } 
        catch (Exception e)
        {
            this.except=27;
            System.out.println(e.getMessage() + "exception in method AcceptCondition");
        }
        return IsAccept;
    }
    ...
    private void MinAcceptCondition(Bid bidReturned) 
    {
        try 
        {
            if(this.minUtilityUhreshold > this.utilitySpace.getUtility(bidReturned))
            {
                this.minUtilityUhreshold = this.utilitySpace.getUtility(bidReturned);
            }
        }
        catch (Exception e)
        {

...
public class CUHKAgent2015 extends AbstractNegotiationParty {
	...
	public AgentID getPartyId() {
		return this.partyId == null ? new AgentID(
				String.format("CUHKAgent2015")) : this.partyId;
	}
					...
					if (estimateRoundLeft(true) > 10) {
						...
						if (IsAccept && !IsTerminate) {
							...
							if (this.utilitySpace
									.getUtility(this.oppPreviousBid_Agent2) > this.reservationValue) {
								action = new Accept(getPartyId());
								System.out
	...
	private double BidToOffer() {
		try {
			...
			if (timeline.getTime() <= AvgConcedeTime) {
				...
				if (this.discountingFactor <= 0.5) {
					this.utilitythreshold_Agent1 = (maximumOfBid * this.discountingFactor)
							/ Math.pow(this.discountingFactor,
									timeline.getTime());
					this.utilitythreshold_Agent2 = (maximumOfBid * this.discountingFactor)
							/ Math.pow(this.discountingFactor,
									timeline.getTime());
				} else {
					this.utilitythreshold_Agent1 = this.minThreshold_Agent1
			...
			while (!accept) {
				...
				if (count < 2) {
					for (double temputil = this.AvgUtilitythreshold; temputil <= this.MaximumUtility; temputil += 0.01) {
						...
						double pchoose1 = (1 / this.relstdevUtility_Agent1 * (1 / Math
								.sqrt(2 * Math.PI)))
								* Math.exp(-Math.pow(temputil
										- this.relavgUtility_Agent1, 2)
										/ (2 * Math.pow(
												this.relstdevUtility_Agent1, 2)));
						double pchoose2 = (1 / this.relstdevUtility_Agent2 * (1 / Math
								.sqrt(2 * Math.PI)))
								* Math.exp(-Math.pow(temputil
										- this.relavgUtility_Agent2, 2)
										/ (2 * Math.pow(
												this.relstdevUtility_Agent2, 2)));
			...
			if (this.OppBids1.get(this.storingcapacity - 1) == null
				...
				while (Bids.size() <= 10) {
					if (this.CurrentrangeNumber <= this.NumberofDivisions - 2) {
						for (int i = 0; i < this.bidsBetweenUtility.get(
								this.CurrentrangeNumber + 1).size(); i++) {
							Bids.add(
									Bids.size(),
									this.bidsBetweenUtility.get(
											this.CurrentrangeNumber + 1).get(i));
						}
					} else {
						break;
					}
	...
	private void updateConcedeDegree_largeDiscountingFactor_Agent1() {
		try {
			...
			this.concedePartOfOppBehaviour_Agent1 = (((((this.relcountLowerBoundMid2_Agent1 / this.relcountUpperBoundMid2_Agent1) * this.slopeOfSlopeOfSessionMax2_Agent1) - ((this.relcountLowerBoundMid1_Agent1 / this.relcountUpperBoundMid1_Agent1) * this.slopeOfSlopeOfSessionMax1_Agent1)) / this.k) / this.relstdevUtility_Agent1)
					- this.N;
	...
	private void updateConcedeDegree_largeDiscountingFactor_Agent2() {
		try {
			...
			this.concedePartOfOppBehaviour_Agent2 = (((((this.relcountLowerBoundMid2_Agent2 / this.relcountUpperBoundMid2_Agent2) * this.slopeOfSlopeOfSessionMax2_Agent2) - ((this.relcountLowerBoundMid1_Agent2 / this.relcountUpperBoundMid1_Agent2) * this.slopeOfSlopeOfSessionMax1_Agent2)) / this.k) / this.relstdevUtility_Agent2)
					- this.N;
	...
	private Bid BidToOffer_original() {
		...
		try {
			...
			Bid bestBidOfferedByOpponent = this
					...
					.getUtility(bestBidOfferedByOpponent2) ? bestBidOfferedByOpponent2
					: bestBidOfferedByOpponent1;
			if (utilitySpace.getUtility(bestBidOfferedByOpponent) >= this.AvgUtilitythreshold

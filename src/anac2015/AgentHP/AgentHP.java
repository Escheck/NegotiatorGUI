...
public class AgentHP extends AbstractNegotiationParty {
	...
	public AgentHP(UtilitySpace utilitySpace,
		...
		for (Issue tmp : utilitySpace.getDomain().getIssues()) {
			...
			for (int j = 0; j < tmpDiscrete.getNumberOfValues(); j++) {
				TemplateLogBidValueHash[issue_num - 1].put(tmpDiscrete
						.getValue(j).toString(), 0.0);
				TemplateLogBidCountHash[issue_num - 1].put(tmpDiscrete
						.getValue(j).toString(), 0);
				addupValueHash[issue_num - 1].put(tmpDiscrete.getValue(j)
						.toString(), 0.0);
			}
		}
	...
	private void initHashArray(int partyNum) {
		...
		for (int j = 1; j <= compareSize; j++) {
			compareResultArray[j - 1] = (2.0 * (double) j - 1.0)
					/ (2.0 * (double) compareSize - 1.0);
		}
	...
	private void makeConcession() {
		...
		// 分散
		double variance = Math.sqrt(((allSum2) / (double) round)
				- (mean * mean));

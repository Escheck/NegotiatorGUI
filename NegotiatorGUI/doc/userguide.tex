...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{Log competitiveness} this computes the amount of discrepancy between the two utility spaces. If X is the bid with maximum utility for side B, then {\em full yield of A} is the utility of X for side A.  The competitiveness ratio is defined as CR(util)=1-(max(fullyield, util) - fullyield) / (1 - fullyield). If this option is enabled, the following info is logged  for side A and side B
		\begin{itemize}
		\item full yield utility of side A.
		\item BSCR = CR(minUtil) where minUtil is the minimum utility that an agent asked for (in one of its bids). 
		\item total CR. which equals to CR(util) where util is the final utility for this agent
...
In this section we assume that you are familiar with programming in Java. In case you are not familiar with Java, please consult the following tutorial: 

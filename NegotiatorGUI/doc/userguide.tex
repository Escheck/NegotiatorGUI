...
\Genius  can run on any machine running Java 8 or higher, including Windows, OSX, Solaris and Linux distributions. Please report any bugs found to \url{negotiation@ii.tudelft.nl}.
...
Note that some agents and scenarios require more memory than allocated by default to Java. This problem can be resolved by using the Xmx and Xms parameters when launching the executable jar, for example \texttt{java -Xmx1536M -Xms1536M -jar genius-XXX.jar}. 
...
We assume that you are familiar with programming in Java. In case you are not familiar with Java, please consult the following tutorial: \url{http://www.oracle.com/technetwork/java/javase/documentation/index.html} 

The recommended way to develop an agent is to create a new project in for example Eclipse or Netbeans. Please refer to Appendix \ref{sec:appendix} where the configuration procedure is explained in detail.
...
\item \raggedright execute the command \verb|javac -cp genius-XXX.jar -source 1.8 -target 1.8 examplepackage/ExampleAgent.java|
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Appendix: Building Agent in Eclipse}
\label{sec:appendix}
...
We assume that the developer has knowledge about Eclipse and Java, and some basic knowledge of the maven-eclipse integration \footnote{it should be all automatic but unfortunately it sometimes needs some manual 'massaging' to keep it working.}.

\subsection{Using Maven}
Select in the workbench File/New Project... and select Maven/new Maven project.
Select "create a simple project" and click next. Enter some group id (eg "ai2019") and some artifactID (eg "group20"). Click finish.

open the pom.xml file and insert this inside the \verb|<project>| part:
...
\begin{enumerate}
\item Run/Debug Configurations...
\item Select "Java Application" and click "new" button.
\item enter \verb|genius.Application| in the "Main Class" field.
\item Click "Debug".
\end{enumerate}

{\em NOTICE}: Strictly, your agent should compile with only this dependency:
\begin{verbatim}
   <dependencies>
      <dependency>
         <groupId>genius</groupId>
...
However, we suggest using the gui dependency for now because of convenience of debugging and running your agent. 
Take care that your agent does not re-use code from old agents, and use only the core APIs.

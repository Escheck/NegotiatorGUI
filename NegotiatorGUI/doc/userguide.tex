...
To summarize: we created three variants of \textit{TheNegotiator}. All variants use a perfect opponent model, and offer the best possible bid for the opponent given the bids under consideration in a round. The variants differ in when they accept, variants with a higher value for $b$ generally accept earlier.
...
\subsubsection{Adding a Bidding Strategy}
Figure~\ref{code:biddingstrategy} visualizes the bidding strategy of \textit{IAMCrazyHaggler}. The first step in creating a component, is creating a new class which extends the \textit{OfferingStrategy}. The \textit{OfferingStrategy} class specifies a simple interface, which forces to implement the \textit{init}, \textit{determineOpeningBid} and \textit{determineNextBid} methods.

Following, we add an empty constructor, and an implementation of the \textit{init} function. In this implementation, we set the \textit{negotiationsession}, \textit{opponent model} and \textit{opponent model strategy}. The \textit{negotiationsession} is an object which stores all information about the current match, including all bids done by the opponent and the agent itself. This entails that no other data structures are required for holding the bidding history. 

Next, we implement the \textit{determineOpeningBid}, which should specify the first bid done by the agent. The final step is to implement \textit{determineNextBid}. In this case, a random bid is generated online with a utility of at least \textit{breakoff}.

An approach often taken by more complex agents, is to first generate all possible bids. While can take a long time, it allows to efficiently look up bids during the negotiation. The \textit{SortedOutcomeSpace} can be used to efficiently generate and access all possible bids during the negotiation. For an example of how to use this class we refer to the class \textit{TimeDependent\_Offering} which implements the bidding strategy of the TimeDependent agent.

Finally, a bidding strategy can use parameters. Using parameters allows to easily create variants of the bidding strategy. For an example on how to use parameters, we refer also refer to the \textit{TimeDependent\_Offering} class.
...
\subsubsection{Adding an Acceptance Condition}
...
Regarding the implementation of the \textit{init}, similar to the bidding strategy we set \textit{negotiation session} and \textit{offering strategy}. In addition, we make a reference to the opponent's bidding history for each access. The framework keeps track of both the agent's and opponent's bidding history. The last bid made by our agent is stored in a special variable \textit{nextBid}. In contrast to the bidding strategy discussed in the previous section, this component expects a parameter $a$ and $b$.
...
In this acceptance condition a bid is only accepted or rejected. In some cases, especially when there are reservation values, it can rational to end the negotiation. This is possible by returning \textit{Actions.Break}.
...
\subsubsection{Adding an Opponent Model}

\subsubsection{Adding an Opponent Model Strategy}

\subsection{Adding Components to the Repository}

\subsection{Multi-Acceptance Criteria (MAC) (TODO)}

With the creation of the BOA framework, there is an enormous space of negotiation strategies.  To make it more manageable to search through this space, the acceptance conditions can be run in parallel given the agent has the same bidding strategy.  This can be done because the agents will accept at a different time during the negotiation, but the bidding behavior will be identical until the acceptance. For example, agent $A$ accepts in the middle of the negotiation, and agent $B$ at the end. The only difference between the complete traces is that the trace of agent $A$ is cut-off in the middle of the negotiation.  This reduces the amount of component combinations from $n^3$ to $n^2$ as the $n$ acceptance conditions reduce to 1. 

\subsubsection{How it works}
In the BOA framework we exploit this property (that the traces are the same between the agents) by running acceptance conditions in parallel, and recording when each acceptance condition accepts. The negotiation is terminated when either the deadline is reached, or when all acceptance conditions have accepted. 



\subsubsection{Using the MAC}
To create a multi-acceptance condition use the example MAC provided with the decoupled agent example.  An place the acceptance conditions in the list.

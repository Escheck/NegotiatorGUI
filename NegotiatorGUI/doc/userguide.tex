...
This section describes the various protocols. We focus on the MultiParty protocols as these have been properly developed. There is also a protocol class for the bilateral negotiation, but this is basically a hard coded Stacked Alternating Offers Protocol and not further developed. 
...
\begin{itemize}
	...
	\item \textbf{Timeout}. The deadline to use. Can be "Round" or "Time". This determines the maximum duration of the session.
\end{itemize}
\end{minipage}
\medskip
...
\begin{itemize}
	...
	\item \textbf{Mediator}. The mediator to use. This option is visible only if the selected protocol needs a mediator. This option enables in the locked position if there are more agents per session than the size of the agents pool.

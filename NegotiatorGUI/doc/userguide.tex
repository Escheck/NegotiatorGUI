...
This protocol uses the following rounds:
\begin{enumerate}
\item Mediator proposes an OfferForVoting
\item The other parties (not the mediator) place a VoteForOfferAcceptance
\item The mediator makes a InformVotingResult 
\end{enumerate}

In this protocol, the parties do not hear the other parties directly. Instead, they only hear the mediator and the mediator hears the bids of all the parties.

% TODO document what the current agreement is with a mediator. RandomFlippingMediator has getLastAcceptedBid()
...
\begin{itemize}
    ...
    \item \textbf{partyRepItems}. This element contains a number of \verb|<item>| elements. Each of these party items contains the following:
      \begin{itemize}
        \item \raggedright \textbf{protocolClassPath}. We do not support other protocols than negotiator.protocol.StackedAlternatingOffersProtocol at this moment.

...
An easy way to create an agent is to create a new project in for example Eclipse or Netbeans. Next, add the negotiationsimulator.jar as a library to the project such that the classes in the project can use the classes of \Genius. Finally, to create an agent create a new class and extend the negotiator.Agent class. Table 1 shows the most important fields and methods of this class. For more information, please refer to the javadoc of \Genius. To implement your agent, you have to override the three methods: ReceiveMessage, init, and chooseAction. Note that \Genius has no problems with loading agents which consist of multiple classes.
...
This section provides an overview of classes which might be useful when implementing an agent. For the documentation of the data structures that are relevant when writing a negotiation agent, please refer to the javadoc that can be found in your download of \Genius. 
...
To compile the agent, you have to put \texttt{YourAgent.java} code in the directory containing the \texttt{negotiator.jar} file, and use the command line: \texttt{javac -cp negosimulator.jar YourAgent.java}. In Eclipse you can more easily copy the class from the ``bin'' folder.

The next step is to load the agent in \Genius. Towards this end, first we should move the agent to the directory of \Genius. Note that the directory in which the agent should be put depends on your project. If for example you have a project with a package ``agents'' in which the agent is located, then the agent should be moved in a folder ``agents'' in the root of your \Genius~installation. An agent may consist of multiple class files. The next step is to add the agent in \Genius. This can be done in two ways:

\begin{itemize}	\item \textbf{Loading the agent using the GUI}. An agent can be easily added by going to the ``Agents'' tab in the ``Components Window'' (see Figure~\ref{fig:addAgent}). Next, pressing right click opens a popup with the option to add a new agent. The final step is to select the main class of your agent.
...
In general, if there is a problem loading your agent the message ``(LOADING FAILED)'' is added after the name of your agent. In this case check that your agent is in the right directory.

...
public class Group18 extends AbstractNegotiationParty {
	...
	public Action BiddingStrategy() {
		...
		return new Offer(PossibleBids[BestBidNo]);
	}
	...
	public Action AcceptStrategy(List<Class<? extends Action>> validActions,
		...
		if (validActions.contains(Accept.class)
				&& (this.getUtility(LastBid) > TimeFactor || this
						.getUtility(LastBid) > this.getUtility(Action
			...
			return new Accept(); // accept if accept is possible and bid is
		...
		if (!(bidListInit)) {
			...
			if (!(sender == null)) {
				...
				if (Action.getBidFromAction(action) != null) {
					partiesBidList[OpponentIDNo].add(Action
							.getBidFromAction(action)); // add the bid to the
														// list add index of ID
				}
			}
		}
		LastBid = Action.getBidFromAction(action); // save bid as last bid
	...
	public void UpdateOpponentModel(AgentID sender, Action action) {
		...
		for (int EvaluatorNo = 0; EvaluatorNo < this.utilitySpace
			...
			switch (OpponentUtilities.get(OpponentIDNo).getDomain()
					.getIssue(EvaluatorNo).getType()) {
			case DISCRETE:
				IssueDiscrete DiscreteIssue = (IssueDiscrete) this.utilitySpace
						.getDomain().getIssue(EvaluatorNo);

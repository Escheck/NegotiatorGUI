...
public class Group20 extends AbstractNegotiationParty {
	...
	private void predictPreferences(Bid agentBid) {
		...
		ArrayList<Issue> opponentIssues = agentBid.getIssues();
		...
		for (Issue lIssue : opponentIssues) {
			...
			try {
				valIndex = lIssueDiscrete
						.getValueIndex((ValueDiscrete) agentBid
								.getValue(issueID)); // find index of value in
														// issue

				/*********** refresh weights *********/
				if (lastOpOffer != null
						&& lastOpOffer.getValue(issueID).equals(
								agentBid.getValue(issueID))) {
					Double[] newWeights = refreshWeight(agentWeights, issueID,
							0.1).clone();
	...
	private Action createBidAction() {
		...
		try {
			...
			System.out.println("Problem with received bid:" + e
					+ ". cancelling bidding");
		}
	...
	private double getMinUtilValue() {
		return MINUTILSCORE
				+ (timeline.getTime() * (MAXUTILSCORE - MINUTILSCORE));
	}
	...
	public double predictUtility(AgentID agentID, Bid bid) throws Exception {
		double opUtility = 0;
		ArrayList<Issue> issueList = bid.getIssues();
		...
		for (Issue lIssue : issueList) {
			...
			int valIndex = lIssueDiscrete.getValueIndex((ValueDiscrete) bid
					.getValue(issueID));
			opUtility += agentWeights[issueID - 1] * values[valIndex]
					/ sumValues;

...
public class Group10 extends AbstractNegotiationParty {
	...
	private void receiveOpponentMessage(AgentID agentID, Action action) {
		...
		if (action instanceof Offer) {
			...
			Bid latestBidOnTable = Action.getBidFromAction(action); // Get the
																	// bid that
																	// he
																	// offered.
			double UndiscountedUtilofLatestBid = getUtility(latestBidOnTable); // Find
																				...
																				// group
																				// 10
			detailsOfLatestBidOnTable = new BidDetails(latestBidOnTable,
					UndiscountedUtilofLatestBid, timeNow); // Combine the offer,
															// utility and time
															// in the
															// BidDetailsObject.
			...
			opponents[opponentNumber]
					.StoreOfferedBid(detailsOfLatestBidOnTable);
		} else if (action instanceof Accept) {
			// The opponent is accepting the bid that is currently on the table
			opponents[opponentNumber]
					.StoreAcceptedBid(detailsOfLatestBidOnTable); // Store the
																	// details
																	// of which
																	// offer he
																	// is
	...
	public Action chooseAction(
		...
		double UndiscountedUtilofPotentialBid = getUtility(potentialBid); // Find
																			...
																			// group
																			// 10
		BidDetails detailsOfPotentialBid = new BidDetails(potentialBid,
				UndiscountedUtilofPotentialBid, timeNow); // Combine the offer,
															// utility and time
															// in the
															// BidDetailsObject.
		...
		// choose what to do.
		Action chosenAction = AcceptanceStrategy.getAction(validActions,
				detailsOfPotentialBid, detailsOfLatestBidOnTable);

...
public class Group14 extends AbstractNegotiationParty {
	...
	private List<BidDetails> allbids, allbidsByNash; // eventually a sorted list
														// of all bids
	private List<BidDetails> firstSetBids, secondSetBids, thirdSetBids,
			fourthSetBids, fifthSetBids, lastSetBids;
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		// oppModel
		if ((myTimeLine.getTime() > 0.4 && count == 0)
				|| (myTimeLine.getTime() > 0.55 && count == 1)
				|| (myTimeLine.getTime() > 0.65 && count == 2)
				|| (myTimeLine.getTime() > 0.75 && count == 3)
				|| (myTimeLine.getTime() > 0.85 && count == 4)
				|| (myTimeLine.getTime() > 0.95 && count == 5)) {
			int phase = count + 1;
			System.out.println("Starting phase " + phase + " at t="
					+ myTimeLine.getTime() + " and round=" + roundNr);
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action instanceof Offer) {
			lastAction = action;
			Bid curBid = Action.getBidFromAction(lastAction);
			if (!(sender == null)) { // if don't have ourselves as a
				...
				if (!oppModelHashMap.containsKey(sender.toString())) {
					oppModelHashMap
							.put(sender.toString(), new OpponentModelling(
									sender.toString(), myUtilSpace.getDomain()
											.getIssues(), curBid));
				...
				BidDetails lastbd = new BidDetails(bidOfLastOffer,
						getUtility(bidOfLastOffer), myTimeLine.getTime());
				oppBidHistory.add(lastbd);
			}

		}

		// System.out.println("The sender of this action is "+sender.toString());
	}
	...
	public Action myOffer() {
		...
		if (myTimeLine.getTime() < 0.4) { // timeslot 1
			ourNextBid = new BidDetails(maxUtilBid(myUtilSpace).getBid(),
		...
		if (getUtility(oppOffer.getBid()) >= getUtility(ourNextBid.getBid())) {
			...
			// System.out.println("Time is:"+myTimeLine.getTime()+" and the offer's utility is :"+getUtility(ourNextBid.getBid()));
			return new Offer(ourNextBid.getBid());
		}
	}
	...
	public void makeLists(List<BidDetails> bidList) {
		...
		// System.out.println("list sizes are 2.1:"+firstSetBids.size()+" - 2.2:"+secondSetBids.size()+" - 2.3:"+thirdSetBids.size()+" - 2.4:"+fourthSetBids.size()+" - 2.5:"+fifthSetBids.size());

		// 1
		if (firstSetBids.isEmpty())
			System.err.println("firstSetBids is empty at t="
					+ myTimeLine.getTime());
		...
		if (secondSetBids.isEmpty())
			System.err.println("secondSetBids is empty at t="
					+ myTimeLine.getTime());
		...
		if (thirdSetBids.isEmpty())
			System.err.println("thirdSetBids is empty at t="
					+ myTimeLine.getTime());
		...
		if (fourthSetBids.isEmpty())
			System.err.println("fourthSetBids is empty at t="
					+ myTimeLine.getTime());
		...
		if (fifthSetBids.isEmpty())
			System.err.println("fifthSetBids is empty at t="
					+ myTimeLine.getTime());
	...
	public List<BidDetails> orderListByNash(List<BidDetails> bidList) {
		...
		for (Map.Entry<String, OpponentModelling> entry : oppModelHashMap
				.entrySet()) {
			nashProd = nashProd
					* entry.getValue().getOppUtil(bidList.get(1).getBid());
		}

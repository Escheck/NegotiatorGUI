...
public class Util {
	...
	public static ArrayList<Value> getValues(Issue issue) {
		...
		case INTEGER:
			...
			break;
		default:
			throw new RuntimeException("Issuetype " + issue.getType()
					+ " not recognized");
		}
	...
	public static double estimatedRoundsLeft(TimeLineInfo time, int currentRound) {
		if (time.getType() == Timeline.Type.Rounds) {
			...
			return (time.getTotalTime() - time.getCurrentTime())
					/ (time.getCurrentTime() / currentRound);
		}
	}

...
public class Group9 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		Bid ourBid = biddingStrat.createBid(previousBidsMap,
		...
		if (!validActions.contains(Accept.class)) {
			...
			if (acceptStrat.determineAcceptance(prevReceivedBid,
					previousBidsMap, ourBid,
					opponentModels.getOpponentUtilities(), this.getTimeLine())) {
				return new Accept();
			} else {
				currentConsideredBid = ourBid;
	...
	private void updateBidhistory(AgentID sender, Bid sendBid) {
		...
		BidDetails tempBidDetails = new BidDetails(sendBid,
				this.getUtility(sendBid), this.getTimeLine().getCurrentTime());

...
public class BayesianAgent extends Agent {
	...
	private double calculateEuclideanDistanceWeghts(double[] pExpectedWeight) {
		...
		try {
			for (Issue lIssue : utilitySpace.getDomain().getIssues()) {
				lDistance = lDistance
						+ sq(fNegotiation.getOpponentWeight(this,
								lIssue.getNumber())
								- pExpectedWeight[i]);
				i++;
			}
		...
		try {
			for (Issue lIssue : utilitySpace.getDomain().getIssues()) {
				lAverageLearnedWeight = lAverageLearnedWeight
						+ pExpectedWeight[i];
				lAverageOriginalWeight = lAverageOriginalWeight
						+ fNegotiation.getOpponentWeight(this,
								lIssue.getNumber());
				i++;
			}
		} catch (Exception e) {
	...
	private double calculateRankingDistanceUtilitySpace(double[] pLearnedUtil,
		...
		lDistance = 2
				* lDistance
				/ (utilitySpace.getDomain().getNumberOfPossibleBids() * (utilitySpace
						.getDomain().getNumberOfPossibleBids()));
		return lDistance;
	}

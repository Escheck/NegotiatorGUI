...
public class Group8 extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class> validActions) {
		...
		this.acceptanceValue = 0.99 + 1 - Math.pow(
				Math.pow(1.9, 1 / this.totalRounds),
				Math.pow(INITIALACCEPTVALUE, (this.totalRounds)
						/ this.roundCounter)
						* (this.roundCounter - 1));

		// first bid - offer a bid with high utility value for self
		if (!validActions.contains(Accept.class)) {
			Bid firstBid = this.generateHigherUtilityBid(this.acceptanceValue)
					.get(0);
		...
		// choosing action after a bid has been made
			...
			try {
				utilityOfMostRecentBid = this.utilitySpace
						.getUtility(this.mostRecentBid);
			} catch (Exception e) {
	...
	private OpponentModel getNextAgentModel() {
		...
		for (OpponentModel opponent : this.opponents) {
			if (opponent.agent.getPartyId().equals(
				...
				int indexOfNextAgent = (indexOfPreviousBidder + 1)
						% this.opponents.size();

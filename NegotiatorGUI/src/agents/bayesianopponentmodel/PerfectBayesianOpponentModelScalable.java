...
//					if(!lBid.getValue(j).equals(lPreviousBid.getValue(j))) {
						...
						lN += fWeightHyps.get(j).get(i).getProbability()*conditionalDistribution(lUtility, opponentUtility);
/*					} else {
...
//					if(!lBid.getValue(j).equals(lPreviousBid.getValue(j))) {				
						lUtility = fWeightHyps.get(j).get(i).getWeight()*getExpectedEvaluationValue(lBid, j) + getPartialUtility(lBid, j);
						lWeightHyps.get(j).get(i).setProbability(fWeightHyps.get(j).get(i).getProbability()*conditionalDistribution(lUtility, opponentUtility)/lN);
/*					} else {

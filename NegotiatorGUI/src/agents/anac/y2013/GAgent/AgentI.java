...
public class AgentI extends Agent
{
	...
	public double changeThreshold(double time){
		return (1-lowerLimit+tim) / ( 1 + Math.exp(sigmoidGain * time)) + lowerLimit;
	}	
	...
	public Action chooseAction(){
		...
		{ 
			if(actionOfPartner==null) {
				action = new Offer(getAgentID(),utilitySpace.getMaxUtilityBid());
			}
			...
			if(actionOfPartner instanceof Offer){
				...
				double offerundiscoutFromOpponent = utilitySpace.getUtility(partnerBid);
	...
	private boolean isAcceptable(double offeredUtilFromOpponent, double myOfferedUtil, double time,Bid parBid) throws Exception	{

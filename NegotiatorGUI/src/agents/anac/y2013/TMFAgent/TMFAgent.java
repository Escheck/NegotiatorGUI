...
public class TMFAgent extends Agent {
	...
	public Action chooseAction() {
			...
			try {
				if (utilitySpace.getUtility(BestOpponentBid) <= utilitySpace
					...
					if (utilitySpace.getUtility(lastBid) > utilitySpace
						...
						return new Accept();
				} else if (utilitySpace.getUtility(BestOpponentBid) > utilitySpace
						.getReservationValueUndiscounted())
					return new Offer(BestOpponentBid);
			} catch (Exception e) {
				e.printStackTrace();
			}
			return new EndNegotiation(); // if the opponent's best offer is
		...
		try {
			if (lastBid != null
					&& utilitySpace.getUtility(lastBid) > cb.utility) // if the
																		// opponent
																		// offered
																		// better
																		// offer
				...
				a = new Accept();
					...
					.getReservationValueUndiscounted()) // if our offer is worse

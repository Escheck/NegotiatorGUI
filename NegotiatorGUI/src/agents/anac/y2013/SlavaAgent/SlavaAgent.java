...
public class SlavaAgent extends Agent
{
	...
	// Stores all the best bids for us that their utility is more than UTILITY_THRESHOLD
	...
	// If the utility of the opponent is higher than these value, accept immediately.
	private final double	GOOD_ENOUGHT_UTILITY = 0.9;

	/**
	 * Method calculates the bid that has a maximum value for us from the domain.
	 * @return
	 * @throws Exception
	 */
	public Bid GetMaxBid() throws Exception {
		
		Bid max = utilitySpace.getDomain().getRandomBid();
		Bid tempBidding = utilitySpace.getDomain().getRandomBid();
	 ...
	 * bestOffer - the best bid for us that this opponent offered us from previous negotiations.
	 * bestBidsMap - stores all the best bids (those who has utility higher than UTILITY_THRESHOLD).
	 */
	public void init()
	{
		try {
	 ...
	 * If it's an offer, it calculates its utility.
	 * If the utility is higher than all previous bids we have seen so far from this opponent, we are update the variables's value.
	 * 
	 * If it's the first time: If we played against this agent, we have a value on the variable "bestOffer" and we initialize the parameter "maxBidRevieved"
	 * to it (of course, only if its lower than it). Otherwise, we initialize "bestOffer" to be that bid.
	 */
		...
		if (actionOfPartner instanceof Offer) {
			...
			try {
				...
				if (this.maxBidRecieved == null) { // The first offer.
					...
					// Initialize the maximum bid received from the opponent (only if its lower than previous negotiations).
				...
				} else if (utility > this.utilitySpace.getUtility(this.maxBidRecieved)) { // Not the first offer.
					this.maxBidRecieved = bid;
				}
			} catch (Exception e) {
				e.printStackTrace();
	 ...
	 * 
	 * If it's an offer that has utility more than GOOD_ENOUGHT_UTILITY, accept immediately.
	 * 
	 * Otherwise, split to 2: Exploration and exploitation.
	 * In exploration part, update the maximal bid for us (if found) and with probability of 0.5, offer it to the opponent and with probability of
	 * 0.5, offer to him a bid that is good enough for us randomly (one that has utility more than UTILITY_THRESHOLD).
	 * In exploitation part, offer always the best bid for us. Accept if the opponent offers a bid that has utility more than MIN_UTILITY_ACCEPT and
	 * has value higher or equals to all his previous offers. Otherwise, offer the best bid for us.
	 * 
	...
	public Action chooseAction()
	{
		...
		try 
		{
			...
			if(actionOfPartner instanceof Offer)
			{
				...
				// if the time is < EXPLORATION_RATE always offer a max bid until so
				if (this.timeline.getTime() <= this.EXPLORATION_RATE)
				{
					...
					if (rand.nextDouble() <= 0.5) {
						action = new Offer(this.maxBid);
					// In a probability of 0.5, offer one of the other best bids.
					} else {
						Bid nextBid = this.bidsAsArray.get((rand.nextInt(this.bidsAsArray.size())));
				...
				else // Exploitation
				{
					...
					try {
						...
						// If the user offered a bid that is more than the maximum so far, accept.
						if (utility >= this.MIN_UTILITY_ACCEPT && utility >= this.utilitySpace.getUtility(this.maxBidRecieved)) {
							action = new Accept(this.getAgentID());
	...
	/**
	 * Method calculates the next bid. It takes the maximum between the global maximum and a new randomized bid within a fixed number of iterations.
	 * @return
	 */
	private Bid calculateNextBid() {
		try {
			...
			// Return the maximum bid between the global maximum and the new randomized bid
	...
	/**
	 * Method iterates fix number of times and returns the maximum bid within all generated random bids.
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Method stores and returns all the best bids for us that are more than a given threshold.
	 * @return
	 */
	private Map<Bid,Double> getBestBidsForUs() {
		...
		for (int i = 0; i < this.MAX_ITERATIONS; i++) {
			...
			try {
				...
				// If the value of the random bid is more than the utility threshold, save it.

	...
	private void getDomainParams() {
		...
		while (issue.hasNext()) { // every issue
			...
			if (e.getType() == EVALUATORTYPE.DISCRETE) {
				...
				tempEU = ((double) (((EvaluatorInteger) e).getUpperBound() + ((EvaluatorInteger) e)
						.getLowerBound())) / 2;
				tempStdevU = Math
				...
				tempEU = (((EvaluatorReal) e).getUpperBound() + ((EvaluatorReal) e)
						.getLowerBound()) / 2;
				tempStdevU = Math
		...
		try {
			...
			if (a == null) {
				...
				if (a instanceof Offer
						&& this.utilitySpace
								.getReservationValueWithDiscount(time) >= utilitySpace
								.getUtilityWithDiscount(((Offer) a).getBid(),
										time)) {

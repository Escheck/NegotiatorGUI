...
public class MetaAgent extends Agent {
	...
	public void init(){
		...
		if (s != null){
			...
			myAgent.internalInit(this.sessionNr, this.sessionsTotal, this.startTime, this.totalTime, this.timeline, this.utilitySpace, this.parametervalues);
	...
	private void getDomainParams(){
		...
		//params = new double[14]; //(Intercept),firstBBUA,slopeBBUA,absSlopeBBUA,avgUtilBBUA,DiscountFactor,DomainSize,numOfIssues,MinSize,MaxSize,AvgUtil,AvgUtilStdev,WeightStdev,Begins
		features = new HashMap<String,Double>();
				
				
		
		int min = Integer.MAX_VALUE, max = -1, size = 1, sumSize = 0;//, relCount = 0;
		
		//params 5+6+7 - Expected Utility of role, Standard deviation of Utility of role, Standard deviation of weights of role
		double EU = 0, stdevU = 0, stdevW = 0, sW = 0, ssW = 0, countW = 0, relevantEU = 0; 
		Iterator<Entry<Objective, Evaluator>> issue = utilitySpace.getEvaluators().iterator();
		...
		while (issue.hasNext()){ //every issue
			...
			if (e.getType() == EVALUATORTYPE.DISCRETE){
				...
				tempEU = ((double)(((EvaluatorInteger)e).getUpperBound() + ((EvaluatorInteger)e).getLowerBound()))/2;
				tempStdevU = Math.sqrt((Math.pow(((EvaluatorInteger)e).getUpperBound() - tempEU, 2)+Math.pow(((EvaluatorInteger)e).getLowerBound() - tempEU, 2))/2);
			}
			else if (e.getType() == EVALUATORTYPE.REAL){
				tempEU = (((EvaluatorReal)e).getUpperBound() + ((EvaluatorReal)e).getLowerBound())/2;
				tempStdevU = Math.sqrt((Math.pow(((EvaluatorReal)e).getUpperBound() - tempEU, 2)+Math.pow(((EvaluatorReal)e).getLowerBound() - tempEU, 2))/2);
			}
			else {
				tempEU = 0.5;
				tempStdevU = 0;
		...
		double avgSize = ((double)sumSize) / ((double)utilitySpace.getEvaluators().size()); 
		...
		double relsumUtility = 0, relcountUtility = 0, relstdevUtility = 0, stdevUtil = 0, countbids = 0, sumbids = 0, relevantStdevU = 0;
		...
		if (relcountUtility > 0){
		    ...
		    relevantStdevU = Math.sqrt((relstdevUtility/relcountUtility) - Math.pow(relevantEU, 2));
		}
		if (countbids > 0){
		...
		features.put("ReservationValue",this.utilitySpace.getReservationValue()); //RV
		features.put("DiscountFactor", this.utilitySpace.getDiscountFactor() == 0 ? 1 : this.utilitySpace.getDiscountFactor()); //DiscountFactor
		...
		try{
			if (!agentInit)
			{
				...
				else //second bid, or first bid as responder (and not initiator)
				{
					...
					getDomainParams();
					UpdateAllAgents();
					manager.SetAvgUtil(features.get("RelevantEU")*Math.pow(utilitySpace.getDiscountFactor(), 0.5), features.get("RelevantStdevU"));
					myAgent = manager.SelectBestAgent();		
					myAgent.internalInit(this.sessionNr, this.sessionsTotal, this.startTime, this.totalTime, this.timeline, this.utilitySpace, this.parametervalues);
					myAgent.setName(this.getName());
					myAgent.setAgentID(this.getAgentID());
	...
	private void UpdateAllAgents() {
		...
		for (String agentName : agents) {
			double predicted = Parser.getMean(manager.GetAgentData(agentName), features);

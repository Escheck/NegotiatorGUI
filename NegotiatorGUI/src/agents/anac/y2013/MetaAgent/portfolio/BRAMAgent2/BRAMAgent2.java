        ...
        if(actionOfPartner == null){
        	...
        	if (offeredUtility >= threshold)//If the utility of the bid that we received from the opponent
        		  ...
        		  action = new Accept(this.getAgentID());
	        		...
	        		//System.out.println("Original BidOffer: " + bidToOffer);

		       }
					
						if(/*( bidToOffer != null ) && */(offeredUtility >= this.utilitySpace.getUtilityWithDiscount(bidToOffer, timeline))){
							action = new Accept(this.getAgentID());
						}
						else if (/*(bidToOffer == null )||*/
						((offeredUtility < this.utilitySpace.getReservationValueWithDiscount(timeline)) && (timeline.getTime() > 177.0/180.0)) && 
						(this.utilitySpace.getReservationValueWithDiscount(timeline) > this.utilitySpace.getUtilityWithDiscount(bidToOffer, timeline))){

 ...
 public abstract class BOAagent extends Agent
 {
   ...
   public Action chooseAction()
   {
     ...
     else
     {
       ...
       {
           System.out.println("Exception occured while determining the next bid in the chooseAction function!");
       }
     ...
     if (decision.equals(Actions.Reject)) {
       try {
           this.negotiationSession.getOwnBidHistory().add(new BidDetails(bid.getBid(),this.negotiationSession.getUtilitySpace().getUtility(bid.getBid()),this.negotiationSession.getTime()));
       } catch (Exception ex) {
       }

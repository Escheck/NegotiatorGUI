 ...
 public class OutcomeSpace
 {
   ...
   public void generateAllBids(AdditiveUtilitySpace utilSpace)
   {
     ...
     while (iter.hasNext()) {
       Bid bid = iter.next();
       try {
         BidDetails BidDetails = new BidDetails(bid, utilSpace.getUtility(bid), -1.0D);
   ...
   public int getIndexOfBidNearUtility(double utility)
   {
     ...
     for (int i = 0; i < this.allBids.size(); i++) {
       if (Math.abs(((BidDetails)this.allBids.get(i)).getMyUndiscountedUtil() - utility) < closesDistance) {
         closesDistance = Math.abs(((BidDetails)this.allBids.get(i)).getMyUndiscountedUtil() - utility);
         best = i;
       }
     }
     return best;

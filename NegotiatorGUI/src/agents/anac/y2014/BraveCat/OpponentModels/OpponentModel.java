...
public abstract class OpponentModel {
	...
	public void init(NegotiationSession negotiationSession,
		...
		this.opponentUtilitySpace = (AbstractUtilitySpace) negotiationSession
	...
	public double getWeight(Issue issue) {
		if (opponentUtilitySpace instanceof AdditiveUtilitySpace) {
			return ((AdditiveUtilitySpace) this.opponentUtilitySpace)
					.getWeight(issue.getNumber());
		}
		return 0;
	}

	public double[] getIssueWeights() {
		double[] estimatedIssueWeights = new double[this.negotiationSession
				.getUtilitySpace().getDomain().getIssues().size()];
		int i = 0;
		for (Issue issue : this.negotiationSession.getUtilitySpace()
				.getDomain().getIssues()) {

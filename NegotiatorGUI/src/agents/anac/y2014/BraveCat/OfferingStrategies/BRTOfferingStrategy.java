 ...
 public class BRTOfferingStrategy extends OfferingStrategy
 {
   ...
   private double Pmax;
   private double Pmin;
   private double e;//This value should be adjusted by the estimated concession value of the opponent.

   public BRTOfferingStrategy(NegotiationSession negoSession, OpponentModel model, OMStrategy oms, double e, double k)
   {
       ...
       if(this.negotiationSession.getUtilitySpace().getType() == UTILITYSPACETYPE.NONLINEAR)
       ...
       try
       {
           ...
           this.Pmin = this.negotiationSession.getUtilitySpace().getReservationValueUndiscounted();
           System.out.println("Pmin: " + this.Pmin);
           this.schedular = new Schedular(negotiationSession);
       }
       catch (Exception ex)
       {
           System.out.println("Exception occured when determining Pmax and Pmin!");
       }
    }
   @Override
   ...
   public BidDetails determineOpeningBid()
   {
       ...
       try {
           ...
           System.out.println("Opening Bid Utility is: " + this.negotiationSession.getUtilitySpace().getUtility(temp.getBid()));
       } catch (Exception ex)
       {
       }
       return temp;
   ...
   public BidDetails determineNextBid()
   {
     ...
     this.Pmin = this.negotiationSession.getUtilitySpace().getReservationValueWithDiscount(time);
     
     //If this is the last bid, the agent chooses to send the best bid received from the opponent, so as to avoid a break off,
     //hoping that this bid is still acceptable for the opponent, and it will result in an agreement.
     if(this.schedular.FinalRounds() || this.schedular.LastRound())
         return negotiationSession.getOpponentBidHistory().getBestDiscountedBidDetails(negotiationSession.getUtilitySpace());
     double targetUtility = TargetUtilityGenerator(time, this.Pmax, this.Pmin, this.e, this.k);
     ...
     {
         System.out.println("Exception occured when calculating behavioral target utility!");
     }
     //System.out.println(targetUtility);
     try {
         if(targetUtility > 1)
     ...
     {
         System.out.println("Exception occured when bringing the target utility into the range!");
     }
     ...
     else
     {
         try
         {
             this.nextBid = this.omStrategy.getBid(this.bidGenerator.NBidsNearUtility(targetUtility, 10));
         } catch (Exception ex)
         {
         }
     }
   ...
   public double BehaviouralTargetUtility(double targetUtility) throws Exception
   {
       if(LastBestBid == null)
       {
           //System.out.println(this.negotiationSession.getOpponentBidHistory().size());
           LastBestBid = this.negotiationSession.getOpponentBidHistory().getLastBidDetails();
           return targetUtility;
       }
       else
       {
           if(this.negotiationSession.getUtilitySpace().getUtility(LastBestBid.getBid()) < this.negotiationSession.getUtilitySpace().getUtility(this.negotiationSession.getOpponentBidHistory().getLastBid()))
           {
               double NewUtility = targetUtility - (this.negotiationSession.getUtilitySpace().getUtility(this.negotiationSession.getOpponentBidHistory().getLastBid()) - this.negotiationSession.getUtilitySpace().getUtility(LastBestBid.getBid()));
               LastBestBid = this.negotiationSession.getOpponentBidHistory().getLastBidDetails();
   ...
   public double TargetUtilityRandomizer(double targetUtility)
   {
       ...
       return targetUtility - ((double) ((1 - targetUtility) * z * (1 - this.negotiationSession.getTime())) / 2.58);
   }
   public double TargetUtilityGenerator(double t, double Pmax, double Pmin, double e, double k)
   {
   ...
   {
       return 1 - ((double) this.negotiationSession.getOpponentBidHistory().numberOfUniqueBidsInBidHistory / this.negotiationSession.getOpponentBidHistory().numberOfTotalBidsInBidHistory);
   }
   @Override

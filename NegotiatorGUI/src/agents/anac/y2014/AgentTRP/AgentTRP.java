...
public class AgentTRP extends Agent {
	...
	/** Ã¨â€¡ÂªÃ¥Ë†â€ Ã£ï¿½Â«Ã£ï¿½Â¨Ã£ï¿½Â£Ã£ï¿½Â¦Ã¤Â¸â‚¬Ã§â€¢ÂªÃ¨â€°Â¯Ã£ï¿½â€žÃ§â€ºÂ¸Ã¦â€°â€¹Ã£ï¿½Å’Ã¦ï¿½ï¿½Ã¦Â¡Ë†Ã£ï¿½â€”Ã£ï¿½Å¸Bid */
	private Bid myBestBidByOpponent;
	/** Ã¨â€¡ÂªÃ¥Ë†â€ Ã£ï¿½Â«Ã£ï¿½Â¨Ã£ï¿½Â£Ã£ï¿½Â¦Ã¤Â¸â‚¬Ã§â€¢ÂªÃ¨â€°Â¯Ã£ï¿½â€žBid */
	private Bid myBestBid;
	/** Ã£ï¿½Å Ã¤Âºâ€™Ã£ï¿½â€žÃ£ï¿½Â«Ã£ï¿½Â¨Ã£ï¿½Â£Ã£ï¿½Â¦Ã¨â€°Â¯Ã£ï¿½â€¢Ã£ï¿½ï¿½Ã£ï¿½â€ Ã£ï¿½ÂªÃ¦â€°â€¹ */
	private Bid bestBid;
	...
	/**
	 * 0 Ã©ï¿½Å¾Ã¥Â¦Â¥Ã¥ï¿½â€� + Ã¦Å½Â¢Ã§Â´Â¢Ã¦Â°â€”Ã¥â€˜Â³
	 * 1 Ã§â€ºÂ¸Ã¦â€°â€¹Ã£ï¿½Å’Ã¥Â¼Â·Ã¦Â°â€”Ã£ï¿½Â¨Ã¥Ë†Â¤Ã¦â€“Â­ -> Ã§â€ºÂ¸Ã¦â€°â€¹Ã£ï¿½Â®Ã£Æ’â„¢Ã£â€šÂ¹Ã£Æ’Ë†Ã£Æ’â€œÃ£Æ’Æ’Ã£Æ’â€°Ã¦Å â€¢Ã£ï¿½â€™Ã£â€šâ€¹
	 * 2 Ã¦â„¢Â®Ã©â‚¬Å¡ -> SA
	 */
	private int mode;
	...
	public void init() {
		 ...
		 * Ã©ï¿½Â¸Ã¦Å Å¾Ã£ï¿½Â®Ã¦â€“Â¹Ã¦Â³â€¢
		 * 1.Ã¥Â¯Â¾Ã¦Ë†Â¦Ã¥â€ºÅ¾Ã¦â€¢Â°
		...
		// Ã¥Ë†ï¿½Ã¦Ë†Â¦
		if(sessionNr < 2) {
			//minimumUtilities = new double[]{0.9,0.9,0.9,0.9,0.9,0.8,0.8,0.8,0.8,0.8};
			mode = 0; // 0
			return;
		} else if(rate > 0.5) {
			//minimumUtilities = new double[]{0.9,0.9,0.9,0.9,0.9,0.8,0.8,0.8,0.75,0.75};
			mode = 3;
			return;
		} else {
			//minimumUtilities = new double[]{0.9,0.9,0.9,0.9,0.9,0.8,0.8,0.8,0.75,0.75};
	...
	public void endSession(NegotiationResult result) {
		 ...
		 * Ã¥ï¿½Ë†Ã¦â€žï¿½Ã£Æ’â€œÃ£Æ’Æ’Ã£Æ’â€°Ã¯Â¼Å’Ã¥ï¿½Ë†Ã¦â€žï¿½Ã¦â„¢â€šÃ©â€“â€œ, Ã¥ï¿½Ë†Ã¦â€žï¿½Ã£ï¿½â€”Ã£ï¿½Å¸Ã£ï¿½â€¹, Ã¦Â±ÂºÃ¨Â£â€šÃ¥â€ºÅ¾Ã¦â€¢Â°Ã¯Â¼Å’Ã¦Â¤Å“Ã§Â´Â¢Ã¯Â¼Å’Ã£Æ’Â©Ã£â€šÂ¦Ã£Æ’Â³Ã£Æ’â€°Ã¦â€¢Â°
		 */
		...
		TRPSessionData mySessionData = new TRPSessionData(lastBid, lastTime, isAgree, myBestBid, myBestBidByOpponent, round, reset);
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		opponentBid = Action.getBidFromAction(opponentAction);
		if (myBestBidByOpponent == null || opponentBid == null) {  // bestBidÃ£ï¿½Å’Ã£ï¿½ÂªÃ£ï¿½â€žÃ£ï¿½Â¨Ã£ï¿½ï¿½  
			myBestBidByOpponent = utilitySpace.getDomain().getRandomBid();	
		} else { 
			if (getUtility(opponentBid) >= getUtility(myBestBidByOpponent)) {  // bestBid Ã£â€šË†Ã£â€šÅ Ã£â€šË†Ã£ï¿½â€žÃ£â€šâ€šÃ£ï¿½Â®Ã£â€šâ€™Ã¦ï¿½ï¿½Ã¦Â¡Ë†Ã£ï¿½â€”Ã£ï¿½Å¸Ã£ï¿½Â¨Ã£ï¿½ï¿½
	...
	public Action chooseAction() {
		...
		if ((time < 0.15 || time >= 0.85) && mySessionData != null) {
			// Ã§â€ºÂ¸Ã¦â€°â€¹Ã£ï¿½Å’Ã©Æ’Â½Ã¥ï¿½Ë†Ã£ï¿½Â®Ã£ï¿½â€žÃ£ï¿½â€žÃ£Æ’â€œÃ£Æ’Æ’Ã£Æ’Ë†Ã£â€šâ€™Ã¦ï¿½ï¿½Ã¦Â¡Ë†Ã£ï¿½â€”Ã£ï¿½Å¸Ã¥Â Â´Ã¥ï¿½Ë† 0.85Ã¤Â»Â¥Ã¤Â¸Å Ã¥ï¿½â€“Ã£â€šÅ’Ã£â€šâ€¹Ã£ï¿½ÂªÃ£â€šâ€°
			if(mySessionData.isAgree && getUtility(mySessionData.lastBid) > 0.85) {
				if(mySessionData.isAgree && round < 100) {
					return new Offer(mySessionData.lastBid);
				}
			} 
		} 
...
//		}
	...
	private Action mode1(double time) {
		if(time < 0.95) {
			...
			return new Offer(myBestBidByOpponent);
		}
	}
	...
	// Ã¥Â¼Â·SA(Ã¦â„¢Â®Ã©â‚¬Å¡Ã£ï¿½Â®SA Ã¨â€¡ÂªÃ¥Ë†â€ Ã£ï¿½Â«Ã£ï¿½Â¨Ã£ï¿½Â£Ã£ï¿½Â¦Ã©Â«ËœÃ£ï¿½â€žÃ£â€šâ€šÃ£ï¿½Â®Ã£â€šâ€™Ã©ï¿½Â¸Ã¦Å Å¾)
	private Action getBidA() {
		double t0 = 10000;
		double t1 = 0.001;
		Bid baseBid;
		if (myBestBid == null || rand.nextInt(10) != 0) {
			baseBid = utilitySpace.getDomain().getRandomBid();
			Bid bid = getSA(baseBid, t0, t1);
			beforeOffer(bid);
			return new Offer(bid);
		} else {
			return new Offer(myBestBid);
		}
	...
	private Bid getSA(Bid bid, double t0, double t1) {
		...
		try {
			do {
				...
				newBid.setValue(issueId, valueInteger);
			...
			// myBestBidÃ£ï¿½Â®Ã¨Â¨Â­Ã¥Â®Å¡ myBestBidÃ£ï¿½Å’Ã£ï¿½ÂªÃ£ï¿½â€žÃ£ï¿½Â¨Ã£ï¿½ï¿½
			if (myBestBid == null) {
	...
	private Action getBidB() throws Exception {
	 ...
	 private Bid nearSearch(Bid baseBid) throws Exception {
		 ...
		 ArrayList<Issue> issues = utilitySpace.getDomain().getIssues(); // Ã¥â€¦Â¨issueÃ£ï¿½Â®Ã¥ï¿½â€“Ã¥Â¾â€”
		 ...
		 for (int i = 0; i < numberIndexes; i++) {
			 ...
			 int issueNumber = lIssueInteger.getNumber(); // issueÃ£ï¿½Â®index 0 Ã£â‚¬Å“ 9
			 ...
			 nextBid.setValue(issueNumber, new ValueInteger(optionIndex)); // Ã§ï¿½Â¾Ã¥Å“Â¨Ã£ï¿½Â®BidÃ£ï¿½â€¹Ã£â€šâ€°IssueÃ£ï¿½Â®Ã¥â‚¬Â¤Ã£â€šâ€™Ã¥â€¦Â¥Ã£â€šÅ’Ã¦â€ºÂ¿Ã£ï¿½Ë†Ã£â€šâ€¹
		 }		 
		 return nextBid;
	 }
	 ...
	 private int selectIndex(IssueInteger issue, Bid baseBid) {
		 ...
		 for(int i = issueIndexMin; i < issueIndexMax; i++) {
			 ...
			 tempBid.setValue(issueNumber, new ValueInteger(i));
			 if(getUtility(sBid) <= getUtility(tempBid)) {
				 candidate = i;
			 }
			 sBid.setValue(issueNumber, new ValueInteger(candidate));
		 }
		 return candidate;
	 }
	 ...
	 private Action getBidD() throws Exception {
		 Bid baseBid = utilitySpace.getDomain().getRandomBid();
		 System.out.println("noisy!!");
		 do {
			 baseBid = nearSearchWithNoise(baseBid);
		 }while(getUtility(baseBid) > 0.55);
		 return new Offer(nearSearchWithNoise(baseBid));
	 }
...
class TRPSessionData implements Serializable {
	...
	public TRPSessionData(Bid lastBid, double lastTime, boolean isAgree, Bid myBestBid, Bid myBestBidByOpponent, int round, boolean reset) {

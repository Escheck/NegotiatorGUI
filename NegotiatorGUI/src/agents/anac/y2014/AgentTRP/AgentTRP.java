...
public class AgentTRP extends Agent {
	...
	public void endSession(NegotiationResult result) {
		...
		if (mySessionData == null) {
			...
			if (getUtility(mySessionData.lastBid) < getUtility(result
					.getLastBid())) {
				lastBid = result.getLastBid();
			} else {
		...
		try {
			do {
				...
				IssueInteger issueInteger = (IssueInteger) bid.getIssues().get(
						selectedIssueId); // Ã£â‚¬â‚¬Ã©ï¿½Â¸Ã¦Å Å¾Ã£ï¿½â€¢Ã£â€šÅ’Ã£ï¿½Å¸issue
	...
	private Bid nearSearch(Bid baseBid) throws Exception {
		...
		for (int i = 0; i < numberIndexes; i++) {
			...
			nextBid = nextBid.putValue(issueNumber, new ValueInteger(
					optionIndex)); // Ã§ï¿½Â¾Ã¥Å“Â¨Ã£ï¿½Â®BidÃ£ï¿½â€¹Ã£â€šâ€°IssueÃ£ï¿½Â®Ã¥â‚¬Â¤Ã£â€šâ€™Ã¥â€¦Â¥Ã£â€šÅ’Ã¦â€ºÂ¿Ã£ï¿½Ë†Ã£â€šâ€¹
		}
		return nextBid;
	}

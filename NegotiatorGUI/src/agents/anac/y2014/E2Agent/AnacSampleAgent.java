...
public class AnacSampleAgent extends Agent {
    ...
    // private ArrayList<BidStorage> partnerBidHistory = null; // ç›¸æ‰‹ã�®Bidå±¥æ­´
    ...
    public void init() {
      ...
      //System.out.println(param);
        ...
        agentKStorategy = new AgentKStorategy(randomnr, new AgentKStorategyComponentImpl());
     ...
     * æœ€é�©ã�ªBidã‚’æŽ¢ç´¢
     */
    private ArrayList<BidStorage> searchOptimumBids(int sampleNr) throws Exception  {
        ...
        for(int index=0; index < sampleNr; ++index) {
            ...
            BidStorage optimumBid = simulatedAnealing.run(startBid, 1.0, SA_K_MAX);
            // Bidå€™è£œè¿½åŠ 
            bids.add(optimumBid);
        }
        return bids;
    ...
    public Action chooseAction() {
        ...
        try {
            ...
            if(actionOfPartner instanceof Offer) { // ç›¸æ‰‹ã�ŒOfferã�—ã�¦ã��ã�Ÿå ´å�ˆ
                ...
                BidStorage partnerBidStorage =
                    new BidStorage(partnerBid, utilitySpace.getUtility(partnerBid), time);
                partnerBidHistory.addBidStorage(partnerBidStorage);
                SummaryStatistics stat = partnerBidHistory.getSummaryStatistics();
                ...
                if(discountFactor > 0.9) {
                	targetUtility = agentKStorategy.fintarget(
                    		time, stat.getAve(), stat.getVar(), tau);
                } else {
                ...
                if(myBidStorage == null) {
                    ...
                    myBidStorage = searchBidWithApproximateUtlity(partnerBid, targetUtility);
                    if(myBidStorage==null) {
            	...
            	if(//worstUtility <= partnerBidStorage.getUtility() ||
            			isAcceptable(time, partnerBidStorage.getUtility(), stat.getAve(), stat.getVar(), tau)) {
                	...
                	if (targetUtility < bestBid.getUtility()) {
	                    ...
	                    //System.out.printf("bestUtility: %f.5 ", bestBid.getUtility());
                	} else {
	                    action = generateOffer(myBidStorage.getBid());
                	}
            	
                	// è‡ªèº«ã�®æœ€ã‚‚æ‚ªã�„Bidã‚’è¨˜æ†¶
	                if((targetUtility - myUtility) < 0.05 && myUtility < worstUtility) {
	                ...
	                /*
	                if(sessionNr>0 && time > 0.999 &&
	                		(reservationValue * discountFactor) * 1.5 < partnerBidStorage.getUtility()) {	
    ...
    private BidStorage search2(Bid bid, double targetUtil) throws Exception {
    	...
    	for(int i=0; i<issues.size(); ++i) {
    		...
    		for(int j=issueIndexMin; j<=issueIndexMax; ++j) {
                ...
                neighbourBid.setValue(i+1, new ValueInteger(j));
     ...
     */
    private boolean isAcceptable(double time, double u,
            double myu, double var, double tau) throws Exception {
    ...
    private double fintarget2(double time, SummaryStatistics stat, Parameters param) {
    	double ret = 1;
    	double target = agentKStorategy.fintarget(time, stat.getAve(), stat.getVar(), 1);
    	...
    	if(target > param.utility) { 
    		...
    		if(param.time - time > 0) {
    			co *= (1 - Math.pow((param.time - time) / param.time, param.alpha));
    		}
    	} 	
    ...
    private double fintarget(double time, SummaryStatistics stat, Parameters param) {
    	double ret = 1;
    	double param_t = param.time * 0.85;
    	double e =  agentKStorategy.emax(stat.getAve(), stat.getVar());
    	
    	if(time < param_t) {
    		ret = 1 - ((1-param.utility) * Math.pow(time / param_t, 1/param.alpha));
    	} else {
    		/*
    		double t = (time - param_t) / (1 - param_t);
    		double ave = stat.getAve() / param.utility;
    		double var = stat.getVar() / Math.pow(param.utility,2);
    		double target = agentKStorategy.fintarget(t, ave, var, 1);
    		ret = target / param.utility;
    		*/
    ...
    private BidStorage searchBidWithApproximateUtlity(Bid startBid, double u) throws Exception {
        // Bid startBid = utilitySpace.getDomain().getRandomBid();
        // ç„¼ã��ã�ªã�¾ã�—æ³•ã�«ã‚ˆã‚‹æœ€é�©è§£æŽ¢ç´¢
    ...
    public void endSession(NegotiationResult result) {
    	...
    	try {
			sessionData.save(sessionNr, partnerBidHistory, result,
					utilitySpace.getUtility(result.getLastBid()), timeline.getTime());
		} catch (Exception e) {
    ...
    public void ReceiveMessage(Action opponentAction) { actionOfPartner = opponentAction; }
    private Offer generateOffer(Bid bid) { return new Offer(getAgentID(), bid); }
    public String getVersion() { return "5.0"; }
    public String getName() { return "Anac Sample Agent"; }

    class AgentKStorategyComponentImpl implements IAgentKStorategyComponent {
        public double g(double t) { return param.g; }
    }
}

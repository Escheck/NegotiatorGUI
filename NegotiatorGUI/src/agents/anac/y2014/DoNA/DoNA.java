...
public class DoNA extends Agent {
	...
	public void init() {
		...
		midReservation = (utilitySpace.getReservationValue() < 0.8 && utilitySpace
				.getReservationValue() > 0.2);// (minUtility+0.75*UtilityRange)
												// &&
												// utilitySpace.getReservationValue()
												// >=
												// (minUtility+0.375*UtilityRange));
		...
		midDiscount = (utilitySpace.getDiscountFactor() < 0.8 && utilitySpace
				.getDiscountFactor() > 0.2);
	...
	public void ReceiveMessage(Action opponentAction) {
		...
		if (opponentAction instanceof Offer) {
			...
			if (currentTimeIndex == numOfSamplesForAvgTime) {
				currentTimeIndex = 0;
				averageResponseTime = (timeArr[numOfSamplesForAvgTime - 1] - timeArr[0])
						/ (numOfSamplesForAvgTime - 1);
				double sumAll = 0;
				for (int i = 1; i < numOfSamplesForAvgTime; i++) {
					sumAll += Math.pow(timeArr[i] - timeArr[i - 1]
							- averageResponseTime, 2);
				}
				stiaResponseTime = Math.pow(sumAll
						/ (numOfSamplesForAvgTime - 1), 0.5);
	...
	public Action chooseAction() {
		...
		if ((lastBidOfOpponentUtil > dblMinUtil || lastBidOfOpponentUtil > myMinUtil)
		...
		if (actionOfPartner != null) {
			Bid Nbid = getBestNigberBid(
					actionOfPartner.getBidFromAction(actionOfPartner), 0);
		...
		if (highDiscount) {
			...
			try {
				...
				System.out.println("Exception in ChooseAction1:"
						+ e.getMessage());

				lastAction = cleanOffer(
						hashBids.get(arrSamples.get(countOffer)),
						lastBidOfOpponentUtil); // best guess if things go
												// wrong.
			...
			if (currentTime >= 1 - FactorOfAverageResponseTime
				...
				if (utilitySpace.getReservationValue() > utilOfBestBidOfOppenent
					...
					if (isPrinting)
						System.out.println("Accept3 Util: "
								+ utilOflastBidOfOpponent);
					if (R > utilitySpace.getUtilityWithDiscount(
							lastBidOfOpponent, timeline.getTime()))
						return cleanEndNegotiation();
					...
					double dblutility = getUtility(this.opponentBidHistory
							.getBestBidInHistory());
					if (isPrinting)
			...
			return FastLastMomentStrategyWithoutEndNegotiation(dblMinUtil
					+ 0.25 * stia, dblMinUtil, currentTime);
		}
		if (lowDiscount) {
			return FastStrategy();
		}
		...
		Bid bid;
		if (currentTime >= 1 - FactorOfAverageResponseTime
				* (averageResponseTime + 2 * stiaResponseTime)) { // last last
																	// moment
			if (utilitySpace.getReservationValue() > utilOfBid) {
				if (isPrinting)
			...
			if (utilitySpace.getReservationValue() > utilOfBestBidOfOppenent
				...
				if (isPrinting)
					System.out.println("Accept5 Util: "
							+ utilOflastBidOfOpponent);
				if (R > utilitySpace.getUtilityWithDiscount(lastBidOfOpponent,
						timeline.getTime()))
					return cleanEndNegotiation();
		...
		return FastLastMomentStrategyWithoutEndNegotiation(dblMinUtil
				- UtilityFactor + 0.25 * stia, dblMinUtil - UtilityFactor,
				currentTime);
	...
	private Action EndNegotiationOrAcceptOrNewOfferr(Bid myOfferedBid)
		...
		// 3rd priority - bestOpponentBidUtility is the best choice
			...
			if (isPrinting)
				System.out
						.println("new Offer(this.opponentBidHistory.getBestBidInHistory())");
			return new Offer(this.opponentBidHistory.getBestBidInHistory());
		}
		...
		// 4th priority - myOfferedBid
			...
			MyOfferedUtility = utilitySpace.getUtility(lastAction
					.getBidFromAction(lastAction));
			if (isPrinting)
				System.out.println("else MyOfferedUtility 333: "
						+ MyOfferedUtility);
			return lastAction;
		}
	}
	...
	public Action FastStrategyWithoutEndNegotiation(double VirtulReservation) {
		...
		try {
			...
			if ((lastBidOfOpponentUtil > MyOfferedUtility || lastBidOfOpponentUtil >= VirtulReservation)
					&& lastBidOfOpponentUtil > R) {
				if (isPrinting)
					System.out
			...
			// Reservation is the best choice
				...
				if (flagStopExplor == false && dbltime < 0.90) {
					...
					if (otherArrSamples.size() > 0) {
						...
						Collections.sort(otherArrSamples);
						double tmpUtil = otherArrSamples.get(otherArrSamples
								.size() - 1);
	...
	private Action bestFromOpponent() {
		double dblUtilVal = alternativeArrSamples.get(alternativeArrSamples
				.size() - 1);
		...
		if (R > utilitySpace
				.getUtilityWithDiscount(lastBid, timeline.getTime()))
			return cleanEndNegotiation();
		else

...
public class ClearDefaultStrategy {
	...
	public void init() {
		try {
			if (utilitySpace.getReservationValue() > 0
				...
				theTimeIAmReadyToInvestInNegotiation = Math.sqrt(Math
						.sqrt(1 - utilitySpace.getReservationValue())
			...
			theFastUtilityInvestInNegotiation = (1 - Math.sqrt(Math
					.sqrt(1 - utilitySpace.getDiscountFactor())
					* Math.sqrt(utilitySpace.getReservationValue()))) * 0.33 + 0.62;

			System.out.println("theTimeIAmReadyToInvestInRiskyNegotiation="
					+ theTimeIAmReadyToInvestInNegotiation);
			...
			opponentBidHistory.initializeDataStructures(utilitySpace
					.getDomain());
	...
	public void ReceiveMessage(Action opponentAction) {
		if (opponentAction instanceof Offer) {
			...
			double remainingTime = theTimeIAmReadyToInvestInNegotiation
					- timeline.getTime();
	...
	public Bid middleBid(int RoundLeft, double MinUtil) throws Exception {
		...
		double utilOfBestOpponentBid = utilitySpace
		...
		double weightOfOurBestBid = UtilityRange * (RoundLeft / MAX_ROUNDS)
				+ MinUtil; // larger constFactor - larger weight to our best
							// bid.
		...
		double averageUtility = weightOfOurBestBid * utilOfMyBestBid
				+ (1 - weightOfOurBestBid) * utilOfBestOpponentBid;
		...
		List<BidDetails> bidDetailsWeAreWillingToAccept = outcomeSpace
				.getBidsinRange(new Range(averageUtility, 1.0));
		...
		Bid bid = opponentBidHistory.ChooseBid(bidsWeAreWillingToAccept,
				utilitySpace.getDomain());
		if (bid == null) {

			try {
				Bid tmp = outcomeSpace.getBidNearUtility(averageUtility)
						.getBid();
				return tmp;
			} catch (Exception e) {
	...
	public Action ExceptionStrategy() {
		return this.FastLastMomentStrategyWithoutEndNegotiation(1, 0,
				timeline.getTime());
	}
	...
	private Action EndNegotiationOrAcceptOrNewOfferr(Bid myOfferedBid)
		...
		double bestOpponentBidUtility = (bestOpponentBid == null) ? utilitySpace
				.getUtility(this.minUtilityBid) : utilitySpace
	...
	public Action OneStepStrategy() {
		try {
			...
			System.out.println("OneStepStrategy: Exception in ChooseAction:"
					+ e.getMessage());
	...
	public Action RandomStepStrategy() {
		try {
			...
			for (BidDetails bd : bidDetailsWeAreWillingToAccept)
				bidsWeAreWillingToAccept.add(bd.getBid());
			Bid bid2 = opponentBidHistory.ChooseBid(bidsWeAreWillingToAccept,
					utilitySpace.getDomain());
			if (bid2 == null) {

				try {
					Bid tmp = outcomeSpace.getBidNearUtility(MyOfferedUtility)
							.getBid();
					return EndNegotiationOrAcceptOrNewOfferr(tmp);
				} catch (Exception e) {
					System.out.println("***********************Exception");
					return EndNegotiationOrAcceptOrNewOfferr(SortedBid.get(0)
							.getBid());
				}
			} else {
	...
	public Action FastStrategy() {
		try {
			return EndNegotiationOrAcceptOrNewOfferr(SortedBid.get(countBid++)
					.getBid());
		} catch (Exception e) {
	...
	public Action FastStrategyWithoutEndNegotiation(double VirtulReservation) {
		try {
			double lastBidOfOpponentUtil = lastBidOfOpponent == null ? 0
					: utilitySpace.getUtility(lastBidOfOpponent);
			MyOfferedUtility = utilitySpace.getUtility(SortedBid.get(countBid)
					.getBid());
			...
			// Reservation is the best choice
				...
				for (BidDetails bd : bidDetailsWeAreWillingToAccept)
					bidsWeAreWillingToAccept.add(bd.getBid());
				Bid bid2 = opponentBidHistory.ChooseBid(
						bidsWeAreWillingToAccept, utilitySpace.getDomain());
				if (bid2 == null) {
					...
					try {
						...
						System.out.println("***********************Exception");
						return EndNegotiationOrAcceptOrNewOfferr(SortedBid.get(
								0).getBid());
					}
				} else {
	...
	public Action LastMomentStrategy(double MinUtil, double dblMyRoundTime) {
		try {
			...
			Bid bid;
			if (dblMyRoundTime >= 1 - FactorOfAverageResponseTime
					* averageResponseTime) { // last last moment
			...
			} else if (dblMyRoundTime >= 1 - Math.pow(2, 1)
					* FactorOfAverageResponseTime * averageResponseTime) { // one
																			// before
																			// last
																			// moment
				double utilOflastBidOfOpponent = utilitySpace
						.getUtility(lastBidOfOpponent);
				double utilOfBestBidOfOppenent = utilitySpace
						.getUtility(this.opponentBidHistory
								.getBestBidInHistory());
			...
			} else if (dblMyRoundTime >= 1 - Math.pow(2, 2)
			...
			System.out.println("LastMomentStrategy: Exception in ChooseAction:"
					+ e.getMessage());
			e.printStackTrace();
	...
	public Action chooseAction() {
		try {
			...
			boolean midReservation = (utilitySpace.getReservationValue() < 0.75 && utilitySpace
					.getReservationValue() > 0.25);// (minUtility+0.75*UtilityRange)
													// &&
													// utilitySpace.getReservationValue()
													// >=
			...
			boolean midDiscount = (utilitySpace.getDiscountFactor() < 0.75 && utilitySpace
					.getDiscountFactor() > 0.25);
			boolean lowDiscount = (utilitySpace.getDiscountFactor() <= 0.25);
			try {
				double currentTime = timeline.getTime()
						/ (theTimeIAmReadyToInvestInNegotiation / 1.2);
				// System.out.println("currentTime: " + currentTime);
				double MyNextOfferedUtility = utilitySpace.getUtility(SortedBid
						.get(countBid).getBid());
				...
				if (highDiscount) {
					...
					if (currentTime <= 0.25 && MyNextOfferedUtility >= 0.95) {
						return FastStrategyWithoutEndNegotiation(0.95);
							...
							&& MyNextOfferedUtility >= 0.85) {
						return FastLastMomentStrategyWithoutEndNegotiation(1,
								0.85, currentTime * 2);

...
public class ClearDefaultStrategy  {
	...
	public void init() {
		try {
			...
			theFastUtilityInvestInNegotiation=(1-Math.sqrt(
					Math.sqrt(1-utilitySpace.getDiscountFactor())
					*
					Math.sqrt(utilitySpace.getReservationValue())
				))*0.33+0.62;
			...
			/*for (int i = 0; i < SortedBid.size(); i++) {
				BidDetails bidDetails= SortedBid.get(i);
				System.out.println("getMyUndiscountedUtil:" + bidDetails.getMyUndiscountedUtil());
			}*/
			countBid = 0;
			MyOfferedUtility = 2;
			IConcedeLast = false;
	...
	public void ReceiveMessage(Action opponentAction) {
		if (opponentAction instanceof Offer) {
			...
			//System.out.println("BIU: time="+timeline.getTime()+" #bids="+numOfRounds+" averageResponseTime="+averageResponseTime);
		}
	}

	/**
	 * @param RoundLeft - estimation of the number of rounds left until the time we decided to quit. Logarithmic scale (1/2, 1/4, 1/8..).
	 * @param MinUtil the minimum utility we want to get from this nego
	 * @return 
	 */
	public Bid middleBid(int RoundLeft,double MinUtil) throws Exception {
		...
		System.out.println(Integer.parseInt(Double.toString(explorationSurface)));
		return SortedBid.get(Integer.parseInt(Double.toString(explorationSurface))).getBid();
		 */
		//System.out.println(utilitySpace.getDiscountFactor() + ", " + utilitySpace.getReservationValue());
		...
		//double constFactor = ((1-0.6+utilitySpace.getDiscountFactor())*RoundLeft)/10 + 0.6*(utilitySpace.getDiscountFactor());

		double weightOfOurBestBid = UtilityRange*(RoundLeft/MAX_ROUNDS) + MinUtil; // larger constFactor - larger weight to our best bid.
		...
		/*
		Range r = new Range(averageUtility,1);
		List<BidDetails> RangeBid = outcomeSpace.getBidsinRange(r);

		int RndInx = Integer.parseInt(Double.toString((Math.random()*RangeBid.size())));
	...
	private Action EndNegotiationOrAcceptOrNewOfferr(Bid myOfferedBid) throws Exception {
		...
		// 4th priority - myOfferedBid
			...
			return new Offer(myOfferedBid);
	}
	
	
	/**
	 * This strategy counts the number of steps the opponent has done towards us (= distinct bids), and does the same number of steps towards him.
	 */
	...
	public Action RandomStepStrategy()
	{
		try {
			...
			List<BidDetails> bidDetailsWeAreWillingToAccept = outcomeSpace.getBidsinRange(new Range(MyOfferedUtility, 1.0));
			List<Bid> bidsWeAreWillingToAccept = new ArrayList<Bid>();
			for (BidDetails bd: bidDetailsWeAreWillingToAccept)
				bidsWeAreWillingToAccept.add(bd.getBid());
	...
	public Action FastStrategyWithoutEndNegotiation(double VirtulReservation)
	{
		try{
			...
			if (lastBidOfOpponentUtil>=bestOpponentBidUtility &&
					 lastBidOfOpponentUtil>=MyOfferedUtility &&
					 lastBidOfOpponentUtil>=VirtulReservation)
			{
				return new Accept();
			}
			// bestOpponentBidUtility is the best choice
			else if (bestOpponentBidUtility>=lastBidOfOpponentUtil &&
					 bestOpponentBidUtility>=MyOfferedUtility &&
			...
			// Reservation is the best choice
					...
					&& VirtulReservation >= bestOpponentBidUtility
	...
	public Action FastLastMomentStrategyWithoutEndNegotiation(double StartMinUtil, double MinUtil ,double dblMyRoundTime)
	{
		double Delta = (StartMinUtil-MinUtil)/11;
			...
			if (dblMyRoundTime >= 1-Math.pow(2,2)*FactorOfAverageResponseTime*averageResponseTime) {  // Other before-last moments - logarithmic scale
	...
	public Action LastMomentStrategy(double MinUtil, double dblMyRoundTime)
	{
		try{
			...
			Bid bid;
			if (dblMyRoundTime >= 1-FactorOfAverageResponseTime*averageResponseTime) {  // last last moment
			...
			} else if (dblMyRoundTime >= 1-Math.pow(2,1)*FactorOfAverageResponseTime*averageResponseTime) {  // one before last moment
				...
				if(utilitySpace.getReservationValue() > utilOfBestBidOfOppenent && utilitySpace.getReservationValue() > utilOflastBidOfOpponent )
					...
					return new Offer(this.opponentBidHistory.getBestBidInHistory());
			} else if (dblMyRoundTime >= 1-Math.pow(2,2)*FactorOfAverageResponseTime*averageResponseTime) {  // Other before-last moments - logarithmic scale
				return FastStrategy();
			} else if (dblMyRoundTime >= 1-Math.pow(2,3)*FactorOfAverageResponseTime*averageResponseTime) {
				return FastStrategy();
			} else if (dblMyRoundTime >= 1-Math.pow(2,4)*FactorOfAverageResponseTime*averageResponseTime) {
				...
				{
					return new Offer(this.maxUtilityBid);
				}
			}
			double utilOflastBidOfOpponent = utilitySpace.getUtility(lastBidOfOpponent);
			double utilOfBid = utilitySpace.getUtility(bid);
			if(utilitySpace.getReservationValue() > utilOfBid && utilitySpace.getReservationValue() > utilOflastBidOfOpponent)
			{
				if(IsPrintForDebug)
	...
	public Action chooseAction() {
		try {
			...
			boolean midReservation   =  (utilitySpace.getReservationValue() <  0.75 && utilitySpace.getReservationValue() > 0.25);//(minUtility+0.75*UtilityRange) && utilitySpace.getReservationValue() >= (minUtility+0.375*UtilityRange));

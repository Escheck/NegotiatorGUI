...
class GenAlg {
	...
	GenAlg(NonlinearUtilitySpace utilitySpace, int initialGenerationSize,
		...
		for (int i = 1; i <= numIssues; i++) {
			...
			int highestVal = ((IssueInteger) utilitySpace.getDomain().getIssue(
					i - 1)).getUpperBound();
			int lowestVal = ((IssueInteger) utilitySpace.getDomain().getIssue(
					i - 1)).getLowerBound();
	...
	Bid getRandomBid(Bid referencebid, int maxDistance) throws Exception {
		...
		for (int i = 0; i < distance; i++) {
			...
			int lowestVal = ((IssueInteger) utilitySpace.getDomain().getIssue(
					issue - 1)).getLowerBound(); // WARNING: the issues are
													// numbered 1 to 30, but
													// when calling getIssue,
													// they are indexed with the
													// values 0 to 29
			int highestVal = ((IssueInteger) utilitySpace.getDomain().getIssue(
					issue - 1)).getUpperBound();

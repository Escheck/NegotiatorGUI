			...
			if(randomnr.nextDouble() < 0.5){
				...
				try {
					if(type == 0 ){
						...
						ret.setValue(lIssueInteger.getNumber(), in1.getValue(lIssueInteger.getNumber()));
					}else{
						IssueDiscrete ID = (IssueDiscrete)issues.get(i);
						ret.setValue(ID.getNumber(), in1.getValue(ID.getNumber()));
					...
					e.printStackTrace();
					System.out.println("GeneCrossErrer Number Out? in MyBidGene Line80");
				}
	...
	public void Mutate(){
		...
		for(int i=0;i<issues.size();i++){
			...
			if(randomnr.nextDouble() < mut){
				...
				if(type == 0){
					IssueInteger lIssueInteger = (IssueInteger)issues.get(i);
					int rndint = randomnr.nextInt(lIssueInteger.getUpperBound() - lIssueInteger.getLowerBound());
					bid.setValue(lIssueInteger.getNumber(), new ValueInteger(lIssueInteger.getLowerBound() + rndint));
				}else{
					IssueDiscrete ID = (IssueDiscrete)issues.get(i);
					int number = randomnr.nextInt(ID.getNumberOfValues());
					bid.setValue(ID.getNumber(), ID.getValue(number));
				}
			}
		}
		utilflag = false;

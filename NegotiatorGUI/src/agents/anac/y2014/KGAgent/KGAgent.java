...
public class KGAgent extends Agent{
	...
	public String getVersion() { return "5.1.1"; }
	...
	public void init()
	{
		...
		System.out.println("reservate = " + reservation + "   timepress = " + timepressur);

		if(utilitySpace.getType() == UTILITYSPACETYPE.LINEAR){
			type = 0;
			System.out.println("UtilitySpaceType = LINEAR");
		}else if(utilitySpace.getType() == UTILITYSPACETYPE.NONLINEAR){
			type = 0;
			System.out.println("UtilitySpaceType =  NONLINEAR");
		}else{
			System.out.println("NULL ??  " + utilitySpace.getType());
			type = 1;
		}
	...
	public Action chooseAction() {
		...
		if(actionOfPartner instanceof Offer){
			...
			//System.out.println("EnemyBid is " +getUtility(((Offer)actionOfPartner).getBid()) + "  AcceptableLine is " + acceptableline );
		...
		try{
			...
			//Bid bid = new Bid(utilitySpace.getDomain(),ai.GetRandomMap(count/100));
	...
	MyBidGene st;
	void MyInit(){
		//MyHyperRectangle input = new MyHyperRectangle(utilitySpace.getDomain().getIssues());

		//NonlinearUtilitySpace nonlinear = new NonlinearUtilitySpace(utilitySpace);
		/*
		ai = new MyRectangleField(utilitySpace.getDomain().getIssues(),nonlinear.getAllInclusiveConstraints());
		ai.Search();
		*/
	...
	ArrayList<MyBidGene> SearchBid2(){
		...
		if(bidlist.size() >0){
			...
			ga = new GA_Main(in,new BidGenerationChange(300,10), new CompMyBidGene(0));

		}else{
			ga = new GA_Main(new BidGenerationChange(300), new CompMyBidGene(0));
		...
		for (int i = 0; i < 60; i++) {
			...
			//System.out.println("Searchdata MyBid " + ((MyBidGene)buf.get(i)).util + " enemybid " + ((MyBidGene)buf.get(i)).enemyutil);
			//System.out.println("in press   MyBid " + getUtility(((MyBidGene)buf.get(i)).bid) + " enemybid " + EnemyPressAll(((MyBidGene)buf.get(i)).enemyutil) + " AddUtil " + ((MyBidGene)buf.get(i)).GetValue(0));
	...
	/*
	 * å®Œå…¨ã�«ãƒ©ãƒ³ãƒ€ãƒ ã�ªBidã‚’ç�?Ÿæˆ�ã�™ã‚‹
	 */
	...
	double EnemyTimePress(double utility){
		...
		return utility * 1.3 * Math.pow(time, (Math.pow(timepressur * gunma_d,2.0)) + gunma_min);
	...
	Boolean Accept(Bid partnerbid){
		...
		if( eutil > (1.0-time)*0.3 + 0.7 ){
			System.out.println("Accept!! for pointhugher");
			return true;
		}
		*/
		return false;
	...
	void BidPrint(Bid bid){
		ArrayList<Issue> issues=bid.getIssues();
		IssueInteger lIssueInteger;

...
public class Atlas extends Agent {
		...
		try {
			...
			if (actionOfPartner instanceof Offer) {
				...
				if (partner_bid_count == 0) {
					...
					partner_bid_utility_weighted_average = (utilitySpace
							.getUtility(partnerBid) + partner_bid_utility_weighted_average) / 2;
				}
				// ‘ŠŽè‚ÌBid‚µ‚½Žž��‚ð‹L˜^‚µ‚Ä‚¨‚­
				partner_prev_last_bid_time = partner_last_bid_time;
				partner_last_bid_time = timeline.getTime();
				time_scale = (((partner_last_bid_time - partner_prev_last_bid_time) + time_scale
						* partner_bid_count) / (partner_bid_count + 1));
	...
	private boolean isAcceptable(Bid partnerBid,
			double offeredUtilFromOpponent, double myOfferedUtil, double time)
			throws Exception {
		// ‘ŠŽè‚ÌŒø—p’l
	...
	private Bid getBid() throws Exception {
		...
		if (!tension) {
			...
			if (timeline.getTime() > (last_time - time_scale * 2)
				...
				if (timeline.getTime() > (last_time - time_scale * 1)) {
					if (utilitySpace.getUtility(false_best_agreement_bid) < utilitySpace
							.getUtility(best_partner_bid)) {
						if (utilitySpace.getUtility(false_best_agreement_bid) > utilitySpace
								.getReservationValue()) {
			...
			if (timeline.getTime() > (last_time - time_scale * 1)
					...
					&& best_partner_bid == null
					&& false_best_agreement_bid != null
					&& utilitySpace.getUtility(false_best_agreement_bid) > utilitySpace
							.getReservationValue()) {
				attack_flag = true;
				return false_best_agreement_bid;
			}
		...
		if (bid == null
				|| utilitySpace.getUtility(bid) < border_bid_utility
			...
			try {
				for (int i = 0; i < SA_ITERATION; i++) {
					if (tension) {
						...
						if (bid == null
								|| utilitySpace.getUtility(check_bid) > utilitySpace
										.getUtility(bid)) {
						...
						if (bid == null
								|| (utilitySpace.getUtility(bid) < border_bid_utility || (utilitySpace
										.getUtility(check_bid) < utilitySpace
										.getUtility(bid))
										&& utilitySpace.getUtility(check_bid) > border_bid_utility)) {
							bid = new Bid(check_bid);
						}
					}
				}
		...
		// �Å‘åBid‚Ì�X�V
		if (bid != null
				&& utilitySpace.getUtility(max_bid) <= utilitySpace
						.getUtility(bid)) {
			max_bid = new Bid(bid);
		}
	...
	private void getBorderUtility() throws Exception {
		...
		if (tension) {
			...
			if (SAMPLE_NUM < partner_bid_count) {
				compensate = (prev_offeredUtilFromOpponent - partner_bid_utility_weighted_average)
				...
				if (compensate * (1.0 + time_compensate) > time_compensate
					...
					border_bid_utility = border_bid_utility - compensate
							* (1.0 + time_compensate);
				} else {
		...
		do {
			...
			while (temperature > 0.0001) {
				...
				for (int i = 0; i < step_num; i++) {
					...
					next_b = next_b.putValue(issueNumber, new ValueInteger(
							next_v)); // ‹ß–T‚ÌBid‚ð‹�‚ß‚é
				}
	...
	private Bid getBestBidbyNear(Bid baseBid) throws Exception {
		...
		for (int i = 0; i < lim; i++) {
			...
			for (int j = min; j <= max; j++) {
				current_b = current_b
						.putValue(issueNumber, new ValueInteger(j)); // ‹ß–T‚ÌBid‚ð‹�‚ß‚é
				current_u = utilitySpace.getUtility(current_b);
		...
		if (sessionNr > 0 && prev != null) {
			...
			if (is_agreement && last_tension) {
				...
				if (false_best_agreement_bid == null) {
					...
					double get_utility_true = (utilitySpace.getDiscountFactor() - false_agreement_average_utility_with_discount)
					...
					utility_true += (utilitySpace
							.getReservationValueWithDiscount(1.0) - false_agreement_average_utility_with_discount)
							* true_failure_num;
			...
			if (isAgreement) {
				...
				if (tension) {
					...
					if (!first_offer_decision_flag) {
						...
						if (attack_flag) {
							...
							if (false_best_agreement_bid != null) {
								...
								if (utilitySpace.getUtility(lastBid) < utilitySpace
										.getUtility(false_best_agreement_bid)
										&& ((tension && last_tension) || (!tension && !last_tension))) {
									partner_attack_num++;
								}
						...
						// •½‹Ï’l‚ð‹L˜^
						false_agreement_average_utility_with_discount = ((false_agreement_average_utility_with_discount * false_agreement_num) + utilitySpace
								.getUtilityWithDiscount(lastBid,
										timeline.getTime()))
						...
						if (utilitySpace.getDiscountFactor() < utilitySpace
								...
								&& utilitySpace
										.getUtility(false_best_agreement_bid) < 1.0) {

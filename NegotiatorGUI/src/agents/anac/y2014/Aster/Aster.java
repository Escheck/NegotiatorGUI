...
public class Aster extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			double maxUtil = opponentMaxUtility > opponentPrevMaxUtility ? opponentMaxUtility
					: opponentPrevMaxUtility;
			...
			if (!selectOpponentBidList.isEmpty()) {
				actionBid = chooseAction.nextOfferingBid(bidTarget, selectMyBidList,
						selectOpponentBidList);
			} else {
				actionBid = chooseAction.nextOfferingBid(bidTarget, selectMyBidList, true);
			}
	...
	public void endSession(NegotiationResult result) {
		...
		try {
			...
			if (result.isAgreement()) {
				...
				if (agreementBidList.size() >= (int) (this.sessionsTotal / 3)) {
					...
					if ((agreediff < AGREE_EPSILON) && (utilitySpace.getDiscountFactor() < 0.95)) {
	...
	private void beginSession() {
		...
		try {
			...
			double maxConcessionUtility = (1.0 - ((opponentMaxUtility - opponentPrevMinUtility) / (1.0 - opponentPrevMinUtility))) * 100;

			// å�ˆæ„�ç‚¹ã�®è¿‘å‚�æŽ¢ç´¢
			if (!agreementBidList.isEmpty()) {
				double target = opponentMaxUtility > opponentPrevMaxUtility ? opponentMaxUtility
						: opponentPrevMaxUtility;
				for (Bid bid : agreementBidList) {
	...
	private Bid isConsiderable() {
		...
		// ************ è­²æ­©åˆ¤å®š ************//
		// 1. ç›¸æ‰‹ã�ŒACCEPT_CONCESSIONä»¥ä¸Šè­²æ­©ã�—ã�¦ã��ã�Ÿå ´å�ˆ
		// 2. å�³æ±ºåˆ¤æ–­ã�Œtrueã�‹ã�¤(ç›¸æ‰‹ã�®æœ€å¤§åŠ¹ç”¨å€¤)=(ç›¸æ‰‹ã�®é�ŽåŽ»ã�®æœ€å¤§åŠ¹ç”¨å€¤)ã�®å ´å�ˆ
		// 3. æ™‚é–“ã�ŒDEAD_LINEã�‹ã�¤(ç›¸æ‰‹ã�®æœ€å¤§åŠ¹ç”¨å€¤)=(ç›¸æ‰‹ã�®é�ŽåŽ»ã�®æœ€å¤§åŠ¹ç”¨å€¤)ã�®å ´å�ˆ
		// 4. DFã�Œæœ‰åŠ¹ã�‹ã�¤ç›¸æ‰‹ã�®æœ€è‰¯Bidã�ŒbidTargetä»¥ä¸Šã�«ã�ªã�£ã�Ÿå ´å�ˆ
		if (concessionDeg >= ACCEPT_CONCESSION) {
			...
			double standardUtil = opponentPrevMaxUtility < bidTarget ? opponentPrevMaxUtility : bidTarget;
	...
	private boolean isAcceptable(double t) throws Exception {
		...
		// ************ å�ˆæ„�åˆ¤å®š ************//
		// 1. ç›¸æ‰‹ã�®åŠ¹ç”¨å€¤ã�Œè‡ªåˆ†ã�®é–¾å€¤ã‚’è¶…ã�ˆã�Ÿå ´å�ˆ
		// 2. AcceptPã�Œä¸€å®šå€¤ã‚’è¶…ã�ˆã�Ÿå ´å�ˆ
	...
	private void addSelectBidList(double bidTarget) {
		...
		double util;
		try {
			for (Iterator<Bid> it = selectAgreementBidList.iterator(); it.hasNext();) {
				bid = it.next();
	...
	private void printCurrentDetails() {
		...
		System.out.println("opponentMin:" + opponentMinUtility);
		System.out
				.println("concessionDeg:"
						+ ((opponentMaxUtility - opponentMinUtility) / (1.0 - opponentMinUtility))
						* 100 + "%");
		System.out.println("prevMax:" + opponentPrevMaxUtility);
		System.out.println("prevMin:" + opponentPrevMinUtility);
		System.out.println("mySize:" + selectMyBidList.size());
		System.out.println("opponentSize:" + selectOpponentBidList.size());
		System.out.println("agreeSize:" + selectAgreementBidList.size());
		System.out.println("immediateDecision:" + immediateDecision);
	}
	*/
}

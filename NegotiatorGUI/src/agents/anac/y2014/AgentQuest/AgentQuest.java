...
public class AgentQuest extends Agent {
	...
	private int alpha = 4; //Acceptable threshold for opponent harheadedness: adjust between 1 and opponentMonitoringWindowSize to negotiate hard (low value) or soft (high value).
	private double beta = 0.6d; //Acceptable threshold for conceding probability: adjust between 0 and 1 to negotiate hard (high value) or soft (low value).
	...
	public void init()
	{
		System.out.println("\n"+getName()+" version("+getVersion()+") initializing...");
		this.myUtilitySpace = super.utilitySpace;
		this.domain = this.myUtilitySpace.getDomain();
		if(this.myUtilitySpace.getReservationValue() != null && this.myUtilitySpace.getReservationValue() > 0.0d)
		{
			this.defaultReservationValue = this.myUtilitySpace.getReservationValue().doubleValue();
		}
	}
	...
	public Action chooseAction() {
		...
		try
		{
			if(this.partnerAction == null)
			{
				...
				addSampleBids(time);
				addSampleBids(time); //Increase bid samples at our side so we dont loose on a low utility due to less samples in first round.
				this.myLastBid = this.myBidHistory.getBestBidDetails().getBid();
				action = new Offer(this.myLastBid);
				this.counter++;
			...
			if(this.partnerAction instanceof Offer)
			{
				...
				//not the first round. measure mean response time and update last round's time
				if(this.counter > 1)
				{
					this.meanResponseTimeOfOpponent += (time - this.lastResponseTime);
				...
				{ //this is first round
					this.lastResponseTime = time;
					addSampleBids(time); //increase bid samples from our side so we dont loose on a low utility due to less samples in first round.
				}
				...
				//Received an Offer from Opponent - process it, generate CounterOffer and decide whether Offer is acceptable or to send CounterOffer
				...
				if(this.opponentLastBid == null){
					...
					opponentUtilityInWindowSofar = this.myUtilitySpace.getUtilityWithDiscount(this.opponentLastBid, time);
				}
				...
				double offeredUtilFromPartner = this.myUtilitySpace.getUtilityWithDiscount(partnerBid, time);
				
				//Tolerance measure:
				if(this.opponentMonitoringWindowIndex > 1 && this.opponentMonitoringWindowIndex <= this.opponentMonitoringWindowSize){ //this.lastTenCounter > 0 &&
					//Maintaining mean utility of opponent's offer with opponent utility in window sofar: 
				...
				//Maintain best Bid:
				if(this.myUtilitySpace.getUtilityWithDiscount(partnerBid, time) > this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time))
				{
					this.bestBid = partnerBid;
				}
				...
				{
					//To deal with the unlikely case if we cant produce a counter offer.
	...
	private void addOpponentBidToHistory(Bid partnerBid, double time)
	{
		if (this.opponentBidHistory.size() < this.MAX_BIDS_TO_STORE)
		{
			try
			{
				this.opponentBidHistory.add( new BidDetails(partnerBid, this.myUtilitySpace.getUtilityWithDiscount(partnerBid, time)) );
			}
			catch (Exception e)
			{
				e.printStackTrace();
		...
		else
		{
			...
			this.opponentBidHistory = new BidHistory(this.opponentBidHistory.getNBestBids(this.MAX_BIDS_TO_STORE-1));
			try
			{
				this.opponentBidHistory.add( new BidDetails(partnerBid, this.myUtilitySpace.getUtilityWithDiscount(partnerBid, time)) );
			}
	...
	private void addSampleBids(double time)
	{
		...
		else
		{
			...
			this.myBidHistory = new BidHistory(this.myBidHistory.getNBestBids(900));
			addBids(time);
		}
	}
	...
	{	
		...
		{
			Bid randomBid = this.domain.getRandomBid();
			try 
			{
				this.myBidHistory.add( new BidDetails(randomBid, this.myUtilitySpace.getUtilityWithDiscount(randomBid, time) ) );
			}
			catch (Exception e)
			{
				e.printStackTrace();
	...
	private BidDetails getCounterOffer(double time)
	{
		...
		List<BidDetails> opponentTop10Bids = this.opponentBidHistory.getNBestBids(10);
		...
		for( int row=0; row< opponentTop10Bids.size(); row++ )
		{
			...
			for( int col=0; col< myTop10Bids.size(); col++ )
			{
				...
				//Compute Euclidean Distance between opponentBidDetails and myBidDetails:
				try {
					comparisonMatrix [row][col] = computeEuclideanDistancePerBid(opponentBidDetails, myBidDetails, time);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		}
		//Now traverse the comparisonMatrix, pick top 10 (least distanced) bids per row (preserving some diversity) and add to a BidHistory HashMap.
		HashMap<Integer,BidDetails> hashMap = new HashMap<Integer,BidDetails>();
		for(int row=0; row<10; row++)
		{
			double closestBid = 99999.999;//some unrealistic number to initialize.
			for( int col=0; col<10; col++)
			{
				if( col > 0 )
				{
					if( (comparisonMatrix [row][col] < comparisonMatrix [row][col-1]) && (comparisonMatrix [row][col] < closestBid ) )
					{
		...
		while(iter.hasNext()) 
		{
		    ...
		    BidDetails bidDetails = (BidDetails)hashMap.get(key);
	...
	private double computeEuclideanDistancePerBid(BidDetails opponentBidDetails, BidDetails myBidDetails, double time) throws Exception
	{
		...
		double myBidUtility;
		opponentBidUtility = this.myUtilitySpace.getUtilityWithDiscount(opponentBid, time);
	...
	private double getMyReservationValue(double time){
		if(this.myUtilitySpace.getReservationValue() != null && this.myUtilitySpace.getReservationValueWithDiscount(time) > 0.0d)
		{
	...
	private Offer genearteCounterOffer( Offer opponentOffer, double time)
	{
		...
		if(time > 0.0d)
		{
			////Sets our counterOffer in this.myLastBid. Resort to last Bid until the following mechanism finds a different/conceding offer:
			if(this.myUtilitySpace.getUtilityWithDiscount(this.myLastBid, time) > this.myUtilitySpace.getUtilityWithDiscount(this.myBidHistory.getBestBidDetails().getBid(), time) )
			{
				counterOffer = new Offer(this.myLastBid);
			}
			else
			{
				BidHistory tempBidHistory = this.myBidHistory.filterBetweenUtility(this.myUtilitySpace.getUtilityWithDiscount(this.myLastBid, time), this.myUtilitySpace.getUtilityWithDiscount(this.myBidHistory.getBestBidDetails().getBid(), time));
				if(tempBidHistory != null && tempBidHistory.size() > 0)
				{
					...
					{	
						this.myLastBid = tempBidHistory.getMedianUtilityBid().getBid();
						counterOffer = new Offer(this.myLastBid);
						
						if(!this.myUtilitySpace.isDiscounted())
						{
							//Starting second level selection only for non-discounted domains
							BidHistory anotherTempBidHistory = tempBidHistory.filterBetweenUtility(this.myUtilitySpace.getUtilityWithDiscount(tempBidHistory.getMedianUtilityBid().getBid(), time),  this.myUtilitySpace.getUtilityWithDiscount(tempBidHistory.getBestBidDetails().getBid(),time) );
							if(anotherTempBidHistory != null && anotherTempBidHistory.size() > 0)
							{
								if(anotherTempBidHistory.getMedianUtilityBid() != null)
								{
									this.myLastBid = anotherTempBidHistory.getMedianUtilityBid().getBid();
									counterOffer = new Offer(this.myLastBid);
								}
							}
				...
				{
					counterOffer = new Offer(this.myLastBid);//retain last offer.
				}
			}
			...
			try
			{
				...
				concessionProbability = Pconcede(this.myUtilitySpace.getUtilityWithDiscount(opponentOffer.getBid(), time), this.timeline.getTime());

				if( (this.hardlineCounter < this.alpha ) && (concessionProbability.compareTo(new BigDecimal(this.beta)) > 0 ) )
				{
					BidDetails tempBidDetails = null;
					tempBidDetails = getCounterOffer(time);
					Bid tempBid = tempBidDetails.getBid();
					double bidUtility = this.myUtilitySpace.getUtilityWithDiscount(tempBid, time);
					if( bidUtility > this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time) && bidUtility >= this.getMyReservationValue(time) ) 
					{
						counterOffer = new Offer(tempBid);
						this.myLastBid = tempBid;
					}
					else if (this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time) >= this.getMyReservationValue(time) )
					{
						counterOffer = new Offer(this.bestBid);
				...
				else
				{
					...
					{
						if( this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time) < this.myUtilitySpace.getUtilityWithDiscount(this.myLastBid, time) )
						{
							counterOffer = new Offer(this.myLastBid);
						}
						else if(this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time) >= this.getMyReservationValue(time))
					...
					{
						counterOffer = new Offer(this.myLastBid);//will execute just once i.e., for round 1.
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	{	
		...
		{
			try {
				opponentOfferUtility = this.myUtilitySpace.getUtilityWithDiscount(opponentOffer.getBid(), time);
				myOfferUtility = this.myUtilitySpace.getUtilityWithDiscount(this.myLastBid, time);
				bestBidUtility = this.myUtilitySpace.getUtilityWithDiscount(this.bestBid, time);
				
				if( opponentOfferUtility >= this.getMyReservationValue(time)
					&& (2.1d*this.meanResponseTimeOfOpponent + time >= 1.0d ) )
				{
					if (  (this.meanResponseTimeOfOpponent + time <= 1.0d ) )
					{
						this.myLastBid = this.bestBid; //most probably our last or last few counter offer(s)!
						result = false;
					}
					else
					{	
	...
	public BigDecimal Pconcede(double u, double t1) throws Exception
	{
		...
		if (t==0.5) 
		{
			...
			else throw new NumberFormatException(u+"");
		}
		double value = (u - 2.*u*t + 2.*(-1. + t + Math.sqrt(sq(-1. + t) + u*(-1. + 2*t))))/(-1. + 2*t);

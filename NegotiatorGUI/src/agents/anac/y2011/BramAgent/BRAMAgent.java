package agents.anac.y2011.BramAgent;
...
public class BRAMAgent extends Agent {
	/* FINAL VARIABLES */
	private final double TIME_TO_CREATE_BIDS_ARRAY = 2000;// The time that we
															// allocate to
															// creating the bids
															// array
		...
		try {
			...
			if (actionOfPartner == null) {
				...
				if (offeredUtility >= threshold)// If the utility of the bid
					...
					if (offeredUtility >= this.utilitySpace
	...
	private void fillBidsArray(double startTime) {
		...
		if (TEST_EQUIVALENCE) {
			BidIterator iterator = new BidIterator(

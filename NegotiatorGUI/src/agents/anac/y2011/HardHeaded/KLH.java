	...
	public void ReceiveMessage(Action pAction) {
		...
		if (pAction instanceof Offer) {
			...
			try {
				...
				opbestvalue = BSelector.BidList.floorEntry(
						utilitySpace.getUtility(opponentbestbid)).getKey();
	...
	private void updateLearner() {
		...
		for (Integer i : lastDiffSet.keySet()) {
			if (lastDiffSet.get(i) == 0
				...
				oppUtility.setWeight(domain.getObjective(i),
		...
		try {
			for (Entry<Objective, Evaluator> e : oppUtility.getEvaluators()) {
				...
				((EvaluatorDiscrete) e.getValue())
						.setEvaluation(
								opponentLastBid.getValue(((IssueDiscrete) e
										.getKey()).getNumber()),
								(LEARNING_VALUE_ADDITION + ((EvaluatorDiscrete) e
										.getValue())
										.getEvaluationNotNormalized(((ValueDiscrete) opponentLastBid
	...
	public Action chooseAction() {
		...
		try {
			...
			// otherwise select some new bids to be offered
				...
				newBid = BSelector.BidList.lowerEntry(bidHistory.getMyLastBid()
						.getKey());
			...
			if (opponentLastBid != null
					&& (utilitySpace.getUtility(opponentLastBid) > lowestYetUtility || utilitySpace
							.getUtility(offerQueue.getFirst().getValue()) <= utilitySpace
							.getUtility(opponentLastBid))) {
				newAction = new Accept(getAgentID());
			}

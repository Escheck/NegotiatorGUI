...
public class PhoenixParty extends AbstractNegotiationParty {
	...
	public void init() {
		try {
			...
			fixedInputSize = (int) Math.min(utilitySpace.getDomain()
					.getNumberOfPossibleBids(), fixedInputSize);
			if (utilitySpace.getDomain().getNumberOfPossibleBids() < 50) {
				...
				} else if (utilitySpace.getDomain().getNumberOfPossibleBids() <= 11) {
					utilityDivisionSize = (int) utilitySpace.getDomain()
							.getNumberOfPossibleBids() / 2;
				} else {
					utilityDivisionSize = (int) utilitySpace.getDomain()
				...
				utilityDivisionSize = Math.min((int) (utilitySpace.getDomain()
						.getNumberOfPossibleBids() / 15), utilityDivisionSize);
			}
	...
	public AgentID getPartyId() {
		if (debug)
			return new AgentID(getName() + "|"
					+ Double.toString(uTargetForBidding) + "|"
					+ Double.toString(uTargetForAcceptance) + "|" + "@"
					+ hashCode());
		return new AgentID(getName() + "@" + hashCode());
	}
		...
		if (arguments instanceof Offer) {
			try {
				...
				if (countOpp == lastCountOpp + 1) {
					...
					opponentPreviousBidUtility1 = getUtility(opponentPreviousBid1);
				}
				// next and next opponent (last opponent)
				else if (countOpp == lastCountOpp + 2) {
					opponentPreviousBid2 = ((Offer) arguments).getBid();
					opponentPreviousBidUtility2 = getUtility(opponentPreviousBid2);
				}
			...
			if (possibleActions.contains(Accept.class)
				...
				if ((myLastBid == null) || (opponentPreviousBid1 == null)
					...
					if (debug && getTime() > 0.02) {
						try {
							OriOut = System.out;
							logTxt = new PrintStream(new FileOutputStream(
									fileName, true));
					...
					if (opponentPreviousBidUtility >= uTargetForBidding
							|| (getTime() > (delta * 0.6 + theta * 0.3) && opponentPreviousBidUtility >= uTargetForAcceptance)) {
						currentAction = new Accept(getPartyId());
						System.out
								.println("Accccccccccccccccccccccccccccccccccc");
					} else {
						double reservationValue = theta
								* Math.pow(delta, currentTime);
						if (theta > 0
								&& ((currentTime - myLastBidTime)
										* timeline.getTotalTime() > opponentNoOfferTime || ((domain
										.getNumberOfPossibleBids() < 100 && currentTime > delta
										* (1.0 - theta + bestReceivedBidUtility2)) && (theta >= bestReceivedBidUtility2))
										&& theta >= 0.4
												* (maxBidUtility - utilitySpace
														.getUtility(opponentFirstBid2))
												+ utilitySpace
														.getUtility(opponentFirstBid2))) {
				...
				if (currentTime >= endTime
					...
					if (opponentPreviousBidUtility > theta) {
						if (theta == 0
								|| opponentPreviousBidUtility > theta * 2
								|| myLastBidUtility
										- opponentPreviousBidUtility < (maxBidUtility - minBidUtility) / 4) {
							currentAction = new Accept(getPartyId());
							System.out
									.println("Accccccccccccccccccccccccccccccccccc");
						}
				...
				if (theta >= uTargetForBidding) {
					currentAction = new EndNegotiation(getPartyId());
					// System.out.println("compareU="+compareU+" uTargetForBidding="+uTargetForBidding);
				}
			} else {
	...
	private void generateUtilitySamples(int m) {
		...
		for (int i = 0; i < m; i++) {
			utilitySamples[i] = minBidUtility + i
					* (maxBidUtility - minBidUtility) / (m * 1.0);
		}
	}
	...
	public Bid BiddingMethod() throws Exception {
		try {
			...
			if (domain.getNumberOfPossibleBids() > 6400) {
				allBids.clear();
				while (limits <= Math.max(
						domain.getNumberOfPossibleBids() / 10, 888)) {
			...
			if (rand.nextDouble() > delta
					* Math.min((Math.pow(Math.min(getTime() + 0.1, 0.8), 0.3)),
							0.5)) {
				bid = tempBL.get(rand.nextInt(tempBL.size()));
			}
			if (bid.equals(opponentFirstBid1) || bid.equals(opponentFirstBid2)) {
				bid = myLastBid;
			}
			controlFreqFunc(bid, 0.5);
			double timeToPlay = (delta * 1.0 - theta) * rand.nextDouble() + 0.1
					* delta;
	...
	private void getTargetUtility() throws Exception {
		...
		rate *= (0.3 + uTargetForAcceptance - Edelta + (1 - Math.pow(1.0, 1.3)) - Math
				.pow(theta, 1.5));
		rate /= opponentConcession();
		double opponentBidDistance = Math.abs(Edelta - uTargetForAcceptance)
				* compaction2;
		uTargetForBidding = (R * Math.pow(3, -getTime() * rate) + Edelta
				* Math.abs(1 - opponentBidDistance) / (2.0))
				/ (Math.abs(1 - opponentBidDistance) / (2.0) + Math.pow(3,
						-getTime() * rate));

		double baseValue = (Math.max(Elow, Edelta) + Edelta / uncertainty + uncertainty
				* Elow)
				/ (1.0 + 1 / uncertainty + uncertainty);
		double diffToElow = 1.2 * Math.abs(uTargetForBidding - baseValue);
		if (delta <= 1) {
			double x = 2 * (1 - getTime()) * delta, y = delta, z = (2 * getTime())
					/ delta;
			double uTargetForBiddingF1 = diffToElow
					* Math.pow(
							5,
			...
			double uTargetForBiddingF2 = diffToElow
					* (1 / ((Math.pow(
							(2 * getTime()) * (2.3 - firstAcceptTime), delta
									+ theta / 3.0) + 0.05) * 20) + 0.4 * delta);
			double uTargetForBiddingF3 = diffToElow
					* (1 / ((Math.log((20.0 / (delta + theta / 3.0))
							* getTime() * (2.3 - firstAcceptTime) + 1.1)) * 10) + 0.3 * delta);
			uTargetForBidding = baseValue
					+ (z * uTargetForBiddingF3 + y * uTargetForBiddingF2 + x
							* uTargetForBiddingF1) / (x + y + z);
		}
		if ((selfVar < 0.05 || varianceOfOpponentUtilities < 0.05)
		...
		if (uTargetForBidding <= uTargetForAcceptance) {
			uTargetForAcceptance = (uTargetForBidding + uTargetForAcceptance) / 2.0;
		}
	}
	...
	private void updateParas() {
		try {
			...
			for (double u : maxUtilitiesAsInputData) {
				varianceOfOpponentUtilities += Math.pow(u
						- meanOfOpponentUtilities, 2);
			}
	...
	private double discountedExpectation(double time) {
		...
		double predictedUtility = (uTargetForAcceptance
				...
				* (1.0 / PredictUncertainty(time)) * Predict(time) + 2 * w
				* meanOfOpponentUtilities)
				/ (2 * w + Math.pow(0.5, getTime()) * (2 + w) + Math.pow(0.5,
						1 - getTime())
						* 3
						* w
						* (1.0 / PredictUncertainty(time)));
		return predictedUtility
				* Math.pow(Math.pow(1.0 + (0.1 * theta + 0.05), 0.7), getTime());
	}
	...
	private double RFunction(double time) {
		try {
			double Elow = lowestExpectation()
					* Math.pow(0.8 + (0.2 * ppp),
			...
			if (getTime() > 10 * trainingTimeSeperator && getTime() < 0.3) {
				...
				double disToPoint = Math.abs(R - comparePoint)
						/ Math.abs(maxBidUtility
								- Math.min(utilitySpace
										.getUtility(opponentFirstBid1),
										utilitySpace
												.getUtility(opponentFirstBid2)));
				ppp = disToPoint;
			...
			double component1 = Math.abs(1 - Math.pow(time,
					1 / Math.pow(Math.abs(0.5), beta)));
			double component2 = Math.cos((1 - Math.pow(1.0, 1.3)
					* Math.pow(compaction2, 0.5))
					/ (omega));
		...
		if (currentTimeIndex != timeSamples.length - 1) {
			if (getTime() > timeSamples[currentTimeIndex + 1]) {
				if (getTime() > 5 * trainingTimeSeperator) {
					EdeltaPast[currentTimeIndex] = discountedExpectation(getTime()
							+ trainingTimeSeperator);
				}
				currentTimeIndex++;
			}
		}
	...
	private void getTrainingData() {
		try {
			...
			for (int i = lastTrainingIndex; i < timeSamples.length; i++) {
				...
				double countU = 0;
				double maxU = 0;
				double timePt = (timeIntervalLowerBound + timeIntervalUpperBound) / 2;

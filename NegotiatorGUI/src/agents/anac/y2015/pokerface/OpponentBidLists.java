...
public class OpponentBidLists {
	...
	public OpponentBidLists(UtilitySpace utilitySpace, boolean single_list) {
		...
		if(single_list) {
			Map<Pair<Integer,Value>,Integer> bid_list = new TreeMap<Pair<Integer,Value>,Integer>(new MapComparator());
	...
	public void insertBid(Object sender, Bid bid) {
		...
		for(int j=0;j<values.size();j++){
			...
			Pair<Integer,Value> pair = new Pair<Integer,Value>(issue_id,value_id);
	...
	public List<Entry<Pair<Integer,Value>, Integer>> getMostFrequentIssueValues(Object sender) {
		...
		Set<Entry<Pair<Integer,Value>,Integer>> pull_through = new TreeSet<Entry<Pair<Integer,Value>,Integer>>(new Comparator<Entry<Pair<Integer,Value>,Integer>>(){
			public int compare(Entry<Pair<Integer,Value>,Integer> e1, Entry<Pair<Integer,Value>,Integer> e2){
	...
	public List<Entry<Pair<Integer,Value>, Double>> weightIssueValues(List<Entry<Pair<Integer,Value>, Integer>> sorted_list) {
		...
		for(Entry<Pair<Integer,Value>, Integer> entry : sorted_list) {
			...
			try{
				...
				temp.setValue(issue_id, entry.getKey().getValue());
				evaluation = evaluator.getEvaluation(utilitySpace, temp, issue_id);
			} catch (Exception e) {
				evaluation = 1.0;
			}
			map.put(entry.getKey(), value*weight*evaluation);
		}
		List<Entry<Pair<Integer,Value>, Double>> weighted_list = new ArrayList<Entry<Pair<Integer,Value>, Double>>(((Map<Pair<Integer,Value>, Double>)map).entrySet());
		Collections.sort(weighted_list, new Comparator<Entry<Pair<Integer,Value>, Double>>()
		        {
	...
	private class MapComparator implements Comparator<Pair<Integer,Value>>{
		...
		public int compare(Pair<Integer,Value> p1, Pair<Integer,Value> p2){
			return (p1.getInteger() + p1.getValue().toString()).compareTo(p2.getInteger() + p2.getValue().toString());
		}
	}
}
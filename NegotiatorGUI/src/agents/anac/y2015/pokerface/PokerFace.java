...
public class PokerFace extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class> validActions) {
		...
		if (timeline instanceof ContinuousTimeline) {
			time_left = Math.floor((total_time - timeline.getCurrentTime())
					/ average_time);
			...
			time_after_walker = ((total_time - 1) * (1.0 - RANDOM_WALKER_TRESHOLD));
		...
		if (!validActions.contains(Accept.class)
				|| (last_utility < threshold && (time_left > 1 || first_to_offer == 1))) {
	...
	private Bid concederBid(
		...
		if (timeline instanceof ContinuousTimeline) {
			actions_left = (int) Math.round((total_time - current_time)
					/ average_time);
			actions_after_walker = (int) ((total_time / average_time) * (1.0 - RANDOM_WALKER_TRESHOLD));
		} else {
			actions_left = (int) total_time - rounds - 1;
			actions_after_walker = (int) (total_time * (1.0 - RANDOM_WALKER_TRESHOLD));
		}
	...
	private void updateTime() {
		...
		if (timeline instanceof ContinuousTimeline) {
			...
			last_rounds_time[rounds % MA_SIZE] = (timeline.getCurrentTime() - current_time)
					/ rounds;
			average_time += last_rounds_time[rounds % MA_SIZE] / MA_SIZE;

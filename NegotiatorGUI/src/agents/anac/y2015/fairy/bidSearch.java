...
{	
	...
	public Bid getBid(Bid baseBid, double threshold) {
		...
		try {
			...
			if (utilitySpace.getUtility(bid) < threshold) { bid = new Bid(maxBid); } // �T���ɂ���ē���ꂽBid��threshold�����������ꍇ�C�ő��p�lBid����Ƃ���
			return bid;
		} catch (Exception e) {
	...
	private Bid getBidbyAppropriateSearch(Bid baseBid, double threshold) {
		...
		try {
			...
			if(negotiatingInfo.isLinerUtilitySpace()){
				...
				if(utilitySpace.getUtility(bid) < threshold){ negotiatingInfo.utilitySpaceTypeisNonLiner(); } // �T���Ɏ��s�����ꍇ�C���`��p��ԗp�̒T���ɐ؂�ւ���
			} 
	...
	{	
		...
		while (currentTemperature > END_TEMPERATURE) { // ���x���\��������܂Ń��[�v
			...
			for (int i = 0; i < STEP_NUM; i++) { // �ߖT��Bid���擾����
				...
				if (maxBid == null || nextBidUtil >= utilitySpace.getUtility(maxBid)) { maxBid = new Bid(nextBid); } // �ő��p�lBid�̍X�V
			}
		...
		if (targetBids.size() == 0) { return new Bid(baseBid); } // ���E�l���傫�Ȍ�p�l������Bid��������Ȃ������Ƃ��́CbaseBid��Ԃ�
		else { return new Bid(targetBids.get(randomnr.nextInt(targetBids.size()))); } // ��p�l�����E�l�t�߂ƂȂ�Bid��Ԃ�
	}
}
...
public class GrandmaAgent extends AbstractNegotiationParty {
	...
	public Action chooseAction(List<Class<? extends Action>> validActions) {
		...
		if (Action.getBidFromAction(lastOfferedBidAction) != null) {
			Bid OppBid = Action.getBidFromAction(lastOfferedBidAction);
		...
		try {
			...
			if (validActions.contains(Accept.class) && isAcceptable(OppBidUtil)) {
				...
				lastOffsteps = 0;
				return new Offer(getBid());
			}
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("unable to check acceptance");
			return new Offer(getRandBid(lowBoundUtil));
		}
	}
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (Action.getBidFromAction(action) != null) {
			...
			lastBid = Action.getBidFromAction(lastOfferedBidAction);
			BH_update((bidNum % partiesN), lastBid);
		} else {
			lastOffsteps = lastOffsteps + 1;
	...
	public String getDescription() {
		return "You are all the same in grandma's eyes";
	}
	...
	private void Counter_update(Bid recBid) throws Exception {
		for (int k = 0; k < issueN; k++) {
			// INTEGERS:
			if (utilitySpace.getDomain().getIssue(k).getType().toString()
					.equals("INTEGER")) {
				// check if value outside bounds
				IssueInteger kIntIssue = (IssueInteger) utilitySpace
						.getDomain().getIssues().get(k);
				ValueInteger recBidInt = (ValueInteger) recBid.getValue(k + 1);
				Integer median = (kIntIssue.getLowerBound() + kIntIssue
						.getUpperBound()) / 2;
			...
			// DISCRETE:
			else if (utilitySpace.getDomain().getIssue(k).getType().toString()
					.equals("DISCRETE")) {
				boolean found = false;
				...
				System.out.println("Wrong Issue Type:"
						+ recBid.getValue(k + 1).getType());
			}
		}
	}
	...
	private void normaliseMean() {
		for (int i = 0; i < IssueAmounts.size(); i++) {
			// DISCRETE Issues
			if (utilitySpace.getDomain().getIssue(i).getType().toString()
					.equals("DISCRETE")) {
				double tot = 0;
			...
			else if (utilitySpace.getDomain().getIssue(i).getType().toString()
					.equals("INTEGER")) {
		...
		for (int i = 0; i < IssueNames.size(); i++) {
			...
			if (utilitySpace.getDomain().getIssue(i).getType().toString()
				...
				for (int k = 0; k < IssueNames.get(i).size(); k++) {
					...
					.equals("INTEGER")) {
				IssueInteger kIntIssue = (IssueInteger) utilitySpace
						.getDomain().getIssues().get(i);
				ValueInteger bidInt = (ValueInteger) bid.getValue(i + 1); // bid
																			// Integer
																			// value
				double nombidnorm = (double) (int) (bidInt.getValue() - kIntIssue
						.getLowerBound());
				double denbidnorm = (double) (int) (kIntIssue.getUpperBound() - kIntIssue
						.getLowerBound());
				Double bidNorm = nombidnorm / denbidnorm; // Normalised bid
															// Integer issue
															// value [0,1]
				double nomcountnorm = (double) (int) intIssCount.get(i).get(1);
				double dencountnorm = (double) (int) (intIssCount.get(i).get(0) + intIssCount
						.get(i).get(1));
	...
	private void lowBoundUtil_update(double lastUt) {
		...
		lowBoundUtil = (MIN_UTIL_BOUND + (1 - MIN_UTIL_BOUND) * discMeanUtil)
				* tfact;// *Math.pow(discMeanUtil, MEAN_POW_COEFF))*tfact;

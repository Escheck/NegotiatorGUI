...
public class AgentHP2_main extends AbstractNegotiationParty {
	...
	public void init(AbstractUtilitySpace utilitySpace, Deadline deadlines,
		...
		// 親クラス初期化メソッド
		super.init(utilitySpace, deadlines, timeline, randomSeed, agentID);
		...
		for (int i = 0; i < expectTimingNum; i++) {
			expectUtilitySpaceTimingArray[i] = (i + 1.0)
					/ (double) expectTimingNum;
		}
	...
	private double getAcceptableProbability() {
		...
		if (offeredUtility >= 0.90 || time >= 0.99
				|| offeredUtility >= underlimitUtility) {
			return 1.0;
		}
		...
		if (isExpectedUtilitySpace) {
			...
			// 提案の提案者から見たAHP評価値を取得
			offeredAHPEvaluation = AHPEvaluator.getAHPEvaluationForOpponents(
					targetID, targetBid);

			// 譲歩率を元に承諾確率計算
			double tmp1 = (Math.pow(time, 5.0) / (5.0 * discountFactor));
			tmp1 += (offeredAHPEvaluation - estimateAHPEvaluationMax[targetID])
					+ (offeredUtility - underlimitUtility);
		...
		for (int i = 0; i < createBidNum; i++) {
			...
			do {
				...
				if (--loop <= 0) {
					...
					&& (util < lowerLimitUtility || util > upperLimitUtility));
			tmpBidArray[i] = tmp;
		}
	...
	private void makeConcessionParameterByAHP() {
		...
		for (int i = 0; i < otherCount; i++) {
			...
			double average = sumAHPEvalationForOpponent[i]
					/ othersOfferCount[i];
			double average2 = sumAHPEvalationForOpponent2[i]
					/ othersOfferCount[i];
			double variance = average2 - average * average;

			// 交渉相手から引き出せるAHP評価値の予測最大
			estimateAHPEvaluationMax[i] = average
					+ ((1.0 - average) * Math.sqrt(12.0) * variance);

			// 予測最大値の小さい方(将来的に譲歩が少ない方)を保存(0除算警戒)
			if (minEstimateMax > estimateAHPEvaluationMax[i]
					&& estimateAHPEvaluationMax[i] > 0.0) {
		...
		// 提案作成用基準効用値作成
		double tmpUtility = 1.0
				- maximumConcessionDegree
				* Math.pow(time, (Math.pow(discountFactor, 3.0) / Math.pow(
						minEstimateMax, 1.0)));
	...
	private void updateAHPEvaluationSum(int partyID, Bid updateTarget) {
		...
		double ahpEval = AHPEvaluator.getAHPEvaluationForOpponents(partyID,
				updateTarget);

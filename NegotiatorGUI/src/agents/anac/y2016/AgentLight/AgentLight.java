...
public class AgentLight extends AbstractNegotiationParty {
	...
	public void init(AbstractUtilitySpace utilitySpace, Deadline deadline,
		...
		super.init(utilitySpace, deadline, timelineInfo, randomSeed, agentID);
		...
		try {
			for (int i = 0; i < possibleBidNum; i++) {
				...
				issueInst = Int2Bid(i);
				ComparableBid bid = new ComparableBid(new Bid(
						utilitySpace.getDomain(), issueInst));
				bidList.add(bid);
			}
	...
	public Action chooseAction(List<Class<? extends Action>> Actions) {
		...
		if (!agentOrder.contains(getPartyId())) {
			...
			if (agentOrder.indexOf(getPartyId()) == (opponentNum - 1))
				...
				nextOpponent = opponentInfo.get(agentOrder.get(agentOrder
						.indexOf(getPartyId()) + 1));
			}
		}
		recordTimes++;

		double remainingRounds = (timeline.getTotalTime() - timeline
				.getCurrentTime()) / (timeline.getCurrentTime() / recordTimes);
		if (recordTimes >= learningTimes) {
			...
			int worthyOpponentBidSize = worthyOpponent.getAgentBidHistory()
					.size();
			opponentCurBid = worthyOpponent.getAgentBidHistory().get(
					worthyOpponentBidSize - 1);
			// opponentPreBid =
		...
		if (isPrinting) {
			...
			// System.out.println("getTimeUtility: "+utilitySpace.getUtilityWithDiscount(lastBidInHistory,
			// timeline.getTime()));
			// System.out.println("currentTimeUtility: "+utilitySpace.getUtilityWithDiscount(lastBidInHistory,
			// timeline.getCurrentTime()));
		}

		double base = ((timeline.getTotalTime() - timeline.getCurrentTime()) / timeline
				.getTotalTime()) > 0.7 ? ((timeline.getTotalTime() - timeline
				.getCurrentTime()) / timeline.getTotalTime()) : 0.7;
		if (recordTimes >= learningTimes && worthyOpponent != null) {
			learningExp = ((worthyOpponent.getOpponentStandardDeviation())[1] + (worthyOpponent
					.getOpponentAverage())[1]);
		}
		minUtility = (reservationValue + (maxUtility - reservationValue)
				* Math.pow(base, learningExp))
				* discountFactor;
		...
		if (lastBidInHistory != null
				...
				|| (bestBidInPotential != null
						&& !myAcceptBidRecord.contains(lastBidInHistory)
						&& !bidPotential.contains(lastBidInHistory) && utilitySpace
						.getUtilityWithDiscount(lastBidInHistory,
								timeline.getTime()) >= utilitySpace
						.getUtilityWithDiscount(bestBidInPotential,
								timeline.getTime()))) {
			if (isPrinting) {
	...
	public OpponentInfo findOpponent(HashMap<AgentID, OpponentInfo> opponentInfo) {
		double minValue = 999;
		OpponentInfo minInfo = null;
		for (AgentID agentId : opponentInfo.keySet()) {
			int opponentBidSize = opponentInfo.get(agentId)
					.getAgentBidHistory().size();
			if (opponentBidSize >= 2) {
				...
				if (value < minValue
						&& (opponentInfo.get(agentId).getOpponentLastAction() instanceof Offer)
	...
	public Action LearningAction() {
		...
		if (bid != null) {
			...
			bidSupport.put(bid, 0);
			myAcceptBidRecord.add(bid);
			return (new Offer(getPartyId(), bidList.get(bidList.size() - 1).bid));
		}
	}
	...
	public Action NegotiationAction(double minUtility) {
		...
		if (recordTimes > 1) {
			...
			tftUtility = (utilitySpace.getUtility(opponentCurBid) - utilitySpace
					...
					* (worthyOpponent.getOpponentStandardDeviation()[1] + worthyOpponent
							.getOpponentAverage()[1]);
		...
		if (offerUtility < maxUtility
				- (utilitySpace.getUtility(worthyOpponent.getBestBid()) - utilitySpace
						.getUtility(worthyOpponent.getAgentBidHistory().get(0))))
			offerUtility = maxUtility
					- (utilitySpace.getUtility(worthyOpponent.getBestBid()) - utilitySpace
							.getUtility(worthyOpponent.getAgentBidHistory()
									.get(0)));
		...
		if (isPrinting) {
			// System.out.println("bestBidUtility: "+utilitySpace.getUtility(worthyOpponent.getBestBid()));
			// System.out.println("lastBidInHistor: "+utilitySpace.getUtility(lastBidInHistory));
			// System.out.println("bestBidInPotential: "+utilitySpace.getUtility(bestBidInPotential));
		...
		if (utilitySpace.getUtility(lastBidInHistory) > minUtility
				...
				&& !myAcceptBidRecord.contains(lastBidInHistory)
				|| (bestBidInPotential != null
						&& !myAcceptBidRecord.contains(lastBidInHistory) && utilitySpace
						.getUtilityWithDiscount(lastBidInHistory, time) > utilitySpace
						.getUtilityWithDiscount(bestBidInPotential, time))) {
			if (isPrinting) {
	...
	public Bid chooseBestBid(double minUtility) {
		...
		while (((ceil > 0) && (ceil < bidList.size() - 1))
				&& ((utilitySpace.getUtilityWithDiscount(bidList.get(ceil).bid,
						time) > minUtility && utilitySpace
						.getUtilityWithDiscount(bidList.get(ceil + 1).bid, time) > minUtility) || (utilitySpace
						.getUtilityWithDiscount(bidList.get(ceil).bid, time) <= minUtility && utilitySpace
						.getUtilityWithDiscount(bidList.get(ceil + 1).bid, time) <= minUtility))) {
			if (utilitySpace
					.getUtilityWithDiscount(bidList.get(ceil).bid, time) > minUtility) {
				...
				if ((int) Math.floor(bidList.size() * Math.pow(0.5, exp)) == 0) {
					ceil += 1;
				} else {
					ceil += (int) Math.floor(bidList.size()
							* Math.pow(0.5, exp));
				}
			}
			exp++;
	...
	public double getOppUtility(Bid bid) {
		...
		for (Issue issue : bid.getIssues()) {
			...
			case INTEGER:
				...
				double lowerWeight = utility += worthyOpponent
						.getIssueWeights().get(issue)[0];
				double upperWeight = utility += worthyOpponent
						.getIssueWeights().get(issue)[1];
				utility = lowerWeight
	...
	public void receiveMessage(AgentID sender, Action action) {
		...
		if (action != null) {
			if (action instanceof Inform
				...
				if (utilitySpace.getUtility(opponentInfo.get(sender)
						.getBestBid()) < utilitySpace.getUtility(bid)) {
					opponentInfo.get(sender).setBestBid(bid);
				}
				opponentInfo.get(sender).setOpponentLastAction(action);
	...
	private class ComparableBid implements Comparable<ComparableBid> {
		...
		public int compareTo(ComparableBid cBid) {
			double time = timeline.getTime();
			return (int) (utilitySpace.getUtilityWithDiscount(this.bid, time) * 1000)
					- (int) (utilitySpace
							.getUtilityWithDiscount(cBid.bid, time) * 1000);
		}
		...
		public void updateBid(Bid bid) {
			...
			for (Issue issue : bid.getIssues()) {
				...
				case DISCRETE:
					...
					if (recordTimes > 1
							&& bid.getValue(issue.getNumber()) == bidHistory
									.get(bidHistory.size() - 2).getValue(
											issue.getNumber())) {
						value.put(bid.getValue(issue.getNumber()), times
								+ frequencyReward + unchangeReward);
						valueWeights.put(issue, value);
					}
					value.put(bid.getValue(issue.getNumber()), times
							+ frequencyReward);
				...
				case INTEGER:
					...
					double midPoint = Math.ceil(lowerBound
					...
					if (midPoint > opponentValueInteger) {
						...
						double newLowEndEvaluation = lowerBoundWeight
								...
								* Math.pow(1 - timeline.getCurrentTime()
										/ timeline.getTotalTime(), learningExp);

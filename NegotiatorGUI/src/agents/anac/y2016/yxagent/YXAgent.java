...
public class YXAgent extends AbstractNegotiationParty {
	...
	public void receiveMessage(AgentID sender, Action opponentAction) {
		super.receiveMessage((AgentID) sender, opponentAction);
		...
		if (!possibleActions.contains(Accept.class)) {
			do {
				...
				myUtility = utilitySpace.getUtility(myLastBid);
													...
													// opponents.size = 0
			return myAction = new Offer(myLastBid);
		}
		...
		if (rounds > 10 && normalizedTime <= 0.9) {// Initialization of Value
			...
			for (Issue issue : issues) {
				...
				if (issue.getType().toString().equals("INTEGER")) {
					for (Value vv : oppValueFrequency.get(hardestOpp)
							.get(issue).keySet()) { // Usual way of key
													// retrieval of Hashmap does
													// not work for INTEGER
													// type, thats why need to
													// loop through
						if (Integer.parseInt(vv.toString()) == Integer
								.parseInt(v.toString())) {
							calUtil += oppIssueWeight.get(hardestOpp)
									.get(issue)
			...
			try {
				calThreshold = (calUtil - ((opponents.size() * deductThreshold) * 3 / 4));
				calThreshold = Math.max(tempThreshold, calThreshold);
				myAction = oppUtility > calThreshold ? new Accept()
						: new Offer(testBid);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			try {
				myAction = oppUtility > tempThreshold ? new Accept()
						: new Offer(testBid);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			for (Object o1 : oppIssueIntegerValue.keySet()) {
				for (Issue issue : issues) {
					if (issue.getType().toString().equals("INTEGER")) {
						...
						double average = (double) 1. / (upperB - lowerB);
						double adjustment;
						...
						if (sumLower > sumUpper) {
							...
							for (int i = lowerB + 1; i < upperB; i++) {
								...
								oppValueFrequency.get(o1).get(issue)
										.put(vI, adjustment);
							}
						} else {
	...
	private void updateModelOppIssueWeight(AgentID sender) {
		if (oppBidHistory.get(sender).size() != 0 && rounds >= 10) {
			Bid previousRoundBid = oppBidHistory.get(sender).get(
					oppBidHistory.get(sender).size() - 1);// -1 since current
															// lastOpponentBid
															// not added yet
			...
			for (Issue issue : issues) {
				Value prevIssueValue = previousRoundBid.getValue(issue
						.getNumber());
				Value currIssueValue = lastOpponentBid.getValue(issue
						.getNumber());
			...
			if (issueContainIntegerType) { // For Discrete & Integer Mix Domain
				if (!issue.getType().toString().equals("INTEGER")) {
					...
					oppValueFrequency
							...
							.get(issue)
							.put(value,
									oppValueFrequency.get(sender).get(issue)
											.get(value)
											+ valueWeightInteger);
				}
	...
	protected Value getRandomValue(Issue currentIssue) throws Exception {
		...
		case REAL:
			...
			index = randomnr.nextInt(lIssueReal
					.getNumberOfDiscretizationSteps());
			currentValue = new ValueReal(
					lIssueReal.getLowerBound()
							+ ((double) ((lIssueReal.getUpperBound() - lIssueReal
		...
		case INTEGER:
			...
			currentValue = new ValueInteger(lIssueInteger.getLowerBound()
					+ index);
			break;
		default:

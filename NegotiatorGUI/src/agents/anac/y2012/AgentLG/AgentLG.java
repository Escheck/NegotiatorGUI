 ...
 * In the beginning it votes in the beginning its best votes. 
 * when the time is 0-0.6 : it votes incrementally from its best utility to a minimum of 0.75* utility 
 * and learns opponent utility. 
 * when the time is 0.6-0.9: it starts to compromise and chooses better bid for the opponents that still good for it 
 * only if the opponent is also comprising (the rate of compromise depends on the opponent and on the discount factor). 
 * when the time over 0.9: the agents starts to "panic" and compromise more frequently. 
 * when the time is over(more then 0.9995 ) it chooses opponents max utility bid for it.
 * The Agent accept an offer only if it offered before by the agent or the time is almost over and the bid is close enough to its worst offer.
 * 
...
public class AgentLG extends Agent {
	...
	public void init() {
		...
		bidChooser = new BidChooser(this.utilitySpace,getAgentID(),oppenentsBid);
	}
	...
	public void ReceiveMessage(Action opponentAction)
	{
		if(opponentAction instanceof Offer) {
			...
			try {
				...
				oppenentsBid.getOpponentBidUtility (this.utilitySpace.getDomain(),oponnetLastBid);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}

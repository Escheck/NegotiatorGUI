	...
	public Action chooseAction()
	{
		...
		{ 
			...
			else if(actionOfPartner instanceof Offer)
			{
				...
				//System.out.println("* " + getName() + ":  Time: + " + this.timeline.getTime() + ", New threshold: " + this.threshold);
...
//				{
//					//if there is no time left, or the threshold is now lower than his best bid so far
...
//				}
	...
	/**
	 * Calculate threshold, all threshold suggested must exceed this threshold. We also accept all offers above this
	 * threshold.
	...
	private double calculateThreshold() throws Exception {
		...
		// bottom - The offer with max utility for the partner (we assume that this is the first bid he suggests).
		...
		// timeFactor - defines where we are on the range l. The thumb rule is that we start close to top and slowly
		// diverge to bottom.
		double timeFactor = getTimeFactor();
		
		// hardThreshold - The threshold which is somewhere between top and bottom.
	...
	/**
	 * Return a factor between 0.0 to 1.0 which defines how our threshold will be influenced by time & discount.
	 * @return
	 */
	...
	{	
		...
		private void BuildValuePreferences() throws Exception
		{
			...
			for (final Issue issue : allIssues)
			{
				...
				case REAL:
					...
					double step = (issueReal.getUpperBound()-issueReal.getLowerBound()) / (double)(issueReal.getNumberOfDiscretizationSteps());
				...
				Value value) throws Exception 
		{
			...
			double util = utilitySpace.getWeight(issue.getNumber()) * utilitySpace.getEvaluation(issue.getNumber(), bid);
		...
		private OfferStep GetOfferStepInIssue(Bid lastOffer, Issue issue)	throws Exception 
		{
			...
			case REAL:	
				...
				oldUtil = this.utilitySpace.getWeight(issue.getNumber()) * this.utilitySpace.getEvaluation(issue.getNumber(), bid);
				...
				newUtil = this.utilitySpace.getWeight(issue.getNumber()) * this.utilitySpace.getEvaluation(issue.getNumber(), bid);
				...
				double newUtilDown = this.utilitySpace.getWeight(issue.getNumber()) * this.utilitySpace.getEvaluation(issue.getNumber(), bid);
	...
	/**
	 * This class is responsible for maintaining the model of the opponent
	 * and for providing API functions to easily select bids offered to the 
	 * opponent
	 * 
	 * @author Ron
	 *

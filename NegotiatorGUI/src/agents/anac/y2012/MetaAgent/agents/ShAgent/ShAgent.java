...
public class ShAgent extends Agent {
		...
		try {
			if (actionOfPartner == null) {
				...
				// }
					...
					Bid ourOffer = utilAnalyzer.GetClosestOffer(partnerBid,
							threshold);
					action = new Offer(this.getAgentID(), ourOffer);
				}
			}
	...
	private void setBestBid(Bid bid) {
		if (this.partnerBestBid == null) {
			...
			double oldUtil = this
					.getUtilityWithoutDiscount(this.partnerBestBid);
	...
	private class UtilityAnalyzer {
		...
		private void BuildValuePreferences() throws Exception {
			...
			for (final Issue issue : allIssues) {
				...
				utilityPerValuePerIssue
						.put(issue, new HashMap<Value, Double>());
				...
				case REAL:
					...
					double step = (issueReal.getUpperBound() - issueReal
							.getLowerBound())
							/ (double) (issueReal
									.getNumberOfDiscretizationSteps());
					for (double dVal = issueReal.getLowerBound(); dVal <= issueReal
							.getUpperBound(); dVal += step) {
				...
				case INTEGER:
					...
					for (int i = issueInteger.getLowerBound(); i <= issueInteger
							.getUpperBound(); i++) {
				...
				Collections.sort(values, new Comparator<Value>() {
					...
					public int compare(Value o1, Value o2) {
						return Double.compare(utilityPerValuePerIssue
								.get(issue).get(o1), utilityPerValuePerIssue
								.get(issue).get(o2));
					}
				});
		...
		private Bid Clone(Bid source) {
			...
			try {
				...
				for (Issue issue : allIssues)
					map.put(issue.getNumber(),
							source.getValue(issue.getNumber()));
		...
		private OfferStep GetOfferStepInIssue(Bid lastOffer, Issue issue)
			...
			ArrayList<Value> issueValues = this.preferredValuesPerIssue
					.get(issue);
	...
	private class OpponentModel {
		...
		public OpponentModel(AdditiveUtilitySpace utilitySpace,
			...
			for (Issue issue : allIssues) {
				opponentPrefferedValuePerIssue.put(issue,
						new ArrayList<Value>());
			}
		}
		...
		public void receiveNewOpponentBid(Bid bid) {
			...
			for (Issue issue : allIssues) {
				try {
					...
					if (opponentPrefferedValuePerIssue.get(issue).contains(
							value)) {
		...
		private void manageTiming() {
			...
			previousTurnTimesArr[turnNumber % previousTurnTimesArr.length] = currTime
					- lastTurnStartTime;

...
public class DNAgent extends Agent {
	...
	public Action chooseAction()
	{
		...
		try 
		{ 
			if(actionOfPartner==null) action = proposeBid();
			if(actionOfPartner instanceof Offer)
			{
				...
				if (minimumOfferedUtil<=offeredUtilFromOpponent){
					//System.out.println("YO: "+myOfferedUtil+ "EL: "+offeredUtilFromOpponent+ " - Gladly accept opponent's bid");
	 ...
	 * @throws Exception 
	 */
	private ArrayList<Bid> performSelection(int selectionNumber) throws Exception{
	...
	private ArrayList<Bid> performMutation(ArrayList<Bid> parents, int mutationNumber) throws Exception{
	...
	private double getFitness(Bid bid) throws Exception{
		if (referenceBid==null){
			...
			if ((discount<=0)||(discount>=1))
				discount=1;
			double beta=Math.pow(1-myTime,(1.0/(3.5*discount*discount)));
			//System.out.println("Discountfactor: "+discount+" Time: "+myTime+" Beta: "+beta);
			return beta*utilitySpace.getUtility(bid)+(1-beta)*getSimilarity(bid,referenceBid)/utilitySpace.getDomain().getIssues().size();
		}
	}
    ...
    /**
	 * Generate a bid based on the current population. Uses tournament selection.
	 * @return new Action(Bid(..)), 
		...
		for(Issue lIssue:issues) 
		{
			switch(lIssue.getType()) {
				...
				case REAL:
					...
					similarity+=1-(Math.abs(((ValueReal)aBid.getValue(issueID)).getValue()-((ValueReal)anotherBid.getValue(issueID)).getValue())/(lIssueReal.getUpperBound()-lIssueReal.getLowerBound()));
					break;
				case INTEGER:
					IssueInteger lIssueInteger = (IssueInteger)lIssue;

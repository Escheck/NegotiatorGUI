...
public class MrFriendly extends Agent{
	...
	/**
	 * When current time fraction is smaller than this boundary, we execute strategy 1.
	 * This means that if we believe our opponent stalls, we won't give in until we are 
	 * in the last 10% of negotiations 
	 */
	...
	/**
	 * The (static) reservation value that we choose (no bids lower than this value are accepted)
	 */
	private static final double RESERVATION_VALUE = 0.3;
	
	/**
	 * The (dynamic) utility threshold above which we will always accept a bid. We keep updating this
	 * value and as time goes by, it should typically decrease slowly (but not during the first minute,
	 * where we try to model the opponent and don't give in much.. we start greedy)
	 */
	private double alwaysAcceptUtility;
	
	/**
	 * Keeps track of our opponent, estimates his issue weights and value preferences
	 */
	...
	/**
	 * Tells us wether or not we are negotiating on a domain where utility is discounted over time
	 */
	...
	public void init(){
		...
		try {
			alwaysAcceptUtility = getUtility(this.utilitySpace.getMaxUtilityBid());
			// If the utility of the highest bid possible is lower than the reservation value, don't use
			// it and set the minimum bid utility to zero.
			if(alwaysAcceptUtility < minimumBidUtility){
				minimumBidUtility = 0;
			}
		} catch (Exception e) {
			// If there are no possible bids in this domain, just set the always accept utility to 1.
			alwaysAcceptUtility = 1;
		}
		discountFactor = this.utilitySpace.getDiscountFactor();
		discountedDomain = (discountFactor != 0) && (discountFactor != 1);
	...
	public Action chooseAction(){
		...
		//TODO: at itexvscypress discount, top0.5% is only 2bids, maybe adjust to small domains..
		
		Bid bid = null;
		Action action = null;
		try 
		{
			Bid partnerBid = bidTable.getLastOpponentBid();
			if(partnerBid != null)
			{
				double offeredUtility = getUtility(partnerBid);
				//factor in discount factor on alwaysAcceptUtility
				double acceptNowUtil = 
					(discountedDomain) ? alwaysAcceptUtility*Math.pow(discountFactor,timeline.getTime()) 
									   : alwaysAcceptUtility;					
					
				if(offeredUtility>=acceptNowUtil || bidTable.weHaveOfferedThisBefore(partnerBid)){
					//just accept if utility offered is higher than the bid we offered that is lowest
...
//			if( timeline.getTime() < LAST_CHANCE_TIME_BOUNDARY){
				//if we're on a discounted domain, start giving in a bit more after 0.5 of the time (AND before .991 of the time)..
				...
				if(discountedDomain && timeline.getTime()>0.5){
						if( timeline.getTime() < FIRST_TIME_BOUNDARY &&
								(	bidTable.getNumberOfOpponentBids() < MIN_OPPONENT_BIDS_FOR_MODEL
									|| (opponentModel.isStalling(bidTable.getConsecutiveBidsDifferent())
											&& !this.weAreStalling())
								 )
						   ){
							//(we're in the first 90% of the time) AND (we don't have enough bids for our model yet, 
							//                                               OR (opponent stalls AND we dont) )
							bid = bidTable.getBestBid();
						}else{
							bid = bidTable.getBestBidUsingModel();
							// Remove the bid from the list(s) so we won't offer the same bid over and over again.
							bidTable.removeBid(bid);	
						}
					
				}else{
					//after 150 received bids we have a pretty good estimation of our opponents preference profile
					//and will use the model for selecting a bid. If our opponent stalls (no 150 bids within 2 minutes,
					//or keeps sending the same bids) we wait for the last 10% of time before we start using the model. 
					if( timeline.getTime() < FIRST_TIME_BOUNDARY &&
							(	bidTable.getNumberOfOpponentBids() < MIN_OPPONENT_BIDS_FOR_MODEL
								|| opponentModel.isStalling(bidTable.getConsecutiveBidsDifferent())
							 )
...
//			}else{ // We are in the last part of the session ( >= .991% of the time).
...
//			}
			...
			alwaysAcceptUtility = Math.max(alwaysAcceptUtility, getUtility(bid)); //update always_accept
			if(alwaysAcceptUtility < bidTable.getMinimumBidUtility()){ // Check if the always accept value isn't below the minimum bid utility.
				alwaysAcceptUtility = bidTable.getMinimumBidUtility();
			}
			
		} catch (Exception e) {
			System.out.println("Exception in chooseAction:"+e.getMessage());
			// If something goes wrong, we try to offer the best bid in the domain.
			try{

...
public class BidTable {
	 ...
	 * the bids from this list if we offer them. However, if the list is empty, we start all over
	 * again and the list will be full again.
	 */
	...
	/**
	 * Contains our utilities for the bids, mapped to the same index as the bids.
	 */
	private HashMap<Integer, Double> utilityTable;

	/**
	 * Contains estimated opponent utility for the bids, mapped to the same index as the bids.
	 */
	...
	/**
	 * The (dynamic) minimum utility that we want to accept from and offer to our opponent. This
	 * starts off the same value as our reservation value, but updates (increases) as we receive bids
	...
	/**
	 * Initializes tables, calls private methods that fill bid tables and utility tables
	 */
	...
	public void setMinimumBidUtility(double mbu){
		if(mbu <= 1.05 && mbu >= 0) minimumBidUtility = mbu; // The environment allows utilities higher than 1.
		else minimumBidUtility = 0;
	}
	...
	/**
	 * Gets a random bid from the top TOP_BIDS_PERCENTAGE % of our list. If TOP_BIDS_PERCENTAGE % is less than 2 bids it chooses
	 * random from the top 2.
	 * @return Bid
	 */
	public Bid getBestBid(){
		...
		if( index > -1 ) return topXBids.get( index );

		//if topFivePercentBids doesnt have any elements for whatever reason, just return the best possible bid
		return bidTable.get(this.getIndexByUtility(Collections.max(utilityTable.values())));
	}
	...
	public Bid getBestBidUsingModel(){
		...
		// If the OpponentModel isn't initialized properly, then just give back the best bid.
		...
		// Select the (unoffered) bids from the list with a utility higher than our desired value.
		// Return the one that maximizes the correspondence to the opponent preferences.
		...
		if(indexBestBid == -1){
			//apparently there were no adequate bids found, or something is wrong with the tables..
			...
			double desiredValue = lo+(OFFERED_UTIL_DECAY*(hi-lo)); //our next bid should be around this value
	...
	/**
	 * Searches for a bid that is good for us and also good for our opponent (as far as we know),
	 * in other words, search for a bid that we expect to be on or close to the Pareto optimal frontier
	 * 
	 * @return int
	 */
	private int getIndexParetoBid(){
		...
		double desiredValue = lo+(OFFERED_UTIL_DECAY*(hi-lo)); //our next bid should be around this value
		if(hi < lo) desiredValue = lo; // if lo somehow surpasses hi, we just look for lo
		
		//look for any Bid that gives us at least our desired value, and at the same time maximizes opponents utility
	...
	/**
	 * Updates our estimated opponent utility table with our current Opponent Model and our estimations for its
	 * preference profile
	 */
	private void updateEstimatedOpponentUtilityTable(){
		...
		for(Entry<Integer, Bid> entry : entryset){
			estimatedOpponentUtilityTable.put(entry.getKey(), opponentModel.getEstimatedUtility(entry.getValue()));
		}		
	}
	 ...
	 * This is necessary, because the function getIndexAroundUtility is able to find bids that are below the
	 * minimumBidUtility. Now we remove them, the bidTable would get empty and checkIfBidTableIsNotEmpty() will
	 * refill the bidTabel (and utilityTable) and we don't have to offer bids with a utility below minimumBidUtility.
	 * This method is called if and only if the minimumBidUtility has changed as a result of an opponent bid.
	 */
	public void updateBidTable() {
		// We keep track of the indexes to delete, because we can't delete them in the first for-loop.
		ArrayList<Integer> indexesToDelete = new ArrayList<Integer>();
		
		// Loop through all utilities. If a utility is lower than the minimumBidUtility, remove the corresponding bid
		// from the bidTable and add the index to the indexesToDelete.
		Set<Entry<Integer, Double>> entrySet = utilityTable.entrySet();
		for(Entry<Integer, Double> entry : entrySet) {
			// If the current entry has a utility lower than the minimumBidUtility, we shouldn't offer it any more,
			// so we remove it from the table.
			if (entry.getValue() < minimumBidUtility) {
				bidTable.remove(entry.getKey()); // Remove the Bid object from the bidTable.
				indexesToDelete.add(entry.getKey()); // Put this index integer in the table with utility indexes to delete. 
			}
		}
		
		// Delete the utility/utilities from the utility table and the opponent utility table if appropriate.
		for(Integer entry : indexesToDelete){
			utilityTable.remove(entry);
			if(estimatedOpponentUtilityTable.size()>0) estimatedOpponentUtilityTable.remove(entry); //if initiated, also delete from opponents utilitytable
		}
	}
	...
	public void removeBid(Bid bid) {
		...
		if(index != -1){
			...
			// The field counter is not lowered, because is has no function after filling the bidTable.
	 ...
	 * 
	 * @param action The last action from the opponent.
	 */
	public void addOpponentAction(Action action){
		...
		// Update the estimated opponent utility table now that the OpponentModel is updated.
		updateEstimatedOpponentUtilityTable();
		// Update the bestOpponentBid if necessary.
		if( bestOpponentBid == null || agent.getUtility(getLastOpponentBid()) > agent.getUtility(bestOpponentBid) ){
	...
	private void updateMinimumBidUtility() {
		...
		if(justReceivedBid != null){ // We only do this if this isn't the round that we start.
	...
	/**
	 * Same as above, but now we are not sure that 'utility' actually appears in the hashmap so
	 * we just seek the nearest bid; first we try within 0.01 margins, if we can't find anything,
	 * we increase the margins, and we keep doing that until we find a proper bid
	 * 
	 * @param utility
	 * @param margin
	 * @return
	...
	private Integer getIndexAroundUtility(Double utility, Double margin) {
		for (Entry<Integer, Double> entry : utilityTable.entrySet()) {
			if (entry.getValue() >= (utility-margin) && entry.getValue() <= (utility+margin)) {
				return entry.getKey();
			}
		}
	...
	/**
	 * Gives the best bid the opponent has offered. This value is updated by the addOpponentAction(Action) method.
	...
	/**
	 * This method refills the bidTable if it became empty. This is necessary, because we remove the bids we
	 * offered from the bidTable (in chooseAction()). When all possible bids (above the minimumBidUtility) are offered,
	 * bidTable is empty and it has to be refilled to enable the agent to do more offers.
	 */
	...
	/**
	 * Fill the array with all bids; exclude bids that are too expensive (>1200e) or too weak (utility <= reservation_value)
	 */
	 ...
	 * 
	 * @param issues ArrayList of all Issue objects in this domain
	 * @param depth The depth of recursion (should never be higher than the number of issues)
	 * @param valueset Hashmap containing the chosen values so far, for the first n (depth) issues
	 */
	private void recursiveArrayFiller(ArrayList<Issue> issues, int depth, HashMap<Integer, Value> valueset){		
		if(depth < issues.size()){
			...
			switch(theIssue.getType()) {
				...
				case INTEGER:
					...
					for(int i=theIssueInteger.getLowerBound(); i<=theIssueInteger.getUpperBound(); i++){
						valueset.put(theIssueInteger.getNumber(), new ValueInteger(i));
						recursiveArrayFiller(issues, (depth+1), valueset);
					}
					break;
			}
			...
			try {
				...
				HashMap<Integer, Value> valuesetClone = (HashMap<Integer, Value>)valueset.clone();
	...
	private void fillTopXHash(){
		...
		//total number of bids in our topTOP_BIDS_PERCENTAGE%, but at least 4, unless we only have less than 4 possible bids (very unlikely)
		int number = Math.max( (int)Math.ceil(bidTable.size()*TOP_BIDS_PERCENTAGE) , Math.min(bidTable.size(), 4));

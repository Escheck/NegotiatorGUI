...
/**
 * @author Yaron, Yulia, Lena
 * Some modifications to the standard Simple Agent.
 */
public class LYYAgent extends Agent
{
    ...
    private static double FIRST_DECISION_LEVEL  = 0.7;  // Utilities below this threshold are rejected.
    private static double SECOND_DECISION_LEVEL = 0.8;  // Two levels of random decision.
    private static double ACCEPT_OFFER_LEVEL    = 0.85; // Utilities above this threshold are accepted.
    ...
    public void ReceiveMessage(Action partnerAction) 
    {
        ...
        if (partnerAction instanceof Offer)
            System.out.println("LYY Agent: Got Offer");
        else if (partnerAction instanceof Accept)
            System.out.println("LYY Agent: Got Accept");
        else
    ...
    public Action chooseAction()
    {
        ...
        { 
            ...
            else if (actionOfPartner instanceof Offer)
            {
                ...
                // If the partner offered an unacceptable offer, return a random bid.
                else
                    action = chooseRandomBidAction();
            }
        } catch (Exception e) { 
    ...
    private boolean isAcceptable(double offeredUtililty, double time) throws Exception
    {
        ...
        if ((offeredUtililty < 0) || (offeredUtililty > 1.05)) throw new Exception("utility "+offeredUtililty+" outside [0,1]");
        if (( time <= 0) || (time > 1)) throw new Exception("time "+time+" outside [0,1]");

        // Reject low utilities.
        if (offeredUtililty < FIRST_DECISION_LEVEL) return false;
        
        // Accept high utilities.
        if (offeredUtililty >= ACCEPT_OFFER_LEVEL)  return true;
        ...
        if (offeredUtililty < SECOND_DECISION_LEVEL) return (rand < squredTime);
    ...
    private Bid getRandomBid() throws Exception
    {
        ...
        /*System.out.println("LYY Agent: getRandomBid - Start(time="+currTime+", diff="+utilDiff+", floor="+utilFloor+")");
        System.out.flush();*/
        ...
        do 
        {
            for(Issue lIssue:issues) 
            {
                ...
                case INTEGER:
                    ...
                    break;
                default: throw new Exception("issue type "+lIssue.getType()+" not supported by LYYAgent");
                }
            }
            bid=new Bid(utilitySpace.getDomain(),values);
        } while (getUtility(bid) < utilFloor);

        /*System.out.println("LYY Agent: getRandomBid - End (my bid util="+getUtility(bid)+")");
        System.out.flush();*/
        return bid;
    }
}

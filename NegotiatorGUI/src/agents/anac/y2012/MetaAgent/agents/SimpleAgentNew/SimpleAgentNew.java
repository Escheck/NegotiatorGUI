...
public class SimpleAgentNew extends Agent {
	...
	public Action chooseAction() {
		...
		try {
			...
			if (actionOfPartner instanceof Offer) {
				...
				offeredBidsCounterMap.put(partnerBid, offeredBidsCounterMap.get(partnerBid) + 1);
			...
			action = new Accept(this.getAgentID()); // best guess if things go
													// wrong.
		}
		return action;
	}
	...
	private Action chooseBidAction(double time) {
		...
		rejectedBidsCounterMap.put(nextBid,	rejectedBidsCounterMap.get(nextBid) + 1);
      ...
      if (utilitySpace.getUtility(nextBid) < utilitySpace.getUtility(opponentBid) )
	...
	private Bid getOpponentBestBid() {
		...
		for (int i=0; i<opponentBidsLst.size() && i < 5; i++){
			try {
				if (utilitySpace.getUtility(opponentBidsLst.get(i)) < utilitySpace.getUtility(maxBid))

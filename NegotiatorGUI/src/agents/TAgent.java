...
{	
   ...
   private static ArrayList<ValueDiscrete>      max_values = null;
    ...
    {
  		return (j==1) ? 0.5 * rvB/n + 0.5 : 0.5 * sq(bids(j-1)) + 0.5;   // adding scaling.
    }
  	//###############################################################################################
			...
			switch ( pie.getType() ) //  silly way to get/set rvB...
			{
				case DISCRETE:
				{
					...
					for ( i = 0 ; i < number_of_values ; i++ )
					{
			            ...
			            if (true)
			            		System.out.println( "######   i=" + i + "\t u(" + value.getValue() + ") = " + getUtility(new Bid(utilitySpace.getDomain(), values)) + "\t" + "  eval(" + value.getValue() + ") = " + evaluation );
					...
					{
						if ( stack_h.get(stack_h.size()-1).values().iterator().next().equals(stack_aux.get(stack_aux.size()-1).values().iterator().next()) )
							stack_h.pop();
						else
							stack_aux.push(stack_h.pop() );
					}
...
/********************************************************************************************************************/
    private int estimated_rounds_left(boolean opponent)
    {
        ...
        double totalDuration =  OpponentMaxDuration + OwnMaxDuration,
        		   round         =  (totalTime - timeline.getCurrentTime()) / totalDuration;
...
/********************************************************************************************************************/
	public static String getVersion() { return "1.0 (Genius 4.2)"; }
...
/********************************************************************************************************************/
	...
	{
		Bid bid = null;
        HashMap<Integer, Value> val = new HashMap<Integer, Value>(); // pairs <issuenumber,chosen value string>
       ...
       //if ( estimated_rounds_left(true) >= stack_aux.size() )    
        {
			...
			{
			     val.put(utilitySpace.getDomain().getIssues().get(0).getNumber(), key);
			     counter++;
				 break;
			}
			...
			System.out.println( 
						      "\n\t       t  =  " + timeline.getTime() 
						     + "\n\t    size  =  " + stack_hc.size()
							 + "\n\t getCurrentTime  =  " + timeline.getCurrentTime() * 1000 
							 + "\n\t getTotalTime    =  " + timeline.getTotalTime() * 1000 		 
							 + "\n\t estimate rounds left    =  " + estimated_rounds_left(false)); 
        }
        else // estimated_rounds_left(true) < stack_aux.size()  =>  bidding from the optimal subset
		{
		...
		if (unused)
		{
			if ( estimated_rounds_left(true) < 10  ) 
			{
				...
				if (true)
				{
					for ( Value key : stack_h.get(stack_h.size() - estimated_rounds_left(true)).keySet() )  
					//				for ( Value key : stack_h.get(0).keySet() )  
					     val.put(utilitySpace.getDomain().getIssues().get(0).getNumber(), key);
				}			
				else // , jump to the last cutoff (best)
				{
			        val.put(utilitySpace.getDomain().getIssues().get(0).getNumber(), (Value) max_values.get(index) );
					System.out.println( "  >>>>>  max_value[" + index + "] = " + max_values.get(index) );
					index++;				
				}

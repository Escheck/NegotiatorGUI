...
/**
 *
 * @author Operations related with the opponent's bid history
 */
public class OpponentBidHistory {
	...
	public void addBid(Bid bid, UtilitySpace utilitySpace) {
		// System.out.println("test in addBid of oppomodel 1" +
		// this.bid_maximum_from_opponent);
		if (bid == null) {
			System.out.println("Received bid is NUll.");
			return;
		}
		...
		try {
			if (bidHistory.size() == 1) {
				...
				if (utilitySpace.getUtility(bid) > utilitySpace
					...
					// System.out.println("test in addBid of oppomodel 2" +
					// this.bid_maximum_from_opponent);
				}
			}
		} catch (Exception e) {
	...
	public void updateOpponentModel(Bid bidToUpdate, Domain domain,
		...
		// if (this.bidHistory.size() <= this.maximumBidsStored) {
		// System.out.println("size of bidHistory,"+this.bidHistory.size());
		// this.updateStatistics(bidToUpdate, false, domain);
		// }
	}

...
public class AgentHP extends AbstractNegotiationParty {
	 ...
	 *
	 * @param validActions 選べる行動のクラスが格納されている(Accept or Offer)
	...
	public Action chooseAction(List<Class> validActions) {
		...
		//System.out.println("Util(Discount) = "+getUtilityWithDiscount(nextBid));
	 ...
	 *
	 * @param sender 行動者の情報
	 * @param action 行動内容(Accept,Offer,Inform)
	 */
	@Override
		...
		if(action.getClass() == Inform.class){
			...
			if(logNum[partyID] == logMax){
				...
				for(int i=0;i<issueNum;i++){
					TmpCountHash[i] = new HashMap<String,Integer>(TemplateLogBidCountHash[i]);
				}
				...
				for(int i=0;i<logMax;i++){
					for(int j=1;j<=issueNum;j++){
						try{
							if(TmpCountHash[j-1].containsKey(logBid[partyID][i].getValue(j).toString())){
								int tmp = TmpCountHash[j-1].get(logBid[partyID][i].getValue(j).toString());
								TmpCountHash[j-1].put(logBid[partyID][i].getValue(j).toString(),tmp + 1);	
							}
						}catch(Exception e){
				...
				for(int i=0;i<issueNum;i++){
					...
					if(tmp >= logMax){
						for (String key : logBidCountHash[partyID][i].keySet()){
							int count = TmpCountHash[i].get(key);
							if(count < logMax){
								TmpCountHash[i].put(key,(int)(logMax / compareSize)+1);
							}
						}
					}
				}
				...
				for(int i=0;i<issueNum;i++){
					...
					for (String key : logBidCountHash[partyID][i].keySet()){
						...
						logBidCountHash[partyID][i].put(key,(now_count + sum_count));
				...
				for(int i=0;i<issueNum;i++){
					...
					for(int j=0;j<=compareSize;j++){
						compareArray[j] = (int)((double)itemCountMax[i] * ((double)j / (double)compareSize));
					}
					...
					for (String key : logBidCountHash[partyID][i].keySet()){
						...
						for(int k=0;k<compareSize;k++){
							if(tmp >= compareArray[k] && tmp <= compareArray[k+1]){
								logBidValueHash[partyID][i].put(key,compareResultArray[k]);
								break;
							}
						}
					}
				...
				for(int i=0;i<=compareSize;i++){
					compareArray[i] = (int)((double)tmpMax * ((double)i / (double)compareSize));
				}
				double tmpWeightArray[] = new double[issueNum];
				for(int i=0;i<issueNum;i++){
					int tmp = itemCountMax[i];
					for(int j=0;j<compareSize;j++){
						if(tmp >= compareArray[j] && tmp <= compareArray[j+1]){
				...
				for(int i=0;i<issueNum;i++){
					for(int j=0;j<issueNum;j++){
						averageMatrix[i][j] = tmpWeightArray[j] / tmpWeightArray[i];
					}
				}
	...
	private double getAcceptableProbability(){
		...
		// 自分にとって好条件 or 時間ギリギリなら他条件に関わらず承諾確定
		...
		if(isEstimated(partyNum)){
			...
			tmp1 += (offeredUtility - estimateMax) + (offeredUtility - utilityBarometer);
	...
	private Bid generateOfferBid(){
		...
		for(int i=0;i<bidNum;i++){
			...
			}while(tmp != null && (util < tmpUnderlLimit || util > upperLimitUtility));
			tmpBid[i] = tmp;
		}
	...
	private void initHashArray(int partyNum){
		...
		for(int i=0;i<otherCount;i++){
			for(int j=0;j<issueNum;j++){
				logBidCountHash[i][j] = new HashMap<String,Integer>(TemplateLogBidCountHash[j]);
			}
		}
		...
		for(int i=0;i<otherCount;i++){
			for(int j=0;j<issueNum;j++){
				logBidValueHash[i][j] = new HashMap<String,Double>(TemplateLogBidValueHash[j]);
			}
		}
		...
		for(int j=1;j<=compareSize;j++){
			compareResultArray[j-1] = (2.0 * (double)j - 1.0) / (2.0 * (double)compareSize - 1.0);
		}
	...
	private void addupUtilSpace(double myWeight){
		...
		// 合算更新の必要がない &予測が作成済みでない場合はreturn 
		int timing = 0;
		...
		// 自分以外の参加者の重み
		double otherWeight = (1.0 - myWeight) / (double)(partyNum - 1);
		...
		for(Issue tmp : utilitySpace.getDomain().getIssues()){                      
			...
			// 論点の重み取得し重み付き格納
			addupWeightArray[issue_num - 1] = utilitySpace.getWeight(issue_num) * myWeight;
			
			// 論点内の項目事の値を取得
			IssueDiscrete tmpDiscrete = (IssueDiscrete)tmp;
			EvaluatorDiscrete evaluator = (EvaluatorDiscrete)utilitySpace.getEvaluator(issue_num);
			for(int j=0;j<tmpDiscrete.getNumberOfValues();j++){
				
				ValueDiscrete value = tmpDiscrete.getValue(j);
				try{
					addupValueHash[issue_num - 1].put(value.toString(),evaluator.getEvaluation(value) * myWeight);
				}catch(Exception e){e.printStackTrace();}
			}
		}
		...
		if(targetBid != null && partyID >= 0){
			...
			for(int i=1;i<=issueNum;i++){
				...
				try{
					...
					utility += (logBidValueHash[partyID][i-1].get(valName) * issueWeightArray[partyID][i-1]);
	...
	private void makeConcession(){
		...
		// 平均
		double mean = allSum / (double)round;
		
		// 分散
		double variance = Math.sqrt(((allSum2) / (double)round) - (mean * mean));
        ...
        // 歩み寄り後の最低効用値(仮)
        // 1:承諾判断用,2:提案作成用
        double tmpConsession1 = 1.0 - (Math.pow(time,alpha) * (1.0 - estimateMax));
        double tmpConsession2 = 1.0 - (Math.pow(time,beta) * (1.0 - estimateMax));

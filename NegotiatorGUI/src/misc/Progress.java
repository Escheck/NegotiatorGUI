...
/**
 * Contains the progress of something. This can be anything as long as it can be counted with an integer and has a
 * known integer end.
 *
 * For example we can count the progress in a tournament if we know that we doe n negotiations in that tournament.
 */
public class Progress {

    /** Holds the current state of progress, the last state of progress, and spinner state */
    private int current, total, spinnerState;

    /** Holds the nano time of when we started keeping progress */
    private long start;

    /** The spinner animation (can safely be changed to any other spinner animation of any length) */
     ...
     * To use timing (ETA), you additionally should call the .start() function as closely to the function to be timed as
     * possible.
     *
     * @param total The total amount of progress that can be made
     */
     ...
     *
     * @return True if current >= total, false otherwise
     */
     ...
     * @param includeSeconds if true, will print up to seconds. if false, will print up to minutes.
     * @return Time passed since .start() as a string (example: 2 hours, 5 minutes)
     */
    ...
    /**
     * Gets the progress string. Will return depending on the used options. use the constant values in this class to
     * generate the options. for example: getProgressString(Progress.ETA | Progress.CURRENT_TOTAL)
     * <p>
     * Progress.ETA: Report estimated time ahead
     * Progress.CURRENT_TOTAL: Report current/total
     * Progress.PERCENTAGE: Report percentage done
     * Progress.CHAR: Show spinner char
     * Progress.ALL: All of the above
    ...
    /**
     * Erases previously printed line and prints the progress string. Will print depending on the used options.
     * use the constant values in this class to generate the options.
     * for example: getProgressString(Progress.ETA | Progress.CURRENT_TOTAL)
     * <p>
     * Progress.ETA: Report estimated time ahead
     * Progress.CURRENT_TOTAL: Report current/total
     * Progress.PERCENTAGE: Report percentage done
    ...
    public void printProgressToConsole(int options) {
        try {
            String data = "\r                                                            "; // to make sure we overwrite

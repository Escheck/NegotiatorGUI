...
/**
 * Class which stores the Pareto-frontier. This class can be
 * used to easily add bids to the Pareto-frontier when
 * calculating it.
 * 
 * @author Tim Baarslag
 */
public class ParetoFrontier  {
	 ...
	 * A bid is added when it strictly dominates a bid in the Pareto-frontier
	 * OR when it is equal to a bid in the Pareto-frontier.
	 * 
	...
	public void mergeIntoFrontier(BidPoint bidpoint)
	{
		for (BidPoint f : frontier)
		{
			...
			if (f.isStrictlyDominatedBy(bidpoint)) { 
				// the bidpoint dominates a Pareto-bid, which is therefore replaced
				merge(bidpoint, f);
				return;
			}
		}
	 ...
	 * @param newParetoBid new Pareto bid.
	 * @param dominatedParetoBid old Pareto bid which must now be removed.
	 */
	private void merge(BidPoint newParetoBid, BidPoint dominatedParetoBid)
	{
		...
		for (BidPoint f : frontier)
		{
			if (f.isStrictlyDominatedBy(newParetoBid))		// delete dominated frontier points
				toBeRemoved.add(f);
		}
	 ...
	 * Pareto-frontier.
	 * 
	 * @param bid to be checked if it is Pareto-optimal.

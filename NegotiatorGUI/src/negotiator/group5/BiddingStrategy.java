 ...
 * 1) In the first round the agent will offer a bid with max utility for itself. 
 * 2) In the subsequent rounds, until a specific time has elapsed, the agent 
 *    will bid selfishly and only concede a tiny bit.
 *    This is because the opponent models are unreliable in the beginning.
 * 3) Afterwards, it will use a tit-for-tat strategy to hopefully reach a Nash point, 
 *    while increasingly concede utility in the later rounds.
 *    To achieve this it will interact with the opponent models to compare the estimated
 *    utilities of the opponents when deciding on bids.
 */
public class BiddingStrategy {
	// The minimum utility for our agent that we will consider when computing bids
	...
	public BiddingStrategy(Domain domain, 
			...
			List<Map<Integer, String>> valuesRev,
			Group5 agent) {
		this.domain = domain;
	...
	/**
	 * Computes all bids in the domain that have an utility of at least {@link #MIN_UTIL}.
	 */
	private void createAllBids() {
		// Count bids
		List<Integer> numValuesEach = new ArrayList<>();
		int numBids = 1;
		...
		List<List<Integer>> bidIndices = new ArrayList<>(Collections.nCopies(numBids, (List<Integer>) null));  
	...
	private Bid createBid(List<Integer> valueIndices) {
		HashMap<Integer, Value> v = new HashMap<>();
	...
	/**
	 * Creates a bid according to the bidding strategy
	 * See the documentation of this class above for a description of the bidding strategy
	 */
	public Bid generateBid() {
		...
		if (deadline != null) {
			//double c = 2.25;
			double c = 3.5;
			maxConcession = Math.exp((round-cutoff)/(deadline/c))/(1.5*Math.exp(c));
			agent.println("Max concession: " + maxConcession);
		}
		
		// 2.2) Do not use the opponent models in the beginning as they are unreliable
		if (round <= cutoff) {
			...
			agent.println("Made selfish move: " + agent.getUtility(b) + ", " + b);
			return b;
		}
		...
		double opponentConcession = (prevOpponentBidUtil-curOpponentBidUtil)/Math.abs(nashBidUtil-prevOpponentBidUtil);
		
		// 3.3) Mirror bid in our utility relative to Nash utility
		double lastBidUtil = agent.getUtility(lastBid);
		double concession = opponentConcession*Math.abs(nashBidUtil-lastBidUtil);
		...
		if (Double.isNaN(opponentConcession) || opponentConcession < 0.001) {
			...
			// then do not concede too much of our utility when offering the next bid
			concession = 0.01;
		}
		
		// 3.4) Make a nice move
		Bid niceMove = niceStepClosestsNash(lastBid, agent.getUtility(lastBid), concession);
		if (niceMove == null || niceMove.equals(lastBid)) {
			agent.println("Warning: could not make a new nice move (making selfish move instead): " + niceMove);
			return selfishStep(maxConcession);
		}
		agent.println("Made nice move: " + agent.getUtility(niceMove) + ", " + niceMove);
		return niceMove;
	}
	...
	/**
	 * Finds a bid with the maximal utility product
	 * Should be called at every round
	 */
	...
	/**
	 * Finds the bid closest to the estimated Nash product with an utility approximately equal
	 * to <code>curUtil</code> but with a difference of up to <code>concession</code>.
	 */
	private Bid niceStepClosestsNash(Bid lastBid, double curUtil, double concession) {

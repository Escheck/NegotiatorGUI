...
public class Group10 extends AbstractNegotiationParty {
	...
	public Group10(UtilitySpace utilitySpace, Deadline deadlines,
	...
	public Action chooseAction(List<Class> validActions) {
		...
		if (!validActions.contains(Accept.class)
				|| (last_utility < treshold && (time_left > 1 || first_to_offer == 1))) {
			if ((current_time / total_time) < RANDOM_WALKER_TRESHOLD) {
				...
				if ((int) time_left < 3) {
					...
					ArrayList<Entry<Pair<Integer, Value>, Integer>> pair_frequency = opponent_bid_list
							.getMostFrequentIssueValues(senders.get(0));
					ArrayList<Entry<Pair<Integer, Value>, Double>> weighted_list = opponent_bid_list
	...
	private Bid concederBid(
			ArrayList<Entry<Pair<Integer, Value>, Double>> ordered_pair) {
		...
		BigInteger steps_per_action = total_steps.divide(BigInteger
				.valueOf(actions_after_walker));
		BigInteger previous_step = current_step;
		current_step = steps_per_action.multiply(BigInteger
				.valueOf(actions_after_walker - actions_left));
		...
		for (BigInteger i = previous_step; i.compareTo(current_step) == -1; i = i

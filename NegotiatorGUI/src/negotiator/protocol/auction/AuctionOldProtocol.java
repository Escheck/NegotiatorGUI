...
public class AuctionOldProtocol extends Protocol {
	...
	protected void calculateTheoreticalOutcome() {
		try {
			...
			for (int i = 0; i < numberOfSellers; i++) {
				AbstractUtilitySpace centerUtilitySpace = getAgentUtilitySpaces(0);
				AbstractUtilitySpace sellerUtilitySpace = getAgentUtilitySpaces(1 + i);
				outcome[i] = Double.NEGATIVE_INFINITY;
				BidIterator iter = new BidIterator(
						centerUtilitySpace.getDomain());
				while (iter.hasNext()) {
					Bid bid = iter.next();
					if (Math.abs(sellerUtilitySpace.getUtility(bid)
							- sellerUtilitySpace
									.getReservationValueUndiscounted()) < ALLOWED_UTILITY_DEVIATION) {
						double lTmpExpecteUtility = centerUtilitySpace
								.getUtility(bid);
			...
			AbstractUtilitySpace centerUtilitySpace = getAgentUtilitySpaces(0);
			AbstractUtilitySpace sellerUtilitySpace = getAgentUtilitySpaces(1 + winner);
			...
			while (iter.hasNext()) {
				...
				if (Math.abs(centerUtilitySpace.getUtility(bid) - lSecondBest) < ALLOWED_UTILITY_DEVIATION) {
					...
					// pOppntBid);
					double lTmpExpecteUtility = sellerUtilitySpace
							.getUtility(bid);
			...
			SimpleElement theoreticalOutcome = new SimpleElement(
					"theoretical_outcome");
			theoreticalOutcome.setAttribute("winner",
					getProfileRepItems(1 + winner).getURL().getFile());
			theoreticalOutcome.setAttribute("center_utility",
					String.valueOf(finalOutcome[1]));
			theoreticalOutcome.setAttribute("seller_utility",
					String.valueOf(finalOutcome[0]));
			...
			for (int i = 0; i < numberOfSellers; i++) {
				...
				space.setAttribute("spaceA", getProfileRepItems(0).getURL()
						.getFile());
				space.setAttribute("spaceB", getProfileRepItems(1 + i).getURL()
						.getFile());
				SimpleElement solution = new SimpleElement("solution");
				optimalPoints.addChildElement(solution);
				solution.setAttribute("type", "Nash");
				BidSpace bidSpace = new BidSpace(getAgentUtilitySpaces(0),
						getAgentUtilitySpaces(i + 1));
				solution.setAttribute("utilityA",
						String.valueOf(bidSpace.getNash().getUtilityA()));
				solution.setAttribute("utilityB",
						String.valueOf(bidSpace.getNash().getUtilityB()));
				solution = new SimpleElement("solution");
				optimalPoints.addChildElement(solution);
				solution.setAttribute("type", "Kalai");
				solution.setAttribute("utilityA", String.valueOf(bidSpace
						.getKalaiSmorodinsky().getUtilityA()));
				solution.setAttribute("utilityB", String.valueOf(bidSpace
						.getKalaiSmorodinsky().getUtilityB()));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void run() {
		try {
			...
			for (int i = 0; i < numberOfSellers; i++) {
				sessions[i] = runNegotiationSession(getAgentRepItem(0),
						getAgentRepItem(i + 1), "Buyer", "Seller",
						getProfileRepItems(0), getProfileRepItems(i + 1),
						getAgentUtilitySpaces(0), getAgentUtilitySpaces(i + 1),
						getAgentParams(0), getAgentParams(i + 1));
			}
			// determine winner
			...
			paramsA.put(new AgentParameterVariable(new AgentParam(
					BayesianAgentForAuction.class.getName(), "role", -1., 1.)),
					new AgentParamValue(0.9));
			paramsA.put(new AgentParameterVariable(new AgentParam(
					BayesianAgentForAuction.class.getName(), "reservation", 0.,
					1.)), new AgentParamValue(lSecondPrice));
			...
			AuctionBilateralAtomicNegoSession secondPhaseSession = runNegotiationSession(
					getAgentRepItem(0),
					getAgentRepItem(1 + winnerSessionIndex), "Buyer", "Seller",
					getProfileRepItems(0),
					getProfileRepItems(1 + winnerSessionIndex),
					getAgentUtilitySpaces(0),
					getAgentUtilitySpaces(1 + winnerSessionIndex), paramsA,
					paramsB);
	...
	protected static AuctionOldProtocol createSession(Tournament tournament,
		...
		// Google creating generic array types in Java
		HashMap<AgentParameterVariable, AgentParamValue>[] params = new HashMap[3];
		params[0] = paramsA;
		params[1] = paramsB;
		params[2] = paramsB;
		AuctionOldProtocol session = new AuctionOldProtocol(agents, profiles,
				params, 1);
		return session;
	...
	private static ArrayList<Protocol> generateAllSessions(Tournament tournament) {
		...
		try {
			...
			// center profiles
			// ProfileRepItem center1 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_center_1.xml"),domain);
			...
			// ProfileRepItem center5 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_center_5.xml"),domain);
			// ProfileRepItem center6 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_center_6.xml"),domain);
			// ProfileRepItem center7 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_center_7.xml"),domain);
			...
			// ProfileRepItem center11 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_center_11.xml"),domain);
			// ProfileRepItem center12 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_center_12.xml"),domain);
			// seller profiles
			// ProfileRepItem seller1 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_1.xml"),domain);
			// ProfileRepItem seller2 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_2.xml"),domain);
			ProfileRepItem seller3 = new ProfileRepItem(new URL(
					"file:etc/templates/SON/son_seller_3.xml"), domain);
			// ProfileRepItem seller4 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_4.xml"),domain);
			// ProfileRepItem seller5 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_5.xml"),domain);
			ProfileRepItem seller6 = new ProfileRepItem(new URL(
					"file:etc/templates/SON/son_seller_6.xml"), domain);
			// ProfileRepItem seller7 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_7.xml"),domain);
			// ProfileRepItem seller8 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_8.xml"),domain);
			// ProfileRepItem seller9 = new ProfileRepItem(new
			// URL("file:etc/templates/SON/son_seller_9.xml"),domain);
			...
			allSessions.add(createSession(tournament, center10, seller3,
					seller6));
			// allSessions.add(createSession(tournament,center7, seller4,
			// seller7));
	...
	/**
	 * do test run of negotiation session. There may be multiple test runs of a
	 * single session, for isntance to take the average score. returns the
	 * result in the global field "outcome"
	 * 
	 * @param nr
	...
	protected AuctionBilateralAtomicNegoSession runNegotiationSession(
			Agent agentA, Agent agentB, AgentRepItem agentARepItem,
			AgentRepItem agentBRepItem, String agentAname, String agentBname,
			ProfileRepItem profileRepItemA, ProfileRepItem profileRepItemB,
...

	{
		...
		AuctionBilateralAtomicNegoSession sessionrunner = new AuctionBilateralAtomicNegoSession(
				this, agentA, agentB, agentAname, agentBname, spaceA, spaceB,
				agentAparams, agentBparams, "Buyer", 3600);
		fireBilateralAtomicNegotiationSessionEvent(sessionrunner,
				profileRepItemA, profileRepItemB, agentARepItem, agentBRepItem,
				"", "");
		if (TournamentConfiguration.getBooleanOption("protocolMode", false)) {
			...
			if (agentA.isUIAgent() || agentB.isUIAgent())
				totalTime = non_gui_nego_time;
			else
				totalTime = gui_nego_time;
			negoThread = new Thread(sessionrunner);
			System.out.println("nego start. " + System.currentTimeMillis()
					/ 1000);
			negoThread.start();
			try {
				synchronized (this) {
	...
	protected AuctionBilateralAtomicNegoSession runNegotiationSession(
			AgentRepItem agentARepItem, AgentRepItem agentBRepItem,
			String agentAname, String agentBname,
			ProfileRepItem profileRepItemA, ProfileRepItem profileRepItemB,
			AbstractUtilitySpace spaceA, AbstractUtilitySpace spaceB,
			HashMap<AgentParameterVariable, AgentParamValue> agentAparams,
			HashMap<AgentParameterVariable, AgentParamValue> agentBparams)
			throws Exception {
		java.lang.ClassLoader loaderA = ClassLoader.getSystemClassLoader()/*
																		 * new
																		 * java
																		 * .net.
																		 * URLClassLoader
																		 ...
																		 * })
																		 */;
		Agent agentA = (Agent) (loaderA.loadClass(agentARepItem.getClassPath())
				.newInstance());
		agentA.setName(agentAname);

		java.lang.ClassLoader loaderB = ClassLoader.getSystemClassLoader();
		Agent agentB = (Agent) (loaderB.loadClass(agentBRepItem.getClassPath())
				.newInstance());
		agentB.setName(agentBname);

		return runNegotiationSession(agentA, agentB, agentARepItem,
				agentBRepItem, agentAname, agentBname, profileRepItemA,
				profileRepItemB, spaceA, spaceB, agentAparams, agentBparams);

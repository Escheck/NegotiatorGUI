...
public abstract class BilateralAtomicNegotiationSession implements Runnable {
	...
	public BilateralAtomicNegotiationSession(Protocol protocol, Agent agentA,
		...
		String domainName = "";
		if (domain == null)
			System.err.println("Warning: domain null in "
					+ spaceA.getFileName());
		else
			domainName = domain.getName();
		matchDataLogger = new CSVlogger(Global.getOQMOutcomesFileName(),
				agentA.getName(), spaceA.getFileName(), agentB.getName(),
				spaceB.getFileName(), domainName);
	...
	protected synchronized void fireNegotiationActionEvent(Agent actorP,
			Action actP, int roundP, long elapsed, double time, double utilA,
			double utilB, double utilADiscount, double utilBDiscount,
			String remarks, boolean finalActionEvent) {
		for (NegotiationEventListener listener : actionEventListener) {
			ActionEvent event = new ActionEvent(this, actorP, actP, roundP,
					elapsed, time, utilA, utilB, utilADiscount, utilBDiscount,
					remarks, finalActionEvent);
	...
	/**
	 * Used by
	 * {@link AlternatingOffersBilateralAtomicNegoSessionSeparateTimelines}.
	 * Does not use time.
	 */
	@Deprecated
	protected synchronized void fireNegotiationActionEvent(Agent actorP,
			Action actP, int roundP, long elapsed, double utilA, double utilB,
			double utilADiscount, double utilBDiscount, String remarks,
			boolean finalActionEvent) {
		for (NegotiationEventListener listener : actionEventListener) {
			listener.handleActionEvent(new ActionEvent(this, actorP, actP,
					roundP, elapsed, -1, utilA, utilB, utilADiscount,
	...
	/**
	 * Checks the space. If not {@link AdditiveUtilitySpace} we return 0 anyway.
	 * 
	 * @param pAgent
	 * @param pIssueID
	 * @return
	 * @throws Exception
	 */
	public double getOpponentWeight(Agent pAgent, int pIssueID)
			throws Exception {

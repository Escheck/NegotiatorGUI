...
public class Group6 extends AbstractNegotiationParty {
	 ...
	 */
	public Group6(UtilitySpace utilitySpace, Deadline deadlines,
	 ...
	 *             outside boundaries
	 */
	private double calculateMinimalAcceptableUtility(double time,
			double reservationValue) throws Exception {
		if (time < 0 || time > 1)
	...
	public void receiveMessage(Object sender, Action action) {
		...
		if (bid != null) {
			...
			if (!opponentModels.containsKey(sender))
				opponentModels
						.put(sender, new FrequencyAnalysisOpponentModel());
		...
		for (Issue issue : this.utilitySpace.getDomain().getIssues()) {
			...
			for (IOpponentModel oppModel : opponentModels.values()) {
				...
				if (opponentValues.get(oppModel.getValue(issue.getNumber())) != null)
					n = opponentValues
							.get(oppModel.getValue(issue.getNumber()));
	...
	private Bid generateBid() throws Exception {
		...
		for (Issue issue : issues) {
			if (majorityValues.get(issue).contains(
					maxBid.getValue(issue.getNumber()))) {
		...
		for (Issue leastImportantIssue : issues) {
			...
			// bid
			if (ourPreviousBid.getValue(leastImportantIssue.getNumber())

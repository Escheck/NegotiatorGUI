...
public class BRAMAgent_Offering extends OfferingStrategy {
	...
	/* FINAL VARIABLES */
	private final int TIME_TO_CREATE_BIDS_ARRAY = 2000;//The time that we allocate to creating the bids array
    ...
    private  ArrayList<Bid> opponentBidsArray;//An Array that contains the last 10 bids that the opponent agent offered
	...
	public void initializeAgent(NegotiationSession negoSession, OpponentModel om, OMStrategy oms) {
	    ...
	    try {
			...
			maxUtility =  negotiationSession.getUtilitySpace().getUtilityWithDiscount(bestBid, negotiationSession.getTimeline());
	...
	public BidDetails determineNextBid() {
		round++;
		threshold = ((BRAMAgentSAS) helper).getNewThreshold(ourBidsArray.get(ourBidsArray.size()-1), bestBid);//Update the threshold according to the discount factor
	...
	private void updateStatistics(Bid bidToUpdate, boolean toRemove)
	{
		try{
			...
			for(Issue lIssue:issues) 
			{
				...
				{
					case DISCRETE:
						if (opponentBidsStatisticsDiscrete != null && opponentBidsStatisticsDiscrete.get(discreteIndex) != null)
						{
	...
	private Bid getBidToOffer() 
	{
		...
		try{
			...
			for (int i=0; i<10; i++) 
			{
				...
				if (currBid != null){
					double currUtility =  negotiationSession.getUtilitySpace().getUtilityWithDiscount(currBid, negotiationSession.getTime());
		   ...
		   for(Issue lIssue:issues) 
		    {
				...
				case REAL:
		            ...
		            ValueReal value = new ValueReal(lIssueReal.getLowerBound() + lOneStep*lOptionIndex + random200.nextDouble()*lOneStep);

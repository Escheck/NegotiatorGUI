...
public class HardHeaded_Offering extends OfferingStrategy {
	...
	public void initializeAgent(NegotiationSession negoSession, OpponentModel model, OMStrategy oms){
		...
		BSelector = new BidSelector(negotiationSession.getUtilitySpace());
	...
	public BidDetails determineNextBid() {
		...
		if (!negotiationSession.getOpponentBidHistory().getHistory().isEmpty()){
			...
			try{
				...
				else if(negotiationSession.getUtilitySpace().getUtility(opponentLastBid) >
				negotiationSession.getUtilitySpace().getUtility(opponentbestbid)) {
					opponentbestbid=opponentLastBid;
				}
	
				opbestvalue=BSelector.getBidList().floorEntry(negotiationSession.getUtilitySpace().getUtility(opponentbestbid)).getKey();
		...
		if (firstRound)
		{
			...
			potentialNextBid = BSelector.getBidList().lowerEntry(negotiationSession.getOwnBidHistory().getLastBidDetails().getMyUndiscountedUtil());				
...
//						}
						...
						try {
							bestBid = new MyEntry<Double, Bid>(negotiationSession.getUtilitySpace().getUtility(selectedBid.getBid()), selectedBid.getBid());
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
		...
		if (offerQueue.isEmpty() || offerQueue == null) {
			...
			try {
				if (opponentLastBid != null
						&& negotiationSession.getUtilitySpace().getUtility(bestBid1) <= negotiationSession.getUtilitySpace().getUtility(opponentLastBid.getBid())) {
					return null;
				} else {
					BidDetails offer = new BidDetails(bestBid1, negotiationSession.getUtilitySpace().getUtility(bestBid1), negotiationSession.getTime());

...
public class TheNegotiator_Offering extends OfferingStrategy{
	...
	public double getUpperThreshold(double threshold, double percentage) {
		int boundary = 0;
		while (boundary < ((TheNegotiatorSAS)helper).getPossibleBids().size() && ((TheNegotiatorSAS)helper).getPossibleBids().get(boundary).getMyUndiscountedUtil() >= threshold) {
			boundary++;
		}
		if (boundary > 0)
			boundary--;
	...
	public Bid getOwnBidBetween(double lowerThres, double upperThres, int counter) {
		...
		if (lB == uB) {
			if (counter == 1) {
				return ((TheNegotiatorSAS)helper).getPossibleBids().get(0).getBid(); // safe fallback value
			}
	...
	public Bid getBestPartnerBids(double threshold) {
		ArrayList<BidDetails> temp = (ArrayList<BidDetails>) new ArrayList<BidDetails>(negotiationSession.getOpponentBidHistory().getHistory()).clone();
		if (TEST_EQUIVALENCE) {

...
public class OMACagent_Offering extends OfferingStrategy {
	...
	public OMACagent_Offering(NegotiationSession negoSession,
	...
	public BidDetails determineNextBid() {
		...
		try {
			if (negotiationSession.getOpponentBidHistory().size() == 0) {
				...
				Bid partnerBid = negotiationSession.getOpponentBidHistory()
						.getLastBid();
				double offeredUtilFromOpponent = getUtility(partnerBid);
				mBidHistory.addOpponentBidnTime(offeredUtilFromOpponent,
						partnerBid, cTime);
	...
	private Bid getFinalBid() {
		...
		if (discount >= discountThreshold) {
			...
			if (cTime <= discount / splitFactor) {
				...
				if (opponentModel instanceof NoModel) {
					...
					Range range = new Range(val * (1.0 - 0.02), val
							* (1.0 + 0.02));
					bid = omStrategy.getBid(outcomespace, range).getBid();
				}
				return bid;
			} else if (cTime > 0.01 * (tCount + (int) Math.floor(discount
					/ splitFactor * 100))) {
				nextUti = getNextUti();
				tCount++;
			}
		...
		if (nextUti == -3.0) {
			...
			val = est_mu + (cTime - est_mt) * (est_u - est_mu)
					/ (est_t - est_mt);
		} else if (nextUti == -1.0) {
			val = getOriU(cTime);
		}
		...
		if (cTime > 0.999
				&& getUtility(mBidHistory.bestOppBid) > MINIMUM_UTILITY
						* Math.pow(discount, cTime)) {
	...
	private double getPre() {
		...
		exma = ma[lenA - 1] + avg(res) + std(res)
				* (1.0 - Math.pow(maxList[lenA - 1], 4))
				* (1.3 + 0.66 * Math.pow(1 - cTime * cTime, 0.4));
	...
	public static double std(double[] arr) {
		...
		std = ((double) len / (double) (len - 1.0))
				* (ssum / (double) len - Math.pow(avg(arr), 2));

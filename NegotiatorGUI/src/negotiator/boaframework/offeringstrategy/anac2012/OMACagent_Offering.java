...
public class OMACagent_Offering extends OfferingStrategy{
	...
	private double EU = 0.95;   // a threshold of expected utility for a session, in use now 
	...
	private Bid chooseBidAction() 
	{
		...
		if (possibleNextBid == null){			
			//return (new Accept(getAgentID())); //need to break off negotiation now
			endNegotiation = true;
	...
	{		
		...
		if(bid == null)
			...
			if( getUtility(mBidHistory.bestOppBid) >= getUtility(bid)){
				//System.out.println("Decoupled bestOppBid1: " + mBidHistory.bestOppBid);
				return mBidHistory.bestOppBid;
			}
		
		if( cTime > 0.999 && getUtility(mBidHistory.bestOppBid) > MINIMUM_UTILITY*Math.pow(discount, cTime)){
			//System.out.println("Decoupled bestOppBid2: " + mBidHistory.bestOppBid);
	...
	private double getPre(){
		...
		int lenA = (int)Math.floor(cTime*100); //don't count the current second as it is not complete		
		if(lenA < len){
			return -1.0;
		}
		...
		exma = ma[lenA-1]+ avg(res)+ std(res)*(1.0-Math.pow(maxList[lenA-1],4))*(1.3+0.66*Math.pow(1-cTime*cTime,0.4));

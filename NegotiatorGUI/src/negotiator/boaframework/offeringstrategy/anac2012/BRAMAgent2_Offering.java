...
public class BRAMAgent2_Offering extends OfferingStrategy {
	...
	private Bid getBidToOffer() 
	{
		...
		try{
			...
			//if (maxUt > endNegotiationUtility){	
				for (int i=0; i<10; i++) 
				{
					...
					if (currBid == null){
						...
						//System.out.println(" BRAM Decoupled - currBid: " + currBid.toString());
						double currUtility =  utilitySpace.getUtilityWithDiscount(currBid, negotiationSession.getTimeline());
			...
			//}
			//else{
			//	return null;//The agent can't make a good enough bid and returns null to end the negotiation - won't get here
			//}
		}catch (Exception e) {
	...
	private Bid selectCurrentBidFromOurBidsArray(){
		...
		//ADDED *********************************//
		if (this.utilitySpace.getUtilityWithDiscount(toSend, negotiationSession.getTimeline())<((BRAMAgentSAS)helper).getThreshold()){

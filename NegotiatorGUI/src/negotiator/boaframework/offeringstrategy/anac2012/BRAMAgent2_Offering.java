...
public class BRAMAgent2_Offering extends OfferingStrategy {
	...
	public BRAMAgent2_Offering(NegotiationSession negoSession,
	...
	private Bid getBidToOffer() {
		...
		try {
			double maxUt = ((BRAMAgentSAS) helper).getThreshold();
			// System.out.println("Decoupled Threshold: " + maxUt);
			...
			for (int i = 0; i < 10; i++) {
				...
				if (currBid == null) {
					...
					// System.out.println(" BRAM Decoupled - currBid: " +
					// currBid.toString());
					double currUtility = utilitySpace.getUtilityWithDiscount(
							currBid, negotiationSession.getTimeline());
	...
	private Bid selectCurrentBidFromOurBidsArray() {
		...
		if (this.utilitySpace.getUtilityWithDiscount(toSend,
				negotiationSession.getTimeline()) < ((BRAMAgentSAS) helper)
				.getThreshold()) {

...
public class AC_CombiMaxInWindowDiscounted extends AcceptanceStrategy {
	...
	public Actions determineAcceptability() {
		...
		if (recentBids.size() > 0){
			BidDetails maxDiscountedBidDetail = recentBids.getBestDiscountedBidDetails(negotiationSession.getUtilitySpace());
			max = negotiationSession.getDiscountedUtility(maxDiscountedBidDetail.getBid(), maxDiscountedBidDetail.getTime());
		} else
			max = 0;

...
public final class AC_TheFawkes extends AcceptanceStrategy
{
    ...
    @Override
    public void init( NegotiationSession nSession, OfferingStrategy biddingStrategy, OpponentModel oppModel, HashMap<String, Double> params ) throws Exception
    {
        ...
        this.minimumAcceptable = total / (double)allBids.size(); // TODO: use median instead of average perhaps?!
    }
    ...
    public Actions determineAcceptability()
    {
        ...
        {
            //Group3_Agent.debug( "REJECT (ACavg) (" + lastOpponentBidUtility + "<" + this.minimumAcceptable + ")" );
            return Actions.Reject;
        }
        else if( lastOpponentBidUtility >= myNextBidUtility )
        {
           // Group3_Agent.debug( "ACCEPT (ACnext) (" + lastOpponentBidUtility + ">=" + myNextBidUtility + ")" );
        ...
        else if( this.negotiationSession.getTime() >= ( 1 - this.OM.getMaxOpponentBidTimeDiff() ) )
        {
            ...
            BidDetails bestOpponentBid = this.negotiationSession.getOpponentBidHistory().filterBetweenTime( time - ( this.OM.getMaxOpponentBidTimeDiff() * 10 ), time ).getBestBidDetails();
            double bestOpponentBidUtility = bestOpponentBid.getMyUndiscountedUtil();
            if( lastOpponentBidUtility >= bestOpponentBidUtility )
            {
                //Group3_Agent.debug( "ACCEPT (ACtime&&ACconst) (" + this.negotiationSession.getTime() + ">=" + ( 1 - this.OM.getMaxOpponentBidTimeDiff() )
                //                    + "&&" + lastOpponentBidUtility + ">=" + bestOpponentBidUtility + ")" );
                return Actions.Accept;
            }
            else
            {
                //Group3_Agent.debug( "REJECT (ACtime&&ACconst) (" + this.negotiationSession.getTime() + ">=" + ( 1 - this.OM.getMaxOpponentBidTimeDiff() )
                //                    + "&& NOT(" + lastOpponentBidUtility + ">=" + bestOpponentBidUtility + "))" );
                return Actions.Reject;
            }
        }
        else
        {
            //Group3_Agent.debug( "REJECT (ACnext) (NOT(" + lastOpponentBidUtility + ">=" + myNextBidUtility + "))" );
            return Actions.Reject;
        }
    }
}

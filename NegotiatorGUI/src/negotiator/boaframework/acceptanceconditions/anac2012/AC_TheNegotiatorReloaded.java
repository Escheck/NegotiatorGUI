...
public class AC_TheNegotiatorReloaded extends AcceptanceStrategy {
	 ...
	 *            mode
	 */
	public AC_TheNegotiatorReloaded(NegotiationSession negoSession,
			OfferingStrategy strat, double a, double b, double ad, double bd,
			double c, double t) {
		this.aNext = a;
		this.bNext = b;
	...
	public void init(NegotiationSession negoSession, OfferingStrategy strat,
		...
		if (parameters.get("a") != null || parameters.get("b") != null
				|| parameters.get("ad") != null || parameters.get("bd") != null
		...
		System.out.println("Domain size: "
				+ negoSession.getDomain().getNumberOfPossibleBids());
	}
	...
	public String printParameters() {
		return "[a: " + aNext + " b: " + bNext + "ad: " + aNextDiscount
				+ " bd: " + bNextDiscount + " time: " + time + " constant: "
				+ constant + "]";
	}
	...
	public Actions determineAcceptability() {
		...
		double nextMyBidDiscountedUtil = negotiationSession.getUtilitySpace()
				.getUtilityWithDiscount(offeringStrategy.getNextBid().getBid(),
						now);
		double lastOpponentBidUtil = negotiationSession.getOpponentBidHistory()
				.getLastBidDetails().getMyUndiscountedUtil();
		double currentRV = negotiationSession.getUtilitySpace()
				.getReservationValueWithDiscount(now);
		...
		if (PANIC_PHASE && discountMode && now > time) {
			...
			BidHistory recentBids = negotiationSession.getOpponentBidHistory()
					.filterBetweenTime(now - window, now);
			double max;
	...
	public Set<BOAparameter> getParameters() {
		...
		set.add(new BOAparameter("a", new BigDecimal(1.0),
				"Scaling factor for AC_next when no discount"));
		set.add(new BOAparameter("b", new BigDecimal(0.0),
				"Constant factor for AC_next when no discount"));
		set.add(new BOAparameter("ad", new BigDecimal(1.05),
				"Scaling factor for AC_next when discount"));
		set.add(new BOAparameter("bd", new BigDecimal(0.0),
				"Constant factor for AC_next when discount"));
		set.add(new BOAparameter("c", new BigDecimal(0.98),
				"Constant specifying utility threshold"));
		set.add(new BOAparameter("t", new BigDecimal(0.99),
				"Time after which AC_max is used"));

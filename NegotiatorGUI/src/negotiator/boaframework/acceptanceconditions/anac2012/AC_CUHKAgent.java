 ...
 * The code was taken from the ANAC2012 CUHKAgent and adapted to work within the BOA framework.
 * 
...
public class AC_CUHKAgent extends AcceptanceStrategy {
	                ...
	                if (negotiationSession.getOwnBidHistory().isEmpty()) {
	                    ...
	                    if (((CUHKAgentSAS) helper).estimateTheRoundsLeft(false,true) > 10) {//still have some rounds left to further negotiate (the major negotiation period)
	                       ...
	                       //we expect that the negotiation is over once we select a bid from the opponent's history.
	                        ...
	                        if (negotiationSession.getTimeline().getTime() > 0.9985 && ((CUHKAgentSAS) helper).estimateTheRoundsLeft(false,true) < 5) {
	...
	private Actions regularDetermineAcceptability(){
		if(activeHelper){
			...
			previousTime = currentTime;
			  ...
			  if (((CUHKAgentSAS) helper).estimateTheRoundsLeft(true,true) > 10) {//still have some rounds left to further negotiate (the major negotiation period)
        ...
        if (roundsLeft > 10) {//still have some rounds left to further negotiate (the major negotiation period)
    ...
    private boolean AcceptOpponentOffer(Bid opponentBid, Bid ownBid) {
        ...
        if (currentUtility >= ((CUHKAgentSAS) helper).getUtilitythreshold() || currentUtility >= nextRoundUtility) {
            ...
            double currentMaximumUtility = this.utilitySpace.getUtilityWithDiscount(negotiationSession.getOpponentBidHistory().getBestBidDetails().getBid(), negotiationSession.getTimeline());
            if (currentMaximumUtility > predictMaximumUtility && negotiationSession.getTime() >  ((CUHKAgentSAS) helper).getConcedeToDiscountingFactor()) {
                try {
                    //if the current offer is approximately as good as the best one in the history, then accept it.
                    if (utilitySpace.getUtility(opponentBid) >= utilitySpace.getUtility(negotiationSession.getOpponentBidHistory().getBestBidDetails().getBid()) - 0.01) {
            ...
            } else if (currentMaximumUtility > ((CUHKAgentSAS) helper).getUtilitythreshold() * Math.pow(this.discountingFactor, negotiationSession.getTime())) {
                try {
                    //if the current offer is approximately as good as the best one in the history, then accept it.
                    if (utilitySpace.getUtility(opponentBid) >= utilitySpace.getUtility(negotiationSession.getOpponentBidHistory().getBestBidDetails().getBid()) - 0.01) {
                        return true;
                    } else {
                        System.out.println("test" + utilitySpace.getUtility(opponentBid) + ((CUHKAgentSAS) helper).getUtilitythreshold());
                        ((CUHKAgentSAS) helper).setConcedeToOpponent(true);
                        return false;
                    }
                } catch (Exception e) {
    ...
    private boolean TerminateCurrentNegotiation(Bid ownBid) {
        ...
        if (currentUtility >= ((CUHKAgentSAS) helper).getUtilitythreshold() || currentUtility >= nextRoundUtility) {
            ...
            double predictMaximumUtility = maximumUtility * this.discountingFactor;
            double currentMaximumUtility = this.utilitySpace.getReservationValueWithDiscount(negotiationSession.getTimeline());
    ...
    private void chooseConcedeToDiscountingDegree() {
       ...
       /* if (this.maximumOfBid > 100) {
        ...
        System.out.println("concedeToDiscountingFactor is " + ((CUHKAgentSAS) helper).getConcedeToDiscountingFactor() + "current time is " + negotiationSession.getTimeline().getTime());
    }

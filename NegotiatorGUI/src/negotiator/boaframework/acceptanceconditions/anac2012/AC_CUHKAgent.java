...
public class AC_CUHKAgent extends AcceptanceStrategy {
	...
	public void initializeAgent(NegotiationSession negotiationSession,
			OfferingStrategy os) throws Exception {
		...
		if (offeringStrategy.getHelper() == null
				|| (!offeringStrategy.getHelper().getName().equals("CUHKAgent"))) {
		...
		if (activeHelper) {
			try {
				...
				this.opponentBidHistory.initializeDataStructures(utilitySpace
						.getDomain());
				((CUHKAgentSAS) helper).setTimeLeftAfter(negotiationSession
	...
	private Actions activeDetermineAcceptability() {
		...
		try {
			...
			// timeline.getTime());
			((CUHKAgentSAS) helper).setTimeLeftBefore(negotiationSession
					.getTimeline().getCurrentTime());
			if (negotiationSession.getOpponentBidHistory().size() >= 1) {// the
																			...
																			// response
																			// secondly
				opponentBid = negotiationSession.getOpponentBidHistory()
						.getLastBidDetails();
				// receiveMessage opponent model first
				this.opponentBidHistory.updateOpponentModel(
						opponentBid.getBid(), utilitySpace.getDomain(),
						this.utilitySpace);
				...
				if (negotiationSession.getOwnBidHistory().isEmpty()) {
					...
					if (((CUHKAgentSAS) helper).estimateTheRoundsLeft(false,
							true) > 10) {// still have some rounds left to
											// further negotiate (the major
											// negotiation period)
						...
						if (negotiationSession.getTimeline().getTime() > 0.9985
								&& ((CUHKAgentSAS) helper)
										.estimateTheRoundsLeft(false, true) < 5) {
							// bid =
							...
							if (offeringStrategy.getNextBid()
									.getMyUndiscountedUtil() < 0.85) {
								...
								// before the deadline
								if (((CUHKAgentSAS) helper)
										.estimateTheRoundsLeft(false, true) < 2) {
									// bid =
			...
			System.out.println(((CUHKAgentSAS) helper).estimateTheRoundsLeft(
					false, false));
	...
	private Actions regularDetermineAcceptability() {
		...
		int roundsLeft;
		if (activeHelper) {
			roundsLeft = ((CUHKAgentSAS) helper).estimateTheRoundsLeft(true,
					true);
		} else {
			roundsLeft = ((CUHKAgentSAS) helper).estimateTheRoundsLeft(false,
					true);
		}
		if (roundsLeft > 10) {// still have some rounds left to further
			...
			if (activeHelper) {
				roundsLeft = ((CUHKAgentSAS) helper).estimateTheRoundsLeft(
						true, true);
			} else {
				roundsLeft = ((CUHKAgentSAS) helper).estimateTheRoundsLeft(
						false, true);
	...
	private boolean AcceptOpponentOffer(Bid opponentBid, Bid ownBid) {
		...
		try {
			...
			if (activeHelper) {
				maximumUtility = utilitySpace.getUtility(utilitySpace
						.getMaxUtilityBid());
			} else {
		...
		// timeline.getTime());
		if (currentUtility >= ((CUHKAgentSAS) helper).getUtilitythreshold()
			...
			double currentMaximumUtility = this.utilitySpace
					.getUtilityWithDiscount(negotiationSession
							.getOpponentBidHistory().getBestBidDetails()
							.getBid(), negotiationSession.getTimeline());
			if (currentMaximumUtility > predictMaximumUtility
					&& negotiationSession.getTime() > ((CUHKAgentSAS) helper)
				...
				try {
					...
					if (utilitySpace.getUtility(opponentBid) >= utilitySpace
							.getUtility(negotiationSession
									.getOpponentBidHistory()
									.getBestBidDetails().getBid()) - 0.01) {
						System.out
			...
			} else if (currentMaximumUtility > ((CUHKAgentSAS) helper)
					.getUtilitythreshold()
	...
	private void chooseConcedeToDiscountingDegree() {
		...
		((CUHKAgentSAS) helper)
				.setConcedeToDiscountingFactor(this.minConcedeToDiscountingFactor
						+ (1 - this.minConcedeToDiscountingFactor) * alpha);
		this.concedeToDiscountingFactor_original = ((CUHKAgentSAS) helper)
				.getConcedeToDiscountingFactor();
		System.out.println("concedeToDiscountingFactor is "
				+ ((CUHKAgentSAS) helper).getConcedeToDiscountingFactor()
				+ "current time is "
	...
	private void calculateBidsBetweenUtility() {
		...
		try {
			...
			double maximumUtility = (((CUHKAgentSAS) helper)
					.getMaximumUtility());
	...
	private void updateConcedeDegree() {
		...
		((CUHKAgentSAS) helper)
				.setConcedeToDiscountingFactor(this.concedeToDiscountingFactor_original
						+ weight

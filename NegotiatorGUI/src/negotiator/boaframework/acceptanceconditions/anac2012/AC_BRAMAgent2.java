 ...
 * The code was taken from the ANAC2012 BRAMAgent2 and adapted to work within the BOA framework.
 * 
...
public class AC_BRAMAgent2 extends AcceptanceStrategy{
	...
	public Actions determineAcceptability() {
		double offeredUtility = utilitySpace.getUtilityWithDiscount(negotiationSession.getOpponentBidHistory().getLastBidDetails().getBid(), negotiationSession.getTime());
    	double threshold;
		...
		if(/*( bidToOffer != null ) && */(offeredUtility >= utilitySpace.getUtilityWithDiscount(offeringStrategy.getNextBid().getBid(), negotiationSession.getTime()))){
			return Actions.Accept;
		}
		...
		(this.utilitySpace.getReservationValueWithDiscount(negotiationSession.getTimeline()) > this.utilitySpace.getUtilityWithDiscount(offeringStrategy.getNextBid().getBid(), negotiationSession.getTimeline()))){

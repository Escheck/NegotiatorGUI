...
public class AC_BRAMAgent2 extends AcceptanceStrategy {
	...
	public void init(NegotiationSession negoSession, OfferingStrategy strat,
		...
		if (offeringStrategy.getHelper() == null
				|| (!offeringStrategy.getHelper().getName().equals("BRAMAgent"))) {
	...
	public Actions determineAcceptability() {
		double offeredUtility = utilitySpace.getUtilityWithDiscount(
				negotiationSession.getOpponentBidHistory().getLastBidDetails()
						.getBid(), negotiationSession.getTime());
		double threshold;

		if (activeHelper) {
			threshold = ((BRAMAgentSAS) helper).getNewThreshold(worstBid,
					bestBid.getBid());
		} else {
		...
		if (((offeredUtility < this.utilitySpace
				.getReservationValueWithDiscount(negotiationSession
						.getTimeline())) && (negotiationSession.getTime() > 177.0 / 180.0))
				&& (this.utilitySpace
						.getReservationValueWithDiscount(negotiationSession
								.getTimeline()) > this.utilitySpace
						.getUtilityWithDiscount(offeringStrategy.getNextBid()

 ...
 * contains functionality for opponent stubbornness.
 * 
 * @author E. Jacobs
 *
 */
public class DiscreteIssueProcessor {
	...
	/**
	 * Creates a DiscreteIssueProcessor for a domain. Use this for opponent modeling
	 * @param d The domain
	 */
	...
	/**
	 * Creates a DiscreteIssueProcessor for a domain, given a certain utility. Use this for modeling your own agent
	 * @param u
	 * @param d
	 */
	...
	/**
	 * Adapts the different issue weights according to the current bid the opponent has made. Issues with more changes get a lower weight
	 * @param currentBid Bid done by the opponent	
	 * @param time Time at which the bid was done
	 */
	public void adaptWeightsByBid(Bid currentBid, double time){
		...
		if (time == 0){
			...
			nBidsPerSecond = (double) nBidsProcessed / (time* (double) totalTime);
		}
		...
		if (previousBid != null){
			...
			for (int i = 0; i < nIssues; i++){
				...
				try {	
					oldValue = (ValueDiscrete) previousBid.getValue(issue.getNumber());
					newValue = (ValueDiscrete) currentBid.getValue(issue.getNumber());
				} catch (Exception e) {}
								
				if (!oldValue.equals(newValue)){
					changeList.set(i, changeList.get(i)+corr/(changeList.get(i)));
				}
			}			
		}
	 ...
	 * @param i Issue for which the weight is required
	...
	/**
	 * Processes each bid to give an indication of the stubbornness of the opponent
	 * @param bid
	 */
	private void processBid(Bid bid){
		...
		//in both cases, reset repeats and allow more different bids to be stored
		...
		//if more then 20 bids are allowed to be stored, reset to 5 bids storage
		...
		if (nBidsProcessed == 0){
			...
			stubbornness = nRepeatLimitOverrides/((double)nBidsProcessed/(double)nRepeatLimit);
		}	
		
		//System.out.println("Updated stubbornness - new value is " + stubbornness + ", total changes:" + changes);
		
	}
	
	/**
	 * Returns the index of the highest weight from some weightList
	 * @param wList The weightList
	...
	public String toString(){
		...
		for (int i = 0;i < nIssues;i++){
			str += "Issue: " + issueList.get(i).getName() + ", changes: " + changeList.get(i) + ", weight: " + weightList.get(i) +"\n";
		}

...
public class Bounds {
	...
	/**
	 * Constructor. Sets the bounds for a given issue based on its 
	 * type (discrete, real or integer)
	 */
	...
	public static Value getIssueValue(Issue pIssue, double pIndex) {
		Value v = null;
		switch(pIssue.getType()) {
			case DISCRETE: v = ((IssueDiscrete) pIssue).getValue((int) pIndex); break;
			case REAL: v = new ValueReal(pIndex); break;
			case INTEGER: v = new ValueInteger((int) pIndex); break;
		}
		return v;
	}
	 ...
	 * it is scaled by the difference between the value and the lower bound divided
	 * by the length of the bounds
	 */
	public static double getScaledIssueValue(Bounds pBounds, Bid pBid, Issue pIssue) throws Exception {
		...
		switch(pIssue.getType()) {
			case DISCRETE: value = ((IssueDiscrete) pIssue).getValueIndex(((ValueDiscrete) v).value); break;
			case REAL: value = ((ValueReal) v).getValue(); break;
			case INTEGER: value = ((ValueInteger) v).getValue(); break;
		}

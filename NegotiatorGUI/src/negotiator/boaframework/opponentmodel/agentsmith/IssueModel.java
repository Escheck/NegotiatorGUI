...
public class IssueModel {
	...
	/**
	 * The utility of if this issue is discrete. Takes the amount of values that are the
	 * same and divides it by the total amount of values. If there are lots of same values
	 * as the bid, the utility will be high, else it will be low.
	 */
	public double getDiscreteUtility(Bid pBid) {
		...
		return  this.fValues.size() == 0 ? 0 : lSame / (double) this.fValues.size();
	}
	...
	public double getRealUtility(Bid pBid) {
		 return 1 - Math.abs(fBounds.normalize(getNumberValue(getBidValue(pBid))) - fBounds.normalize(getAverage()));
	}
	...
	/**
	 * Gets the weight of this value. If there are lots of changes, it's not important,
	...
	public double getDiscreteWeight() {
		...
		for(Value lValue:fValues) {
			ValueDiscrete lDiscreteValue = (ValueDiscrete) lValue;
			int lCount = lCounter.get(lDiscreteValue.value) != null ?  lCounter.get(lDiscreteValue.value) : 0;
			lCount++;
			lCounter.put(lDiscreteValue.value, lCount);
		}
		double lMax = 0;
		double lTotal = 0;

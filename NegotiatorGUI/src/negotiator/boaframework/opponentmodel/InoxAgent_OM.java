...
public class InoxAgent_OM extends OpponentModel {
	...
	public double getBidEvaluation(Bid bid) {
		...
		try {
			...
			if (smallHistory.size() > 0.2 * negotiationSession
					.getOpponentBidHistory().size()) {
				result = opponentUtilitySpace.getUtility(bid);
				// Else receiveMessage with scaling
			} else {
				Objective root = opponentUtilitySpace.getDomain()
						.getObjectivesRoot();
				Enumeration<Objective> issueEnum = root
						.getPreorderIssueEnumeration();
				while (issueEnum.hasMoreElements()) {
					Objective is = (Objective) issueEnum.nextElement();
					Evaluator eval = (Evaluator) opponentUtilitySpace
							.getEvaluator(is.getNumber());
	...
	public double valueEval(Objective obj, Bid bid) throws Exception {
		EvaluatorDiscrete lEval = (EvaluatorDiscrete) opponentUtilitySpace
				.getEvaluator(obj.getNumber());
		int variable = 0;

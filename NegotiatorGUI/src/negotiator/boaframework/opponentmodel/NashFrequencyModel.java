...
public class NashFrequencyModel extends OpponentModel {
	...
	private void initModel() {
		...
		for (int index = 0; index < issues.size(); index++) {
			...
			if (issue instanceof IssueDiscrete)
				issueEvaluation = new IssueEvaluationDiscrete(
						(IssueDiscrete) issue);
				...
				Range ourNonZeroUtilityRange = new Range(
						issueI.getLowerBound(), issueI.getUpperBound());
				issueEvaluation = new IssueEvaluationInteger(
						(IssueInteger) issue, ourNonZeroUtilityRange);
			} else if (issue instanceof IssueReal) {
				// We use the range in which our utility is non-zero to estimate
				// the distribution of the opponent.
				IssueReal issueR = (IssueReal) issue;
				Range ourNonZeroUtilityRange = new Range(
						issueR.getLowerBound(), issueR.getUpperBound());
				issueEvaluation = new IssueEvaluationReal((IssueReal) issue,
						ourNonZeroUtilityRange);
			} else
				throw new UnsupportedOperationException(
						"There is no implementation for that issueType.");
		...
		if (bid != null) {
			...
			for (Issue issue : issues) {
				try {
					...
					this.issueEvaluationList.updateIssueEvaluation(issueID,
							offeredValue);
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		if (issueEvaluationList.isReady()) {
			...
			for (Issue issue : issues) {
				try {
					...
					double issueWeight = this.issueEvaluationList
							.getNormalizedIssueWeight(issueID);

					// Get the estimated normalized weight of the value of the
					// issue, which corresponds to the value that has
					// been offered in the bid.
					Value offeredValue = bid.getValue(issueID);
					AIssueEvaluation issueEvaluation = this.issueEvaluationList
							.getIssueEvaluation(issueID);
					double offeredValueWeight = issueEvaluation
							.getNormalizedValueWeight(offeredValue);
	...
	public double getWeight(Issue issue) {
		...
		return this.issueEvaluationList.getNormalizedIssueWeight(issue
				.getNumber());
	}
	...
	public AdditiveUtilitySpace getOpponentUtilitySpace() {
		if (issueEvaluationList.isReady()) {
			return new UtilitySpaceAdapter(this, negotiationSession
					.getUtilitySpace().getDomain());
		} else {
			System.out
					.println("Returned own utilityspace to avoid an error (normal on first turn).");

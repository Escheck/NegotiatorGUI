 ...
 * The opponentmodel will be based on frequency/distribution analysis. 
 * 
 * - The importance of a discrete issue and it's values will be calculated by the difference in the number of times
 * 		a certain value is chosen.
 * - The importance of a numerical issue will be calculated by the first offered values and the range in which our own
...
public class NashFrequencyModel extends OpponentModel
{
	...
	/**
	 * This initializes the negotiatormodel, creating an issueEvaluation 
	 * for each issue in the utilitySpace.
	 */
	private void initModel()
	{
		...
		//This will later contain all information we can gather on the negotiator.
		for(int index = 0; index < issues.size(); index++)
		{
			...
			{
				//We use the range in which our utility is non-zero to estimate the distribution of the opponent.
	...
	public void updateModel(Bid bid, double time)
	{
		if (bid != null) {
			...
			//After all issueEvaluations have been updated, we can calculate the new
	 ...
	 * The utility estimation is done by multiplying the normalized issue weights with the
	 * normalized offered-value weight of that issue.
	 * 
	 * Note that the utility of a bid will always be in between 0 - 1.
	 * Note that this method does not take discount into account.
	 * 
	 * @param bid The bid that has just been offered by the negotiator that we are evaluating.
	 * @return The estimated utility of the bid that has been offered by the negotiator we are evaluating.
	 */
		...
		if (issueEvaluationList.isReady()) {
			...
			for(Issue issue : issues)
			{
				try 
				{
					...
					//Get the estimated normalized weight of the issue, which indicates how important the issue is.
					double issueWeight = this.issueEvaluationList.getNormalizedIssueWeight(issueID);
					
					//Get the estimated normalized weight of the value of the issue, which corresponds to the value that has
					...
					//Since all issueWeights combined should add up to 1, and the maximum value of the valueWeight is 1,
					//the estimated utility should be exactly 1 if all offered valueWeights are 1. So to calculate the partial estimated
					//utility for this specific issue-value combination, we need to multiply the weights.

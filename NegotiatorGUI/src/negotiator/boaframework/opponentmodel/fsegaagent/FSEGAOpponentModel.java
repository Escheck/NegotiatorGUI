...
public class FSEGAOpponentModel extends OpponentModel
{
	...
	public FSEGAOpponentModel(AdditiveUtilitySpace pUS)
	{
		...
		ArrayList<Issue> issues = fDomain.getIssues();
		...
		for(int i = 0; i < numberOfIssues; i++)
		{
			...
			switch(uUS.getEvaluator(issues.get(i).getNumber()).getType())
			{
				case DISCRETE:
					...
					EvaluatorHypothesis lEvaluatorHypothesis = new EvaluatorHypothesis(lDiscreteEvaluator);
					...
					lEvaluatorHypothesis = new EvaluatorHypothesis(lDiscreteEvaluator);
				...
				case REAL:
					lEvalHyps = new ArrayList<EvaluatorHypothesis>();
					aaEvaluatorHypothesis.add(lEvalHyps);
					IssueReal lRealIssue = (IssueReal)(fDomain.getIssue(i)); //Laptop | Harddisk | Monitor
		...
		EvaluatorHypothesis[] ehTmp = new EvaluatorHypothesis[uUS.getNrOfEvaluators()];

		buildEvaluationHypothesis(evalHypothesis, ehTmp, uUS.getNrOfEvaluators() - 1, aaEvaluatorHypothesis);
	...
	private void buildEvaluationHypothesis(ArrayList<EvaluatorHypothesis[]> pHyps, EvaluatorHypothesis[] pEval, int m, ArrayList<ArrayList<EvaluatorHypothesis>> paaEval)
	{
		if(m==0)
		{
			ArrayList<EvaluatorHypothesis> lEvalHyps = paaEval.get(uUS.getNrOfEvaluators()-1);
			for (int i=0;i<lEvalHyps.size();i++)
			{
				pEval[uUS.getNrOfEvaluators()-1] = lEvalHyps.get(i);
				EvaluatorHypothesis[] lTmp = new EvaluatorHypothesis[uUS.getNrOfEvaluators()];
		...
		{
			ArrayList<EvaluatorHypothesis> lEvalHyps = paaEval.get(uUS.getNrOfEvaluators()-m-1);
			for (int i=0;i<lEvalHyps.size();i++)
			{
	...
	private void buildUtilitySpaceHypothesis(WeightHypothesis[] pWeightHypothesis, ArrayList<EvaluatorHypothesis[]> pEvalHypothesis)
	{
		...
		for(int i = 0; i < pWeightHypothesis.length; i++)
		{
			for(int j=0; j < pEvalHypothesis.size(); j++) {
				UtilitySpaceHypothesis lUSHyp = new UtilitySpaceHypothesis(fDomain, uUS, pWeightHypothesis[i], pEvalHypothesis.get(j));

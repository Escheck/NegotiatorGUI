...
public class SortedOutcomeSpace extends OutcomeSpace {
	...
	public List<BidDetails> getBidsinRange(Range r) {
		...
		if (allBids.get(upperboundIndex).getMyUndiscountedUtil() <= r
				.getUpperbound() && upperboundIndex > 0) {
			upperboundIndex--;
		}
		// test lowerbound index element is under lowerbound
		if (allBids.get(lowerboundIndex).getMyUndiscountedUtil() >= r
				.getLowerbound() && lowerboundIndex < allBids.size()) {
			lowerboundIndex++;
		}
		...
		ArrayList<BidDetails> subList = new ArrayList<BidDetails>(
				allBids.subList(upperboundIndex, lowerboundIndex));
		return subList;
	...
	public int getIndexOfBidNearUtility(double utility) {
		...
		int newIndex = -1;
		double closestDistance = Math.abs(allBids.get(index)
				.getMyUndiscountedUtil() - utility);

		// checks if the BidDetails above the selected is closer to the
		// targetUtility
		if (index > 0
				&& Math.abs(allBids.get(index - 1).getMyUndiscountedUtil()
						- utility) < closestDistance) {
			newIndex = index - 1;
			closestDistance = Math.abs(allBids.get(index - 1)
					.getMyUndiscountedUtil() - utility);
		}

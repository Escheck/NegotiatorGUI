...
public class PartialPreferenceModels {
	...
	private ArrayList<ValuePreferenceGraphMap> preferenceOrderingMap; // a collection of models for each agent
	private ArrayList<Issue> issues;
	


	public PartialPreferenceModels(Bid firstBid, int numberOfParties) { //maybe we can get the bid directly and use also domain knowledge later...
		
		preferenceOrderingMap=new ArrayList<ValuePreferenceGraphMap>(numberOfParties);
	...
	public void updateIssuePreferenceList(int issueIndex, Value previousValue, Value currentValue, ArrayList<Feedback> feedback) {
		
		for (int i=0; i<feedback.size(); i++)
			preferenceOrderingMap.get(i).updateValuePreferenceGraph(issueIndex, previousValue, currentValue, feedback.get(i));
	}	
	...
	public Value getNashValue(int issueIndex) {
		...
		if (preferenceOrderingMap.get(0).getAllValues(issueIndex).size()==1) // if there exists only one value, return it.
			return preferenceOrderingMap.get(0).getAllValues(issueIndex).get(0);
		
		for (Value currentValue:preferenceOrderingMap.get(0).getAllValues(issueIndex)) {
		
			currentProduct=1.0;
			for (int i=0; i<preferenceOrderingMap.size(); i++) {
				currentProduct*=preferenceOrderingMap.get(i).getEstimatedUtility(issueIndex, currentValue);
			}
	...
	public ArrayList<Value> getNashValues(int issueIndex) {
		...
		if (preferenceOrderingMap.get(0).getAllValues(issueIndex).size()==1) // if there exists only one value, return it.
			 nashValues.add(preferenceOrderingMap.get(0).getAllValues(issueIndex).get(0));
		else {
	...
	// because of the time constraint, I wrote the simple sorting but not efficient (since there are no much nash bids, it will not be a problem)
	...
	public ArrayList<Bid> estimatePossibleNashBids(Bid sampleBid) throws Exception{
		...
		for (Issue currentIssue:issues)
		{
			...
			firstBid.setValue(currentIssue.getNumber(), nashIssueValues.get(currentIssue.getNumber()).get(0));			
		}
		...
		for (Issue currentIssue: issues) {
			...
			for (Value currentValue: nashIssueValues.get(currentIndex)) {
				...
				for (int i=0; i<nashBids.size(); i++){
					...
					currentBid.setValue(currentIndex, currentValue);
	...
	public ArrayList<Value> getIncomparableValues(int issueIndex, Value currentValue) {
		...
		Value incomparable;
		for (int i=0; i<preferenceOrderingMap.size(); i++) {
			incomparable=preferenceOrderingMap.get(i).getIncomparableValue(issueIndex, currentValue);

...
public class ValuePreferenceGraph {
	...
	public Integer getDepth(Value issueValue) {
		...
		if (!containsIssueValue(issueValue))
			return -1;
		else return partialGraph.get(issueValue).getDepth()-lowestDepth+1; // to ensure the lowest depth is equal to one..
		
	}
	
	
	public double getEstimatedUtility(Value issueValue) { // according to depth heuristic
	...
	public void updateLowHighDepths() {
        ...
        lowestDepth=highestDepth; // lowerDepth can be greater than it was before 
        while(valueIterator.hasNext()){
	...
	public Value getMissingValue() {
		...
		case REAL: //needs to be checked!
			...
			for (int i=0; i<issueReal.getNumberOfDiscretizationSteps(); i++) {
				value=new ValueReal(issueReal.getLowerBound() + (((double)((issueReal.getUpperBound()-issueReal.getLowerBound()))/(issueReal.getNumberOfDiscretizationSteps()))*i));
				if (!containsIssueValue(value)) 
		    	  return value;
			}
			break;	
	...
	private void searchCondition (Value willBeChecked, ArrayList<Value> visitedList, ArrayList<Value> selectedValues, ArrayList<Value> forbiddenList)   {
	...
	private void increasePreferredValueNodeDepths (Value lessPreferredValue, Value morePreferredValue,int difference)   {
		...
		// the depth of the nodes related to morePreferredValue except forbidden list will be increaded by "difference"
	...
	public void addPreferenceOrdering(Value firstValue, Value secondValue, Feedback feedback) {
		...
		if (firstValue.equals(secondValue)) //if the values are same, do nothing
			return;
			...
			addNewNode(firstValue, secondValue, feedback);
				  ...
				  if (feedback==Feedback.BETTER) {
					  ...
					  if(firstValueNode.getDepth()!= secondValueNode.getDepth()) {
						  ...
						  if (firstValueNode.getDepth()<secondValueNode.getDepth()) {//second node is higher  
							  
							  increasePreferredValueNodeDepths(secondValue, firstValue,secondValueNode.getDepth()-firstValueNode.getDepth());
							  firstValueNode.setDepth(secondValueNode.getDepth());					  
						  }  
						  else { //first node is higher
							  increasePreferredValueNodeDepths(firstValue, secondValue, firstValueNode.getDepth()-secondValueNode.getDepth());
						  }					  
	...
	// FirstValue always exists in the system but newValudoes not exist for this method
	...
	public static void main(String[] args) {
		...
		mygraph.addPreferenceOrdering(new ValueDiscrete("b"), new ValueDiscrete("c"), Feedback.BETTER);
		mygraph.addPreferenceOrdering(new ValueDiscrete("c"), new ValueDiscrete("d"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("c"), new ValueDiscrete("e"), Feedback.SAME);
		mygraph.addPreferenceOrdering(new ValueDiscrete("e"), new ValueDiscrete("f"), Feedback.BETTER);
		mygraph.addPreferenceOrdering(new ValueDiscrete("a"), new ValueDiscrete("g"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("a"), new ValueDiscrete("h"), Feedback.SAME);
		mygraph.addPreferenceOrdering(new ValueDiscrete("h"), new ValueDiscrete("k"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("f"), new ValueDiscrete("m"), Feedback.SAME);
		...
		for (Value current: mygraph.getAllValues()) {
		    ...
		    System.out.println("\nEstimated Utility:"+mygraph.getEstimatedUtility(current));
		...
		System.out.println("*******************************************************************");		
		System.out.println("Comparables-a :"+mygraph.getAllComparableValues(new ValueDiscrete("a")));
		System.out.println("Comparables-b :"+mygraph.getAllComparableValues(new ValueDiscrete("b")));
		System.out.println("Comparables-c :"+mygraph.getAllComparableValues(new ValueDiscrete("c")));
		System.out.println("Comparables-d :"+mygraph.getAllComparableValues(new ValueDiscrete("d")));
		System.out.println("Comparables-e :"+mygraph.getAllComparableValues(new ValueDiscrete("e")));
		System.out.println("Comparables-f :"+mygraph.getAllComparableValues(new ValueDiscrete("f")));
		System.out.println("Comparables-g :"+mygraph.getAllComparableValues(new ValueDiscrete("g")));
		System.out.println("Comparables-h :"+mygraph.getAllComparableValues(new ValueDiscrete("h")));
		System.out.println("Comparables-k :"+mygraph.getAllComparableValues(new ValueDiscrete("k")));
		System.out.println("Comparables-m :"+mygraph.getAllComparableValues(new ValueDiscrete("m")));
		
		
		System.out.println("*******************************************************************");		
		System.out.println("InComparables-a :"+mygraph.getAllIncomparableValues(new ValueDiscrete("a")));
		System.out.println("InComparables-b :"+mygraph.getAllIncomparableValues(new ValueDiscrete("b")));
		System.out.println("InComparables-c :"+mygraph.getAllIncomparableValues(new ValueDiscrete("c")));
		System.out.println("InComparables-d :"+mygraph.getAllIncomparableValues(new ValueDiscrete("d")));
		System.out.println("InComparables-e :"+mygraph.getAllIncomparableValues(new ValueDiscrete("e")));
		System.out.println("InComparables-f :"+mygraph.getAllIncomparableValues(new ValueDiscrete("f")));
		System.out.println("InComparables-g :"+mygraph.getAllIncomparableValues(new ValueDiscrete("g")));
		System.out.println("InComparables-h :"+mygraph.getAllIncomparableValues(new ValueDiscrete("h")));
		System.out.println("InComparables-k :"+mygraph.getAllIncomparableValues(new ValueDiscrete("k")));
		System.out.println("InComparables-m :"+mygraph.getAllIncomparableValues(new ValueDiscrete("m")));
			*/
		
/*
   	ValuePreferenceGraph mygraph=new ValuePreferenceGraph(new ValueDiscrete("a"));
		mygraph.addPreferenceOrdering(new ValueDiscrete("a"), new ValueDiscrete("b"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("b"), new ValueDiscrete("c"), Feedback.BETTER);
		mygraph.addPreferenceOrdering(new ValueDiscrete("c"), new ValueDiscrete("d"), Feedback.BETTER);
		mygraph.addPreferenceOrdering(new ValueDiscrete("d"), new ValueDiscrete("a"), Feedback.BETTER);
		mygraph.addPreferenceOrdering(new ValueDiscrete("d"), new ValueDiscrete("e"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("b"), new ValueDiscrete("e"), Feedback.WORSE);

		mygraph.addPreferenceOrdering(new ValueDiscrete("d"), new ValueDiscrete("f"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("d"), new ValueDiscrete("g"), Feedback.BETTER);
		mygraph.addPreferenceOrdering(new ValueDiscrete("b"), new ValueDiscrete("f"), Feedback.SAME);
		mygraph.addPreferenceOrdering(new ValueDiscrete("d"), new ValueDiscrete("k"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("c"), new ValueDiscrete("m"), Feedback.WORSE);
		mygraph.addPreferenceOrdering(new ValueDiscrete("m"), new ValueDiscrete("n"), Feedback.BETTER);
	    mygraph.addPreferenceOrdering(new ValueDiscrete("k"), new ValueDiscrete("m"), Feedback.BETTER);
	//    mygraph.addPreferenceOrdering(new ValueDiscrete("e"), new ValueDiscrete("n"), Feedback.SAME);
		System.out.println(mygraph.toString());
	*/
	/*
		System.out.println("*******************************************************************");		
		...
		System.out.println("InComparables-k :"+mygraph.getAllIncomparableValues(new ValueDiscrete("k")));
		System.out.println("InComparables-m :"+mygraph.getAllIncomparableValues(new ValueDiscrete("m")));
		System.out.println("InComparables-n :"+mygraph.getAllIncomparableValues(new ValueDiscrete("n")));
		*/

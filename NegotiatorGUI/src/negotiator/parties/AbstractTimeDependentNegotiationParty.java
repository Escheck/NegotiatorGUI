 ...
 */
public abstract class AbstractTimeDependentNegotiationParty extends
	...
	public Action chooseAction(List<Class> possibleActions) {
		...
		double nextUtil = nextBid != null ? utilitySpace
				.getUtilityWithDiscount(nextBid, timeline) : 0;
		...
		// Voting based actions
			...
			return new Reject(getPartyId()); // Accept is higher up the chain
	...
	public double getTargetUtility() {
		...
		double offset = timeline instanceof DiscreteTimeline ? 1d / ((DiscreteTimeline) timeline)
				.getTotalRounds() : 0d;

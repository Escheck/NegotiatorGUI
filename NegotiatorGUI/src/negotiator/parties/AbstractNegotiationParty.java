...
public abstract class AbstractNegotiationParty implements NegotiationParty {
    ...
    /**
     * map of all deadlines set for this party. Deadlines are combined using "or". So for example
     * if we have Time and Round deadline, it means that the deadline is reached when either time
     * or round will have reached it's deadline
     */
    protected final Map<DeadlineType, Object> deadlines;
     ...
     */
    public AbstractNegotiationParty(
    ...
    protected Value getRandomValue(Issue currentIssue) throws Exception {
        ...
        switch (currentIssue.getType()) {
            ...
            case REAL:
                ...
                index = rand.nextInt(realIss.getNumberOfDiscretizationSteps());  // check this!
                currentValue = new ValueReal(realIss.getLowerBound() + (((realIss.getUpperBound()
                        - realIss.getLowerBound())) / (realIss.getNumberOfDiscretizationSteps()))
                        * index);
                break;
            case INTEGER:
                IssueInteger integerIssue = (IssueInteger) currentIssue;
                index = rand.nextInt(integerIssue.getUpperBound() - integerIssue.getLowerBound()
                        + 1);
     ...
     *
     * @param bid The bid to get the utility for
     * @return A double value between [0, 1] (inclusive) that represents the bids utility
     */
    @Override
    ...
    public AgentID getPartyId() {
        return partyId == null ? new AgentID(""+getClass()+"@"+hashCode()) : partyId;
    }

    /**
     * Sets the party id
     *
     * @param partyId The uniquely identifying party id.
     */
    @Override

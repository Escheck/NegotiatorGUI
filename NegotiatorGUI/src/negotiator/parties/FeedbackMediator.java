 ...
 * <p/>
 * This class was adapted from Reyhan's parties.SmartMediatorOnlyFeedback class (see svn history for
 * details about that class), and adapted to fit into the new framework.
 *
 * @author David Festen
...
public class FeedbackMediator extends Mediator {
     ...
     */
    public FeedbackMediator(UtilitySpace utilitySpace, Map<DeadlineType, Object> deadlines,
    ...
    public Action chooseAction(List<Class> possibleActions) {
        if (possibleActions.contains(OfferForFeedback.class)) roundNumber++;
        ...
        try {
            ...
            } else if (roundNumber == 1) { //initially generate a random bid and create the
            // preference model
                currentBid = generateRandomBid();
                lastBid = new Bid(currentBid);
                preferenceList = new PartialPreferenceModels(new Bid(currentBid), numListeners);
                return (new OfferForFeedback(getPartyId(), currentBid));
                // when we stop searching part and start voting, inform the agents about the last
                // accepted bid by all
            } else {
                ...
                if (roundNumber > 2) {
                    ...
                    preferenceList.updateIssuePreferenceList(currentIndex,
                            lastBid.getValue(currentIndex), currentBid.getValue(currentIndex),
                            currentFeedbackList);
                    lastBid = new Bid(currentBid);
                }
            ...
            System.out.println("" + roundNumber + " :Cannot generate random bid or update " +
...
//        if ((getRound() <= ((double) getTotalRoundOrTime() / 2)) || (epsilon > Math.random()))
    ...
    private double getNormalizedRound() {
        if (!deadlines.containsKey(DeadlineType.ROUND)) return 0d;

        double totalRounds = (Integer) deadlines.get(DeadlineType.ROUND);
    ...
    private Value searchForNewValue() throws Exception {
        ...
        do {
            ...
            do {
                ...
                if ((!newValue.equals(lastBid.getValue(currentIndex))) && (preferenceList
                        .mayImproveAll(currentIndex, lastBid.getValue(currentIndex), newValue)))
                    return newValue;

...
public class NegotiationLogParser {
	...
	static class ResultsParser extends DefaultHandler {
		...
		private boolean processTournamentBasedQM(String nsURI, String strippedName, String tagName, Attributes attributes) {		
			...
			if (tagName.equals("NegotiationOutcome")) {
				...
				outcome.setRunNr(Integer.parseInt(attributes.getValue("runNumber")));
				...
				outcome.setBiddingStrategyA(attributes.getValue("offering_strategy"));
				outcome.setAcceptanceStrategyA(attributes.getValue("acceptance_strategy"));
				...
				outcome.setAgentAutility(Double.parseDouble(attributes.getValue("finalUtility")));
				...
				outcome.setBiddingStrategyB(attributes.getValue("offering_strategy"));
				outcome.setAcceptanceStrategyB(attributes.getValue("acceptance_strategy"));
				...
				outcome.setAgentButility(Double.parseDouble(attributes.getValue("finalUtility")));
		 ...
		 */
		private boolean outcomeInArray(ArrayList<OutcomeInfoDerived> outcomes, OutcomeInfoDerived outcome) {
			boolean found = false;
			for (int i = 0; i < outcomes.size(); i++) {
				if (outcomes.get(i).getAgentAname().equals(outcome.getAgentAname()) &&
						outcomes.get(i).getAgentBname().equals(outcome.getAgentBname()) &&
						outcomes.get(i).getAgentAutilSpaceName().equals(outcome.getAgentAutilSpaceName()) &&
						outcomes.get(i).getAgentButilSpaceName().equals(outcome.getAgentButilSpaceName())) {
					found = true;
					break;
				}
				if (outcomes.get(i).getAgentAname().equals(outcome.getAgentBname()) &&
						outcomes.get(i).getAgentBname().equals(outcome.getAgentAname()) &&
						outcomes.get(i).getAgentAutilSpaceName().equals(outcome.getAgentButilSpaceName()) &&
						outcomes.get(i).getAgentButilSpaceName().equals(outcome.getAgentAutilSpaceName())) {
					found = true;
					break;
				}
			}
	...
	public static ResultsParser parseLog(String log) throws Exception{
		...
		xr.parse(log);
		System.out.println("Parsed " + handler.getOutcomes().size() + " outcome logs");
		return handler;
	}

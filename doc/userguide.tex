...
\subsection{Running a Multi-Party Negotiation}\label{sec:singlesessionrun}
...
\begin{itemize}
	...
	\item \textbf{Data Persistency}. What kind of persistent data is available to the parties. The options are 
	\begin{itemize}
	\item \textbf{Disabled}. Parties do not have persistent data. This is the default
	\item \textbf{Serializable}. Parties can save anything serializable
	\item \textbf{Standard}. Parties receive a prepared, read only StandardInfo object.
	\end{itemize}

\item bilateral options. These appear only if you have exactly 2 parties added. The sub-options of this panel are
	\begin{itemize}
...
The Tournament tab will appear similar to Figure~\ref{Fig:multipartytournament}. This panel shows a set of tournament options:

\begin{itemize}
	\item \textbf{Protocol}. The protocol to use for each session. See Chapter \ref{sec:protocols}
	...
	\item \textbf{Agent Repetition}. If selected, parties drawn from the Agents pool with return. If not selected, parties are drawn without return. This option can only be disabled if 'Agents per session' is less or equal to the size of the Agents pool.
	\item \textbf{Randomize session order}. If set, all sessions within a single run of the tournament are randomized in order. Each repetition of the tournament is randomized separately.
...
After you click "Start", the tournament starts. The panel then is swapped for a tournament progress panel. 
...
The parties and profiles for each session are determined by created by all possible permutations of N parties from the agents pool  and permutations of N profiles from the profile pool, where N is the selected number of "Agents per Session". The mediator is not taking part in this permutation, and is added to each session. The value of "Agents per session" is excluding the mediator.
...
\item A party item  (and mediator) contains:
  \begin{itemize}
    \item \raggedright \textbf{protocolClassPath}. This is the protocol that this party assumes to be used. 
    		You will get errors like "used with wrong protocol" if your party is used in the wrong protocol context. 
		Protocols were discussed in section \ref{sec:protocols}.
...
\begin{itemize}
\item Protocol. The protocol value is used for all sessions. See section \ref{sec:protocols}.
\item Mediator. The mediator to use for all sessions (ignored if the protocol does not need a mediator)
\item Deadline. The deadline is used for all sessions.  A deadline contains two values:
     \begin{itemize}
      ...
      \end{itemize}
\item repeats. This is also called 'number of tournaments' and determines the number of times a complete tournament will be run.
\item Randomize Session Order. Whether all generated sessions within a tournament must be randomized.
\item Parties per session. The number of agents to draw for each session. This excludes a possible mediator.
\item Party Repetition. true if agents are to be drawn from the agents pool with return, false if they are to be drawn without return.
\item Parties and Profile pool for side A
\item Parties and Profile pool for side B
\end{itemize}

The tournament generation works as follows. 

If there are exactly 2 agents per session and the agents and profiles for side B have been set, then bilateral generation is used. Otherwise, multilateral generation is used. This generation method creates an ordered list of sessions for 1 tournament.  If the 'Randomize Session Order' is set, the list is randomized. All sessions use the same protocol, mediator, deadline.
...
To implement a multi-party negotiation party, at a minimum one needs to implement a class that implements the \\texttt{negotiator.parties.NegotiationParty} interface. Also your implementation must have a public default (no-argument) constructor. The interface is as follows: 

\begin{lstlisting}
public interface NegotiationParty {
	/**
	 * Initializes the party. This MUST be called immediately after construction
	 * of any class implementing this. Tells which utility space and timeline
	 * the party is running in. This must be called one time only.
	 * 
	 * @param utilSpace
	 ...
	 *            persistent over sessions. Each <agentclass, profiles> tuple
	 *            has its own unique storage that persists only during the run
	 *            of a tournament. Between sessions, this data is saved to disk
	 *            to avoid memory issues when other agents are running. If the
	 *            storage is not empty, this data is retrieved at the start of
	 *            each session and saved at the end of each session. The load
	 *            time is charged on the total available negotiation time. The
	 *            save time is limited to 1 second. The programmer should ensure
	 *            that storage is actually serializable.
	 */
	public void init(AbstractUtilitySpace utilSpace, Deadline deadline, 
			TimeLineInfo timeline, long randomSeed,
			AgentID agentID, SerializableMap storage);

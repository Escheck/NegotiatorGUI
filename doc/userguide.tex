...
%===========================================================================
\title{Using the \Genius Framework for Running Autonomous Negotiating Agents}
...
\begin{itemize}
	\item \textbf{Multi-Party Negotiation Session}. A single negotiation session in which a number of agents negotiate.
...
Genius logs quality measures to log files. Logs are written both  in \verb|.csv| and \verb|.xml| format.
...
\item the agent that acted.
\item the action that the agent did. The action consists of the action type name ("Offer", "Accept", "EndNegotiation", etc) and the bid details if available.
\end{enumerate}
...
and then, for each of the agents that participated a "resultsOfAgent" element containing:
\begin{enumerate}
\item the agent name. 
\item the agent's description
\item the agent's utilityspace filename
\item the full agent class path
\item the final utility (un-discounted) of this agent, or 0 if there was no agreement
...
The logStats.xml file contains for each of the agents that participated in the tournament statistical info:

\begin{enumerate}
\item agentname: the agent name (full class path)
...
\subsection{Set up your Workspace}\label{sec:setupworkspace}
...
\item Now you can proceed working on your code in \verb|src/main/java|.  For a quick start, you can just copy one of the examples provided in the zip.
\end{enumerate}
 \FloatBarrier

Eclipse now automatically downloads and includes all dependencies into your project and build your code into target/classes. 



\subsection{Example agents}
The \Genius zip file contains two examples: the Multiparty example and the Storage example. To compile an example, set up your workspace (Section \ref{sec:setupworkspace}) and copy the example folder into  \verb|src/main/java|.
...
\begin{table*}[t]
  ...
  \hline\hline
    ...
    getProtocol & The actual supported MultilateralProtocol. Usually this returns StackedAlternating\-Offers\-Protocol. Your agent should override this if it supports a another protocol\\
...
For convenience, you can also extend the class \texttt{negotiator.parties.AbstractNegotiationParty} which is a basic implementation of NegotiationParty. This class also provides convenient support functions for building your agent.

Your agent might need to check the exact type of the provided AbstractUtilitySpace (inside NegotiationInfo), for instance if your agent supports for example only AdditiveutilitySpace.
...
\hline
\texttt{UtilitySpace}\\
The preference profile of the scenario allocated to the agent. It is recommended to use this class when implementing a model of the opponent's preference profile.\\
\hline
\texttt{Timeline }\\
...
\hline
\texttt{BidHistory}\\
a structure to keep track of the bids presented by the agent and the opponent.\\
\hline
\texttt{SortedOutcomeSpace}\\
a structure which stores all possible bids and their utilities by using BidIterator. In addition, it implements search algorithms that can be used to search the space of possible bids for bids near a given utility or within a given utility range.\\
\hline
\texttt{BidIterator}\\
...
\hline
{\bf WARNING} \\
Many of the provided BOA components currently assume a single opponent party. \\
These will behave incorrectly when used with multiple opponents. \\
We recommend checking the source code of the BOA components you want to use, \\
or write your own components if you are creating a NegotiationParty.\\
\hline
\end{tabular}
...
A boa agents can be edited in the "Boa Parties" repository tab (Figure \ref{fig:boaparties}). Right-click in the panel to add items. Select an item and right-click to remove or edit an item.   
...
\subsection{Set up a Workspace}
BOA components must be compiled before they can be loaded into Genius.
To compile a BOA component, follow the steps in (Section \ref{sec:setupworkspace}). Then, create your BOA code into \verb|src/main/java/| (instead of agent code). For a quick start, you can copy the boaexample folder into \verb|src/main/java/|. Maven automatically compiles your BOA components into \verb|target/classes|.

Please refer to chapter \ref{sec:debugging} for instructions on running \Genius in debug mode to debug your components.



\subsection{Creating BOA Components}
...
To run or debug your agent as it runs in Genius, right click on the project root and select Run As.../Java Application. Then select \verb|Application - genius| and click ok.

...
\abstract{\noindent \Genius~is a negotiation environment that implements an open architecture for heterogeneous negotiating agents. \Genius~can be used to implement, or simulate, real life negotiations. This document describes how you can install the required environment, work with the provided scenarios and agents, and write, compile, and run an agent yourself.}
...
Note that some agents and scenarios require more memory than allocated by default to Java. This problem can be resolved by using the Xmx and Xms parameters when running Java, for example \texttt{java -Xmx1536M -Xms1536M -jar negosimulator.jar}.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Access partner preferences}.
		\item \textbf{Allow pausing timeline}.
	\end{itemize}
	\item \textsc{Session generation}
	\begin{itemize}
		\item \textbf{Play both sides}. When generating the matches, whether each pair of agents should play both sides on a scenario or not.
	...
	\begin{itemize}
		...
		\item \textbf{Log final accuracy}.
		\item \textbf{Log competitiveness}.
		\item \textbf{Append mode and deadline}.
	\end{itemize}
...
A large set of quality measures have been incorporated in \Genius~4.0. Most quality measures are automatically available, while for others an option must be selected in the tournament options.

In addition, there are now two logs in \Genius: the standard log and the tournament log. This standard log captures the outcome of each negotiation in a tournament by logging the results of the quality measures for both agents. The tournament log uses the standard log to calculate averages and standard deviations of functions of the quality measures in the standard log, for example the average utility for all matches which resulted in an agreement.
...
\Genius~4.0 incorporates four types of quality measures: standard measures, detailed measures, opponent model accuracy measures, and competitiveness measures. In the following sections we discuss each of these measure types in detail.

\subsubsection{Standard Measures}
The standard measures are the measures which are enabled by default in \Genius~and cannot be disabled. Table~\ref{tab:standardmeasures} provides an overview of all default quality measures.
...
\begin{table}[h!]
	...
	\begin{tabular}{|p{4.7cm}|p{9.2cm}|}
		...
		errors & Errors encountered during the negotiation. Not reaching an agreement before the deadline is also an error.\\[-0.3ex]\hline
		...
		normalized\_utility & The final utility divided by the maximum possible utility for the agent. In normal domains the result should be equal to the final utility.\\[-0.3ex]\hline
...
\subsubsection{Detailed Measures}
...
\begin{table}[h!]
	...
	\begin{tabular}{|p{4.7cm}|p{9.2cm}|}
		...
		\textbf{Attribute} & \textbf{Description}\\[-0.2ex] 
		\hline\hline
		concession\_moves & The percentage of which in which the agent relative to the prev\\[-0.3ex]\hline
		...
		fortunate\_moves & The percentage of moves in which the agent, relative to the previous offer, offered a bid which increased both its own and its opponents utility.\\[-0.3ex]\hline
		...
		social\_welfare & The sum of the utility for both agents.\\[-0.3ex]
...
\hline
\end{tabular}
	\caption{Opponen model accuracy measures in \Genius~in alphabetic order.}
	\label{tab:omaccuracyMeasures}
\end{table} 

\subsubsection{Competitiveness Measures}
Log competitiveness
...
\begin{table}[h!]
	...
	\begin{tabular}{|p{4.7cm}|p{9.2cm}|}
		...
		BS\_CR & \\[-0.3ex]
		FYU & \\[-0.3ex]
		minDemandedUtility & \\[-0.3ex]
		Normalized\_AC\_CR & \\[-0.3ex]
		Total\_CR & \\[-0.3ex]
		\hline
		\hline
	\end{tabular}
	\caption{Competitiveness quality measures in \Genius~in alphabetic order.}
	\label{tab:competitivenessMeasures}
\end{table}
...
\subsection{Analyzing the Results using Excel}~\label{sec:analysisExcel}
...
\item \textbf{SortedOutcomeSpace} is a structure which stores all possible bids and their utilities. This class implements efficient search algorithms that can be used to efficiently search the space of possible bids for bids satisfying specific properties.

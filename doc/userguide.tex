...
Like the Simple Mediator Based Protocol, the parties do not hear the other parties directly. Instead, they only hear the mediator and the mediator hears the bids of all the parties. The mediator determines which bid will be voted on, collects the votes and determines the outcome. The mediator is just another NegotiationParty, but it extends Mediator.
 ...
 The mediator generates its first bid randomly and sends it to the negotiating agents. After each bid, each negotiating agent comparing the current bid with the mediator\vtick s previous bid and gives feedback (`better', `worse' or `same')  to the mediator. For its further bids, the mediator updates the previous bid by flipping one of the issues. This process continues iteratively until reaching a predefined number of bids. This protocol is explained in detail in \cite{MultiMediatedNegoProtocolsWithFeedback}.
...
The accepted bid is the last bid that was not receiving a `worse' vote. The negotiation runs on until the deadline (unless some party crashes).
...
public interface NegotiationParty {
	 ...
	 * instantiation. Tells which utility space and timeline we 
	 * are running in. This is called one time only.
	 *
	 * @param utilSpace
	 * 	(a copy of/readonly version of) the {@link UtilitySpace} 
	...
	/**
	 * When this function is called, it is expected that the Party 
	 * chooses one of the actions from the possible action list 
	 * and returns an instance of the chosen action.
	 *
	...
	/**
	 * This method is called when another {@link NegotiationParty} 
	 * chose an{@link Action}.
	 *
	 * @param sender
	 * 	The initiator of the action. May be null if initiator is 
	 *	not an agent.
	 * @param arguments
...
You may want to check AbstractUtilitySpace, if your agent supports for example only AdditiveutilitySpace.

We recommend to use the javadoc included with the distribution to check the details of all the involved classes. 

For convenience, you can also extend the class \texttt{negotiator.parties.AbstractNegotiationParty} directly. That class provides convenient support functions. 
